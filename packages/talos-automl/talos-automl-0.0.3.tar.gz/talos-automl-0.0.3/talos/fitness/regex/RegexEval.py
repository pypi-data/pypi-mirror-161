...
from talos.fitness.regex.testing.RegexTimer import time_regex_test_case
        ...
        for test_case in RegexEval.test_cases:
            results.append(
                time_regex_test_case(
                    compiled_regex, test_case, testing_iterations)
            )
        return results
    ...
    def evaluate(self, ind, **kwargs):
        ...
        if RegexEval.pstartup is None:
            RegexEval.pstartup = Process(
                target=self.call_fitness, name="self.call_fitness"
            )
