...
class ThorlabsPM100x:
    ...
    def list_devices(self):
        ...
        '''
        if hasattr(self, 'rm'):                 #This makes sure that the Resource Manager of pyvisa (if it was already initialized) is closed and cleared before looking for available devices
            self.rm.close()                     #If a device was previously connected but was unplugged/turned off without doing a proper disconnection, it will not show up in the list         
            self.rm.visalib._registry.clear()   #of available devices (or it will generate an error when querying with '*IDN?'), unless we first close and clear the rm object.
        self.rm = visa.ResourceManager()
