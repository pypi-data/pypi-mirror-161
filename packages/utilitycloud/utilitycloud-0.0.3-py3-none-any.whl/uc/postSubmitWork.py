...
def postWork(token, user, wo_id, wfid, aid, fields):
    ...
    response = postWork_payload(token, right_now, user_guid, user, user_full_name, aid, wfid, wo_id, fields)
    return response


def postWork_payload(token, right_now, user_guid, user, user_full_name, aid, wfid, wo_id, fields):
    payload = json.dumps(
        {
            "ReportId": 0,
    ...
    response = postWork_raw(token, payload)
    return response


def postWork_raw(token, payload,*args,**kwargs):
    ...
    apiAction = "workorder/submitwork"
