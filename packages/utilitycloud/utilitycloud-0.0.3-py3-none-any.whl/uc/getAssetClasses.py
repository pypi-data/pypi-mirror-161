...
def getAssetClasses(token,*args,**kwargs):
    ...
    apiAction = "assetclass/getassetclassbybillingaccount"
...
def getAcRead(token,assetclassId,*args,**kwargs):
  ...
  body = json.dumps({
    'assetclassid':str(assetclassId)
  })

  apiAction = "api/AssetClassDetailsController/Read"
  urlBuild = url+apiAction
  print("------LOG---- URL BUILD "+urlBuild)
...
def postAcSave(token,payload,*args,**kwargs):
  domain = kwargs.get('domain','https://ucld.us/')
  url = domain
  ...
  apiAction = "api/AssetClassDetailsController/SAVE_ASSET_CLASS_DETAILS"
  urlBuild = url+apiAction
  print("------LOG---- URL BUILD "+urlBuild)
  response = requests.post(urlBuild,headers=headers,data=body)
  #print("------LOG------- Workflow Fields")
