...
class Maestro:
    ...
    def get_prompt(self, conn: psycopg.Connection) -> Optional[Prompt]:
        ...
        maybe_prompt = cursor.execute(
            "UPDATE prompt_queue SET status='assigned', assigned_at=now(), hostname=%s WHERE id = %s AND status='pending' RETURNING id AS prompt_id, prompt, params, webhook, inserted_ts;",
            [hostname, prompt_id],
        ).fetchone()
        if not maybe_prompt:
