...
from .._constants import TimeSeriesInternal, TimeSeriesWebLinks
from .._diagnostics.azureml_error import AzureMLError
from .._diagnostics.contract import Contract
from .._diagnostics.error_definitions import TimeseriesInvalidDateOffsetType
from .._diagnostics.validation import Validation
from .._types import CoreDataSingleColumnInputType, GrainType
...
def get_tsdf_frequency_and_start(
    ...
    if X.index.names[0] is not None:
        ...
        X, reverse_dict = _temp_rename_columns(X, set(X.index.names).intersection(set(X.columns)))
    try:
        ...
        if time_colname_null.any():
            tsds.data.drop(tsds.time_index[time_colname_null], level=time_column_Name, inplace=True)
    ...
    if len(fixed_frequencies_not_nan[FREQ].unique()) == 1:
        coverage = groupby_ob.apply(
            lambda x: check_coverage_one_grain(tsds.from_data_frame_and_metadata(x, False), fixed_frequencies)
        ).sum()
...
def check_coverage_one_grain(one_grain: TimeSeriesDataSet, freqs: Union[pd.DataFrame, pd.Series]) -> int:
    ...
    if isinstance(freqs, pd.Series):
        ...
        df = pd.merge(one_grain.data, freqs, on=one_grain.time_series_id_column_names, how="left", right_index=False)
        df.reset_index(drop=True, inplace=True)
        freq = df[FREQ][0]
        start = df[START][0]
...
def improved_infer_freq_one_grain(
    ...
    if best_coverage / len(df_grain.data) <= FREQUENCY_REJECT_TOLERANCE or data_grid_too_big:
        ...
        frequencues_to_try = (
            get_frequencies_choices(td_mode, ix_time.to_series()) if user_frequency is None else [user_frequency]
        )
...
def _series_freq_start_for_grain(
    df_one_grain: TimeSeriesDataSet, freq: pd.DateOffset, start: pd.Timestamp, grain: GrainType, n_points_in_grid: int
) -> pd.Series:
    """
...
def get_frequencies_choices(delta: pd.Timedelta, df_time: pd.Series) -> List[pd.DateOffset]:
    ...
    """
    from ..featurization.timeseries.forecasting_heuristic_utils import timedelta_to_freq_safe
...
def has_dominant_frequency(
    timedeltas: pd.Series, delta_mode: pd.Timedelta, dominant_frequency: pd.DateOffset, tolerance: float
) -> bool:
    """
...
def get_dominant_timedeltas(frequency: pd.DateOffset, default: pd.Timedelta) -> Set[pd.Timedelta]:
    ...
    if hasattr(frequency, "delta"):
        ...
        isinstance(frequency, pd.tseries.offsets.MonthBegin)
        or isinstance(frequency, pd.tseries.offsets.MonthEnd)
        or frequency == DateOffset(months=1)
    ):
        timedeltas = {pd.Timedelta(days=30), pd.Timedelta(days=31), pd.Timedelta(days=28), pd.Timedelta(days=29)}
    elif (
        isinstance(frequency, pd.tseries.offsets.YearBegin)
        or isinstance(frequency, pd.tseries.offsets.YearEnd)
        or frequency == DateOffset(years=1)
    ):
        timedeltas = {pd.Timedelta(days=365), pd.Timedelta(days=366)}
    elif isinstance(frequency, pd.tseries.offsets.QuarterBegin) or isinstance(
        frequency, pd.tseries.offsets.QuarterEnd
    ):
...
def fix_df_frequency(
    ...
    Contract.assert_true(
        (grain_column_names is None) == isinstance(start_times, pd.Timestamp),
        message="If the data frame contains multiple data time series, the start times should be represented by a "
                "dictionary in case of a single series, it must be a pd.Timestamp.",
    )
    ...
    for grain, df in X.groupby(groupers, as_index=False, group_keys=False):
        if grain not in start_times.keys():
            ...
            dfs.append(fix_frequency_one_grain(df, freq, start_time, time_column_name))
    if dfs:
...
def _correct_start_time(
    one_grain: pd.DataFrame, time_column_name: str, start: pd.Timestamp, freq: Optional[pd.DateOffset]
) -> pd.Timestamp:
    """
...
def fix_frequency_one_grain(
    X: pd.DataFrame, freq: Optional[pd.DateOffset], start: pd.Timestamp, time_column_name: str
) -> pd.DataFrame:
    """
    Remove outlier data points from the data frame which does not comply with the given frequency.
...
def fix_data_set_regularity_may_be(
    ...
    """
    user_freq = str_to_offset_safe(freq, freq_ref_code)
    aggregation_enabled = target_aggregation_function is not None and freq is not None
    ...
    if aggregation_enabled:
        ts_data = TimeSeriesDataConfig(
            X, y, time_column_name, grain_column_names, user_freq, target_aggregation_function, featurization_config
        )
        X_fixed, y_fixed = aggregate_dataset(ts_data, new_freq)
        was_aggregated = X_fixed.shape != X.shape
        return FixedDataSet(X_fixed, y_fixed, False, was_aggregated, user_freq)

    if y is not None:
        # If y was provided, align it with X.
        X[TimeSeriesInternal.DUMMY_TARGET_COLUMN] = y
    X_fixed = fix_df_frequency(X, time_column_name, grain_column_names, starts, new_freq)
    if y is not None:
    ...
    return FixedDataSet(X_fixed, y_fixed, False, X_fixed.shape[0] != X.shape[0], new_freq)


def str_to_offset_safe(freq: Optional[str], ref_code: Optional[str] = None) -> pd.DateOffset:
    """
    ...
    :raises: AzureMLError if the frequency can not be converted to the pd.DateOffset.
    """
    try:
...
def _temp_rename_columns(X: pd.DataFrame, col_set: Set[str]) -> Tuple[pd.DataFrame, Dict[str, str]]:
    """
