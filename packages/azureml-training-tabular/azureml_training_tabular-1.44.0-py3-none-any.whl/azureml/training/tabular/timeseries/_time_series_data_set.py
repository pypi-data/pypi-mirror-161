...
from .._constants import TimeSeries, TimeSeriesInternal, TimeSeriesWebLinks
...
from .._diagnostics.error_strings import ErrorStrings
    ...
    **Note:** This class will not copy the data frame, but will set its index to
    ...
    def _validate(self) -> None:
        ...
        if self.time_series_id_column_names is not None and any(
            ...
            raise AzureMLError.create(
                TimeseriesDfColumnTypeNotSupported,
                target="time_series_id_column_names",
        ...
        if self.origin_time_column_name and not isinstance(self.origin_time_column_name, str):
            raise AzureMLError.create(
                TimeseriesDfColumnTypeNotSupported,
                target="origin_time_column_name",
        ...
        if self.data.index.to_frame().duplicated().sum() > 0:
            raise AzureMLError.create(
                TimeseriesDfDuplicatedIndex, target="index_keys", reference_code=ReferenceCodes._TSDS_DUPLICATED_INDEX
            )
    ...
    def concat(
        ...
        if any(set(df.index.names) != names for df in data_list[1:]):
            raise AzureMLError.create(
                IncompatibleIndexError, reference_code=ReferenceCodes._TSDS_CONCAT_INDEX_ERROR, target="index"
            )
        ...
        if any(not names.issubset(set(df.columns)) for df in data_list[1:]):
            raise AzureMLError.create(
                IncompatibleColumnsError, reference_code=ReferenceCodes._TSDS_CONCAT_COLUMNS_ERROR, target="columns"
            )
    ...
    def _infer_freq_by_ts_id(self) -> pd.Series:
        ...
        Contract.assert_true(
            ...
            ErrorStrings.TIMESERIES_DF_CANNOT_INFER_FREQ_WITHOUT_TIME_IDX,
            log_safe=True,
        )
