...
from ..._diagnostics.error_definitions import ClientError, DiskFull, InsufficientMemory
...
class Downloader:
    ...
    def get_download_path(cls, file_name: str, target_dir: str, prefix: Optional[str] = None) -> str:
        ...
        """
        return os.path.join(target_dir, "{prefix}_{file_name}".format(prefix=prefix or "", file_name=file_name))
    ...
    def download(
        ...
        download_path = Downloader.get_download_path(file_name=file_name, target_dir=target_dir, prefix=prefix)
    ...
    def _download_with_retries(
        cls, download_url: str, download_path: str, retries: int, md5hash: Optional[str] = None
    ) -> bool:
        download_succeeded = False
        for retry_attempt in range(0, retries + 1):
            try:
                download_succeeded = cls._download(
                    download_url=download_url, download_path=download_path, md5hash=md5hash
                )
                break
    ...
    def _download(cls, download_url: str, download_path: str, md5hash: Optional[str] = None) -> bool:
        ...
        try:
            ...
            raise ClientError("Request exception when trying to download the file")
            ...
            raise ClientError("Exception when trying to download the file")
        finally:
        ...
        if md5hash and download_hash != md5hash:
            error_message = "md5hash validation failed. This downloaded file will not be used " "and will be removed."
            Downloader.remove_file(download_path)
            logging_utilities.log_traceback(ClientError(error_message), logger, is_critical=False)
            has_download_failed = True
    ...
    def _download_file(download_url: str, download_path: str) -> str:
        ...
        with requests.get(download_url, stream=True) as r, open(download_path, "wb") as fd:
            if r.status_code != 404:
                ...
                logger.info(
                    "Embeddings download time: {time_taken}".format(time_taken=download_end_time - download_start_time)
                )
    ...
    def remove_file(file_path: str) -> None:
        ...
        try:
            ...
            logging_utilities.log_traceback(ex, logger, override_error_msg="Removing file failed", is_critical=False)
