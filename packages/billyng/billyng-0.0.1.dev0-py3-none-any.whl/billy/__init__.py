...
def checkUrlIsSet(func):
    def wrapper(*args, **kwargs):
        if not isinstance(Billy.url, str) or Billy.url.strip() == "":
            raise RuntimeError("billy url is not set")
...
def _post(url, *args, **kwargs):
    return _handle(requests.post(f"{Billy.url}{url}", *args, **kwargs))


@billyError
def _delete(url, *args, **kwargs):
    return _handle(requests.delete(f"{Billy.url}{url}", *args, **kwargs))


@billyError
def _get(url, *args, **kwargs):
    return _handle(requests.get(f"{Billy.url}{url}", *args, **kwargs))




@decorateAll(checkUrlIsSet)
class Billy:
    url: str = None

    @staticmethod
    def createCustomer(email: str, address: dict = None, description: str = None, metadata: dict = {}, name: str = None, phone: str = None, initialCredits: int = 0):
    ...
    @staticmethod
    def createSubscription(customerId: str, productId: str, option: Union[str,int] = "base", cancelExistingSubscriptions: bool = True, invoiceNow: bool = True, prorate: bool = True, cancelAtPeriodEnd: bool = False, trialUntil: int = None):
    ...
    @staticmethod
    def cancelSubscription(customerId: str, productId: str, invoiceNow: bool = True, prorate: bool = True):
