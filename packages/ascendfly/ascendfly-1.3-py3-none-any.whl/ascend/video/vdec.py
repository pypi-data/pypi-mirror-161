...
class Vdec():
    ...
    def __check_para(self, context, channel, en_type):
        ...
        if channel < 0:
            raise ValueError(f"Configurated channel_id is negative.")
    ...
    def __callback(self, input_stream_desc, output_pic_desc, user_data):
        ...
        if input_stream_desc:
            in_stream_data = acl.media.dvpp_get_stream_desc_data(input_stream_desc)
            in_stream_dsize = acl.media.dvpp_get_stream_desc_size(input_stream_desc)
            ret = acl.media.dvpp_free(in_stream_data)
            if ret != ACL_SUCCESS:
                Log(WARNING, f'free in_stream_data memory failed, return {ret}.')
    ...
    def process(self, frame, skipped=False):
        ...
        if skipped:
            ret = acl.media.vdec_send_skipped_frame(self.channel_desc,
                                            stream_desc, 
                                            self._frame_conf, 
                                            user_data)
        else:
    ...
    def channel_id(self, channel_id):
        ...
        if channel_id < 0:
            raise ValueError(f"Configurate channel_id should not be a negative.")
