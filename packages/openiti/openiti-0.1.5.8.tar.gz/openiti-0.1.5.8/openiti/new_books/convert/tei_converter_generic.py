...
from openiti.helper.ara import deNoise, normalize_composites
...
##        self.unique_tags = dict()
    ...
    def get_metadata(self, text):
        ...
        """
        text = deNoise(normalize_composites(text))
        ...
        self.witnesses = []
        try:
            for w in sourceDesc.find_all("witness"):
                self.witnesses.append(w.text.strip())
            if self.witnesses:
                metadata += "#META# Manuscript witnesses: {}\n".format(", ".join(self.witnesses))
        except Exception as e:
            print(e)
            pass
        print("witnesses:", self.witnesses)

        for tag in fileDesc.find_all("respStmt"):
            metadata += "#META# {}\n".format(re.sub("[\r\n ]+", " ", tag.text).strip())
