...
def secrets_add(ctx: click.Context, name: str, data_file):
    ...
    max_len = 64 * 1024
    value_binary: bytes = data_file.read(max_len + 1)
    if len(value_binary) > max_len:
        click.secho(f"Secret value is too long (max={max_len})")
        exit(1)

    # Send raw bytes to storage
    success = secrets_client.add(name, value_binary)
    if success:
...
@secrets_group.command("get_as_text")
@click.argument("name", required=True)
@click.pass_context
def secrets_get_as_text(ctx: click.Context, name: str):
    """Output secret value as text, assuming it is a utf8 encoded string"""
    secrets_client: SecretsApiClient = get_secrets_client()
    value = secrets_client.get_as_text(name)
    click.echo(value)


@secrets_group.command("get_as_bytes")
@click.argument("name", required=True)
@click.pass_context
def secrets_get_as_bytes(ctx: click.Context, name: str):
    """Output raw bytes of binary secret"""
    secrets_client: SecretsApiClient = get_secrets_client()
    value = secrets_client.get_as_bytes(name)
    click.echo(value, nl=False, color=False)


@secrets_group.command("get_as_base64")
@click.argument("name", required=True)
@click.pass_context
def secrets_get_as_base64(ctx: click.Context, name: str):
    """Output base64 encoding of binary secret value in"""
    secrets_client: SecretsApiClient = get_secrets_client()
    value = secrets_client.get_as_base64(name)
    click.echo(value)
...
def secrets_list(ctx: click.Context):
    ...
    assert databutton.utils.deploy.get_auth_token() == "test"
    secrets_client: SecretsApiClient = get_secrets_client()
