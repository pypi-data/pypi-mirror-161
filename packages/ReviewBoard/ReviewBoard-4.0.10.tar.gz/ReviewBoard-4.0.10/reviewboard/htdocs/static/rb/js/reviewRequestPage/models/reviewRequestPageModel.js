...
RB.ReviewRequestPage.ReviewRequestPage = RB.ReviewablePage.extend({
  ...
  _loadUpdates() {
    ...
    if (entries.length > 0) {
      ...
      for (let entryTypeID in allEntryIDs) {
        if (allEntryIDs.hasOwnProperty(entryTypeID)) {
          ...
          urlEntryTypeIDs.push("".concat(entryTypeID, ":").concat(entryIDs));
        }
      }

      urlQuery.push("entries=".concat(urlEntryTypeIDs.join(';')));
    }
    /*
    ...
    urlQuery.sort();
    const urlQueryStr = urlQuery.length > 0 ? "?".concat(urlQuery.join('&')) : '';
    Backbone.sync('read', this, {
      url: "".concat(updatesURL).concat(urlQueryStr),
  ...
  _reloadFromUpdate(model, metadata, html) {
    this.trigger("applyingUpdate:".concat(metadata.type), metadata, html);

    if (model) {
      this.trigger("applyingUpdate:".concat(metadata.type, ":").concat(model.id), metadata, html);
      ...
      this.trigger("appliedModelUpdate:".concat(metadata.type, ":").concat(model.id), metadata, html);

      if (_.isFunction(model.afterApplyUpdate)) {
        model.afterApplyUpdate(metadata);
      }

      this.trigger("appliedUpdate:".concat(metadata.type, ":").concat(model.id), metadata, html);
    }

    this.trigger("appliedUpdate:".concat(metadata.type), metadata, html);
  }
