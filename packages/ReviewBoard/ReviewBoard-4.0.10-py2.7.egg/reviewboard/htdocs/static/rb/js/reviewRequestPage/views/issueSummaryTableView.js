...
RB.ReviewRequestPage.IssueSummaryTableView = Backbone.View.extend({
  ...
  _sortByCol(colIndex, ascending) {
    this._$tbody.html(this._getIssueRows().sort((issueA, issueB) => {
      const $issueA = $(issueA);
      const $issueB = $(issueB);
      const $columnA = $issueA.children("td:nth-child(".concat(colIndex, ")"));
      const $columnB = $issueB.children("td:nth-child(".concat(colIndex, ")"));
      let value1;
      let value2;
  ...
  _buildMaps() {
    ...
    _.each(this._getIssueRows(), issueEl => {
      ...
      if (!_.has(this.reviewerToSelectorMap, reviewer)) {
        this.reviewerToSelectorMap[reviewer] = "[data-reviewer=\"".concat(reviewer, "\"]");
  ...
  _onIssueStatusChanged(comment, oldStatus, timestamp) {
    ...
    const $oldCounter = this._$tabs.children("[data-issue-state=".concat(oldStatus, "]")).find('.rb-c-issue-summary-table__counter');

    const $newCounter = this._$tabs.children("[data-issue-state=".concat(newStatus, "]")).find('.rb-c-issue-summary-table__counter');
