  ...
  RB.ReviewRequestPage.ReviewRequestPageView = RB.ReviewablePageView.extend({
    ...
    initialize() {
      ...
      this.listenTo(this.model, 'applyingUpdate:entry', (metadata, html) => {
        ...
        this.listenToOnce(this.model, "appliedModelUpdate:entry:".concat(entryID), (metadata, html) => this._reloadView(entryView, html));
        this.listenToOnce(this.model, "appliedUpdate:entry:".concat(entryID), metadata => {
    ...
    _onIssueClicked(params) {
      ...
      const selector = "#".concat(prefix, "comment").concat(params.commentID);
