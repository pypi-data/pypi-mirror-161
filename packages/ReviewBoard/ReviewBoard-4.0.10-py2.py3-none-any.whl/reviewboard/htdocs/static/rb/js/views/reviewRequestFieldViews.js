...
(function () {
  ...
  Fields.TextFieldView = Fields.BaseFieldView.extend({
     ...
     */
    richTextAttr() {
      return this.allowRichText ? "".concat(_.result(this, 'fieldName'), "RichText") : null;
    },
    ...
    initialize(options) {
      ...
      this.jsonTextTypeFieldName = this.jsonFieldName === 'text' ? 'text_type' : "".concat(this.jsonFieldName, "_text_type");
    },
    ...
    render() {
      ...
      const inlineEditorOptions = {
        el: this.$el,
        formClass: "".concat(this.$el.prop('id'), "-editor"),
      ...
      this.listenTo(this.model, "change:".concat(this.editableProp), (model, editable) => {
        if (editable) {
      ...
      this.listenTo(this.model, "fieldChanged:".concat(fieldName), this._formatField);
      return this;
    },
      ...
      this.inlineEditorView.$field.rbautocomplete({
        formatItem: data => {
          ...
          if (ac.descKey && data[ac.descKey]) {
            s += " <span>(".concat(_.escape(data[ac.descKey]), ")</span>");
          }
        ...
        error: xhr => {
          ...
          try {
            ...
            text = "HTTP ".concat(xhr.status, " ").concat(xhr.statusText);
          }
    ...
    _loadRichTextValue() {
      if (this.useExtraData) {
        ...
        console.assert(textType === 'plain' || textType === 'markdown', "Text type \"".concat(textType, "\" in field \"").concat(textTypeFieldName, "\" ") + "not supported.");
