...
class database(object):
    def __init__(self):
        ...
        if self.__path == None:
            ...
            cursor = conn.cursor()
            cursor.execute(
                'create table interface (id varchar(40) primary key, method varchar(40),fullname varchar(200),DStype varchar(20), header varchar(20), moduel varchar(20), framework varchar(20))')
            cursor.close()
            conn.commit()
            conn.close()
    ...
    def insert(self, type, framework, header='None', moduel='None', method='None', fullname='None'):
        ...
        if len(cursor.fetchall()) > 0:
            ...
            cursor.execute(
                "insert into interface (id, method, fullname, DStype, header, moduel, framework) values ('%s','%s','%s', '%s', '%s', '%s', '%s')" % (
                id, method, fullname, type, header, moduel, framework))
