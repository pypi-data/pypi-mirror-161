...
def test_slip_int_for_monotone_interpolation():
    ...
    aux = [0.0] *dim
    ll.ConvertXDataAUX( dim, npts,  XData, aux)
...
def test_procedural_interface():
    ...
    # LipIntInfValueCons(int *Dim, int *Ndata, int* Cons, double* x, double* Xd,double* y,  double Lipconst, int* Index);
    lc = 4.0
    Cons = [0, 0, 1, 0]
...
def test_smoothing():
    ...
    T = ll.LipIntSmoothLipschitz(dim, npts,XData,YData, LipConst, None, None, None, None, None, None)
    print( "T: ", T)
    sys.exit()
