...
class CSVFolder:
    def __init__(
        self, root_folder, partition_pattern="", delimiter=",", filter_pattern=""
    ):
        if partition_pattern:
            warnings.warn(
                "partition_pattern is deprecated. Use filter_pattern instead",
                DeprecationWarning,
            )
        if partition_pattern and filter_pattern:
            raise ValueError(
                "partition_pattern and filter_pattern cannot be passed at the same time."
            )
        self.root_folder = root_folder
        self.partition_pattern = partition_pattern
        self.filter_pattern = filter_pattern if filter_pattern else partition_pattern
        self.delimiter = delimiter


class ParquetFolder:
    def __init__(self, root_folder, partition_pattern="", filter_pattern=""):
        if partition_pattern:
            warnings.warn(
                "partition_pattern is deprecated. Use filter_pattern instead",
                DeprecationWarning,
...
class JSONFolder:
    def __init__(
        self, root_folder, partition_pattern="", multiline=False, filter_pattern=""
    ):
        if partition_pattern:
            warnings.warn(
                "partition_pattern is deprecated. Use filter_pattern instead",
