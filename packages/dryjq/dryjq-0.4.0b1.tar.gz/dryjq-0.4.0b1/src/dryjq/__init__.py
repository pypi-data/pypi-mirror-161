...
class DataStructureAddress:
    ...
    def get_value(self, data_structure: Any) -> Any:
        ...
        for element in self.__complete_path:
            # logging.debug("Found value at %r: %r", element, current_value)
...
class DataHandler:
    ...
    def execute_single_query(self, query: str) -> None:
        ...
        logging.debug("Filtered data before executing %r:", query)
        logging.debug("%r", self.filtered_data)
        ds_data_path, new_value = QueryParser.parse_query(query)
        if new_value is None:
            self.filtered_data = ds_data_path.get_value(self.filtered_data)
        else:
            self.filtered_data = ds_data_path.replace_value(
                self.filtered_data, yaml.safe_load(new_value.strip())
            )
            self.updated_data = True
        #
        logging.debug("Filtered data after executing %r:", query)
        logging.debug("%r", self.filtered_data)
