...
def handle_io(arguments: argparse.Namespace) -> int:
    ...
    if arguments.yaml_file == "-":
        if arguments.inplace:
            logging.warning("Cannot modify <stdin> inplace")
        #
...
def main() -> int:
    ...
    main_parser.set_defaults(loglevel=logging.INFO, query=".", yaml_file="-")
    ...
    main_parser.add_argument(
        ...
        "--inplace",
        action="store_true",
        help="Modify the YAML file in place (when used on a JSON file,"
        " this will convert the file to YAML). NOT AVAILABLE YET.",
    )
    main_parser.add_argument(
        "query",
    ...
    main_parser.add_argument(
        "yaml_file",
        help="The YAML (or JSON) file to read",
    )
    ...
    # XXX: fuse
    if arguments.inplace:
        logging.warning("Early development fuse: deactivating --inplace")
