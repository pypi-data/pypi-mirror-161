...
class RhinoView extends base_1.DOMWidgetView {
    ...
    render() {
        ...
        load3dmModel(this.scene, '/tree/' + this.path, {
            ...
            .catch(() => {
            this.showError('Error: path "' + this.model.get('path') + '" was not found');
            return;
        });
        //position camera
    ...
    handleLighting() {
        if (this.ambientLight > 0) {
            const ambientLight = new THREE.AmbientLight(0xffffff, this.ambientLight);
    ...
    addHelpersElements() {
        ...
        if (this.grid !== null) {
            const gridHelper = new THREE.GridHelper(this.grid.size, this.grid.division);
