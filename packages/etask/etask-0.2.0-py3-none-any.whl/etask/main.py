...
def parse_args():
    ...
    sub_autoloads = subparser.add_parser(
        "autoloads",
        help=tasks.Tasks.autoloads.__doc__
    )
    ...
    sub_clean = subparser.add_parser(
        "clean",
        help=tasks.Tasks.elisp_clean.__doc__
    )
    ...
    sub_compile = subparser.add_parser(
        "compile",
        help=tasks.Tasks.elisp_compile.__doc__
    )
    ...
    sub_compile_dir = subparser.add_parser(
        "compile-dir",
        help=tasks.Tasks.elisp_compile_dir.__doc__
    )
    ...
    sub_eval = subparser.add_parser(
        "eval",
        help=tasks.Tasks.elisp_eval.__doc__
    )
    ...
    sub_install_local = subparser.add_parser(
        "install-local",
        help=tasks.Tasks.install_local.__doc__
    )
    ...
    sub_install_remote = subparser.add_parser(
        "install-remote",
        help=tasks.Tasks.install_remote.__doc__
    )
    ...
    subparser.add_parser(
        "load-path",
        help=tasks.Tasks.load_path.__doc__
    )
    # ^ Takes no arguments.

    sub_test = subparser.add_parser(
        "test",
        help=tasks.Tasks.test.__doc__
    )
    ...
    sub_test_dir = subparser.add_parser(
        "test-dir",
        help=tasks.Tasks.test_dir.__doc__
    )
...
def main():
    ...
    if args.verbose:
        pinfo(emacs.Emacs(debug=args.debug))
    ...
    if not args.command and args.verbose:
        ...
        emacs_tasks.install_local(args.path)

    elif args.command == "install-remote":
        emacs_tasks.install_remote(
            args.package,
