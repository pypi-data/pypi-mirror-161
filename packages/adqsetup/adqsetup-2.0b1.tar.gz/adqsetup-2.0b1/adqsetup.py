...
class Ethtool(object):
    ...
    def _ioctl(self, data): # type: (bytearray|str|bytes) -> bytearray
        ...
        ioctl(
            ...
            pack("16sP", self.ifname.encode(), addr)
        )
    ...
    def features(self, modify=None): # type: (dict[str, bool]) -> dict[str, bool]
        ...
        if modify and len(modify):
            ...
            for n, v in modify.items():
                self._log("ethtool --features %s %s %s" % (self.ifname, n, 'on' if v else 'off'))
    ...
    def flags(self, modify=None): 
        ...
        if modify and len(modify):
            ...
            for n, v in modify.items():
                self._log("ethtool --set-priv-flags %s %s %s" % (self.ifname, n, 'on' if v else 'off'))
    ...
    def _coalesce_queues_get(self, queues):
        ...
        offset = int(8 + math.ceil(self.MAX_NUM_QUEUE / 8))
        resp = self._ioctl(data)[offset:]
        results = []
...
class NLAttr(namedtuple("NLAttr", "type data")):
    ...
    TYPE_BITFIELD32   = 15
    _hdr_struct = Struct("2H")

    class _int_struct:
        def __init__(self, pattern): # type: (str) -> None
            self.struct = Struct(pattern)
        def pack(self, obj): # type: (int) -> bytes
            return self.struct.pack(int(obj))
    ...
    class _str_struct:
        ...
        def pack(self, obj): # type: (str) -> bytes
            if self.null:
                return str(obj).encode() + b'\x00'
            else:
                return str(obj).encode()
        @staticmethod
        def unpack(data): # type: (bytes) -> str
            return data.partition(b'\x00')[0].decode()
    ...
    class _bitfield32_struct():
        ...
        @classmethod
        def unpack(cls, data): # type: (bytes) -> NLAttr._bitfield32
            return NLAttr._bitfield32(*cls.struct.unpack(data))
    ...
    class _ipaddr_struct:
        ...
        def pack(cls, obj): # type: (str) -> bytes
            ...
            return cls.struct.pack(*octets)            
        @classmethod
        def unpack(cls, data): # type: (bytes) -> str
            return '{}.{}.{}.{}'.format(*cls.struct.unpack(data))
    ...
    @classmethod
    def unpack(cls, data): # type: (bytes|bytearray) -> NLAttr
        length, type = cls._hdr_struct.unpack(data[:cls._hdr_struct.size])
        data = data[cls._hdr_struct.size:length]
        # print(cls, length, type, data)
        if type in cls._struct and cls._struct[type]:
            data = cls._struct[type].unpack(data)
        # else:
        #     raise ValueError("unknown type: %d" % type)
        return cls(type, data)

    @classmethod
    def list(cls, data): # type: (bytes|bytearray) -> list[NLAttr]
        results = []
        offset = 0
        while offset < len(data):
            attr = cls.unpack(data[offset:])
            results.append(attr)
            offset += attr.size
        return results

    @classmethod
    def dict(cls, data): # type: (bytes|bytearray) -> dict[int, any]
        return {
            a.type: a.data for a in cls.list(data)
        }
...
class NLMessage(object):
    ...
    def _pack_header(self): # type: () -> bytes
        return self._hdr_struct.pack(len(self), self.type, self.flags, self.seq, 0)
    ...
    def unpack(cls, data): # type: (bytes) -> NLMessage
        length, type, flags, seq, _ = cls._hdr_struct.unpack(
            data[:cls._hdr_struct.size])
        payload = data[cls._hdr_struct.size:length]
...
class GNLConn(NLConn):
    ...
    class Genlmsghdr(StructTemplate,
        ...
        struct = Struct("BBH")
        def __new__(cls, cmd, version):
            # type: (int, int) -> GNLConn.Genlmsghdr
            obj = super(GNLConn.Genlmsghdr, cls).__new__(cls, cmd, version, 0)
            return obj
        @classmethod
        def unpack(cls, data): # type: (bytes) -> GNLConn.Genlmsghdr
            return cls(*cls.struct.unpack(data[:cls.struct.size])[:2])
    ...
    def family(self, name):
        ...
        # print(GNLAttr(GNLAttr.TYPE_FAMILY_NAME, name).pack())
        try:
            msgs = self.send(NLMessage(
                type=self.GENL_ID_CTRL,
                flags=NLMessage.FLAG_REQUEST | NLMessage.FLAG_ACK,
                data=GNLConn.Genlmsghdr(self.CTRL_CMD_GETFAMILY, 1).pack() +
                    GNLAttr(GNLAttr.TYPE_FAMILY_NAME, name).pack()
            ))
        except:
        ...
        for m in msgs:
            ...
            msg = GNLConn.Genlmsghdr.unpack(m.data)
            name = None
            pos = msg.struct.size
            ...
            cmds = []
            # print(GNLAttr.list(attrs.get(GNLAttr.TYPE_OPS, b'\x04\x00\x00\x00')))
            for attr in GNLOps.list(attrs.get(GNLAttr.TYPE_OPS, b'\x04\x00\x00\x00')):
        ...
        with RNLConn() as conn:
            ...
            for m in msgs:
                ...
                while pos < len(m.data):
                    attr = IFLAttr.unpack(m.data[pos:]) 
                    pos += attr.size
...
class TCSKBAttr(NLAttr):
    ...
    class _skb_params_struct():
        struct = Struct("2I3i")
        @classmethod
        def pack(cls, obj): # type: (TCSKBAttr._params) -> bytes
            return cls.struct.pack(*obj)
        @classmethod
        def unpack(cls, data): # type: (bytes) -> TCSKBAttr._params
            return TCSKBAttr._params(*cls.struct.unpack(data))
...
class TCtool(object):
        ...
        with RNLConn() as conn:
            ...
            for m in msgs:
                ...
                if msg.index != self.ifindex:
                    continue
                kind = ''
                for attr in TCAttr.list(m.data[msg.struct.size:]):
    ...
    def qdisc_add(self, parent=None, kind=None, pmap=None, count=None, offset=None):
        ...
        with RNLConn() as conn:
            ...
            msg.data = msg.data + TCAttr(TCAttr.TYPE_KIND, kind).pack()
            if kind == 'mqprio':
                ...
                msg.data = msg.data + TCAttr(
                    ...
                    TCMQAttr(TCMQAttr.TYPE_MODE, TCMQAttr.MODE_CHANNEL).pack()
                ).pack()
            conn.send(msg)
            if self.log:
    ...
    def _filter_list(self, parent):
        ...
        with RNLConn() as conn:
            msgs = conn.send(NLMessage(
                ...
                flags=NLMessage.FLAG_REQUEST | NLMessage.FLAG_DUMP,
                data=self.TCmsg(socket.AF_UNSPEC, self.ifindex, 0, parent, 0).pack()
            ))
            results = []
            for m in msgs:
                ...
                for attr in TCAttr.list(m.data[msg.struct.size:]):
                    if attr.type == TCAttr.TYPE_KIND:
                        ...
                        if proto == 'tcp':
                            dst_port = attrs.get(TCFLAttr.TYPE_KEY_TCP_DST, None)
                            src_port = attrs.get(TCFLAttr.TYPE_KEY_TCP_SRC, None)
                        elif proto == 'udp':
                            dst_port = attrs.get(TCFLAttr.TYPE_KEY_UDP_DST, None)
                            src_port = attrs.get(TCFLAttr.TYPE_KEY_UDP_SRC, None)
                        else:
                            dst_port = None
                            src_port = None
                        if TCFLAttr.TYPE_ACT in attrs:
                            ...
                            if actions[TCAttr.TYPE_KIND] == "skbedit":
                                action = 'skbedit'
                                options = TCSKBAttr.dict(actions[TCAttr.TYPE_OPTIONS])
    ...
    def filter_add(self, direction='ingress', prio=1, proto='tcp', 
        ...
        with RNLConn() as conn:
            ...
            if direction == 'ingress':
                parent = self._tc_h_make(self.TC_H_CLSACT, self.TC_H_MIN_INGRESS)
            elif direction == 'egress':
                parent = self._tc_h_make(self.TC_H_CLSACT, self.TC_H_MIN_EGRESS)
            else:
            ...
            if src_addr is not None:
                ...
                options.append((TCFLAttr.TYPE_KEY_IPV4_DST_MASK, mask))
            if proto.lower() == 'tcp':
                if src_port is not None:
                    options.append(TCFLAttr(TCFLAttr.TYPE_KEY_TCP_SRC, src_port))
                if dst_port is not None:
                    options.append(TCFLAttr(TCFLAttr.TYPE_KEY_TCP_DST, dst_port))
            elif proto.lower() == 'udp':
                if src_port is not None:
                    options.append(TCFLAttr(TCFLAttr.TYPE_KEY_UDP_SRC, src_port))
                if dst_port is not None:
                    options.append(TCFLAttr(TCFLAttr.TYPE_KEY_UDP_DST, dst_port))
            flags = 0
            if direction == 'ingress' and tc is not None:
                ...
                options.append(TCFLAttr(TCFLAttr.TYPE_ACT, TCAction(1, 
                    TCAttr(TCAttr.TYPE_KIND, action).pack() + 
                    TCAttr(TCAttr.TYPE_OPTIONS, 
                        TCSKBAttr(TCSKBAttr.TYPE_PRIORITY, priority).pack() + 
                        TCSKBAttr(TCSKBAttr.TYPE_PARMS, 
                            TCSKBAttr._params(0, 0, TCSKBAttr.ACT_PIPE, 0, 0)).pack()
                    ).pack()
                ).pack()))
            ...
            msg = NLMessage(
                ...
                data=self.TCmsg(
                    ...
                    self._tc_h_make(prio << 16, socket.htons(NLConn.ETH_P_IP))
                    ).pack() 
                    + TCAttr(TCAttr.TYPE_KIND, 'flower').pack() 
                    + TCAttr(TCAttr.TYPE_OPTIONS, data).pack()
            )
            conn.send(msg)
            if self.log:
                # send simulated commands to the log
        ...
        with GNLConn() as conn:
            msgs = conn.send(NLMessage(
                ...
                data=GNLConn.Genlmsghdr(
                    ...
                    DLAttr(DLAttr.TYPE_BUS_NAME, "pci").pack() + 
                    DLAttr(DLAttr.TYPE_DEV_NAME, self.device).pack()
            ))
            for m in msgs:
                if m.type != self._fid:
                    continue
                hdr = GNLConn.Genlmsghdr.unpack(m.data)
                if hdr.cmd != self.CMD_INFO_GET:
                    continue
                return DLAttr.dict(m.data[hdr.struct.size:])
        ...
        with GNLConn() as conn:
            ...
            msgs = conn.send(NLMessage(
                ...
                data=GNLConn.Genlmsghdr(
                    ...
                    DLAttr(DLAttr.TYPE_BUS_NAME, "pci").pack() + 
                    DLAttr(DLAttr.TYPE_DEV_NAME, self._device).pack() + 
                    DLAttr(DLAttr.TYPE_PARAM_NAME, name).pack() + 
                    DLAttr(DLAttr.TYPE_PARAM_VALUE_CMODE, 
                        DLAttr.PARAM_CMODE_RUNTIME).pack() 
            ))
            for m in msgs:
                if m.type != self._fid:
                    continue
                ...
                if hdr.cmd != self.CMD_PARAM_GET:
                    continue
                attr = DLAttr.dict(m.data[hdr.struct.size:])
                if DLAttr.TYPE_PARAM in attr:
                    ...
                    if attrs.get(DLAttr.TYPE_PARAM_NAME, None) == name:
                        ...
                        if DLAttr.TYPE_PARAM_VALUES_LIST not in attrs:
                            raise Exception("devlink param value format missing list")
                        ptype = attrs[DLAttr.TYPE_PARAM_TYPE]
                        pvlist = DLAttr.dict(attrs[DLAttr.TYPE_PARAM_VALUES_LIST]) 
                        pvalue = DLAttr.dict(pvlist[DLAttr.TYPE_PARAM_VALUE])
                        # print(ptype, pvlist, pvalue)
                        if ptype == DLAttr.MNL_TYPE_FLAG:
                            value = DLAttr.TYPE_PARAM_VALUE_DATA in pvalue
                        elif ptype in ptype_struct:
                            # print(ptype_struct[ptype], pvalue[DLAttr.TYPE_PARAM_VALUE_DATA])
                            value = unpack(
                                ptype_struct[ptype], pvalue[DLAttr.TYPE_PARAM_VALUE_DATA]
                            )[0]
            ...
            if value is not None and modify is not None and modify != value:
                ...
                msg = NLMessage(
                    ...
                    data=GNLConn.Genlmsghdr(
                        ...
                        DLAttr(DLAttr.TYPE_PARAM_VALUE_CMODE, 
                            DLAttr.PARAM_CMODE_RUNTIME).pack() + 
                        DLAttr(DLAttr.TYPE_PARAM_TYPE, ptype).pack()
                )
                ...
                conn.send(msg)
                self._log("devlink dev param set pci/%s name %s value %s cmode runtime" % 
...
class Config(object):
    ...
    def __init__(self, source=None, log=None, verbose=False): # type: (any, str, any, bool) -> None
        '''
    ...
    def _cleanup(self):
        ...
        try:
            ...
            # _devlink_param(self.globals.dev, 'num_qps_per_poller', 0, log=self.log)
        ...
        self._print("- cleared any flow filters")
        settings.arp_announce = 0
        settings.arp_ignore = 0
        settings.arp_notify = 0
        settings.apply()

    def _set_tcs(self): # type(bool, bool) -> None
        ...
        if self._filters and not any([t.kind == 'clsact' for t in tc.qdisc_list()]):
        ...
        if self._filters:
            self._printhead("creating traffic filters")
        ...
        for name, section in self:
            if name != 'globals':
                if section.mode == 'exclusive' and self.settings.flow_director is None:
                    devlink = Devlink(self.globals.dev, self.log)
                    devlink.param('tc%d_inline_fd' % tc_idx, True)
                    # _devlink_param(
            ...
            for f in filters['egress']:
                ...
                for n, v in f._asdict().items():
                    if v is not None:
                        ...
                        params.append("%s %s" % (n, str(v)))
                self._print("  %s" % ' '.join(params))
        if sideband and self._filters:
            self._print("- flow filters:")
            for n in ntuples:
                self._print(
                    "  flow-type %s dst-port %d -> queue %d" % 
                    (n['flow-type'].lower(), n['dst-port'], n['action'])
                )
    ...
    def _set_affinity(self): # type () -> None
        self._printhead("setting interrupt affinity")
        self._log("## queue affinity ##")
        # get nic interrupts
        ...
        for i, irq in enumerate(irqs):
            ...
            self._log("echo %s > /proc/irq/%d/smp_affinity" % (mask, irq))
        # display setup
        self._print("- affinitized %d interrupts" % len(irqs))
    ...
    def _set_symmetry(self): # type () -> None
        ...
        for i in range(queues):
            ...
            self._log("echo %s > /sys/class/net/%s/queues/tx-%d/xps_rxqs" % (mask, self.globals.dev, i))
        for i in range(queues):
            _writefile("/sys/class/net/%s/queues/tx-%d/xps_cpus" % (self.globals.dev, i), '0')
            self._log("echo 0 > /sys/class/net/%s/queues/tx-%d/xps_cpus" % (self.globals.dev, i))
        # display setup
        self._print("- aligned %d queues" % queues)

    def _set_pollers(self): # type () -> None
        self._printhead("enabling independent pollers")
        self._log("## independent pollers ##")
        tc_idx = 0
        for name, section in self:
            if name != 'globals' and section.pollers > 0:
                ...
                try:
                    devlink = Devlink(self.globals.dev, self.log)
                    devlink.param('tc%d_qps_per_poller' % tc_idx, num_queues)
                    devlink.param('tc%d_poller_timeout' % tc_idx, int(section.poller_timeout))
                except:
                    raise Exception("current driver or kernel does not support pollers")
                # _devlink_param(
...
def check_depends(log=None, verbose=False): # type (any, bool) -> None
    '''
    ...
    if len(needed):
        ...
        _exec(["yum", "-y", "install"] + needed, log=log, echo=verbose)
...
def check_services(log=None, verbose=False): # type (any, bool) -> None
    ...
    if _exec(['systemctl', 'status', 'irqbalance'], check=True, echo=verbose):
        print('- warning: irqbalance is installed and running - this may impact performance')
        time.sleep(2)


def reload_driver(driver=None, log=None, verbose=False): # (str, bool, any, bool) -> None
    '''
...
# def check_driver(dev, log=None, verbose=False): # type (str, any, bool) -> None
#     '''
...
def check_interface(dev, log=None, verbose=False): # type (str, any, bool) -> None
    '''
    ...
    try:
        if args.command in ['apply', 'persist']:
            ...
            while len(params):
                ...
                group[n] = v
            config = Config({name: group}, args.log, args.verbose)
