...
class RXnfcFlowAddr(StructTempl, namedtuple("RXnfcFlowExt",
...
class Ethtool(object):
    ...
    def ntuple_add(self, proto='tcp', src_port=0, dst_port=0, action=0, id=None):
        ...
        self._log("ethtool --config-ntuple %s flow-type %s4 dst-port %d action %d" % 
...
class NLAttr(namedtuple("NLAttr", "type data")):
    ...
    def _from(cls, data, offset=0): # type: (bytes|bytearray, int) -> NLAttr
        ...
        # print(type, length, '%r' % data[offset:offset+length])
    ...
    def list(cls, data, offset=0): 
        ...
        results = []
        # print(offset, data[offset:offset+32])
        while offset < len(data):
            attr = cls._from(data, offset)
            # print(attr, data[offset:offset+attr.size])
            results.append(attr)
            offset += attr.size
            # print(offset, data[offset:offset+32])
        return results
    ...
    def qdisc_add(self, parent=None, kind=None, pmap=None, count=None, offset=None):
        ...
        with RNLConn() as conn:
            ...
            msg = NLMessage(
                ...
                data=self.TCmsg(socket.AF_UNSPEC, self.ifindex, handle, parent, 0)._bytes
            )
    ...
    def _filter_list(self, parent):
        ...
        with RNLConn() as conn:
            msgs = conn.send(NLMessage(
                ...
                flags=NLMessage.FLAG_REQUEST | NLMessage.FLAG_DUMP,
                data=self.TCmsg(socket.AF_UNSPEC, self.ifindex, 0, parent, 0)._bytes
            ))
            results = []
            for m in msgs:
                ...
                options.append(TCFLAttr(TCFLAttr.TYPE_ACT, TCAction(1, 
                    ...
                    TCAttr(TCAttr.TYPE_OPTIONS, 
                        ...
                        TCSKBAttr(TCSKBAttr.TYPE_PARMS, 
                            TCSKBAttr._params(0, 0, TCSKBAttr.ACT_PIPE, 0, 0))._bytes
                    )._bytes
                )._bytes))
...
class Config(object):
        ...
        for name, section in self:
            if name != 'globals' and section.mode == 'shared':
                ...
                if self._filters:
                    ...
                    for _, port in enumerate(section.ports):
                        self.settings.ethtool.ntuple_add(
                            proto=section.protocol,
                            dst_port=port, action=queue_idx
                        )
                        # ntuples.append({
    ...
    def _set_affinity(self): # type () -> None
        ...
        for i, irq in enumerate(irqs):
            ...
            _writefile("/proc/irq/%d/smp_affinity_list" % irq, cpu)
            self._log("echo %s > /proc/irq/%d/smp_affinity_list" % (cpu, irq))
        # display setup
