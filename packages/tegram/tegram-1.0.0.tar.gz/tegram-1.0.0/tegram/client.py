...
import tegram
from tegram import __version__, __license__
from tegram import enums
from tegram import raw
from tegram import utils
from tegram.crypto import aes
from tegram.errors import CDNFileHashMismatch
from tegram.errors import (
    SessionPasswordNeeded,
    VolumeLocNotFound, ChannelPrivate,
    AuthBytesInvalid, BadRequest
)
from tegram.handlers.handler import Handler
from tegram.methods import Methods
from tegram.session import Auth, Session
from tegram.storage import FileStorage, MemoryStorage
from tegram.types import User, TermsOfService
from tegram.utils import ainput
...
class Client(Methods):
    """tegram Client, the main means for interacting with Telegram.
    ...
    Parameters:
        ...
        in_memory (``bool``, *optional*):
            ...
            :meth:`~tegram.Client.export_session_string` before stopping the client to get a session string you can
        ...
        workdir (``str``, *optional*):
            ...
            The working directory is the location in the filesystem where tegram will store the session files.
    ...
    APP_VERSION = f"tegram {__version__}"
    ...
    async def authorize(self) -> User:
        ...
        print(f"Welcome to tegram (version {__version__})")
        print(f"tegram is free software and comes with ABSOLUTELY NO WARRANTY. Licensed\n"
    ...
    def set_parse_mode(self, parse_mode: Optional["enums.ParseMode"]):
        ...
        Parameters:
            parse_mode (:obj:`~tegram.enums.ParseMode`):
        ...
        if session_empty:
            if not self.api_id or not self.api_hash:
                raise AttributeError("The API key is required for new authorizations. "
                                     "More info: https://docs.tegram.org/start/auth")
    ...
    async def handle_download(self, packet):
        ...
        try:
            async for chunk in self.get_file(file_id, file_size, 0, 0, progress, progress_args):
                file.write(chunk)
        except tegram.StopTransmission:
            if not in_memory:
                file.close()
                os.remove(file.name)
