...
import tegram
from tegram.methods.utilities.idle import idle
...
class Run:
    def run(
        ...
        :meth:`~tegram.Client.start`, :meth:`~tegram.idle` and :meth:`~tegram.Client.stop` in sequence.
        ...
        operation. This is almost the same as :py:obj:`asyncio.run` except for the fact that tegram's ``run`` uses the
        current event loop instead of a new one.

        If you want to run multiple clients at once, see :meth:`tegram.compose`.
