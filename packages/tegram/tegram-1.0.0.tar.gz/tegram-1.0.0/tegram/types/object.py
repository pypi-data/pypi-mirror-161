...
class Object:
    def __init__(self, client: "tegram.Client" = None):
        self._client = client

    def bind(self, client: "tegram.Client"):
        """Bind a Client instance to this and to all nested tegram objects.

        Parameters:
            client (:obj:`~tegram.types.Client`):
                The Client instance to bind this object with. Useful to re-enable bound methods after serializing and
                deserializing tegram objects with ``repr`` and ``eval``.
        """
        self._client = client
