...
class SentCode(Object):
    ...
    Parameters:
        type (:obj:`~tegram.enums.SentCodeType`):
            Type of the current sent code.

        phone_code_hash (``str``):
            Confirmation code identifier useful for the next authorization steps (either
            :meth:`~tegram.Client.sign_in` or :meth:`~tegram.Client.sign_up`).

        next_type (:obj:`~tegram.enums.NextCodeType`, *optional*):
            Type of the next code to be sent with :meth:`~tegram.Client.resend_code`.

        timeout (``int``, *optional*):
            Delay in seconds before calling :meth:`~tegram.Client.resend_code`.
    """
