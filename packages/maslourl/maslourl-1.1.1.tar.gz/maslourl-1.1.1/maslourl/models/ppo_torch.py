...
class MaslouPPOAgent(ABC):
    ...
    def __init__(self, args=None, discrete=True, eval=False):
        if not eval and args is None:
            raise ValueError("If agent is in train state then u nust give args, else put eval=True")
        self.eval = eval
        if not self.eval:
            self.args = args
            ...
            self.env = self.make_env("", 42, 0, False, 0, "test")
    ...
    def save_agent(self):
        if not os.path.exists("./models/"):
            os.makedirs("./models/")
        torch.save(self.agent, "models/model.pt")
        if self.args.track and self.args.save_best_to_wandb:
            if self.args.verbose:
                print("Saving file to w&b", os.path.join(wandb.run.dir, "models/model.pt"))
            if not os.path.exists(os.path.join(wandb.run.dir, "models/")):
                os.makedirs(os.path.join(wandb.run.dir, "models/"))
            torch.save(self.agent, os.path.join(wandb.run.dir, "models/model.pt"))
    ...
    def train(self):
        if self.eval:
            raise ValueError("The agent is in eval state")
