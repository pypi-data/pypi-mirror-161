...
class Covid19:
    ...
    def province_covid_data(self, province_name='北京', show_timeline: int = 0):
        ...
        if province_name == 'auto':
            ...
            if country_name != '中国':
                raise CovidException(
                    f'目前只支持获取中国的详细数据，然而你当前的所在地为{country_name}。')
            if place['provinceName'] != 'Failed':
                province_name = place['provinceName']
            else:
                raise CovidException(f'获取地区失败，请检查网络连接！')
        cities = []
        data = {}
        c_data = self.c_data
        timeline_url = ''
    ...
    def danger_areas_data(self, city_name=None):
        ...
        if city_name == 'auto':
            ...
            if country_name != '中国':
                raise CovidException(
                    '仅支持获取中国的中高风险地区，然而你当前所在的国家是' + country_name)
            if province_name in ['香港', '澳门', '台湾']:
                raise CovidException(
                    '由于防疫政策不同，目前不支持获取港澳台的疫情数据，然而你当前的所在地是' + province_name)
