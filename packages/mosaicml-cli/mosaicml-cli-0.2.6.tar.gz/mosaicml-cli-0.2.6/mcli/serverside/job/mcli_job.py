...
from mcli.models import IntegrationType, MCLIEnvVar, MCLIIntegration, MCLIPlatform, RunInput
...
class MCLIJob():
    ...
    def build_docker(self, status: Optional[Status] = None):
        """Build docker container for integrations that require it
        """
        for integration in self.integrations:
            if integration.integration_type == IntegrationType.local:
                integration.build_to_docker(self.image, status=status)
