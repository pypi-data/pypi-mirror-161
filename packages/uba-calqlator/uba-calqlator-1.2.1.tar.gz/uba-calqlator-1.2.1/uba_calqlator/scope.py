...
def _get_time_series_data(inventory_id: str, data_frame: DataFrame) -> dict:
    ...
    def get_data_points(data_frame: DataFrame) -> dict:
        unit = data_frame.name[0]
        resolution_time_unit = data_frame.name[1]
        resolution_factor = data_frame.name[2]
        data_points = [{ 'timestamp': item[0].strftime(_TIMESTAMP_FORMAT), 'value': item[1] } for item in zip(data_frame.index, data_frame.value)]
        return {
    ...
    def get_data(data_frame: DataFrame) -> Series:
        data = (data_frame
            .groupby(['unit', 'resolution.timeUnit', 'resolution.factor'])
            .apply(get_data_points)
            .reset_index()
