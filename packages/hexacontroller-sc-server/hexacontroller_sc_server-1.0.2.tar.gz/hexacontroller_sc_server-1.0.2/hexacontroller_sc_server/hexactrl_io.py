...
class Xil_i2c():
    ...
    def __init__(self, bus_id):
        ...
        *Parameter*
        bus_id
          Bus id to start the SMBus module with, occupies the hardware
    ...
    def write(self, address, data: Union[int, bytearray]):
        ...
        *Parameters*
        address
          The address of the peripheral on the bus to write data to,
          must be 0-127

        data
          The data to be sent to the peripheral. If an `int` is passed a single
          transaction is perfomed, if a `bytearray` is passed then the entire
          data is written as a single i2c transaction (aka frames)
        """
    ...
    def read(self, address: int, count: int = 1):
        ...
        *Parameters*
        address
          The address of the peripheral on the bus to read data from,
          must be 0-127

        count
          The number of bytes to read form the bus if the count is larger
          than one a block read is performe.

        *Returns*
        data
          array of the values read for the different bytes as list of ints
        """
        if count == 1:
    ...
    def scan(self):
        ...
        *Returns*
        ``scan_responses: list(int)``
          a list of addresses that responded to the read attempt
        """
        scan_responses = []
...
class Xil_gpio():
    ...
    def __init__(self, chip, pin, mode):
        ...
        *Parameters*
        chip: str
          The name of the GPIO driver that the pin is connected to
        pin: str
          The name of the Pin that should be driven. The name is registered
          in the Device tree that linux uses at boot
        mode: str
          either ``input`` or ``output``, determins the direction of the GPIO.
          When ``input`` is set the write action raises an Error

        *Notes*
        The mode pesists over the lifetime of the object
        ...
        self.transaction_logger.info("Initializing pin")
    ...
    def write(self, val):
        ...
        *Parameters*
        val:
          either ``0`` to set the pin to logic *LOW* or ``1`` to set the
          pin to logic *HIGH*
        """
        if val not in [0, 1]:
            raise ValueError(
    ...
    def read(self):
        ...
        *Returns*
        state:
          0 if the pin is logical LOW and 1 if the pin is set/pulled to HIGH
        """
