...
from ..exceptions import ArgumentCombinationError, InstanceTypeError
from ..utils import (
    COILED_LOGGER_NAME,
...
class ClusterBeta(Cluster, Generic[IsAsynchronous]):
    ...
    def __init__(
        ...
        *,
        ...
        self.package_sync_only = (
            package_sync if isinstance(package_sync, list) else None
        )
            ...
            if self.package_sync:
                ...
                if not self.package_sync_only:
                    ...
                    for critical_package in critical_packages:
                        if critical_package not in [
                            p["name"] for p in [*env[0], *env[1]]
                        ]:
                            halting_failures.append(critical_package)
                            self.auto_env_issues[critical_package] = (
                                {
                                    ...
                                    "specifier": "n/a",
                                    "include": False,
                                    "issue": "Could not detect package locally",
                                },
                                100,
                            )
                ...
                for pkgInfo, level in self.auto_env_issues.values():
                    if level > self.package_sync_fail_on:
                        halting_failures.append(pkgInfo["name"])
                    if level >= 100:
                ...
                if self.auto_env_issues and HAS_RICH and self.show_widget:
                    from .widgets.rich import print_rich_package_warning_table

                    print_rich_package_warning_table(pkg_warnings=self.auto_env_issues)
                if halting_failures:
                    failure_str = ", ".join([pkg for pkg in halting_failures])
                    raise RuntimeError(
                        f"Could not find valid install candidates for {failure_str}, check stdout for more details"
                    )
