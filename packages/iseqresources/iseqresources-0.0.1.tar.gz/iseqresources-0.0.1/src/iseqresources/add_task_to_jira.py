...
class AddTaskToJira:
    ...
    def __init__(self, tool: dict, email: str, token: str, jira_project_info: dict):
        self.tool = tool
        self.jira_project_info = utils.load_json(jira_project_info)
        self.email = email
        self.token = token
        self.server = self.jira_project_info["server"]
        self.epic_id = self.jira_project_info["epic_id"]
        self.project_key = self.jira_project_info["project_key"]
