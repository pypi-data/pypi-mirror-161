...
def country_code_list(country_name):
    ...
    '''
    json_data = {}
    file_path = 'worldstock/dataset/yahoo_country_code.json'
...
def refine_dataset_maker(country_name):
    handwork_dataset = pd.read_csv('worldstock/dataset/handwork_dataset/' + country_name + '_handwork_name_dataset.csv',encoding='utf-8-sig') # 또는 DB에서 로드
    handwork_dataset = handwork_dataset[['symbol','refine_name']]
    # us를 입력받아 us_name_dataset 가져오기
    name_dataset = pd.read_csv('worldstock/dataset/' + country_name + '_name_dataset.csv',encoding='utf-8-sig') # 또는 DB에서 로드
    # refine
    ...
    name_dataset.to_csv('worldstock/dataset/' + country_name + '_refine_name_dataset.csv', encoding='utf-8-sig', index = False )
    return name_dataset