...
def read_registers(reg_cfg, slave):
    global regMaskRecords
    global startDateTime

    # Read the register
    try:
        reg_val = int(slave.read_register(reg_cfg['reg_address']))
    except IOError:
        printLog('Error: Failed to read ' + reg_cfg['serial_port'] + ' ' + '{0:#0{1}x}'.format(reg_cfg['slave_address'], 4) + ' ' + '{0:#0{1}x}'.format(reg_cfg['reg_address'], 6))
        return

    # Find all register masks with a matching address
    for idx, mask_cfg in enumerate(cfg['register_masks']):
        try:
            if mask_cfg['address'] == reg_cfg['reg_address']:
                ...
                temp_val = reg_val & mask_cfg['mask']
                ...
                val = float(temp_val) / (10 ** mask_cfg['decimals'])
...
def main():
    ...
    printLog('Opening serial ports...')

    # Schedule each Modbus register read with delays based on configuration
    for reg_cfg in cfg['registers']:
        # Open the serial port and set the slave address
        try:
            slave = minimalmodbus.Instrument(reg_cfg['serial_port'], reg_cfg['slave_address'])
            slave.serial.baudrate = reg_cfg['serial_baud']
            slave.serial.timeout = float(reg_cfg['query_timeout']) / 1000
        except Exception:
            printLog('Error: Failed to open ' + reg_cfg['serial_port'] + ' ' + '{0:#0{1}x}'.format(reg_cfg['slave_address'], 4))
            continue

        timer = RepeatTimer(float(reg_cfg['query_period']) / 1000, read_registers, [reg_cfg, slave])
        timer.daemon = True
        timer.start()
