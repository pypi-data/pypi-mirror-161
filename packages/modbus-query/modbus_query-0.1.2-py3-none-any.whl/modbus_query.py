...
def read_registers(reg_addr):
    ...
    # Read the register
    try:
        val = slave.read_register(reg_addr)
    except IOError as e:
        printLog('Error when reading register address ' + '{0:#0{1}x}'.format(reg_addr, 6) + ': ' + e.args[0])
        return
...
def main():
    ...
    if not os.path.exists(os.path.join(userDirectory, 'config.yaml')):
        shutil.copyfile('config.yaml', os.path.join(userDirectory, 'config.yaml'))
    ...
    writeCSV('Time (s)', csv)
    printLog('Created file: ModbusQueryData\\' + dataFileName)
    ...
    for reg_cfg in cfg['registers']:
        schedule.every(reg_cfg['period']).seconds.do(read_registers, reg_cfg['address'])
