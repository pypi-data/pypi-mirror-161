...
class DockerBuildContext(_AbstractRunConfigElement):
    ...
    def from_local_directory(workspace, path, dockerfile_path="Dockerfile"):
        ...
        blob_service_client = BlockBlobService(account_name=storage_account, account_key=storage_account_key)
        ...
        for relative_path, source_path in files.items():
            ...
            blob_service_client.create_blob_from_path(_DOCKER_BUILD_CONTEXT_CONTAINER, target_path, source_path)

        location = blob_service_client.make_blob_url(_DOCKER_BUILD_CONTEXT_CONTAINER, path_prefix + "/")
