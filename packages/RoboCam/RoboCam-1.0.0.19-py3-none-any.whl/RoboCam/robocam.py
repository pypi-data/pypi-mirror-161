...
class RoboCam():
    ...
    def TrainFaceData(self, path:str =pkg_resources.resource_filename(__package__,"res/face/")):
        if os.path.isdir(path) is False:
        ...
        for filename in filenames:
            ...
            image = cv2.imread(path + filename, cv2.IMREAD_ANYCOLOR)
    ...
    def DeleteFaceData(self, name:str, path:str=pkg_resources.resource_filename(__package__,"res/face/")):
        if os.path.isdir(path) is False:
            print(path +" is not directory.")
            return

        self.__faceR.RemoveFace(name, path)
    ...
    def GetArucoRectPoint(self,id:int) -> list:
        ret = []
        if self.__arucoDetectedIds is not None and id in self.__arucoDataDict:
            ret = self.__arucoDataDict[id].corners
        return ret
    ...
    def GetRecognizedNumberPoint(self)->list:
        if self.__numberDetectedList is not None:
