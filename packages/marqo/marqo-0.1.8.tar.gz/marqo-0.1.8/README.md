...
responses = mq.index("my-multimodal-index").add_documents([{
...
results = mq.index("my-multimodal-index").search('spaceman')
...
```python
results = mq.index(index_name).search('https://api.claylings.io/api/image/190', searchable_attributes=['Image'])
```
