    ...
    def save_avi(self, seconds =  1, name = "" ):
        ...
        if seconds == 0:
            ...
            if not self.zmqsender is None:
                ...
                if (telegramnow-self.telegramlast).total_seconds()>20:
                    y = threading.Thread(target=telegram.bot_send, args=("ALERT_NPI", f"CAMERA {self.zmqrpi_name} {zmqnow.strftime('%a %H:%M:%S')}", self.frame))
                    y.start()
