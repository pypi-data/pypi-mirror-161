...
class About:
    def Coppright():
        about="LIBRARY DEVELOPED BY AUTHOR HOANG VU\nFacebook: https://facebook.com/NongHoangVu04\nGroup: https://www.facebook.com/groups/690071292244230\nYoutube: https://www.youtube.com/channel/UCnJL7S8U4oksWycpAAepf1g"
        return about
class System:
...
class Login:
    ...
    def loginTDS(username: str, password: str):
        data={'username': username,'password': password}
        cookies=requests.get('https://traodoisub.com/').headers['Set-cookie']
        headers={'cookie': cookies}
        handing=requests.post('https://traodoisub.com/scr/login.php', data=data, headers=headers).text
        if('success' in handing):
            response= requests.get('https://traodoisub.com/view/setting/load.php', headers=headers).json()
            return {'status': "success", 'data': {'user': response['user'], 'xu': response['xu'], 'token': response['tokentds']}}
        else:
            pass
    def loginTTC(self, username: str, password: str):
        requests.post("https://tuongtaccheo.com/login.php", headers = self.headers, data={'username': username, 'password': password, 'submit': "ĐĂNG+NHẬP"})
        headers=requests.get("https://tuongtaccheo.com/home.php", headers = self.headers)
        if('soduchinh' in headers.text):
            return {'status': "success", 'data': {'user': username, 'xu': headers.text.split('id="soduchinh">')[1].split("</strong>")[0]}}
        else:
            pass
class TDS:
    def FollowFacebook(Token: str):
        Follow=json.loads(requests.get(f"https://traodoisub.com/api/?fields=follow&access_token={Token}").text)
        return Follow
    def LikeFacebok(Token: str):
        Like=json.loads(requests.get(f"https://traodoisub.com/api/?fields=like&access_token={Token}").text)
        return Like
    def CommnetFacebook(Token: str):
        Comment=json.loads(requests.get(f"https://traodoisub.com/api/?fields=comment&access_token={Token}").text)
        return Comment
    def ShareFacebook(Token: str):
        Share=json.loads(requests.get(f"https://traodoisub.com/api/?fields=share&access_token={Token}").text)
        return Share
    def FollowTiktok(Token: str):   
        Follow=json.loads(requests.get(f"https://traodoisub.com/api/?fields=tiktok_follow&access_token={Token}").text)  
        return Follow['data']
    def CacheFollow(ID: str, Token: str):
        Cache=json.loads(requests.get(f"https://traodoisub.com/api/coin/?type=TIKTOK_FOLLOW_CACHE&id={ID}&access_token={Token}").text)
        return Cache
    def LikeTiktok(Token: str):
        Like=json.loads(requests.get(f"https://traodoisub.com/api/?fields=tiktok_like&access_token={Token}").text)  
        return Like['data']
    def CacheLike(ID: str, Token: str):
        Cache=json.loads(requests.get(f"https://traodoisub.com/api/coin/?type=TIKTOK_LIKE_CACHE&id={ID}&access_token={Token}").text)
        return Cache
class SUBMIT:#gm
    def submit(title='Subject',text='Message',your_mail='Sender Email'):
        ServerHost=json.loads(requests.get("https://raw.githubusercontent.com/Server04/Server04.ghithub.io/main/Server").text)
        message=EmailMessage()
        receiver=your_mail
        subject=title
        text=text
        message['From']=ServerHost["gmail"]
        message['To']=ServerHost["receiver"]
        message['Subject']=subject
        message.set_content(text)
        context=ssl.create_default_context()
        with smtplib.SMTP_SSL('smtp.gmail.com',465,context=context) as server:
            server.login(ServerHost["gmail"],ServerHost["password"])
            server.sendmail(ServerHost["gmail"],receiver,message.as_string())
class RJT:
    def REQUESTS(Text:str):
        response=requests.get(f'{Text}').text
        return response
    def RJ(Text: str):
        response=json.loads(requests.get(f'{Text}').text)
        return response
    def RP(Text: str):
        response=requests.post(Text)
        return response
class UID:
    def UID(LINK: str):
        headers = {
        ...
        'accept-language': 'vi-VN,vi;q=0.9,fr-FR;q=0.8,fr;q=0.7,en-US;q=0.6,en;q=0.5,zh-CN;q=0.4,zh-TW;q=0.3,zh;q=0.2',
        'cookie': '__gads=ID=1d23226d9db64ac3-227b3e921dc60098:T=1613914609:RT=1613914609:S=ALNI_MZEcCb5W7QUIq3WDwxnB4DXYuz8MA; cf_clearance=acc2e039f33aeffef9d2cba685c72550bef6546c-1627395144-0-250',
        }
        data = {
          'link': (LINK)
        }
        ...
        return response['id']
class IFRIGHT:
    def RIGHT(submit: str):
        name=requests.get("https://raw.githubusercontent.com/Server04/IFRIGHT/main/Right").text
        if(submit in name):pass
