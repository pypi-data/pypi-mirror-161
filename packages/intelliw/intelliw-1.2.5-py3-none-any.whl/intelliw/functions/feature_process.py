...
LastEditTime: 2022-07-29 17:20:47
LastEditors: Hexu
Description: 按列的特征处理
...
def feature_process(data, config: list, stage: str) -> Iterable:
    ...
    if processor_mode == "infer":
        ...
        if target_data is not None:
            ...
            logger.info(
                "set new target data and final target col add to param")
        ...
        new_config = json.dumps(
            new_config, ensure_ascii=False, cls=JsonEncoder)
...
def bucket(params, col_frame, mode):
    if not ('boundaries' in params and len(params['boundaries']) > 0):
        raise FeatureProcessException("for bucket transform ,boundaries can't be empty!")
    bins = [float('-inf')]
