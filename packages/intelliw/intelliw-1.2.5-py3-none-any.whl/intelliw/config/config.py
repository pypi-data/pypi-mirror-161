...
# 数据集相关
SOURCE_TYPE = 3  # 输入数据源类型，0 空，1 远程 csv，2 智能分析，3 本地 csv， 4 图片数据源， 5 数据工场,  6 nlp语料
INPUT_ADDR = ''
INPUT_GETROW_ADDR = ''
INPUT_MODEL_ID = ''
INPUT_DATA_SOURCE_ID = ''
# cv: 0-自有 1-labelme 2-voc 3-coco  nlp: 1-txt 2-csv 3-json
...
# nlp语料存储文件名
NLP_CORPORS_PATH = ''     # 本地nlp数据源路径
NLP_CORPORS_FILEPATH = "tmp_local_nlp_corpora_data/"
NLP_CORPORS_TRAIN_FILEPATH = os.path.join(NLP_CORPORS_FILEPATH, "train/")
NLP_CORPORS_VAL_FILEPATH = os.path.join(NLP_CORPORS_FILEPATH, "val/")
NLP_CORPORS_TEST_FILEPATH = os.path.join(NLP_CORPORS_FILEPATH, "test/")
...
def set_dataset_config(dataset_id: str = None, dataset_ratio: list = [0.7, 0.3, 0], loacl_csv: str = None, loacl_corpus: str = None):
    ...
    if not dataset_id:
        if loacl_csv:
            ...
            setattr(module, "NLP_CORPORS_PATH", loacl_corpus)
    else:
