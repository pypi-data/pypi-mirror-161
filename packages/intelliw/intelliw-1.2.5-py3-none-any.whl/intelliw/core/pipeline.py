...
class Pipeline:
    ...
    def _feature_helper(self, stage, data):
        ...
        if params is not None:
            return functions.feature_process(data, params, stage)
        return data
    ...
    def _get_train_kwargs(self, reader, column_meta):
        ...
        # target
        final_target_col, target_cols, target_metadata = gl.pop(
            "final_target_col"), gl.pop("target_cols"), gl.pop("target_metadata")
        if target_cols is None and target_metadata is not None and len(target_metadata) > 0:
            target_cols = [
                {"col": m["target_col"], "column_name":m["target_name"]} for m in target_metadata]
    ...
    def train(self, datasets: DataSets):
        ...
        try:
            ...
            if len(varnames) == 2 and "kwargs" not in varnames:
                train_func(self.instance.train_set, self.instance.valid_set)
            elif "kwargs" in varnames:
                kwargs = self._get_train_kwargs(
                    train_set, datasets.column_meta)
                if len(varnames) == 3:
                    train_func(self.instance.train_set,
                               self.instance.valid_set, **kwargs)
                elif len(varnames) == 1:
                    train_func(**kwargs)
                else:
    ...
    def _raise_exception(self, msg: message.CommonResponse, e: Exception = None, is_farmeworl_error=True):
        raise_logger = logger if is_farmeworl_error else get_user_logger()

        # 用于传递错误给prepare_env.sh
        err_stack = "\n"
        if isinstance(e, (exception.DatasetException, exception.DataSourceDownloadException, exception.FeatureProcessException)):
            err_stack += str(e)  # 数据集错误不需要调用栈，知道什么问题就行
        elif e:
            err_stack += traceback.format_exc()
        else:
            err_stack = ""
        err_content = "[{}] {}".format(msg.msg, err_stack)
