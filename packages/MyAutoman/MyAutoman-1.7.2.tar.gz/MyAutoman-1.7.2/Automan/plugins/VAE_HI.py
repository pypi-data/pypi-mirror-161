...
import torch.nn.functional as FDataset
...
class vae_hi(object):
    def __init__(self, train_data, scale_type=2, fill_dict={np.nan:0}, used_vars=[],
                learning_rate = 0.001, batch_size = 128, num_epochs = 10, num_hidden_1=12, num_latent=4):
        '''函数功能
        初始化AE类
    ...
    def train(self):
        ...
        model = VariationalAutoencoder(data_x.shape[1],
                                                 self.num_hidden_1, self.num_latent, device)
        ...
        for epoch in range(self.num_epochs):
            ...
            for batch_idx, features in enumerate(loader):
                ...
                z_mean, z_log_var, encoded, decoded = self.model(features)
    ...
    def predict(self, oot_data, output_method=1):
        ...
        with torch.set_grad_enabled(False):
            for features in loader:
                features = features.to(device)
                z_mean, z_log_var, encoded, decoded = self.model.forward_predict(features)
                loss0 = torch.mean(torch.abs(features - decoded), dim=1)
...
class VariationalAutoencoder(torch.nn.Module):
    ...
    def encoder_predict(self, features):
        ### ENCODER
        x = self.hidden_1(features)
        x = F.leaky_relu(x)
        z_mean = self.z_mean(x)
    ...
    def forward_predict(self, features):
        ...
        z_mean, z_log_var, encoded = self.encoder_predict(features)
        decoded = self.decoder(encoded)
