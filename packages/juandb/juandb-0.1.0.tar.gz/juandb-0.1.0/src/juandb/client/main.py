...
class Client(BaseClient):
    def __init__(
        ...
        super().__init__(bot, requests_params)
    ...
    def get_open_orders(self, store: str, symbol: str = None, side: str = None) -> t.Dict:
        result = self._get('deals/openDeals', signed=False, headers={'x-storeClient': store})

        if symbol is not None:
            # data = self._pick(result.get('result').get('orders'), "symbol", symbol)
            # result['result']['orders'] = data
            raise NotImplementedError('Symbol filtering is not implemented yet')

        if side is not None:
            raise NotImplementedError('Side filtering is not implemented yet')
...
class AsyncClient(BaseClient):
    ...
    async def get_open_orders(self, store: str, symbol: str = None, side: str = None) -> t.Dict:
        result = await self._get('deals/openDeals', signed=False, headers={'x-storeClient': store})
