...
logger = _jobs_common.get_executor_logger()
...
def execute() -> None:
    ...
    if not _jobs_common.running_in_executor():
        ...
        logger.error(_log_message(error_message))
        return
    ...
    try:
        ...
        logger.info(_log_message('succeeded'))
    except BaseException as e:
        logger.error(_log_message(e), exc_info=e)
    finally:
        # Cleanup
...
def _merge_notebook(notebook_file_scheduled: Union[str, Path]) -> None:
    ...
    try:
        ...
        # Get kernel language
        language = _jobs_common.get_notebook_language(nb_notebook_scheduled)
        if not language:
        ...
        execution_notebook = _jobs_common.get_execution_notebook(language)
        ...
        # Log to executor
        logger.debug(_log_message('successfully merged execution notebook with scheduled notebook'))
    except BaseException as e:
        logger.error(_log_message(e), exc_info=e)
        raise e
...
def _execute(target_file_folder: Union[str, Path]) -> NotebookNode:
    ...
    try:
        ...
        # Log to executor
        logger.debug(_log_message('successfully executed merged notebook'))

        # Python logger has no TRACE level. Special logging case here since dumping notebook contents can clog the
        # log
        if _jobs_common.is_log_level_trace_from_executor():
            logger.debug(
                _log_message(f'Executed notebook contents from {notebook_file_merged}:{nb_notebook_merged}'))
        ...
        # Log to executor
        logger.debug(_log_message('successfully edited merged notebook'))
...
def _convert(job_result_file: str) -> None:
    ...
    try:
        ...
        # Log to executor
        logger.debug(_log_message('successfully exported merged notebook'))
    except BaseException as e:
        logger.error(_log_message(e), exc_info=e)
        raise e
...
def _compose_filename(index, label, scheduled_file_filename, scheduled_file_folder, extension='.html', result=True):
    folder_path: Union[str, Path] = Path(scheduled_file_folder, _jobs_common.get_results_folder()) if result else \
