...
class Client:
    ...
    **AIRT_SERVICE_PASSWORD**, and **AIRT_SERVER_URL** environment variables.
    ...
    def __init__(
        ...
        Client.sso_provider = sso_provider
        Client.sso_nonce = sso_nonce
        Client.username = username

    @classmethod
    def _add_otp_and_sso_fields(
        cls,
        password: str,
        user_otp: Optional[int] = None,
        sso_provider: Optional[str] = None,
        sso_nonce: Optional[str] = None,
    ) -> str:
        """Add OTP and SSO provider to password and convert it into a JSON encoded string

        If passed, then user_otp and sso_provider parameter will be added and the
        password will be encoded as JSON string.

        Args:
            password: Password for the developer account.
            otp: Dynamically generated six-digit verification code from the authenticator app.
            sso_provider: The name of the Single sign-on (SSO) provider.
            sso_nonce: A cryptographically strong random string.

        Returns:
            The JSON encoded password
        """
        password_dict = {"password": password}

        if user_otp is not None:
            password_dict["user_otp"] = user_otp  # type: ignore

        if sso_provider is not None:
            password_dict["sso_provider"] = sso_provider
            password_dict["nonce"] = sso_nonce  # type: ignore
    ...
    def get_token(  # type: ignore
        ...
        *,
        ...
        is_sso_or_otp_passed = (otp is not None) or (sso_provider is not None)

        if is_sso_or_otp_passed:
            sso_nonce = secrets.token_hex()
            password = Client._add_otp_and_sso_fields(
                password, otp, sso_provider, sso_nonce
            )
        ...
        authorization_url = response.get("authorization_url", None)
    ...
    def set_sso_token(cls):
        ...
        response = get_data(
            url=f"{cls.server}/sso/token/?username={cls.username}&sso_provider={cls.sso_provider}&nonce={cls.sso_nonce}",
            token=None,
        )
