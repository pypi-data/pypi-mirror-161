...
def token(
    ...
    You can also enable Single sign-on (SSO) for your account and generate tokens. Currently, we support only Google as an SSO provider and will be supporting more providers in the upcoming releases.
    ...
    try:
        ...
        if sso_provider is None:
            ...
            typer.echo(
                "\nPlease open the below authorization URL in your preferred browser and proceed with the Single sign-on (SSO) authentication."
            )
            typer.echo(
                "\nAn access token will be generated if the authentication from the external SSO provider is successful and completed within 10 minutes. Else the Single sign-on (SSO) authentication will be a timeout and this command will terminate. "
            )
            typer.echo(f"\n\n{authorization_url}")
