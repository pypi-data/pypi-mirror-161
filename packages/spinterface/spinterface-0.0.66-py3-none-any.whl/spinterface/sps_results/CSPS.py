    ...
    def _create_visualizer(self) -> Dict[str, IVisStage]:
        ...
        if self._iinitial:
            l_stages['INITIAL'] = CVisInitial(strategy=1, info_file=self.info_initial,
                                              ...
                                              arrowscale_evec=self.arrowscale_evec)
        if self._idisp:
    ...
    def _check_settings(settings: Dict[str, List]) -> Dict[str, Any]:
        ...
        for (key, valuetype) in const.KEYS.items():
            ...
            if (key=='disp2_superpos_modenrs_low') or (key=='disp2_superpos_modenrs_high'):
                try:
                    l_sps_attempts = settings['sps_attempts']
                except KeyError:
                    raise ValueError(f"The following necessary key is not provided in input file: sps_attempts.")
                if len(l_values)!=l_sps_attempts:
                    raise ValueError(f'Number of attempts must equal the number of options provided for escape mode indices.')
                vs = []
                for val in l_values:
