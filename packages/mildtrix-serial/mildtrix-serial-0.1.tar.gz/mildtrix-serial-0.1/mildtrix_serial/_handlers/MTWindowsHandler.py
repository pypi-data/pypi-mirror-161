...
class MTWindowsHandler(MTSettings, IMTNative):
    #is Windows unicode.
    __unicode = None

    def __init__(self):
        """Constructor."""
        MTSettings.__init__(self)
    ...
    def isUnicode(self):
        ...
        if MTWindowsHandler.__unicode is None:
            try:
                h = _stdcall_libraries['kernel32'].CreateEventW(0, 0, 0, 0)
                ctypes.windll.Kernel32.CloseHandle(h)
                MTWindowsHandler.__unicode = True
            except AttributeError:
                MTWindowsHandler.__unicode = False
        return MTWindowsHandler.__unicode
