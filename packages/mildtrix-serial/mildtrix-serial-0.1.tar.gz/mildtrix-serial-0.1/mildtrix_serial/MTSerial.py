...
from mildtrix_common.enums import TraceLevel, MediaState, TraceTypes
from mildtrix_common.IMTMedia import IMTMedia
from mildtrix_common.MediaStateEventArgs import MediaStateEventArgs
from mildtrix_common.TraceEventArgs import TraceEventArgs
from mildtrix_common.PropertyChangedEventArgs import PropertyChangedEventArgs
from mildtrix_common.ReceiveEventArgs import ReceiveEventArgs
from ._MTSynchronousMediaBase import _MTSynchronousMediaBase
if os.name == 'nt':  # sys.platform == 'win32':
    from ._handlers.MTWindowsHandler import MTWindowsHandler
elif os.name == 'posix':
    from ._handlers.MTLinuxHandler import MTLinuxHandler

# pylint: disable=too-many-public-methods, too-many-instance-attributes,
# too-many-arguments
class MTSerial(IMTMedia):
    def __init__(self,
                 port,
        ...
        self.__syncBase = _MTSynchronousMediaBase(100)
    ...
    def __initialize(cls):
        ...
        if os.name == 'nt':
            return MTWindowsHandler()
        if os.name == 'posix':
            return MTLinuxHandler()
    ...
    def getAvailableBaudRates(self):
        ...
        return (mildtrix_common.io.BaudRate.BAUD_RATE_300, mildtrix_common.io.BaudRate.BAUD_RATE_600,
                mildtrix_common.io.BaudRate.BAUD_RATE_1800, mildtrix_common.io.BaudRate.BAUD_RATE_2400,
                mildtrix_common.io.BaudRate.BAUD_RATE_4800, mildtrix_common.io.BaudRate.BAUD_RATE_9600,
                mildtrix_common.io.BaudRate.BAUD_RATE_19200, mildtrix_common.io.BaudRate.BAUD_RATE_38400)
    ...
    def send(self, data, receiver=None):
        ...
        if not isinstance(data, bytes):
            data = bytes(_MTSynchronousMediaBase.toBytes(data))
        self.__h.write(data)
    ...
    def __handleReceivedData(self, buff, info):
        ...
        if self.getIsSynchronous():
            ...
            with self.__syncBase.getSync():
                ...
                if self.eop:
                    tmp = _MTSynchronousMediaBase.toBytes(self.eop)
                    totalCount = _MTSynchronousMediaBase.indexOf(buff, tmp, 0, len(buff))
                if totalCount != -1:
    ...
    def __getBaudRate(self):
        if self.__h.isOpen():
            self.__h.baudRate = mildtrix_common.io.BaudRate(self.__h.getBaudRate())
    ...
    def __getParity(self):
        if self.__h.isOpen():
            self.__h.parity = mildtrix_common.io.Parity(self.__h.getParity())
    ...
    def __getStopBits(self):
        if self.__h.isOpen():
            self.__h.stopBits = mildtrix_common.io.StopBits(self.__h.getStopBits())
    ...
    def getSettings(self):
        ...
        if self.__h.baudRate != mildtrix_common.io.BaudRate.BAUD_RATE_9600:
            sb += "<BaudRate>"
            sb += str(int(self.__h.baudRate))
            sb += "</BaudRate>"
            sb += nl

        if self.__h.stopBits != mildtrix_common.io.StopBits.ONE:
            sb += "<StopBits>"
            sb += str(int(self.__h.stopBits))
            sb += "</StopBits>"
            sb += nl

        if self.__h.parity != mildtrix_common.io.Parity.NONE:
            sb += "<Parity>"
    ...
    def setSettings(self, value):
        ...
        self.__portName = ""
        self.__h.baudRate = mildtrix_common.io.BaudRate.BAUD_RATE_9600
        self.__h.stopBits = mildtrix_common.io.StopBits.ONE
        self.__h.parity = mildtrix_common.io.Parity.NONE
        self.__h.dataBits = 8
