...
class FirstBatchClient:
    def __init__(self, public_key, secret, bearer):
        self.__requests = FirstBatchRequests()
        self.__public_key = public_key
        self.__secret = secret
        # self.__bearer_token = self.__requests.call_auth(signed_hash=self.__requests.auth_sign(self.__secret), public_key=self.__public_key)["data"]["bearer"]
        self.__bearer_token = bearer
    ...
    def get_statistics(self, event: FirstBatchEvent):
        return self.__safe_call(self.__requests.call_event_statistics, {"event_id":event.event_id})
