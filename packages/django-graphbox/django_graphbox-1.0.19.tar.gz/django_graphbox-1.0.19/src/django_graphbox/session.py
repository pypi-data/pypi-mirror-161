...
class Manager:
    ...
    def __init__(self, user_model, use_social_session=False, rol_field_name='rol', login_id_field_name='uname', password_field_name='password', name_field_name="name", photo_field_name="photo", social_id_field=None, social_origin_field=None, active_field_name=None, session_expiration_time=12, groups={}, modify_permissions={}, security_key=None):
        ...
        if self.user_model!=None:
            try:
                if origin==self.social_origin_facebook:
                    ...
                    return False, None, '', ErrorManager.get_error_by_code(INVALID_TOKEN)
                if self.user_model.objects.filter(**{self.social_id_field:id}).exists():
                    user_instance=self.user_model.objects.get(**{self.social_id_field:id})
                    if self.active_field_name==None or getattr(user_instance, self.active_field_name):
                    ...
                    if self.active_field_name==None or not getattr(user_instance, self.active_field_name):
                        ...
                        return False, None, '', ErrorManager.get_error_by_code(USER_ALREADY_EXISTS)
                else:
