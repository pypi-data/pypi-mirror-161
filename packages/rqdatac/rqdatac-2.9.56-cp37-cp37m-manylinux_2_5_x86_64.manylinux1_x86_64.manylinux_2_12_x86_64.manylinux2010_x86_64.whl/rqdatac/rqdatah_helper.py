...
def http_conv_trading_hours(hours):
    ...
    return header + '\n'.join(f'{str(s)},{str(e)}' for (s, e) in hours)
...
def tick_to_dict(t):
    ...
    if isinstance(asks, Iterable):
        d.update((f'ask{i}', v) for i, v in enumerate(asks))

    ask_vols = d.pop('ask_vols')
    if isinstance(ask_vols, Iterable):
        d.update((f'ask_vol{i}', v) for i, v in enumerate(ask_vols))

    bids = d.pop('bids')
    if isinstance(bids, Iterable):
        d.update((f'bid{i}', v) for i, v in enumerate(bids))

    bid_vols = d.pop('bid_vols')
    if isinstance(bids, Iterable):
        d.update((f'bid_vol{i}', v) for i, v in enumerate(bid_vols))
