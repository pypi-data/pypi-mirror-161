    ...
    def wrapper(self, *args, **kwargs):
        ...
        while True:
            try:
                ...
                print(f" - The requests by pass proxy server {current_proxy} has failed. Trying to next proxy server {next_proxy}...(rotate rate: {rotate_count}/{len(self.proxies)})")
            else:
                return response
        raise RotateProxiesDone(f"All requests through {len(self.proxies)} proxies fail")
    return wrapper