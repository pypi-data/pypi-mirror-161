...
class CaseOpti(GenMapManager):
    ...
    def read_param(self, dir, copyDefault=False, callback=None):
        ...
        if not os.path.exists(self.launcherDir):
            try:
                ...
                shutil.copyfile(self.workingDir+"launcher.param.default", self.launcherDir+"launcher.param")
                shutil.copyfile(self.workingDir+"launcher.param.default", self.launcherDir+"launcher.param.default")
            except OSError:
...
class Optimisation(wx.Frame):
    ...
    def new(self, event):
        ...
        newCase = CaseOpti()
        newCase.read_param(launcherDir, copyDefault=True, callback=self.update_parameters_launcher)
    ...
    def apply_optim(self, event, idLauncher=0):
        ...
        filePath = self.myCases[idLauncher].refCatchment.workingDir + "Subbasin_" + str(self.myCases[idLauncher].refCatchment.myEffSortSubBasins[0]) + "\\"
    ...
    def associate_ptr_q_all(self, idOpti=1, idLauncher=0):
        ...
        for iSub in self.myCases[idLauncher].refCatchment.myEffSortSubBasins:
            ...
            ptrArray = self.dllFortran.get_cptr_py(ct.byref(ct.c_int(idOpti)),ct.byref(ct.c_int(idLauncher+1)), 
                                            ct.c_int(cste.ptr_q_all), pointerDims)
            curSub.myHydro = None
            curSub.myHydro = self.make_nd_array(ptrArray, shape=(dims[0],dims[1]), dtype=ct.c_double, order='F', own_data=False)
    ...
    def launch_semiDistributed_optimisation(self, event, idOpti=1, idLauncher=0):
        ...
        if (self.optiParam.get_group("Semi-Distributed"))!=None:
            nbRefs = int(self.optiParam.get_param("Semi-Distributed","nb"))
            doneList = []
            sortJct = []
            readDict = {}
            # Read all ref data
            ...
            for iOpti in range(len(sortJct)):
                ...
                self.myCases[idLauncher].refCatchment.activate_usefulSubs(blockJunction=doneList)
