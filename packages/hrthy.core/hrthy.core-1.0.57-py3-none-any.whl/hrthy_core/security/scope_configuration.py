...
from hrthy_core.security.exceptions import WrongScopesConfigurationScopeNotFound, WrongScopesConfigurationHiddenScope, \
    WrongScopesConfigurationMissingDependency
from hrthy_core.security.hierarchy import HIERARCHY_SCOPES
...
class ScopeConfiguration:
    ...
    def __init__(self, scopes: List[str], current_scopes: List[str]):
        super().__init__()
        self.scopes = self._check_configuration_permission(scopes, current_scopes)
        self.scopes = self._add_default_scopes(self.scopes)

    def _check_configuration_permission(self, scopes: List[str], current_scopes: List[str]) -> List[str]:
        for scope in scopes:
            if scope not in HIERARCHY_SCOPES.keys():
                raise WrongScopesConfigurationScopeNotFound(scope)
            scope_to_check = HIERARCHY_SCOPES.get(scope.strip().lower()).value
    ...
    def _add_default_scopes(self, scopes: List[str]) -> List[str]:
