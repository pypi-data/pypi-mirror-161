...
class Proxy:
    ...
    def get_proxy_block_transaction_count_by_number(tag: str) -> str:
        url = (
            ...
            f"{modules.PROXY}"
            f"{fields.ACTION}"
            f"{actions.AVAX_GET_BLOCK_TRANSACTION_COUNT_BY_NUMBER}"
            f"{fields.TAG}"
            f"{tag}"
        )
        return url
    ...
    def get_proxy_transaction_by_hash(txhash: str) -> str:
        url = (
            ...
            f"{modules.PROXY}"
            f"{fields.ACTION}"
            f"{actions.AVAX_GET_TRANSACTION_BY_HASH}"
            f"{fields.TXHASH}"
            f"{txhash}"
        )
        return url
    ...
    def get_proxy_transaction_by_block_number_and_index(tag: str, index: str) -> str:
        url = (
            ...
            f"{modules.PROXY}"
            f"{fields.ACTION}"
            f"{actions.AVAX_GET_TRANSACTION_BY_BLOCK_NUMBER_AND_INDEX}"
            f"{fields.TAG}"
            f"{tag}"
            f"{fields.INDEX}"
            f"{index}"
    ...
    def get_proxy_transaction_count(address: str) -> str:
        url = (
            ...
            f"{modules.PROXY}"
            f"{fields.ACTION}"
            f"{actions.AVAX_GET_TRANSACTION_COUNT}"
            f"{fields.ADDRESS}"
            f"{address}"
            f"{fields.TAG}"
            f"{tags.LATEST}"
    ...
    def get_proxy_transaction_receipt(txhash: str) -> str:
        url = (
            ...
            f"{modules.PROXY}"
            f"{fields.ACTION}"
            f"{actions.AVAX_GET_TRANSACTION_RECEIPT}"
            f"{fields.TXHASH}"
            f"{txhash}"
        )
        return url
