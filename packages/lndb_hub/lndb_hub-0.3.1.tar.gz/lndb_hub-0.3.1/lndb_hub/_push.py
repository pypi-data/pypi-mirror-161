...
try:
    ...
    print("Could not import lamindb.")
...
class hub:
    ...
    def share_instance(cls):
        ...
        dobject_df = load.entity("dobject")
    ...
    def share_dobject(cls, id: str, v: str, hub: Client = None):
        ...
        dobject = (
            load.entity("dobject").loc[[(id, v)]].reset_index().to_dict("records")[0]
        )
        dsource_id = dobject["dsource_id"]
        with sqm.Session(settings.instance.db_engine()) as session:
            dtransform = session.get(schema_core.dtransform, dsource_id)
        if not jupynb_exists(dtransform.jupynb_id, dtransform.jupynb_v, hub):
            logger.info(
                f"Published jupynb ({dtransform.jupynb_id}, {dtransform.jupynb_v})."
            )
            insert_jupynb(
                dtransform.jupynb_id, dtransform.jupynb_v, instance["id"], hub
            )
        else:
            logger.warning(
                f"jupynb ({dtransform.jupynb_id}, {dtransform.jupynb_v}) already exists"
                " and is not pushed."
            )
        if not dobject_exists(id, v, hub):
            logger.info(f"Published dobject ({id}, {v}).")
            insert_dobject(id, v, dtransform.jupynb_id, dtransform.jupynb_v, hub)
...
def delete_jupynb(id, v, hub: Client):
    try:
        ...
        logger.warning(
            "Did not delete jupynb ({id}, {v}) probably because it's still attached to"
            " another dobject."
        )


def insert_jupynb(id, v, instance_id, hub: Client):
    jupynb = load.entity("jupynb").loc[[(id, v)]].reset_index().to_dict("records")[0]
    data = (
        hub.table("jupynb")
        .insert(
            {
...
def insert_dobject(id, v, jupynb_id, jupynb_v, hub: Client):
    dobject = load.entity("dobject").loc[[(id, v)]].reset_index().to_dict("records")[0]
    data = (
        hub.table("dobject")
        .insert(
            {
