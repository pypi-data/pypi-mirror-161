...
def _ensure_connection(host_attr, plugin_attr, widgets):
    ...
    # throw away the platform field since maya is Linux only.
    host_names = [" ".join(h.split()[:2]) for h in software_data.supported_host_names()]
    AEcommon.ensure_populate_menu(
        widgets["host_menu"], host_names)
...
def _sync_plugin_software_menus(host_attr, plugin_attr, plugin_layout):
    ...
    # When we get the plugin software, we have to qualify the host software by it's platform.
    platform = list(coredata.platforms())[0]
    software_data = coredata.data()["software"]
    full_host_name = "{} {}".format(host_attr.get(), platform)
    supported_plugins = software_data.supported_plugins(full_host_name)
