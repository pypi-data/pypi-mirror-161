...
Options:
    ...
    -a --all        显示全部参数
    --arch=<arch_type> 架构
    --num=<num_of_items> 显示多少条数据
    --build=<build_version> build 版本号
"""
__version__="0.1.9"
...
class CommandLineTool:
    ...
    def replace_lib(self, zip_path):
        print('🤖 Start to replace lib...')
        os.system('unzip -o' + zip_path)
        print('🤖 Replace finished')
        os.system('rm -rf ./' + zip_path)
        return True

    def checkout_all(self, branch):
    ...
    def rollback(self, branch, build_version, arch_type):
        ...
        if self.checkout_repo(build):
            ...
            if arch_type == 'mac_x86_64':
                dest_path += ('Mac/Release' + '/x86_64_libs_' + roll_back_build['name'] + '.zip')
                release_path += 'Mac/Release'
            else:
                dest_path += ('Mac_arm64/Release' + '/arm64_libs_' + roll_back_build['name'] + '.zip')
                release_path += 'Mac_arm64/Release'
            self.download_by_aria(url=build['lib_url'], dst=release_path)
            self.replace_lib(dest_path)
