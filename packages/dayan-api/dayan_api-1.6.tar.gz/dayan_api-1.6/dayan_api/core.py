...
from dayan_api.connect import Connect
from dayan_api.constants import PACKAGE_NAME
from dayan_api.exception import RayvisionError, RayvisonTaskIdError
from dayan_api.operators import QueryOperator
from dayan_api.operators import RenderEnvOperator
from dayan_api.operators import TagOperator
from dayan_api.operators import TaskOperator
from dayan_api.operators import TransmitOperator
from dayan_api.operators import UserOperator
...
class RayvisionAPI(object):
    ...
    def submit_cc(self, task_id, option=None, param=None):
        if not isinstance(task_id, int):
            raise RayvisonTaskIdError(10006, "task_id must int !!!!")

        self.task.submit_task_to_cc(task_id, option, param)
        return True
