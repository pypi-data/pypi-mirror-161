...
def generate_headers_body_str(domain_name, api_url, header, body):
    ...
    if 'queryTaskInfo' in str(api_url):
        header_body_dict = headers_body_sort(header, body, True)
    else:
        header_body_dict = headers_body_sort(header, body)
    header_body_list = [
...
def headers_body_sort(header, body, task_info=False):
    ...
    for key in sorted_key_list:
        if key is "channel" and task_info:
            new_dict['bFilterOsgb'] = '0'
