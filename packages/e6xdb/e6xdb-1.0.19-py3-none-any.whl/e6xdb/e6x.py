...
class Cursor(DBAPICursor):
    ...
    def _fetch_all(self):
        ...
        self._data = list()
        while len(self._data) < self.rowcount():
    ...
    def fetchmany(self, size=10000):
        _logger.info("fetching all from overriden method")
        size = min(size, self.rowcount())
        self.rows_count = 0
        self._data = list()
        while self.rows_count < size:
            _logger.info("fetching next batch from fetch many")
            _logger.info(len(self._data))
            _logger.info("no  of rows accumulated ".format(self.rows_count))
            rows = self.fetch_batch()
            if rows is None:
                return None
            self.rows_count = self.rows_count + len(rows)
            self._data = self._data + rows
        return self._data if len(self._data) == size else self._data[0:size]
