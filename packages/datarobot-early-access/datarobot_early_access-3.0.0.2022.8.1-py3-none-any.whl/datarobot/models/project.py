...
from ..helpers.partitioning_methods import DatetimePartitioningSpecification, PartitioningMethod
from ..utils import (
    camelize,
    datetime_to_string,
    from_api,
...
class Project(APIObject, BrowserMixin):
    ...
    datetime_partition_spec: DatetimePartitioningSpecification, optional
        for time series projects, partitioning attributes; otherwise, None
    positive_class : str
    ...
    _converter = t.Dict(
        {
            ...
            t.Key("datetime_partition_spec", optional=True): DatetimePartitioning._converter,
    ...
    def __init__(
        ...
        self.datetime_partition_spec = datetime_partition_spec
    ...
    def start(
        ...
        partitioning_method : PartitioningMethod object, optional
            ...
            ``datarobot.helpers.partitioning_methods``. For time series projects, as an alternative, use
            ``Project.set_datetime_partitioning`` to set the partitioning for the project.
    ...
    def set_target(
        ...
        partitioning_method : PartitioningMethod object, optional
            Instance of one of the :ref:`Partition Classes <partitions_api>` defined in
            ``datarobot.helpers.partitioning_methods``. For time series projects, as an alternative,
            use ``Project.set_datetime_partitioning`` to set the partitioning for the project.
        positive_class : str, float, or int; optional
            Specifies a level of the target column that should be treated as the
            positive class for binary classification.  May only be specified
            for binary classification targets.
        ...
        if self.datetime_partition_spec and not partitioning_method:
            self._load_partitioning_method(self.datetime_partition_spec, aim_payload)
            self.datetime_partition_spec.prep_payload(self.id, max_wait=max_wait)
        elif self.datetime_partition_spec and partitioning_method:
            warnings.warn(
                "The `partitioning_method` passed directly into `set_target` will be used, but \
                `self.datetime_partition_spec` is non-null. If you wish to instead use \
                `self.datetime_partition_spec`, call `set_datetime_partitioning` on your project \
                instance and do not pass a partitioning method directly to `set_target`."
            )
            self._load_partitioning_method(partitioning_method, aim_payload)
            partitioning_method.prep_payload(self.id, max_wait=max_wait)
        elif not self.datetime_partition_spec and partitioning_method:
            self._load_partitioning_method(partitioning_method, aim_payload)
            partitioning_method.prep_payload(self.id, max_wait=max_wait)
    ...
    def set_datetime_partitioning(
        ...
        **kwargs,
        ...
        Updates ``datetime_partition_spec`` if already set previously (does not replace it).
        ...
        if datetime_partition_spec is not None:
        ...
        elif self.datetime_partition_spec is not None:
            self.datetime_partition_spec._update_settings(**kwargs)
            ...
            self.datetime_partition_spec = spec

        full_partitioning = DatetimePartitioning.generate(
            self.id, self.datetime_partition_spec, target=self.target
        )
    ...
    def list_datetime_partition_spec(self) -> Optional[Dict]:
        ...
        settings from `project.datetime_partition_spec`.
        ...
        """
        if self.datetime_partition_spec and self.stage != PROJECT_STAGE.MODELING:
            res = vars(self.datetime_partition_spec)
            return res
        elif not self.datetime_partition_spec and self.stage != PROJECT_STAGE.MODELING:
            return
