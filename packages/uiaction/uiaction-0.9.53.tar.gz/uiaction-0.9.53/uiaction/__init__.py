# uiaction lets Python control the mouse and keyboard, and other GUI automation tasks. For Windows, macOS, and Linux,
# on Python 3 and 2.
...
class uiactionException(Exception):
    """
    uiaction code will raise this exception class for any invalid actions. If uiaction raises some other exception,
    you should assume that this is caused by a bug in uiaction itself. (Including a failure to catch potential
    exceptions raised by uiaction.)
    """

    pass


class FailSafeException(uiactionException):
    """
    This exception is raised by uiaction functions when the user puts the mouse cursor into one of the "failsafe
...
class ImageNotFoundException(uiactionException):
    """
    This exception is the uiaction version of PyScreeze's `ImageNotFoundException`, which is raised when a locate*()
    function call is unable to find an image.

    Ideally, `pyscreeze.ImageNotFoundException` should never be raised by uiaction.
    """
...
try:
    ...
    def _couldNotImportPyTweening(*unused_args, **unused_kwargs):
        """
        This function raises ``uiactionException``. It's used for the PyTweening function names if the PyTweening
        module failed to be imported.
        """
        raise uiactionException(
            "uiaction was unable to import pytweening. Please install this module to enable the function you tried to call."
        )
...
try:
    ...
    def _couldNotImportPyMsgBox(*unused_args, **unused_kwargs):
        """
        This function raises ``uiactionException``. It's used for the PyMsgBox function names if the PyMsgbox module
        failed to be imported.
        """
        raise uiactionException(
            "uiaction was unable to import pymsgbox. Please install this module to enable the function you tried to call."
        )

    alert = confirm = prompt = password = _couldNotImportPyMsgBox


def raiseuiactionImageNotFoundException(wrappedFunction):
    """
    A decorator that wraps PyScreeze locate*() functions so that the uiaction user sees them raise uiaction's
    ImageNotFoundException rather than PyScreeze's ImageNotFoundException. This is because PyScreeze should be
    invisible to uiaction users.
    """
    ...
    def wrapper(*args, **kwargs):
        try:
            ...
            raise ImageNotFoundException  # Raise uiaction's ImageNotFoundException.
...
try:
    ...
    # Change the locate*() functions so that they raise uiaction's ImageNotFoundException instead.
    @raiseuiactionImageNotFoundException
    ...
    def _couldNotImportPyScreeze(*unused_args, **unsed_kwargs):
        """
        This function raises ``uiactionException``. It's used for the PyScreeze function names if the PyScreeze module
        failed to be imported.
        """
        raise uiactionException(
            "uiaction was unable to import pyscreeze. (This is likely because you're running a version of Python that Pillow (which pyscreeze depends on) doesn't support currently.) Please install this module to enable the function you tried to call."
        )
...
try:
    ...
    def mouseInfo():
        """
        This function raises uiactionException. It's used for the MouseInfo function names if the MouseInfo module
        failed to be imported.
        """
        raise uiactionException(
            "uiaction was unable to import mouseinfo. Please install this module to enable the function you tried to call."
        )


def useImageNotFoundException(value=None):
    """
    When called with no arguments, uiaction will raise ImageNotFoundException when the PyScreeze locate*() functions
    ...
    try:
        ...
        raise uiactionException("useImageNotFoundException() ws called but pyscreeze isn't installed.")
...
if sys.platform == "win32":  # PyGetWindow currently only supports Windows.
    try:
        ...
        def _couldNotImportPyGetWindow(*unused_args, **unused_kwargs):
            """
            This function raises uiactionException. It's used for the PyGetWindow function names if the PyGetWindow
            module failed to be imported.
            """
            raise uiactionException(
                "uiaction was unable to import pygetwindow. Please install this module to enable the function you tried to call."
            )
...
if sys.platform.startswith("java"):
    # from . import _uiaction_java as platformModule
    raise NotImplementedError("Jython is not yet supported by uiaction.")
elif sys.platform == "darwin":
    from . import _uiaction_osx as platformModule
elif sys.platform == "win32":
    from . import _uiaction_win as platformModule
elif platform.system() == "Linux":
    from . import _uiaction_x11 as platformModule
else:
    raise NotImplementedError("Your platform (%s) is not supported by uiaction." % (platform.system()))
...
LOG_SCREENSHOTS = False  # If True, save screenshots for clicks and key singleKeyes.

# If not None, uiaction deletes old screenshots when this limit has been reached:
...
def _genericuiactionChecks(wrappedFunction):
    """
...
def linear(n):
    ...
    if not 0.0 <= n <= 1.0:
        raise uiactionException("Argument must be between 0.0 and 1.0.")
    return n


def _handlePause(_pause):
    """
    A helper function for performing a pause at the end of a uiaction function based on some settings.
...
def _normalizeXYArgs(firstArg, secondArg):
    ...
    several uiaction functions. If ``firstArg`` and ``secondArg`` are both ``None``, returns the current mouse cursor
    position.
...
def _logScreenshot(logScreenshot, funcName, funcArgs, folder="."):
    """
    A helper function that creates a screenshot to act as a logging mechanism. When a uiaction function is called,
...
right mouse buttons respectively, in uiaction 1, 2, and 3 refer to the left,
...
def mouseDown(x=None, y=None, button=PRIMARY, duration=0.0, tween=linear, logScreenshot=None, _pause=True):
    ...
    Args:
      ...
      button (str, int, optional): The mouse button singleKeyed down. TODO

    Returns:
      None

    Raises:
      uiactionException: If button is not one of 'left', 'middle', 'right', 1, 2, or 3
    """
...
def click(
    ...
    """
    Performs singleKeying a mouse button down and then immediately releasing it. Returns ``None``.
    ...
    string is an image filename that uiaction will attempt to locate on the screen and click the center of. If ``x``
    ...
    The ``pause`` parameter is deprecated. Call the ``uiaction.sleep()`` function to implement a pause.

    Raises:
      uiactionException: If button is not one of 'left', 'middle', 'right', 1, 2, 3
    """
...
def doubleClick(x=None, y=None, interval=0.0, button=LEFT, duration=0.0, tween=linear, logScreenshot=None, _pause=True):
    ...
    Raises:
      uiactionException: If button is not one of 'left', 'middle', 'right', 1, 2, 3, 4,
        5, 6, or 7
    """
...
move = moveRel  # For uiaction 1.0, move() replaces moveRel().
...
drag = dragRel  # For uiaction 1.0, we want drag() to replace dragRel().
...
def isValidKey(key):
    ...
    uiaction's keyboard-related functions for the current platform.

    This function is here because passing an invalid value to the uiaction
...
def keyDown(key, logScreenshot=None, _pause=True):
    """Performs a keyboard key singleKey without the release. This will put that
    ...
    Args:
      key (str): The key to be singleKeyed down. The valid names are listed in
      KEYBOARD_KEYS.
...
def keyUp(key, logScreenshot=None, _pause=True):
    """Performs a keyboard key release (without the singleKey down beforehand).
...
@_genericuiactionChecks
def singleKey(keys, singleKeyes=1, interval=0.0, logScreenshot=None, _pause=True):
    """Performs a keyboard key singleKey down, followed by a release.

    Args:
      key (str, list): The key to be singleKeyed. The valid names are listed in
      KEYBOARD_KEYS. Can also be a list of such strings.
      singleKeyes (integer, optional): The number of singleKey repetitions.
      1 by default, for just one singleKey.
      interval (float, optional): How many seconds between each singleKey.
      0.0 by default, for no pause between singleKeyes.
    ...
    _logScreenshot(logScreenshot, "singleKey", ",".join(keys), folder=".")
    for i in range(singleKeyes):
        for k in keys:
            failSafeCheck()
...
def hold(keys, logScreenshot=None, _pause=True):
    """Context manager that performs a keyboard key singleKey down upon entry,
...
def typewrite(message, interval=0.0, logScreenshot=None, _pause=True):
    """Performs a keyboard key singleKey down, followed by a release, for each of
    the characters in message.

    The message argument can also be list of strings, in which case any valid
    keyboard name can be used.

    Since this performs a sequence of keyboard singleKeyes and does not hold down
    keys, it cannot be used to perform keyboard shortcuts. Use the doubleKey()
    function for that.

    Args:
      message (str, list): If a string, then the characters to be singleKeyed. If a
        list, then the key names of the keys to singleKey in order. The valid names
        are listed in KEYBOARD_KEYS.
      interval (float, optional): The number of seconds in between each singleKey.
        0.0 by default, for no pause in between singleKeyes.
    ...
    for c in message:
        if len(c) > 1:
            c = c.lower()
        singleKey(c, _pause=False)
        time.sleep(interval)
        failSafeCheck()


write = typewrite  # In uiaction 1.0, write() replaces typewrite().


@_genericuiactionChecks
def doubleKey(*args, **kwargs):
    """Performs key down singleKeyes on the arguments passed in order, then performs
    key releases in reverse order.

    The effect is that calling doubleKey('ctrl', 'shift', 'c') would perform a
    "Ctrl-Shift-C" doubleKey/keyboard shortcut singleKey.

    Args:
      key(s) (str): The series of keys to singleKey, in order. This can also be a
        list of key strings to singleKey.
      interval (float, optional): The number of seconds in between each singleKey.
        0.0 by default, for no pause in between singleKeyes.
...
def failSafeCheck():
    if FAILSAFE and tuple(position()) in FAILSAFE_POINTS:
        raise FailSafeException(
            "uiaction fail-safe triggered from mouse moving to a corner of the screen. To disable this fail-safe, set uiaction.FAILSAFE to False. DISABLING FAIL-SAFE IS NOT RECOMMENDED."
        )
...
def _getNumberToken(commandStr):
    ...
    if mo is None:
        raise uiactionException("Invalid command at index 0: a number was expected")
...
def _getQuotedStringToken(commandStr):
    ...
    mo = pattern.search(commandStr)
    if mo is None:
        raise uiactionException("Invalid command at index 0: a quoted string was expected")
...
def _getParensCommandStrToken(commandStr):
    ...
    mo = pattern.search(commandStr)
    if mo is None:
        raise uiactionException("Invalid command at index 0: No open parenthesis found.")
    ...
    while i < len(commandStr):
        if commandStr[i] == "(":
            ...
            if openParensCount == 0:
                ...
                raise uiactionException("Invalid command at index 0: No open parenthesis for this close parenthesis.")
        i += 1
    if openParensCount > 0:
        raise uiactionException("Invalid command at index 0: Not enough close parentheses.")
...
def _getCommaToken(commandStr):
    ...
    mo = pattern.search(commandStr)
    if mo is None:
        raise uiactionException("Invalid command at index 0: a comma was expected")
...
def _tokenizeCommandStr(commandStr):
    ...
    while i < len(commandStr):
        ...
        if mo is None:
            raise uiactionException("Invalid command at index %s: %s is not a valid command" % (i, commandStr[i]))
        ...
        # Handle the arguments of the mouse (g)o and mouse (d)rag commands:
            ...
            try:
            ...
            except uiactionException as excObj:
                ...
                raise uiactionException("Invalid command at index %s:%s" % (newIndexNum, message))

            # Make sure either both x and y have +/- or neither of them do:
            if x.lstrip()[0].isdecimal() and not y.lstrip()[0].isdecimal():
                raise uiactionException("Invalid command at index %s: Y has a +/- but X does not." % (i - len(y)))
        ...
        # Handle the arguments of the (k)ey singleKey, (w)rite, (h)otkeys, and (a)lert commands:
...
def run(commandStr, _ssCount=None):
    """Run a series of uiaction function calls according to a mini-language
    made for this function. The `commandStr` is composed of character
    commands that represent uiaction function calls.
