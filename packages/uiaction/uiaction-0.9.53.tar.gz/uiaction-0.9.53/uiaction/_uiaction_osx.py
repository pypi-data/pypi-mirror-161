...
try:
    ...
    assert False, "You must first install pyobjc-core and pyobjc: https://uiaction.readthedocs.io/en/latest/install.html"
import AppKit
...
if sys.platform !=  'darwin':
    raise Exception('The uiaction_osx module should only be loaded on an OS X system.')
...
def _normalKeyEvent(key, upDown):
    ...
    try:
        if uiaction.isShiftCharacter(key):
            ...
            time.sleep(uiaction.DARWIN_CATCH_UP_TIME)
...
def _dragTo(x, y, button):
    if button == LEFT:
        ...
        assert False, "button argument not in ('left', 'middle', 'right')"
    time.sleep(uiaction.DARWIN_CATCH_UP_TIME) # needed to allow OS time to catch up.
