...
for store_id_batch in helper.batch(stores['store-id'], batch_size):
    ...
    uri = s3.save_df_to_s3(df=recon_df[column_order],
                           file_name=f"inventory/ledger/{start_date.replace('-', '/')}/batch_{batch}.csv", index=False)
    ...
    s3.write_df_to_db(df=recon_df[table_info['column_name']], table_name="inventory-ledger", db=w_db,
