...
"""
@本类是用于树莓派IO引脚控制PCA9685芯片产生PWM波形，控制舵机转动角度
"""
class Servo:
    ...
    @angle: 舵机转动的大概角度，值不与实际的角度对应，方便微调，取值范围：100~~600
    """
    def set_servo(self,servo_channel,angle):
        if (servo_channel > 0) and (servo_channel < 7):
            if (angle >= 100) and (angle < 601):
                self.pwm.set_pwm(servo_channel, 0,angle)
            else:
                print('\033[31m warning: servo angle must be 100,101,102...600 \033[0m')
