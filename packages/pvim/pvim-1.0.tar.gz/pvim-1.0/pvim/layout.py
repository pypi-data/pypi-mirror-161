...
import pvim.window_arrangement as window_arrangement
...
class vimScrollOffsets(ScrollOffsets):
    def __init__(self, editor):
        self.editor = editor
        self.left = 0
        self.right = 0
...
def get_terminal_title(editor):
    ...
    e.g.: "filename.py (/directory) - vim"
    """
    eb = editor.current_editor_buffer
    if eb is not None:
        return '%s - vim' % (eb.location or '[New file]', )
    else:
        return 'vim'
