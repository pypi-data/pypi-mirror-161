...
'''
if 'Windows' in pf:
	from . import diodeIV, induction, npnCEout, pendulumVelocity
	from . import plotIV, RCtransient, RLCsteadystate, RLCtransient
	from . import RLtransient, rodPendulum, scope, soundBeats, soundFreqResp, soundVelocity
	from . import  utils, logger, tof
...
class MainWindow(QMainWindow):
	...
	def scope_help(self,e):
		...
		if self.expName != 'scope':
			explib = importlib.import_module('.scope',package='eyesjrGUI')
			try:
	...
	def callExpt(self, e):
		...
		explib = importlib.import_module('.'+module_name,package='eyesjrGUI')
		try:
			if self.expWidget != None:
				self.expWidget.timer.stop()	 # Stop the timer loop of current widget			
			self.hwin = None
