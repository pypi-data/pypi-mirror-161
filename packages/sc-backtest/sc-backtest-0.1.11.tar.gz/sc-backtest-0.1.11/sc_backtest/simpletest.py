...
class simpletest():
    ...
    def simple_pnl_cs(self, x, y, dn=False, ax=None, horizon=3, **kwargs):
        if len(x) != len(y):
            print(F'mismatched length of x, y, len(x) == {len(x)} len(y) == {len(y)}')
            return
    ...
    def plot_composite_cs(self, x, y,
                          markout_periods=20, cdf_period2=5, fig_return=False, horizon=3,
                          **kwargs):
        """"""
        if len(x) != len(y):
            print(F'mismatched length of x, y, len(x) == {len(x)} len(y) == {len(y)}')
        ...
        self.simple_pnl_cs(x, y, ax=ax5, horizon=horizon, title=title)
