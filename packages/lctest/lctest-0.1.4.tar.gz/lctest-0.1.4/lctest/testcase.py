...
def testcase(*testcases, log=False, param_length_limit=100):
    ...
    def deco_fun(fun):
        ...
        print(textformat(f"<cyan>Test result for</c> <b:blue><b>{fun.__name__}</>"))
        return wrapper()
    ...
    def deco_class(cls):
        def wrapper():
            for fun_name, testcases in get_runnable_methods_and_testcases():
                obj = cls()
                print(textformat(f"<b:yellow>Test result for</> <b><i>{cls.__name__}</>.<brightmagenta><b>{fun_name}</>"))
...
class _TestPrinter:
    ...
    def print_result(self, result, runtime):
        ...
        runtime_text = textformat(f"<gray><i>{runtime:.3f}ms</>")
