...
def distributed_diagnostics(self, signature=None, level=1):
    ...
    def dump(sig):
        kmkbt__zprw = self.overloads[sig]
        vdxbc__xdme = kmkbt__zprw.metadata.get('distributed_diagnostics', None)
        if vdxbc__xdme is None:
            docrs__ibypw = 'No distributed diagnostic available'
            raise bodo.utils.typing.BodoError(docrs__ibypw)
        vdxbc__xdme.dump(level, self.get_metadata(sig))
...
def master_mode_wrapper(numba_jit_wrapper):
    ...
    def _wrapper(pyfunc):
        sacjo__recal = numba_jit_wrapper(pyfunc)
        return master_mode.MasterModeDispatcher(sacjo__recal)
    return _wrapper
...
def jit(signature_or_function=None, pipeline_class=None, **options):
    ...
    if 'distributed' in options and isinstance(options['distributed'], bool):
        fri__ulkmj = options.pop('distributed')
        pipeline_class = (pipeline_class if fri__ulkmj else bodo.compiler.
            BodoCompilerSeq)
    if 'replicated' in options and isinstance(options['replicated'], bool):
        uskz__uluw = options.pop('replicated')
        pipeline_class = (bodo.compiler.BodoCompilerSeq if uskz__uluw else
            pipeline_class)
    udi__hpte = numba.jit(signature_or_function, pipeline_class=
        pipeline_class, **options)
    if master_mode.master_mode_on and bodo.get_rank(
        ) == master_mode.MASTER_RANK:
        if isinstance(udi__hpte, numba.dispatcher._DispatcherBase):
            return master_mode.MasterModeDispatcher(udi__hpte)
        else:
            return master_mode_wrapper(udi__hpte)
    else:
        return udi__hpte
