...
def _global_except_hook(exctype, value, traceback):
    ...
    pih__jxxh = MPI.COMM_WORLD
    jah__urgsh = pih__jxxh.Get_rank()
    ebned__uozuh = 3.0
    nczq__denn = True
    ejq__cdhhr = pih__jxxh.Ibarrier()
    vugcs__blsmi = time.time()
    while time.time() - vugcs__blsmi < ebned__uozuh:
        time.sleep(0.1)
        if ejq__cdhhr.Test():
            nczq__denn = False
            break
    try:
        ...
        if nczq__denn:
            sys.stderr.write(
                '\n*****************************************************\n')
            sys.stderr.write(
                f'   Uncaught exception detected on rank {jah__urgsh}. \n')
