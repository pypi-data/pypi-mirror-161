...
def box_regex_flag(typ, val, c):
    fcju__shil = c.box(types.int64, val)
    gxyg__rwx = c.pyapi.unserialize(c.pyapi.serialize_object(re.RegexFlag))
    return c.pyapi.call_function_objargs(gxyg__rwx, (fcju__shil,))


@unbox(RegexFlagsType)
def unbox_regex_flag(typ, obj, c):
    fcju__shil = c.pyapi.object_getattr_string(obj, 'value')
    return c.unbox(types.int64, fcju__shil)
...
def cast_match_obj_bool(context, builder, fromty, toty, val):
    out = cgutils.alloca_once_value(builder, context.get_constant(types.
        bool_, True))
    ljbe__mwe = context.get_python_api(builder)
    wte__gzem = builder.icmp_signed('==', val, ljbe__mwe.borrow_none())
    with builder.if_then(wte__gzem):
...
def init_const_pattern(typingctx, pat, pat_const=None):
    jbba__wceil = get_overload_const_str(pat_const)

    def codegen(context, builder, sig, args):
        return impl_ret_borrowed(context, builder, sig.return_type, args[0])
    return RePatternType(jbba__wceil)(pat, pat_const), codegen
...
def overload_pat_findall(p, string, pos=0, endpos=9223372036854775807):
    if p.pat_const:
        myywc__owh = re.compile(p.pat_const).groups
        typ = types.List(string_type)
        if myywc__owh > 1:
            typ = types.List(types.Tuple([string_type] * myywc__owh))
...
def overload_match_group(m, *args):
    ...
    yji__hdm = types.optional(string_type)
    if len(args) == 1:

        def _match_group_impl_one(m, *args):
            jph__ysqe = args[0]
            with numba.objmode(out=yji__hdm):
                out = m.group(jph__ysqe)
            return out
        return _match_group_impl_one
    xxpo__apv = 'tuple_str_{}'.format(len(args))
    setattr(types, xxpo__apv, types.Tuple([yji__hdm] * len(args)))
    pmf__rwdtp = ', '.join('group{}'.format(tbb__ycy + 1) for tbb__ycy in
        range(len(args)))
    ykrr__fmox = 'def _match_group_impl(m, *args):\n'
    ykrr__fmox += '  ({}) = args\n'.format(pmf__rwdtp)
    ykrr__fmox += "  with numba.objmode(out='{}'):\n".format(xxpo__apv)
    ykrr__fmox += '    out = m.group({})\n'.format(pmf__rwdtp)
    ykrr__fmox += '  return out\n'
    mio__ahsof = {}
    exec(ykrr__fmox, globals(), mio__ahsof)
    wcw__xzb = mio__ahsof['_match_group_impl']
    return wcw__xzb
...
def overload_match_groups(m, default=None):
    fntw__fjziv = types.List(types.optional(string_type))

    def _match_groups_impl(m, default=None):
        with numba.objmode(out=fntw__fjziv):
...
def overload_match_groupdict(m, default=None):
    ...
    def _check_dict_none(out):
        if any(snvfm__kju is None for snvfm__kju in out.values()):
            raise BodoError(
...
def overload_match_lastgroup(p):
    ...
    def _match_lastgroup_impl(p):
        with numba.objmode(lastgroup=yji__hdm):
