...
def get_dtype_size(typingctx, dtype=None):
    ...
    def codegen(context, builder, sig, args):
        ycl__sebb = context.get_abi_sizeof(context.get_data_type(dtype.dtype))
        return context.get_constant(types.intp, ycl__sebb)
...
def tofile_overload(arr, fname):
    if fname == string_type or isinstance(fname, types.StringLiteral):
        ...
        def tofile_impl(arr, fname):
            ...
            mbtv__fcfqg = np.ascontiguousarray(arr)
            dtype_size = get_dtype_size(mbtv__fcfqg.dtype)
            file_write(unicode_to_utf8(fname), mbtv__fcfqg.ctypes, 
                dtype_size * mbtv__fcfqg.size)
...
def file_write_parallel_overload(fname, arr, start, count):
    if fname == string_type:
        ...
        def _impl(fname, arr, start, count):
            mbtv__fcfqg = np.ascontiguousarray(arr)
            dtype_size = get_dtype_size(mbtv__fcfqg.dtype)
            dlo__ueg = dtype_size * bodo.libs.distributed_api.get_tuple_prod(
                mbtv__fcfqg.shape[1:])
            _file_write_parallel(unicode_to_utf8(fname), mbtv__fcfqg.ctypes,
                start, count, dlo__ueg)
            bodo.utils.utils.check_and_propagate_cpp_exception()
        return _impl
...
def get_file_size_overload(fname, count, offset, dtype_size):
    if fname == string_type:
        ...
        def impl(fname, count, offset, dtype_size):
            if offset < 0:
                return -1
            cqoq__emypd = _get_file_size(unicode_to_utf8(fname)) - offset
            bodo.utils.utils.check_and_propagate_cpp_exception()
            if count != -1:
                cqoq__emypd = min(cqoq__emypd, count * dtype_size)
            if cqoq__emypd < 0:
                return -1
            return cqoq__emypd
        return impl
