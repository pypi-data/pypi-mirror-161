...
def remove_dead_pq(pq_node, lives_no_aliases, lives, arg_aliases, alias_map,
    func_ir, typemap):
    dtbbn__zior = pq_node.out_vars[0].name
    oiebb__nsur = pq_node.out_vars[1].name
    if dtbbn__zior not in lives and oiebb__nsur not in lives:
        return None
    elif dtbbn__zior not in lives:
        pq_node.col_indices = []
        pq_node.df_colnames = []
        pq_node.out_used_cols = []
    elif oiebb__nsur not in lives:
