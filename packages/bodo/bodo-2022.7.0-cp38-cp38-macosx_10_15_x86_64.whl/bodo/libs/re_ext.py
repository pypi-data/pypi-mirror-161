...
def box_regex_flag(typ, val, c):
    ska__dwca = c.box(types.int64, val)
    vmj__ahnt = c.pyapi.unserialize(c.pyapi.serialize_object(re.RegexFlag))
    return c.pyapi.call_function_objargs(vmj__ahnt, (ska__dwca,))


@unbox(RegexFlagsType)
def unbox_regex_flag(typ, obj, c):
    ska__dwca = c.pyapi.object_getattr_string(obj, 'value')
    return c.unbox(types.int64, ska__dwca)
...
def cast_match_obj_bool(context, builder, fromty, toty, val):
    out = cgutils.alloca_once_value(builder, context.get_constant(types.
        bool_, True))
    ioy__ipme = context.get_python_api(builder)
    kqc__ahgi = builder.icmp_signed('==', val, ioy__ipme.borrow_none())
    with builder.if_then(kqc__ahgi):
...
def init_const_pattern(typingctx, pat, pat_const=None):
    rwd__akyl = get_overload_const_str(pat_const)

    def codegen(context, builder, sig, args):
        return impl_ret_borrowed(context, builder, sig.return_type, args[0])
    return RePatternType(rwd__akyl)(pat, pat_const), codegen
...
def overload_pat_findall(p, string, pos=0, endpos=9223372036854775807):
    if p.pat_const:
        slnb__kski = re.compile(p.pat_const).groups
        typ = types.List(string_type)
        if slnb__kski > 1:
            typ = types.List(types.Tuple([string_type] * slnb__kski))
...
def overload_match_group(m, *args):
    ...
    qzeb__bipty = types.optional(string_type)
    if len(args) == 1:

        def _match_group_impl_one(m, *args):
            ksdyc__rcj = args[0]
            with numba.objmode(out=qzeb__bipty):
                out = m.group(ksdyc__rcj)
            return out
        return _match_group_impl_one
    yyr__nqxvv = 'tuple_str_{}'.format(len(args))
    setattr(types, yyr__nqxvv, types.Tuple([qzeb__bipty] * len(args)))
    jkfar__sqy = ', '.join('group{}'.format(nyje__einl + 1) for nyje__einl in
        range(len(args)))
    rdcg__wxmj = 'def _match_group_impl(m, *args):\n'
    rdcg__wxmj += '  ({}) = args\n'.format(jkfar__sqy)
    rdcg__wxmj += "  with numba.objmode(out='{}'):\n".format(yyr__nqxvv)
    rdcg__wxmj += '    out = m.group({})\n'.format(jkfar__sqy)
    rdcg__wxmj += '  return out\n'
    zcet__tpla = {}
    exec(rdcg__wxmj, globals(), zcet__tpla)
    fdn__oqb = zcet__tpla['_match_group_impl']
    return fdn__oqb
...
def overload_match_groups(m, default=None):
    zig__lvl = types.List(types.optional(string_type))

    def _match_groups_impl(m, default=None):
        with numba.objmode(out=zig__lvl):
...
def overload_match_groupdict(m, default=None):
    ...
    def _check_dict_none(out):
        if any(eqw__xagp is None for eqw__xagp in out.values()):
            raise BodoError(
...
def overload_match_lastgroup(p):
    ...
    def _match_lastgroup_impl(p):
        with numba.objmode(lastgroup=qzeb__bipty):
