...
def distributed_diagnostics(self, signature=None, level=1):
    ...
    def dump(sig):
        mryj__iylvc = self.overloads[sig]
        bdsl__hkt = mryj__iylvc.metadata.get('distributed_diagnostics', None)
        if bdsl__hkt is None:
            slrqw__ukuqo = 'No distributed diagnostic available'
            raise bodo.utils.typing.BodoError(slrqw__ukuqo)
        bdsl__hkt.dump(level, self.get_metadata(sig))
...
def master_mode_wrapper(numba_jit_wrapper):
    ...
    def _wrapper(pyfunc):
        wjthr__oaed = numba_jit_wrapper(pyfunc)
        return master_mode.MasterModeDispatcher(wjthr__oaed)
    return _wrapper
...
def jit(signature_or_function=None, pipeline_class=None, **options):
    ...
    if 'distributed' in options and isinstance(options['distributed'], bool):
        zqz__tbrkx = options.pop('distributed')
        pipeline_class = (pipeline_class if zqz__tbrkx else bodo.compiler.
            BodoCompilerSeq)
    if 'replicated' in options and isinstance(options['replicated'], bool):
        vxrdv__gyptn = options.pop('replicated')
        pipeline_class = (bodo.compiler.BodoCompilerSeq if vxrdv__gyptn else
            pipeline_class)
    vyjrc__yee = numba.jit(signature_or_function, pipeline_class=
        pipeline_class, **options)
    if master_mode.master_mode_on and bodo.get_rank(
        ) == master_mode.MASTER_RANK:
        if isinstance(vyjrc__yee, numba.dispatcher._DispatcherBase):
            return master_mode.MasterModeDispatcher(vyjrc__yee)
        else:
            return master_mode_wrapper(vyjrc__yee)
    else:
        return vyjrc__yee
