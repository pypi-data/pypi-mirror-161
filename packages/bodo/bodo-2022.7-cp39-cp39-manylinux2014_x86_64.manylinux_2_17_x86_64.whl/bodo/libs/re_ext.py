...
def box_regex_flag(typ, val, c):
    kljx__dvyc = c.box(types.int64, val)
    gewp__bfhrc = c.pyapi.unserialize(c.pyapi.serialize_object(re.RegexFlag))
    return c.pyapi.call_function_objargs(gewp__bfhrc, (kljx__dvyc,))


@unbox(RegexFlagsType)
def unbox_regex_flag(typ, obj, c):
    kljx__dvyc = c.pyapi.object_getattr_string(obj, 'value')
    return c.unbox(types.int64, kljx__dvyc)
...
def cast_match_obj_bool(context, builder, fromty, toty, val):
    out = cgutils.alloca_once_value(builder, context.get_constant(types.
        bool_, True))
    kjtib__nlp = context.get_python_api(builder)
    xyb__uvcjk = builder.icmp_signed('==', val, kjtib__nlp.borrow_none())
    with builder.if_then(xyb__uvcjk):
...
def init_const_pattern(typingctx, pat, pat_const=None):
    xfvu__xnmx = get_overload_const_str(pat_const)

    def codegen(context, builder, sig, args):
        return impl_ret_borrowed(context, builder, sig.return_type, args[0])
    return RePatternType(xfvu__xnmx)(pat, pat_const), codegen
...
def overload_pat_findall(p, string, pos=0, endpos=9223372036854775807):
    if p.pat_const:
        qsvp__ame = re.compile(p.pat_const).groups
        typ = types.List(string_type)
        if qsvp__ame > 1:
            typ = types.List(types.Tuple([string_type] * qsvp__ame))
...
def overload_match_group(m, *args):
    ...
    myxrq__noyy = types.optional(string_type)
    if len(args) == 1:

        def _match_group_impl_one(m, *args):
            mvqkq__bdox = args[0]
            with numba.objmode(out=myxrq__noyy):
                out = m.group(mvqkq__bdox)
            return out
        return _match_group_impl_one
    wkf__vhs = 'tuple_str_{}'.format(len(args))
    setattr(types, wkf__vhs, types.Tuple([myxrq__noyy] * len(args)))
    burs__huz = ', '.join('group{}'.format(bbm__qhh + 1) for bbm__qhh in
        range(len(args)))
    imbn__mevv = 'def _match_group_impl(m, *args):\n'
    imbn__mevv += '  ({}) = args\n'.format(burs__huz)
    imbn__mevv += "  with numba.objmode(out='{}'):\n".format(wkf__vhs)
    imbn__mevv += '    out = m.group({})\n'.format(burs__huz)
    imbn__mevv += '  return out\n'
    jsxwr__hzl = {}
    exec(imbn__mevv, globals(), jsxwr__hzl)
    zxvaq__ijyka = jsxwr__hzl['_match_group_impl']
    return zxvaq__ijyka
...
def overload_match_groups(m, default=None):
    rpx__qsqy = types.List(types.optional(string_type))

    def _match_groups_impl(m, default=None):
        with numba.objmode(out=rpx__qsqy):
...
def overload_match_groupdict(m, default=None):
    ...
    def _check_dict_none(out):
        if any(byo__uzo is None for byo__uzo in out.values()):
            raise BodoError(
...
def overload_match_lastgroup(p):
    ...
    def _match_lastgroup_impl(p):
        with numba.objmode(lastgroup=myxrq__noyy):
