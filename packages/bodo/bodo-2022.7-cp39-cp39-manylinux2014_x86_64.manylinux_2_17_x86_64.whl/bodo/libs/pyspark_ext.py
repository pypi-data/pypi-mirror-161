...
def box_spark_session(typ, val, c):
    axm__sqpbn = c.context.insert_const_string(c.builder.module, 'pyspark')
    wkwhc__xmz = c.pyapi.import_module_noblock(axm__sqpbn)
    tca__ttjbz = c.pyapi.object_getattr_string(wkwhc__xmz, 'sql')
    elqj__vucl = c.pyapi.object_getattr_string(tca__ttjbz, 'SparkSession')
    dzzxn__jbsp = c.pyapi.object_getattr_string(elqj__vucl, 'builder')
    spgma__onsqa = c.pyapi.call_method(dzzxn__jbsp, 'getOrCreate', ())
    c.pyapi.decref(wkwhc__xmz)
    c.pyapi.decref(tca__ttjbz)
    c.pyapi.decref(elqj__vucl)
    c.pyapi.decref(dzzxn__jbsp)
    return spgma__onsqa
...
class RowType(types.BaseNamedTuple):
    ...
    def __init__(self, types, fields):
        ...
        dkrs__ebxnm = 'Row({})'.format(', '.join(f'{imw__qvx}:{yxy__pag}' for
            imw__qvx, yxy__pag in zip(self.fields, self.types)))
        super(RowType, self).__init__(dkrs__ebxnm)
...
class RowModel(models.StructModel):
    ...
    def __init__(self, dmm, fe_type):
        kpnex__wzj = [(imw__qvx, yxy__pag) for imw__qvx, yxy__pag in zip(
            fe_type.fields, fe_type.types)]
        super(RowModel, self).__init__(dmm, fe_type, kpnex__wzj)
...
def typeof_row(val, c):
    ...
    return RowType(tuple(numba.typeof(hdt__ogpu) for hdt__ogpu in val), fields)


@box(RowType)
def box_row(typ, val, c):
    tpxhr__kuc = c.pyapi.unserialize(c.pyapi.serialize_object(pyspark.sql.
        types.Row))
    if all(imw__qvx.startswith(ANON_SENTINEL) for imw__qvx in typ.fields):
        uwh__yudqn = [c.box(yxy__pag, c.builder.extract_value(val, i)) for 
            i, yxy__pag in enumerate(typ.types)]
        brcoo__kfuh = c.pyapi.call_function_objargs(tpxhr__kuc, uwh__yudqn)
        for obj in uwh__yudqn:
            c.pyapi.decref(obj)
        c.pyapi.decref(tpxhr__kuc)
        return brcoo__kfuh
    args = c.pyapi.tuple_pack([])
    uwh__yudqn = []
    cuo__srff = []
    for i, yxy__pag in enumerate(typ.types):
        rwiah__mnaqz = c.builder.extract_value(val, i)
        obj = c.box(yxy__pag, rwiah__mnaqz)
        cuo__srff.append((typ.fields[i], obj))
        uwh__yudqn.append(obj)
    kws = c.pyapi.dict_pack(cuo__srff)
    brcoo__kfuh = c.pyapi.call(tpxhr__kuc, args, kws)
    for obj in uwh__yudqn:
        c.pyapi.decref(obj)
    c.pyapi.decref(tpxhr__kuc)
    c.pyapi.decref(args)
...
class RowConstructor(AbstractTemplate):
    ...
    def generic(self, args, kws):
        ...
        owugw__sbn = ', '.join(f'arg{i}' for i in range(len(args)))
        axs__tmcr = ', '.join(f"{amqwx__hrt} = ''" for amqwx__hrt in kws)
        func_text = f'def row_stub({owugw__sbn}{axs__tmcr}):\n'
        func_text += '    pass\n'
        sweq__jpso = {}
        exec(func_text, {}, sweq__jpso)
        kplzt__ochs = sweq__jpso['row_stub']
        cwj__jxsv = numba.core.utils.pysignature(kplzt__ochs)
        if args:
            juay__miibm = RowType(args, tuple(f'{ANON_SENTINEL}{i}' for i in
                range(len(args))))
            return signature(juay__miibm, *args).replace(pysig=cwj__jxsv)
        kws = dict(kws)
        juay__miibm = RowType(tuple(kws.values()), tuple(kws.keys()))
        return signature(juay__miibm, *kws.values()).replace(pysig=cwj__jxsv)
...
class SparkDataFrameModel(models.StructModel):
    ...
    def __init__(self, dmm, fe_type):
        kpnex__wzj = [('df', fe_type.df)]
        super(SparkDataFrameModel, self).__init__(dmm, fe_type, kpnex__wzj)
...
def overload_create_df(sp_session, data, schema=None, samplingRatio=None,
    ...
    eqs__cicw = data.dtype.fields
    gyjz__dinuy = len(data.dtype.types)
    func_text = (
        'def impl(sp_session, data, schema=None, samplingRatio=None, verifySchema=True):\n'
        )
    func_text += f'  n = len(data)\n'
    hjqpz__eir = []
    for i, yxy__pag in enumerate(data.dtype.types):
        yhuq__mjix = dtype_to_array_type(yxy__pag)
        func_text += (
            f'  A{i} = bodo.utils.utils.alloc_type(n, arr_typ{i}, (-1,))\n')
        hjqpz__eir.append(yhuq__mjix)
    func_text += f'  for i in range(n):\n'
    func_text += f'    r = data[i]\n'
    for i in range(gyjz__dinuy):
        func_text += (
            f'    A{i}[i] = bodo.utils.conversion.unbox_if_timestamp(r[{i}])\n'
            )
    xmih__urca = '({}{})'.format(', '.join(f'A{i}' for i in range(
        gyjz__dinuy)), ',' if len(eqs__cicw) == 1 else '')
    func_text += (
        '  index = bodo.hiframes.pd_index_ext.init_range_index(0, n, 1, None)\n'
        )
    func_text += f"""  pdf = bodo.hiframes.pd_dataframe_ext.init_dataframe({xmih__urca}, index, __col_name_meta_value_create_df)
"""
    func_text += f'  pdf = bodo.scatterv(pdf)\n'
    func_text += f'  return bodo.libs.pyspark_ext.init_spark_df(pdf)\n'
    sweq__jpso = {}
    grwxs__wnq = {'bodo': bodo, '__col_name_meta_value_create_df':
        ColNamesMetaType(tuple(eqs__cicw))}
    for i in range(gyjz__dinuy):
        grwxs__wnq[f'arr_typ{i}'] = hjqpz__eir[i]
    exec(func_text, grwxs__wnq, sweq__jpso)
    impl = sweq__jpso['impl']
    return impl
...
def overload_df_to_rows(df):
    ...
    jtvw__iwsjv = ', '.join(f'{c}=A{i}[i]' for i, c in enumerate(df.columns))
    func_text += f'    out.append(Row({jtvw__iwsjv}))\n'
    func_text += '  return out\n'
    sweq__jpso = {}
    grwxs__wnq = {'bodo': bodo, 'Row': pyspark.sql.types.Row}
    exec(func_text, grwxs__wnq, sweq__jpso)
    impl = sweq__jpso['impl']
    return impl
...
def _gen_df_select(spark_df, cols, avoid_stararg=False):
    ...
    for col in cols:
        ...
        data, ocw__cdsnb = _gen_col_code(col, df_type)
        func_text += ocw__cdsnb
        out_data.append(data)
    return _gen_init_spark_df(func_text, out_data, out_col_names)


def _gen_init_spark_df(func_text, out_data, out_col_names):
    xmih__urca = '({}{})'.format(', '.join(out_data), ',' if len(out_data) ==
        1 else '')
    zjr__ijenf = '0' if not out_data else f'len({out_data[0]})'
    func_text += f'  n = {zjr__ijenf}\n'
    func_text += (
        '  index = bodo.hiframes.pd_index_ext.init_range_index(0, n, 1, None)\n'
        )
    func_text += f"""  pdf = bodo.hiframes.pd_dataframe_ext.init_dataframe({xmih__urca}, index, __col_name_meta_value_init_spark_df)
"""
    func_text += f'  return bodo.libs.pyspark_ext.init_spark_df(pdf)\n'
    sweq__jpso = {}
    grwxs__wnq = {'bodo': bodo, 'np': np,
...
def overload_show(spark_df, n=20, truncate=True, vertical=False):
    lar__sdxwo = dict(truncate=truncate, vertical=vertical)
    lxh__gru = dict(truncate=True, vertical=False)
    check_unsupported_args('SparkDataFrameType.show', lar__sdxwo, lxh__gru)
...
def overload_with_column(spark_df, colName, col):
    ...
    jzrli__hixw = spark_df.df.columns
    qla__tjbuc = jzrli__hixw if col_name in jzrli__hixw else jzrli__hixw + (
        col_name,)
    sei__gmte, oqlld__mxsmo = _gen_col_code(col, spark_df.df)
    out_data = [(sei__gmte if c == col_name else
        f'bodo.hiframes.pd_dataframe_ext.get_dataframe_data(df, {jzrli__hixw.index(c)})'
        ) for c in qla__tjbuc]
    func_text = 'def impl(spark_df, colName, col):\n'
    func_text += '  df = spark_df._df\n'
    func_text += oqlld__mxsmo
    return _gen_init_spark_df(func_text, out_data, qla__tjbuc)
...
def overload_with_column_renamed(spark_df, existing, new):
    ...
    ugqus__htb = get_overload_const_str(existing)
    omth__ihc = get_overload_const_str(new)
    jzrli__hixw = spark_df.df.columns
    qla__tjbuc = tuple(omth__ihc if c == ugqus__htb else c for c in jzrli__hixw
        )
    out_data = [f'bodo.hiframes.pd_dataframe_ext.get_dataframe_data(df, {i})'
         for i in range(len(jzrli__hixw))]
...
def overload_dataframe_columns(spark_df):
    zykz__fbo = list(str(amqwx__hrt) for amqwx__hrt in spark_df.df.columns)
    func_text = 'def impl(spark_df):\n'
    func_text += f'  return {zykz__fbo}\n'
    sweq__jpso = {}
    exec(func_text, {}, sweq__jpso)
    impl = sweq__jpso['impl']
    return impl
...
def init_col_from_name(typingctx, col=None):
    ...
    vgekg__zenet = get_overload_const_str(col)
    ubitl__pewg = ColumnType(ExprType('col', (vgekg__zenet,)))

    def codegen(context, builder, signature, args):
        return context.get_constant_null(ubitl__pewg)
    return ubitl__pewg(col), codegen
...
def init_f_sum(typingctx, col=None):
    ubitl__pewg = ColumnType(ExprType('sum', (col.expr,)))
...
def _gen_col_code_expr(expr, df_type):
    ...
    if expr.op == 'sum':
        rea__fjoow, dtrd__voedv = _gen_col_code_expr(expr.children[0], df_type)
        i = ir_utils.next_label()
        func_text = f"""  A{i} = np.asarray([bodo.libs.array_ops.array_op_sum({rea__fjoow}, True, 0)])
"""
        return f'A{i}', dtrd__voedv + func_text


def _gen_col_code_colname(col_name, df_type):
    mcdcz__rkxp = df_type.columns.index(col_name)
    i = ir_utils.next_label()
    func_text = (
        f'  A{i} = bodo.hiframes.pd_dataframe_ext.get_dataframe_data(df, {mcdcz__rkxp})\n'
        )
