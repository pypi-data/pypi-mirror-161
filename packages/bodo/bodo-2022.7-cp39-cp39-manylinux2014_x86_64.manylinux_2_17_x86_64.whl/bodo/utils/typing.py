...
def decode_if_dict_array_overload(A):
    if isinstance(A, types.BaseTuple):
        rkt__kixrn = len(A.types)
        qqp__zdvtj = 'def f(A):\n'
        mpinb__nio = ','.join(f'decode_if_dict_array(A[{i}])' for i in
            range(rkt__kixrn))
        qqp__zdvtj += '  return ({}{})\n'.format(mpinb__nio, ',' if 
            rkt__kixrn == 1 else '')
        jbu__csx = {}
        exec(qqp__zdvtj, {'decode_if_dict_array': decode_if_dict_array},
            jbu__csx)
        impl = jbu__csx['f']
        return impl
    if isinstance(A, types.List):
        ...
        def impl(A):
            ...
            for a in A:
                rkt__kixrn += 1
            iluu__nan = []
            for i in range(rkt__kixrn):
                iluu__nan.append(decode_if_dict_array(A[i]))
            return iluu__nan
        return impl
    ...
    if isinstance(A, bodo.SeriesType):
        ...
        def impl(A):
            mgayb__rnjre = bodo.hiframes.pd_series_ext.get_series_data(A)
            snnl__cmg = bodo.hiframes.pd_series_ext.get_series_index(A)
            name = bodo.hiframes.pd_series_ext.get_series_name(A)
            mmi__utaj = decode_if_dict_array(mgayb__rnjre)
            return bodo.hiframes.pd_series_ext.init_series(mmi__utaj,
                snnl__cmg, name)
        return impl
...
def to_str_arr_if_dict_array(t):
    ...
    if isinstance(t, bodo.TableType):
        ocwcr__yvdg = tuple(to_str_arr_if_dict_array(t) for t in t.arr_types)
        return bodo.TableType(ocwcr__yvdg, t.has_runtime_cols)
...
class BodoError(NumbaError):
    ...
    def __init__(self, msg, loc=None, locs_in_msg=None):
        ...
        pdz__npt = numba.core.errors.termcolor().errmsg
        super(BodoError, self).__init__(pdz__npt(msg), loc)
...
def raise_bodo_error(msg, loc=None):
    if bodo.transforms.typing_pass.in_partial_typing:
        ...
        rgcw__fqe = [] if loc is None else [loc]
        raise BodoError(msg, locs_in_msg=rgcw__fqe)
...
class FileInfo:
    ...
    def get_schema(self, fname):
        kna__zoo = self.get_full_filename(fname)
        return self._get_schema(kna__zoo)
...
class FilenameModel(models.StructModel):
    ...
    def __init__(self, dmm, fe_type):
        ftky__mntm = dmm.lookup(bodo.typeof(fe_type.fname))
        jmx__qvm = [(a, innv__xtf) for a, innv__xtf in zip(ftky__mntm.
            _fields, ftky__mntm._members)]
        super().__init__(dmm, fe_type, jmx__qvm)
...
def is_initial_value_type(t):
    if not isinstance(t, types.InitialValue) or t.initial_value is None:
        return False
    yeh__wihn = t.initial_value
    if isinstance(yeh__wihn, dict):
        yeh__wihn = yeh__wihn.values()
    return not any(isinstance(pnm__qdlo, (types.Poison, numba.core.
        interpreter._UNKNOWN_VALUE)) for pnm__qdlo in yeh__wihn)
...
def is_overload_constant_dict(val):
    return isinstance(val, types.LiteralStrKeyDict) and all(is_literal_type
        (pnm__qdlo) for pnm__qdlo in val.types) or is_initial_value_dict_type(
...
def is_overload_bool_list(val):
    return is_overload_constant_list(val) and all(is_overload_constant_bool
        (pnm__qdlo) for pnm__qdlo in get_overload_const_list(val))
...
def get_overload_const(val):
    ...
    if isinstance(val, types.LiteralList):
        oxyl__yntib = []
        for pnm__qdlo in val.literal_value:
            csnd__iyihg = get_overload_const(pnm__qdlo)
            if csnd__iyihg == NOT_CONSTANT:
                return NOT_CONSTANT
            else:
                oxyl__yntib.append(csnd__iyihg)
        return oxyl__yntib
    ...
    if isinstance(val, types.BaseTuple):
        oxyl__yntib = []
        for pnm__qdlo in val.types:
            csnd__iyihg = get_overload_const(pnm__qdlo)
            if csnd__iyihg == NOT_CONSTANT:
                return NOT_CONSTANT
            else:
                oxyl__yntib.append(csnd__iyihg)
        return tuple(oxyl__yntib)
...
def ensure_constant_arg(fname, arg_name, val, const_type):
    csnd__iyihg = get_overload_const(val)
    pqqev__bzgl = _const_type_repr.get(const_type, str(const_type))
    if not isinstance(csnd__iyihg, const_type):
        raise BodoError(
            f"{fname}(): argument '{arg_name}' should be a constant {pqqev__bzgl} not {val}"
            )


def ensure_constant_values(fname, arg_name, val, const_values):
    csnd__iyihg = get_overload_const(val)
    if csnd__iyihg not in const_values:
        raise BodoError(
            f"{fname}(): argument '{arg_name}' should be a constant value in {const_values} not '{csnd__iyihg}'"
            )
...
def check_unsupported_args(fname, args_dict, arg_defaults_dict,
    ...
    fye__ulji = False
    for a in args_dict:
        idt__btgv = get_overload_const(args_dict[a])
        gadxc__ceegd = arg_defaults_dict[a]
        if (idt__btgv is NOT_CONSTANT or idt__btgv is not None and 
            gadxc__ceegd is None or idt__btgv is None and gadxc__ceegd is not
            None or idt__btgv != gadxc__ceegd or idt__btgv is not _no_input and
            gadxc__ceegd is _no_input or idt__btgv is _no_input and 
            gadxc__ceegd is not _no_input):
            paj__tgypi = (
                f'{fn_str}: {a} parameter only supports default value {gadxc__ceegd}'
                )
            fye__ulji = True
            break
    if fye__ulji and package_name == 'pandas':
        if module_name == 'IO':
            paj__tgypi += """
...
def get_overload_constant_dict(val):
    if isinstance(val, types.LiteralStrKeyDict):
        return {get_literal_value(gen__xoac): get_literal_value(pnm__qdlo) for
            gen__xoac, pnm__qdlo in val.literal_value.items()}
    ...
    if isinstance(val, types.DictType):
        ...
        return val.initial_value
    kpkdc__vcgam = [get_overload_const(pnm__qdlo) for pnm__qdlo in val.
        types[1:]]
    return {kpkdc__vcgam[2 * i]: kpkdc__vcgam[2 * i + 1] for i in range(len
        (kpkdc__vcgam) // 2)}
...
def get_overload_const_list(val):
    ...
    if is_initial_value_list_type(val):
        return val.initial_value
    if isinstance(val, types.LiteralList):
        return [get_literal_value(pnm__qdlo) for pnm__qdlo in val.literal_value
            ]
...
def parse_dtype(dtype, func_name=None):
    ...
    if isinstance(dtype, types.Number) or dtype == bodo.string_type:
        return dtype
    try:
        eijvw__nybvy = get_overload_const_str(dtype)
        if eijvw__nybvy.startswith('Int') or eijvw__nybvy.startswith('UInt'):
            return bodo.libs.int_arr_ext.typeof_pd_int_dtype(pd.api.types.
                pandas_dtype(eijvw__nybvy), None)
        if eijvw__nybvy == 'boolean':
            return bodo.libs.bool_arr_ext.boolean_dtype
        if eijvw__nybvy == 'str':
            return bodo.string_type
        return numba.np.numpy_support.from_dtype(np.dtype(eijvw__nybvy))
    except:
        pass
...
def get_index_names(t, func_name, default_name):
    ...
    malvw__bytzl = '{}: index name should be a constant string'.format(
        func_name)
    if isinstance(t, MultiIndexType):
        vze__bbxm = []
        for i, jas__edsj in enumerate(t.names_typ):
            if jas__edsj == types.none:
                vze__bbxm.append('level_{}'.format(i))
                continue
            if not is_overload_constant_str(jas__edsj):
                raise BodoError(malvw__bytzl)
            vze__bbxm.append(get_overload_const_str(jas__edsj))
        return tuple(vze__bbxm)
...
if types.SliceLiteral in numba.core.pythonapi._boxers.functions:
    ...
    def box_slice_literal(typ, val, c):
        ozezn__gex = typ.literal_value
        wkkho__zbrch = []
        for gbo__xffs in ('start', 'stop', 'step'):
            nqub__ecpl = getattr(typ.literal_value, gbo__xffs)
            lcwj__flzx = c.pyapi.make_none(
                ) if nqub__ecpl is None else c.pyapi.from_native_value(types
                .literal(nqub__ecpl), nqub__ecpl, c.env_manager)
            wkkho__zbrch.append(lcwj__flzx)
        hvhy__yojds = c.pyapi.unserialize(c.pyapi.serialize_object(slice))
        tsi__qrafp = c.pyapi.call_function_objargs(hvhy__yojds, wkkho__zbrch)
        for a in wkkho__zbrch:
            c.pyapi.decref(a)
        c.pyapi.decref(hvhy__yojds)
        return tsi__qrafp
...
def box_list_literal(typ, val, c):
    ozbh__wywp = typ.literal_value
    hfsg__wyjrj = [c.pyapi.from_native_value(types.literal(pnm__qdlo),
        pnm__qdlo, c.env_manager) for pnm__qdlo in ozbh__wywp]
    qibxf__esb = c.pyapi.list_pack(hfsg__wyjrj)
    for a in hfsg__wyjrj:
        c.pyapi.decref(a)
    return qibxf__esb
...
def list_literal_to_list(context, builder, fromty, toty, val):
    ...
    uvzv__eiu = types.List(toty.dtype)
...
def is_literal_type(t):
    ...
    return isinstance(t, (types.Literal, types.Omitted)) and not isinstance(t,
        ...
        is_literal_type(pnm__qdlo) for pnm__qdlo in t.types
...
def get_literal_value(t):
    ...
    if isinstance(t, types.Literal):
        if isinstance(t, types.LiteralStrKeyDict):
            return {get_literal_value(gen__xoac): get_literal_value(
                pnm__qdlo) for gen__xoac, pnm__qdlo in t.literal_value.items()}
        if isinstance(t, types.LiteralList):
            return [get_literal_value(pnm__qdlo) for pnm__qdlo in t.
                literal_value]
        return t.literal_value
    if isinstance(t, types.Omitted):
        return t.value
    if isinstance(t, types.BaseTuple):
        return tuple(get_literal_value(pnm__qdlo) for pnm__qdlo in t.types)
...
def dtype_to_array_type(dtype):
    ...
    if isinstance(dtype, (types.Number, types.Boolean, types.NPDatetime,
        types.NPTimedelta)):
        mgayb__rnjre = types.Array(dtype, 1, 'C')
        if iox__fnagp:
            return to_nullable_type(mgayb__rnjre)
        return mgayb__rnjre
...
def get_udf_out_arr_type(f_return_type, return_nullable=False):
    ...
    luqwy__pwyeu = dtype_to_array_type(f_return_type)
    luqwy__pwyeu = to_nullable_type(luqwy__pwyeu
        ) if return_nullable else luqwy__pwyeu
    return luqwy__pwyeu
...
def types_equality_exists(t1, t2):
    vaj__ldgcb = numba.core.registry.cpu_target.typing_context
    try:
        vaj__ldgcb.resolve_function_type(operator.eq, (t1, t2), {})
        return True
    except:
        return False
...
def is_hashable_type(t):
    whitelist_types = (types.UnicodeType, types.StringLiteral, types.
        ...
        PandasTimestampType)
    wts__ifyu = (types.bool_, bodo.datetime64ns, bodo.timedelta64ns, bodo.
        pd_timedelta_type)
    if isinstance(t, whitelist_types) or t in wts__ifyu:
        return True
    vaj__ldgcb = numba.core.registry.cpu_target.typing_context
    try:
        vaj__ldgcb.resolve_function_type(hash, (t,), {})
        return True
    except:
        return False
...
def to_nullable_type(t):
    ...
    if isinstance(t, DataFrameType):
        smlb__fyjor = tuple(to_nullable_type(t) for t in t.data)
        return DataFrameType(smlb__fyjor, t.index, t.columns, t.dist, t.
            is_table_format)
...
def is_common_scalar_dtype(scalar_types):
    emk__chh, qvg__ybe = get_common_scalar_dtype(scalar_types)
    return qvg__ybe
...
def get_common_scalar_dtype(scalar_types):
    ...
    try:
        phwgg__qfxg = np.find_common_type([numba.np.numpy_support.as_dtype(
            t) for t in scalar_types], [])
        if phwgg__qfxg != object:
            return numba.np.numpy_support.from_dtype(phwgg__qfxg), True
    except numba.core.errors.NumbaNotImplementedError as uare__avb:
        pass
    ...
    cot__zvhyi = itertools.groupby(scalar_types)
    if next(cot__zvhyi, True) and not next(cot__zvhyi, False):
...
def get_nullable_and_non_nullable_types(array_of_types):
    ...
    for typ in array_of_types:
        if typ == bodo.libs.bool_arr_ext.boolean_array:
            jrh__lhin.append(types.Array(types.bool_, 1, 'C'))
        elif isinstance(typ, bodo.libs.int_arr_ext.IntegerArrayType):
            jrh__lhin.append(types.Array(typ.dtype, 1, 'C'))
        elif isinstance(typ, types.Array):
            if typ.dtype == types.bool_:
                jrh__lhin.append(bodo.libs.bool_arr_ext.boolean_array)
            if isinstance(typ.dtype, types.Integer):
                jrh__lhin.append(bodo.libs.int_arr_ext.IntegerArrayType(typ
                    .dtype))
        jrh__lhin.append(typ)
    return jrh__lhin
...
def _gen_objmode_overload(func, output_type, attr_name=None, is_function=
    ...
    if is_function:
        rggq__akki = getfullargspec(func)
        assert rggq__akki.varargs is None, 'varargs not supported'
        assert rggq__akki.varkw is None, 'varkw not supported'
        defaults = [] if rggq__akki.defaults is None else rggq__akki.defaults
        lqx__uuj = len(rggq__akki.args) - len(defaults)
        args = rggq__akki.args[1:] if attr_name else rggq__akki.args[:]
        apdlu__owqw = []
        for i, xrac__mxsdb in enumerate(rggq__akki.args):
            if i < lqx__uuj:
                apdlu__owqw.append(xrac__mxsdb)
            elif str(defaults[i - lqx__uuj]) != '<deprecated parameter>':
                apdlu__owqw.append(xrac__mxsdb + '=' + str(defaults[i -
                    lqx__uuj]))
            else:
                args.remove(xrac__mxsdb)
        if rggq__akki.kwonlyargs is not None:
            for xrac__mxsdb in rggq__akki.kwonlyargs:
                args.append(f'{xrac__mxsdb}={xrac__mxsdb}')
                apdlu__owqw.append(
                    f'{xrac__mxsdb}={str(rggq__akki.kwonlydefaults[xrac__mxsdb])}'
                    )
        sig = ', '.join(apdlu__owqw)
        args = ', '.join(args)
    else:
        sig = 'self'
    ...
    wsd__zgxna = f'self.{attr_name}' if attr_name else f'{func_name}'
    qqp__zdvtj = f'def overload_impl({sig}):\n'
    qqp__zdvtj += f'    def impl({sig}):\n'
    if single_rank:
        qqp__zdvtj += f'        if bodo.get_rank() == 0:\n'
        xmg__qexi = '    '
    else:
        xmg__qexi = ''
    qqp__zdvtj += (
        f"        {xmg__qexi}with numba.objmode(res='{type_name}'):\n")
    if is_function:
        qqp__zdvtj += f'            {xmg__qexi}res = {wsd__zgxna}({args})\n'
    else:
        qqp__zdvtj += f'            {xmg__qexi}res = {wsd__zgxna}\n'
    qqp__zdvtj += f'        return res\n'
    qqp__zdvtj += f'    return impl\n'
    jbu__csx = {}
    sfeis__miby = globals()
    if not attr_name:
        sfeis__miby[func_name] = func
    exec(qqp__zdvtj, sfeis__miby, jbu__csx)
    rcz__mspce = jbu__csx['overload_impl']
    return rcz__mspce


def gen_objmode_func_overload(func, output_type=None, single_rank=False):
    try:
        rcz__mspce = _gen_objmode_overload(func, output_type, is_function=
            True, single_rank=single_rank)
        overload(func, no_unliteral=True)(rcz__mspce)
    except Exception as uare__avb:
        pass
...
def gen_objmode_method_overload(obj_type, method_name, method, output_type=
    ...
    try:
        rcz__mspce = _gen_objmode_overload(method, output_type, method_name,
            True, single_rank)
        overload_method(obj_type, method_name, no_unliteral=True)(rcz__mspce)
    except Exception as uare__avb:
        pass
...
def gen_objmode_attr_overload(obj_type, attr_name, attr, output_type=None,
    single_rank=False):
    try:
        rcz__mspce = _gen_objmode_overload(attr, output_type, attr_name, 
            False, single_rank)
        overload_attribute(obj_type, attr_name, no_unliteral=True)(rcz__mspce)
    except Exception as uare__avb:
        pass
...
class NumTypeStaticGetItem(AbstractTemplate):
    ...
    def generic(self, args, kws):
        val, npo__dzu = args
        if isinstance(npo__dzu, slice) and (isinstance(val, types.
            NumberClass) or isinstance(val, types.TypeRef) and isinstance(
            val.instance_type, (types.NPDatetime, types.NPTimedelta))):
            return signature(types.TypeRef(val.instance_type[npo__dzu]), *args)
...
def construct_pysig(arg_names, defaults):
    qqp__zdvtj = f'def stub('
    for xrac__mxsdb in arg_names:
        qqp__zdvtj += xrac__mxsdb
        if xrac__mxsdb in defaults:
            if isinstance(defaults[xrac__mxsdb], str):
                qqp__zdvtj += f"='{defaults[xrac__mxsdb]}'"
            else:
                qqp__zdvtj += f'={defaults[xrac__mxsdb]}'
        qqp__zdvtj += ', '
    qqp__zdvtj += '):\n'
    qqp__zdvtj += '    pass\n'
    jbu__csx = {}
    exec(qqp__zdvtj, {}, jbu__csx)
    kenv__bbwiq = jbu__csx['stub']
    return numba.core.utils.pysignature(kenv__bbwiq)


def fold_typing_args(func_name, args, kws, arg_names, defaults,
    unsupported_arg_names=()):
    kws = dict(kws)
    nmiqe__snv = len(arg_names)
    ldkfs__qingg = len(args) + len(kws)
    if ldkfs__qingg > nmiqe__snv:
        pnhu__iuj = 'argument' if nmiqe__snv == 1 else 'arguments'
        nib__gnoi = 'was' if ldkfs__qingg == 1 else 'were'
        raise BodoError(
            f'{func_name}(): Too many arguments specified. Function takes {nmiqe__snv} {pnhu__iuj}, but {ldkfs__qingg} {nib__gnoi} provided.'
            )
    bgv__pugiq = bodo.utils.typing.construct_pysig(arg_names, defaults)
    try:
        oloa__pkdd = bodo.utils.transform.fold_argument_types(bgv__pugiq,
            args, kws)
    except Exception as euryo__azd:
        raise_bodo_error(f'{func_name}(): {euryo__azd}')
    if unsupported_arg_names:
        ...
        for i, arg_name in enumerate(arg_names):
            if arg_name in unsupported_arg_names:
                ...
                ottnl__vwzz[arg_name] = oloa__pkdd[i]
                gtfws__jpvzf[arg_name] = defaults[arg_name]
        check_unsupported_args(func_name, ottnl__vwzz, gtfws__jpvzf)
    return bgv__pugiq, oloa__pkdd
...
class BodoArrayIteratorModel(models.StructModel):
    ...
    def __init__(self, dmm, fe_type):
        jmx__qvm = [('index', types.EphemeralPointer(types.uintp)), (
            'array', fe_type.arr_type)]
        super(BodoArrayIteratorModel, self).__init__(dmm, fe_type, jmx__qvm)
...
def iternext_bodo_array(context, builder, sig, args, result):
    [erdx__zkxt] = sig.args
    [whg__fof] = args
    chvu__qfmsd = context.make_helper(builder, erdx__zkxt, value=whg__fof)
    sfuk__pij = signature(types.intp, erdx__zkxt.arr_type)
    rwcc__mfbmw = context.compile_internal(builder, lambda a: len(a),
        sfuk__pij, [chvu__qfmsd.array])
    snnl__cmg = builder.load(chvu__qfmsd.index)
    evd__iqewb = builder.icmp_signed('<', snnl__cmg, rwcc__mfbmw)
    result.set_valid(evd__iqewb)
    with builder.if_then(evd__iqewb):
        xklgm__tzlf = signature(erdx__zkxt.yield_type, erdx__zkxt.arr_type,
            types.intp)
        value = context.compile_internal(builder, lambda a, i: a[i],
            xklgm__tzlf, [chvu__qfmsd.array, snnl__cmg])
        result.yield_(value)
        wgzt__lge = cgutils.increment_index(builder, snnl__cmg)
        builder.store(wgzt__lge, chvu__qfmsd.index)


def index_typ_from_dtype_name_arr(elem_dtype, name, arr_typ):
    lcd__bei = type(get_index_type_from_dtype(elem_dtype))
    if name is None:
        rophi__ciu = None
    else:
        rophi__ciu = types.StringLiteral(name)
    if lcd__bei == bodo.hiframes.pd_index_ext.NumericIndexType:
        ftitl__vafp = lcd__bei(elem_dtype, rophi__ciu, arr_typ)
    elif lcd__bei == bodo.hiframes.pd_index_ext.CategoricalIndexType:
        ftitl__vafp = lcd__bei(bodo.CategoricalArrayType(elem_dtype),
            rophi__ciu, arr_typ)
    else:
        ftitl__vafp = lcd__bei(rophi__ciu, arr_typ)
    return ftitl__vafp
...
def check_objmode_output_type(ret_tup, ret_type):
    return tuple(_check_objmode_type(pnm__qdlo, t) for pnm__qdlo, t in zip(
...
def _fix_objmode_df_type(val, val_typ, typ):
    ...
    if val_typ != typ:
        pvlw__yhx = pd.Index(val_typ.columns)
        sah__ztixj = pd.Index(typ.columns)
        ckxzu__zebiv = pvlw__yhx.argsort()
        odxt__rjewj = sah__ztixj.argsort()
        yleg__emhd = val_typ.copy(data=tuple(np.array(val_typ.data)[
            ckxzu__zebiv]), columns=tuple(pvlw__yhx[ckxzu__zebiv]))
        todj__yiy = typ.copy(data=tuple(np.array(typ.data)[odxt__rjewj]),
            columns=tuple(sah__ztixj[odxt__rjewj]))
        if yleg__emhd == todj__yiy:
            val_typ = typ
