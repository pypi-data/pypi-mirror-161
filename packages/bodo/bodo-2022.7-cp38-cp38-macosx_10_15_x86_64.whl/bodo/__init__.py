...
def _global_except_hook(exctype, value, traceback):
    ...
    rpomw__umbsa = MPI.COMM_WORLD
    hoc__isv = rpomw__umbsa.Get_rank()
    dqj__mgl = 3.0
    eplj__senb = True
    ygmy__oxap = rpomw__umbsa.Ibarrier()
    tod__mhuor = time.time()
    while time.time() - tod__mhuor < dqj__mgl:
        time.sleep(0.1)
        if ygmy__oxap.Test():
            eplj__senb = False
            break
    try:
        ...
        if eplj__senb:
            sys.stderr.write(
                '\n*****************************************************\n')
            sys.stderr.write(
                f'   Uncaught exception detected on rank {hoc__isv}. \n')
