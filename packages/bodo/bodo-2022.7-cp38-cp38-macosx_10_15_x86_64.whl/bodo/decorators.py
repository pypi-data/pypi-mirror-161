...
def distributed_diagnostics(self, signature=None, level=1):
    ...
    def dump(sig):
        ypc__ori = self.overloads[sig]
        kdt__zxv = ypc__ori.metadata.get('distributed_diagnostics', None)
        if kdt__zxv is None:
            kxp__npebv = 'No distributed diagnostic available'
            raise bodo.utils.typing.BodoError(kxp__npebv)
        kdt__zxv.dump(level, self.get_metadata(sig))
...
def master_mode_wrapper(numba_jit_wrapper):
    ...
    def _wrapper(pyfunc):
        tasc__met = numba_jit_wrapper(pyfunc)
        return master_mode.MasterModeDispatcher(tasc__met)
    return _wrapper
...
def jit(signature_or_function=None, pipeline_class=None, **options):
    ...
    if 'distributed' in options and isinstance(options['distributed'], bool):
        cysyk__sofk = options.pop('distributed')
        pipeline_class = (pipeline_class if cysyk__sofk else bodo.compiler.
            BodoCompilerSeq)
    if 'replicated' in options and isinstance(options['replicated'], bool):
        esg__qjvsp = options.pop('replicated')
        pipeline_class = (bodo.compiler.BodoCompilerSeq if esg__qjvsp else
            pipeline_class)
    ukft__gvlsu = numba.jit(signature_or_function, pipeline_class=
        pipeline_class, **options)
    if master_mode.master_mode_on and bodo.get_rank(
        ) == master_mode.MASTER_RANK:
        if isinstance(ukft__gvlsu, numba.dispatcher._DispatcherBase):
            return master_mode.MasterModeDispatcher(ukft__gvlsu)
        else:
            return master_mode_wrapper(ukft__gvlsu)
    else:
        return ukft__gvlsu
