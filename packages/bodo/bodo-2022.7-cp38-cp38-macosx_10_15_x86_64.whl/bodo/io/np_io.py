...
def get_dtype_size(typingctx, dtype=None):
    ...
    def codegen(context, builder, sig, args):
        qsp__ngud = context.get_abi_sizeof(context.get_data_type(dtype.dtype))
        return context.get_constant(types.intp, qsp__ngud)
...
def tofile_overload(arr, fname):
    if fname == string_type or isinstance(fname, types.StringLiteral):
        ...
        def tofile_impl(arr, fname):
            ...
            hlt__otztg = np.ascontiguousarray(arr)
            dtype_size = get_dtype_size(hlt__otztg.dtype)
            file_write(unicode_to_utf8(fname), hlt__otztg.ctypes, 
                dtype_size * hlt__otztg.size)
...
def file_write_parallel_overload(fname, arr, start, count):
    if fname == string_type:
        ...
        def _impl(fname, arr, start, count):
            hlt__otztg = np.ascontiguousarray(arr)
            dtype_size = get_dtype_size(hlt__otztg.dtype)
            twf__zzt = dtype_size * bodo.libs.distributed_api.get_tuple_prod(
                hlt__otztg.shape[1:])
            _file_write_parallel(unicode_to_utf8(fname), hlt__otztg.ctypes,
                start, count, twf__zzt)
            bodo.utils.utils.check_and_propagate_cpp_exception()
        return _impl
...
def get_file_size_overload(fname, count, offset, dtype_size):
    if fname == string_type:
        ...
        def impl(fname, count, offset, dtype_size):
            if offset < 0:
                return -1
            zltbp__epbi = _get_file_size(unicode_to_utf8(fname)) - offset
            bodo.utils.utils.check_and_propagate_cpp_exception()
            if count != -1:
                zltbp__epbi = min(zltbp__epbi, count * dtype_size)
            if zltbp__epbi < 0:
                return -1
            return zltbp__epbi
        return impl
