...
def _global_except_hook(exctype, value, traceback):
    ...
    obqxb__pfg = MPI.COMM_WORLD
    vkvf__tub = obqxb__pfg.Get_rank()
    jci__rfj = 3.0
    afkr__tbp = True
    xph__xcly = obqxb__pfg.Ibarrier()
    ckq__wjnry = time.time()
    while time.time() - ckq__wjnry < jci__rfj:
        time.sleep(0.1)
        if xph__xcly.Test():
            afkr__tbp = False
            break
    try:
        ...
        if afkr__tbp:
            sys.stderr.write(
                '\n*****************************************************\n')
            sys.stderr.write(
                f'   Uncaught exception detected on rank {vkvf__tub}. \n')
