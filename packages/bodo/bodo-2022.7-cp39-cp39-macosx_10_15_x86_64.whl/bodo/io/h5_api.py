...
def overload_h5_file_keys(obj_id):
    ...
    def h5f_keys_impl(obj_id):
        uxcgo__ykt = []
        gjlgk__pjah = h5g_get_num_objs(obj_id)
        for mznhi__fwug in range(gjlgk__pjah):
            wqdo__umz = h5g_get_objname_by_idx(obj_id, mznhi__fwug)
            uxcgo__ykt.append(wqdo__umz)
        return uxcgo__ykt
...
def overload_h5_file_create_dataset(obj_id, name, shape=None, dtype=None,
    ...
    nasdl__tdex = parse_dtype(dtype)
    klvw__dmcv = np.int32(numba_to_c_type(nasdl__tdex))
...
def overload_getitem_file(in_f, in_idx):
    if in_f in (h5file_type, h5dataset_or_group_type
        ...
        def impl(in_f, in_idx):
            ijm__aawf = h5_open_dset_or_group_obj(unify_h5_id(in_f),
...
def h5read(dset_id, ndim, starts, counts, is_parallel, out_arr):
    pypxi__nehia = tuple_to_ptr(starts)
    enfyt__opu = tuple_to_ptr(counts)
    bmiap__azfo = bodo.libs.distributed_api.get_type_enum(out_arr)
    return _h5_read(cast_to_h5_dset(dset_id), ndim, pypxi__nehia,
        enfyt__opu, is_parallel, out_arr.ctypes, bmiap__azfo)
...
def h5write(dset_id, ndim, starts, counts, is_parallel, out_arr):
    ...
    enfyt__opu = tuple_to_ptr(counts)
    bmiap__azfo = bodo.libs.distributed_api.get_type_enum(out_arr)
    return _h5_write(cast_to_h5_dset(dset_id), ndim, pypxi__nehia,
        enfyt__opu, is_parallel, out_arr.ctypes, bmiap__azfo)
...
class H5ReadType(AbstractTemplate):
    ...
    def generic(self, args, kws):
        ...
        iso__hdh = types.Array(dtype, ndim, 'C')
        duoad__ehev = args[3]
        iso__hdh = self.context.resolve_function_type(operator.getitem, [
            iso__hdh, duoad__ehev], {}).return_type.copy(layout='C')
        return signature(iso__hdh, *args)
...
def get_filter_read_indices(bool_arr):
    lmml__lgnv = bool_arr.nonzero()[0]
    hzu__fxofv = bodo.libs.distributed_api.get_rank()
    zol__gff = bodo.libs.distributed_api.get_size()
    xgv__azvh = np.empty(zol__gff, np.int64)
    qznu__jew = len(bool_arr)
    bodo.libs.distributed_api.allgather(xgv__azvh, qznu__jew)
    omp__gsxo = xgv__azvh.cumsum()[hzu__fxofv] - qznu__jew
    lmml__lgnv += omp__gsxo
    orarz__bwsij = bodo.libs.distributed_api.dist_reduce(len(lmml__lgnv),
        np.int32(sum_op))
    ravt__gotiw = bodo.libs.distributed_api.gatherv(lmml__lgnv)
    if hzu__fxofv == 0:
        kuk__pigi = ravt__gotiw
    else:
        kuk__pigi = np.empty(orarz__bwsij, lmml__lgnv.dtype)
    bodo.libs.distributed_api.bcast(kuk__pigi)
    shi__tarp = bodo.libs.distributed_api.get_start(orarz__bwsij, zol__gff,
        hzu__fxofv)
    prkum__zux = bodo.libs.distributed_api.get_end(orarz__bwsij, zol__gff,
        hzu__fxofv)
    return kuk__pigi[shi__tarp:prkum__zux]
...
def tuple_to_ptr(typingctx, tuple_tp=None):
    ...
    def codegen(context, builder, sig, args):
        fhbdi__olw = cgutils.alloca_once(builder, args[0].type)
        builder.store(args[0], fhbdi__olw)
        return builder.bitcast(fhbdi__olw, lir.IntType(8).as_pointer())
...
def h5read_filter(dset_id, ndim, starts, counts, is_parallel, out_arr,
    ...
    enfyt__opu = tuple_to_ptr(counts)
    bmiap__azfo = bodo.libs.distributed_api.get_type_enum(out_arr)
    return _h5read_filter(cast_to_h5_dset(dset_id), ndim, pypxi__nehia,
        enfyt__opu, is_parallel, out_arr.ctypes, bmiap__azfo, read_indices.
