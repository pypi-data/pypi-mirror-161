...
def lower_pyarrow_table_schema(context, builder, ty, pyval):
    nbmq__mkj = context.get_python_api(builder)
    return nbmq__mkj.unserialize(nbmq__mkj.serialize_object(pyval))
...
def get_arrow_timestamp_type(pa_ts_typ):
    umpp__yzqca = 'ns', 'us', 'ms', 's'
    if pa_ts_typ.unit not in umpp__yzqca:
        return types.Array(bodo.datetime64ns, 1, 'C'), False
    elif pa_ts_typ.tz is not None:
        eli__vwz = pa_ts_typ.to_pandas_dtype().tz
        tzg__jbwzb = bodo.libs.pd_datetime_arr_ext.get_pytz_type_info(eli__vwz)
        return bodo.DatetimeArrayType(tzg__jbwzb), True
    else:
...
def _get_numba_typ_from_pa_typ(pa_typ: pa.Field, is_index,
    ...
    if isinstance(pa_typ.type, pa.ListType):
        tokb__tszz, blyv__kggps = _get_numba_typ_from_pa_typ(pa_typ.type.
            value_field, is_index, nullable_from_metadata, category_info)
        return ArrayItemArrayType(tokb__tszz), blyv__kggps
    if isinstance(pa_typ.type, pa.StructType):
        ejo__adnph = []
        eabh__gyhfl = []
        blyv__kggps = True
        for wcxp__nxmey in pa_typ.flatten():
            eabh__gyhfl.append(wcxp__nxmey.name.split('.')[-1])
            xlu__nvyxs, tedy__vrrew = _get_numba_typ_from_pa_typ(wcxp__nxmey,
                is_index, nullable_from_metadata, category_info)
            ejo__adnph.append(xlu__nvyxs)
            blyv__kggps = blyv__kggps and tedy__vrrew
        return StructArrayType(tuple(ejo__adnph), tuple(eabh__gyhfl)
            ), blyv__kggps
    ...
    if isinstance(pa_typ.type, pa.DictionaryType):
        ...
        rjqyy__mbh = _pyarrow_numba_type_map[pa_typ.type.index_type]
        vshhn__peiq = PDCategoricalDtype(category_info[pa_typ.name], bodo.
            string_type, pa_typ.type.ordered, int_type=rjqyy__mbh)
        return CategoricalArrayType(vshhn__peiq), True
    if isinstance(pa_typ.type, pa.lib.TimestampType):
        ...
        sns__gjul = _pyarrow_numba_type_map[pa_typ.type]
        blyv__kggps = True
    else:
        raise BodoError('Arrow data type {} not supported yet'.format(
            pa_typ.type))
    if sns__gjul == datetime_date_type:
        return datetime_date_array_type, blyv__kggps
    if sns__gjul == bytes_type:
        return binary_array_type, blyv__kggps
    tokb__tszz = (string_array_type if sns__gjul == string_type else types.
        Array(sns__gjul, 1, 'C'))
    if sns__gjul == types.bool_:
        tokb__tszz = boolean_array
    qmjw__znuhy = (use_nullable_int_arr if nullable_from_metadata is None else
        nullable_from_metadata)
    if qmjw__znuhy and not is_index and isinstance(sns__gjul, types.Integer
        ) and pa_typ.nullable:
        tokb__tszz = IntegerArrayType(sns__gjul)
    return tokb__tszz, blyv__kggps
