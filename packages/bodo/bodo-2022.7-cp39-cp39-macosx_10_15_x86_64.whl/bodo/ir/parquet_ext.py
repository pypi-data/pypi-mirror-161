...
def remove_dead_pq(pq_node, lives_no_aliases, lives, arg_aliases, alias_map,
    func_ir, typemap):
    psqn__oysq = pq_node.out_vars[0].name
    uhlx__zpbz = pq_node.out_vars[1].name
    if psqn__oysq not in lives and uhlx__zpbz not in lives:
        return None
    elif psqn__oysq not in lives:
        pq_node.col_indices = []
        pq_node.df_colnames = []
        pq_node.out_used_cols = []
    elif uhlx__zpbz not in lives:
