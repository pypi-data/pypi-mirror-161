...
def get_dtype_size(typingctx, dtype=None):
    ...
    def codegen(context, builder, sig, args):
        nitq__mqz = context.get_abi_sizeof(context.get_data_type(dtype.dtype))
        return context.get_constant(types.intp, nitq__mqz)
...
def tofile_overload(arr, fname):
    if fname == string_type or isinstance(fname, types.StringLiteral):
        ...
        def tofile_impl(arr, fname):
            ...
            oqxrc__rmgq = np.ascontiguousarray(arr)
            dtype_size = get_dtype_size(oqxrc__rmgq.dtype)
            file_write(unicode_to_utf8(fname), oqxrc__rmgq.ctypes, 
                dtype_size * oqxrc__rmgq.size)
...
def file_write_parallel_overload(fname, arr, start, count):
    if fname == string_type:
        ...
        def _impl(fname, arr, start, count):
            oqxrc__rmgq = np.ascontiguousarray(arr)
            dtype_size = get_dtype_size(oqxrc__rmgq.dtype)
            fhhu__kmcea = (dtype_size * bodo.libs.distributed_api.
                get_tuple_prod(oqxrc__rmgq.shape[1:]))
            _file_write_parallel(unicode_to_utf8(fname), oqxrc__rmgq.ctypes,
                start, count, fhhu__kmcea)
            bodo.utils.utils.check_and_propagate_cpp_exception()
        return _impl
...
def get_file_size_overload(fname, count, offset, dtype_size):
    if fname == string_type:
        ...
        def impl(fname, count, offset, dtype_size):
            if offset < 0:
                return -1
            kgcb__cfq = _get_file_size(unicode_to_utf8(fname)) - offset
            bodo.utils.utils.check_and_propagate_cpp_exception()
            if count != -1:
                kgcb__cfq = min(kgcb__cfq, count * dtype_size)
            if kgcb__cfq < 0:
                return -1
            return kgcb__cfq
        return impl
