...
def _install_mpl_types():
    ikqd__pkm = [('mpl_figure_type', matplotlib.figure.Figure), (
    ...
    for oqjg__bhw, wrlik__upd in ikqd__pkm:
        install_py_obj_class(types_name=oqjg__bhw, python_type=wrlik__upd,
            module=this_module)
...
def generate_matplotlib_signature(return_typ, args, kws, obj_typ=None):
    kws = dict(kws)
    mkc__wglzi = ', '.join(f'e{kxp__xafjm}' for kxp__xafjm in range(len(args)))
    if mkc__wglzi:
        mkc__wglzi += ', '
    frk__llk = ', '.join(f"{pmr__kvlrf} = ''" for pmr__kvlrf in kws.keys())
    qyjmj__pyjho = 'matplotlib_obj, ' if obj_typ is not None else ''
    vrv__hxx = f'def mpl_stub({qyjmj__pyjho} {mkc__wglzi} {frk__llk}):\n'
    vrv__hxx += '    pass\n'
    igns__pwq = {}
    exec(vrv__hxx, {}, igns__pwq)
    gsvzn__vefus = igns__pwq['mpl_stub']
    siii__qtfk = numba.core.utils.pysignature(gsvzn__vefus)
    vgxtt__pym = ((obj_typ,) if obj_typ is not None else ()) + args + tuple(kws
        .values())
    return signature(return_typ, *unliteral_all(vgxtt__pym)).replace(pysig=
        siii__qtfk)


def generate_axes_typing(mod_name, nrows, ncols):
    jrge__dyv = '{}.subplots(): {} must be a constant integer >= 1'
    if not is_overload_constant_int(nrows):
        raise_bodo_error(jrge__dyv.format(mod_name, 'nrows'))
    if not is_overload_constant_int(ncols):
        raise_bodo_error(jrge__dyv.format(mod_name, 'ncols'))
    fyd__mrp = get_overload_const_int(nrows)
    nvfx__yxes = get_overload_const_int(ncols)
    if fyd__mrp < 1:
        raise BodoError(jrge__dyv.format(mod_name, 'nrows'))
    if nvfx__yxes < 1:
        raise BodoError(jrge__dyv.format(mod_name, 'ncols'))
    if fyd__mrp == 1 and nvfx__yxes == 1:
        avtfg__oipe = types.mpl_axes_type
    else:
        if nvfx__yxes == 1:
            qtg__ris = types.mpl_axes_type
        else:
            qtg__ris = types.Tuple([types.mpl_axes_type] * nvfx__yxes)
        avtfg__oipe = types.Tuple([qtg__ris] * fyd__mrp)
    return avtfg__oipe


def generate_pie_return_type(args, kws):
    onylr__hcg = args[4] if len(args) > 5 else kws.get('autopct', types.none)
    if onylr__hcg == types.none:
...
def generate_xcorr_return_type(func_mod, args, kws):
    mhvw__erzm = args[4] if len(args) > 5 else kws.get('usevlines', True)
    if not is_overload_constant_bool(mhvw__erzm):
        raise_bodo_error(
            f'{func_mod}.xcorr(): usevlines must be a constant boolean')
    if is_overload_true(mhvw__erzm):
...
class SubplotsTyper(AbstractTemplate):
    ...
    def generic(self, args, kws):
        ...
        vwq__rqpg = generate_axes_typing('matplotlib.pyplot', nrows, ncols)
        return generate_matplotlib_signature(types.Tuple([types.
            mpl_figure_type, vwq__rqpg]), args, kws)
...
def overload_subplots(fig, nrows=1, ncols=1, sharex=False, sharey=False,
    ...
    vwq__rqpg = generate_axes_typing('matplotlib.figure.Figure', nrows, ncols)
    oqjg__bhw = str(vwq__rqpg)
    if not hasattr(types, oqjg__bhw):
        oqjg__bhw = f'objmode_type{ir_utils.next_label()}'
        setattr(types, oqjg__bhw, vwq__rqpg)
    vrv__hxx = f"""def impl(
        fig,
        nrows=1,
        ncols=1,
        ...
        with numba.objmode(axes="{oqjg__bhw}"):
            axes = fig.subplots(
                nrows=nrows,
                ncols=ncols,
                sharex=sharex,
    ...
    """
    igns__pwq = {}
    exec(vrv__hxx, {'numba': numba, 'np': np}, igns__pwq)
    impl = igns__pwq['impl']
    return impl
