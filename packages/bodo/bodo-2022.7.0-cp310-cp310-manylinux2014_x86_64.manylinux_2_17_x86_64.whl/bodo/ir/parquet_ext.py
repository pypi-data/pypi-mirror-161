...
def remove_dead_pq(pq_node, lives_no_aliases, lives, arg_aliases, alias_map,
    func_ir, typemap):
    jol__memh = pq_node.out_vars[0].name
    dqlh__dzt = pq_node.out_vars[1].name
    if jol__memh not in lives and dqlh__dzt not in lives:
        return None
    elif jol__memh not in lives:
        pq_node.col_indices = []
        pq_node.df_colnames = []
        pq_node.out_used_cols = []
    elif dqlh__dzt not in lives:
