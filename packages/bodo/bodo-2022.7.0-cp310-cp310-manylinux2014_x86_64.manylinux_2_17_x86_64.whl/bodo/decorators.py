...
def distributed_diagnostics(self, signature=None, level=1):
    ...
    def dump(sig):
        kxzan__cij = self.overloads[sig]
        dfiu__nse = kxzan__cij.metadata.get('distributed_diagnostics', None)
        if dfiu__nse is None:
            dain__hfe = 'No distributed diagnostic available'
            raise bodo.utils.typing.BodoError(dain__hfe)
        dfiu__nse.dump(level, self.get_metadata(sig))
...
def master_mode_wrapper(numba_jit_wrapper):
    ...
    def _wrapper(pyfunc):
        qrtmu__ezacy = numba_jit_wrapper(pyfunc)
        return master_mode.MasterModeDispatcher(qrtmu__ezacy)
    return _wrapper
...
def jit(signature_or_function=None, pipeline_class=None, **options):
    ...
    if 'distributed' in options and isinstance(options['distributed'], bool):
        zsyd__nshw = options.pop('distributed')
        pipeline_class = (pipeline_class if zsyd__nshw else bodo.compiler.
            BodoCompilerSeq)
    if 'replicated' in options and isinstance(options['replicated'], bool):
        rri__njy = options.pop('replicated')
        pipeline_class = (bodo.compiler.BodoCompilerSeq if rri__njy else
            pipeline_class)
    fyt__lojcv = numba.jit(signature_or_function, pipeline_class=
        pipeline_class, **options)
    if master_mode.master_mode_on and bodo.get_rank(
        ) == master_mode.MASTER_RANK:
        if isinstance(fyt__lojcv, numba.dispatcher._DispatcherBase):
            return master_mode.MasterModeDispatcher(fyt__lojcv)
        else:
            return master_mode_wrapper(fyt__lojcv)
    else:
        return fyt__lojcv
