...
def get_dtype_size(typingctx, dtype=None):
    ...
    def codegen(context, builder, sig, args):
        qbepm__bkek = context.get_abi_sizeof(context.get_data_type(dtype.dtype)
            )
        return context.get_constant(types.intp, qbepm__bkek)
...
def tofile_overload(arr, fname):
    if fname == string_type or isinstance(fname, types.StringLiteral):
        ...
        def tofile_impl(arr, fname):
            ...
            ozspl__sxzsc = np.ascontiguousarray(arr)
            dtype_size = get_dtype_size(ozspl__sxzsc.dtype)
            file_write(unicode_to_utf8(fname), ozspl__sxzsc.ctypes, 
                dtype_size * ozspl__sxzsc.size)
...
def file_write_parallel_overload(fname, arr, start, count):
    if fname == string_type:
        ...
        def _impl(fname, arr, start, count):
            ozspl__sxzsc = np.ascontiguousarray(arr)
            dtype_size = get_dtype_size(ozspl__sxzsc.dtype)
            nxt__qzotx = dtype_size * bodo.libs.distributed_api.get_tuple_prod(
                ozspl__sxzsc.shape[1:])
            _file_write_parallel(unicode_to_utf8(fname), ozspl__sxzsc.
                ctypes, start, count, nxt__qzotx)
            bodo.utils.utils.check_and_propagate_cpp_exception()
        return _impl
...
def get_file_size_overload(fname, count, offset, dtype_size):
    if fname == string_type:
        ...
        def impl(fname, count, offset, dtype_size):
            if offset < 0:
                return -1
            qjzc__wwgzv = _get_file_size(unicode_to_utf8(fname)) - offset
            bodo.utils.utils.check_and_propagate_cpp_exception()
            if count != -1:
                qjzc__wwgzv = min(qjzc__wwgzv, count * dtype_size)
            if qjzc__wwgzv < 0:
                return -1
            return qjzc__wwgzv
        return impl
