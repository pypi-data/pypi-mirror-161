...
def decode_if_dict_array_overload(A):
    if isinstance(A, types.BaseTuple):
        oarun__xeh = len(A.types)
        eas__omlpf = 'def f(A):\n'
        eoyf__ljh = ','.join(f'decode_if_dict_array(A[{i}])' for i in range
            (oarun__xeh))
        eas__omlpf += '  return ({}{})\n'.format(eoyf__ljh, ',' if 
            oarun__xeh == 1 else '')
        mgqw__tsxjc = {}
        exec(eas__omlpf, {'decode_if_dict_array': decode_if_dict_array},
            mgqw__tsxjc)
        impl = mgqw__tsxjc['f']
        return impl
    if isinstance(A, types.List):
        ...
        def impl(A):
            ...
            for a in A:
                oarun__xeh += 1
            udyns__tujx = []
            for i in range(oarun__xeh):
                udyns__tujx.append(decode_if_dict_array(A[i]))
            return udyns__tujx
        return impl
    ...
    if isinstance(A, bodo.SeriesType):
        ...
        def impl(A):
            mvgf__ldcla = bodo.hiframes.pd_series_ext.get_series_data(A)
            hpe__vuv = bodo.hiframes.pd_series_ext.get_series_index(A)
            name = bodo.hiframes.pd_series_ext.get_series_name(A)
            qaju__afj = decode_if_dict_array(mvgf__ldcla)
            return bodo.hiframes.pd_series_ext.init_series(qaju__afj,
                hpe__vuv, name)
        return impl
...
def to_str_arr_if_dict_array(t):
    ...
    if isinstance(t, bodo.TableType):
        ecbi__vhc = tuple(to_str_arr_if_dict_array(t) for t in t.arr_types)
        return bodo.TableType(ecbi__vhc, t.has_runtime_cols)
...
class BodoError(NumbaError):
    ...
    def __init__(self, msg, loc=None, locs_in_msg=None):
        ...
        qanf__lmzcl = numba.core.errors.termcolor().errmsg
        super(BodoError, self).__init__(qanf__lmzcl(msg), loc)
...
def raise_bodo_error(msg, loc=None):
    if bodo.transforms.typing_pass.in_partial_typing:
        ...
        flq__nnf = [] if loc is None else [loc]
        raise BodoError(msg, locs_in_msg=flq__nnf)
...
class FileInfo:
    ...
    def get_schema(self, fname):
        ygdkx__mbjca = self.get_full_filename(fname)
        return self._get_schema(ygdkx__mbjca)
...
class FilenameModel(models.StructModel):
    ...
    def __init__(self, dmm, fe_type):
        iinnd__pnfc = dmm.lookup(bodo.typeof(fe_type.fname))
        yzynj__qmev = [(a, vhaoj__zah) for a, vhaoj__zah in zip(iinnd__pnfc
            ._fields, iinnd__pnfc._members)]
        super().__init__(dmm, fe_type, yzynj__qmev)
...
def is_initial_value_type(t):
    if not isinstance(t, types.InitialValue) or t.initial_value is None:
        return False
    hkly__grve = t.initial_value
    if isinstance(hkly__grve, dict):
        hkly__grve = hkly__grve.values()
    return not any(isinstance(dhq__guh, (types.Poison, numba.core.
        interpreter._UNKNOWN_VALUE)) for dhq__guh in hkly__grve)
...
def is_overload_constant_dict(val):
    return isinstance(val, types.LiteralStrKeyDict) and all(is_literal_type
        (dhq__guh) for dhq__guh in val.types) or is_initial_value_dict_type(val
...
def is_overload_bool_list(val):
    return is_overload_constant_list(val) and all(is_overload_constant_bool
        (dhq__guh) for dhq__guh in get_overload_const_list(val))
...
def get_overload_const(val):
    ...
    if isinstance(val, types.LiteralList):
        hxp__wnw = []
        for dhq__guh in val.literal_value:
            zugy__eyb = get_overload_const(dhq__guh)
            if zugy__eyb == NOT_CONSTANT:
                return NOT_CONSTANT
            else:
                hxp__wnw.append(zugy__eyb)
        return hxp__wnw
    ...
    if isinstance(val, types.BaseTuple):
        hxp__wnw = []
        for dhq__guh in val.types:
            zugy__eyb = get_overload_const(dhq__guh)
            if zugy__eyb == NOT_CONSTANT:
                return NOT_CONSTANT
            else:
                hxp__wnw.append(zugy__eyb)
        return tuple(hxp__wnw)
...
def ensure_constant_arg(fname, arg_name, val, const_type):
    zugy__eyb = get_overload_const(val)
    wpj__koliv = _const_type_repr.get(const_type, str(const_type))
    if not isinstance(zugy__eyb, const_type):
        raise BodoError(
            f"{fname}(): argument '{arg_name}' should be a constant {wpj__koliv} not {val}"
            )


def ensure_constant_values(fname, arg_name, val, const_values):
    zugy__eyb = get_overload_const(val)
    if zugy__eyb not in const_values:
        raise BodoError(
            f"{fname}(): argument '{arg_name}' should be a constant value in {const_values} not '{zugy__eyb}'"
            )
...
def check_unsupported_args(fname, args_dict, arg_defaults_dict,
    ...
    duku__obdr = False
    for a in args_dict:
        dwyw__kzjlt = get_overload_const(args_dict[a])
        gujzv__ddap = arg_defaults_dict[a]
        if (dwyw__kzjlt is NOT_CONSTANT or dwyw__kzjlt is not None and 
            gujzv__ddap is None or dwyw__kzjlt is None and gujzv__ddap is not
            None or dwyw__kzjlt != gujzv__ddap or dwyw__kzjlt is not
            _no_input and gujzv__ddap is _no_input or dwyw__kzjlt is
            _no_input and gujzv__ddap is not _no_input):
            aar__ccq = (
                f'{fn_str}: {a} parameter only supports default value {gujzv__ddap}'
                )
            duku__obdr = True
            break
    if duku__obdr and package_name == 'pandas':
        if module_name == 'IO':
            aar__ccq += """
...
def get_overload_constant_dict(val):
    if isinstance(val, types.LiteralStrKeyDict):
        return {get_literal_value(auzc__dcx): get_literal_value(dhq__guh) for
            auzc__dcx, dhq__guh in val.literal_value.items()}
    ...
    if isinstance(val, types.DictType):
        ...
        return val.initial_value
    otg__mzst = [get_overload_const(dhq__guh) for dhq__guh in val.types[1:]]
    return {otg__mzst[2 * i]: otg__mzst[2 * i + 1] for i in range(len(
        otg__mzst) // 2)}
...
def get_overload_const_list(val):
    ...
    if is_initial_value_list_type(val):
        return val.initial_value
    if isinstance(val, types.LiteralList):
        return [get_literal_value(dhq__guh) for dhq__guh in val.literal_value]
...
def parse_dtype(dtype, func_name=None):
    ...
    if isinstance(dtype, types.Number) or dtype == bodo.string_type:
        return dtype
    try:
        owmni__bgjw = get_overload_const_str(dtype)
        if owmni__bgjw.startswith('Int') or owmni__bgjw.startswith('UInt'):
            return bodo.libs.int_arr_ext.typeof_pd_int_dtype(pd.api.types.
                pandas_dtype(owmni__bgjw), None)
        if owmni__bgjw == 'boolean':
            return bodo.libs.bool_arr_ext.boolean_dtype
        if owmni__bgjw == 'str':
            return bodo.string_type
        return numba.np.numpy_support.from_dtype(np.dtype(owmni__bgjw))
    except:
        pass
...
def get_index_names(t, func_name, default_name):
    ...
    uvh__tuonw = '{}: index name should be a constant string'.format(func_name)
    if isinstance(t, MultiIndexType):
        dragz__tcrco = []
        for i, eag__akbis in enumerate(t.names_typ):
            if eag__akbis == types.none:
                dragz__tcrco.append('level_{}'.format(i))
                continue
            if not is_overload_constant_str(eag__akbis):
                raise BodoError(uvh__tuonw)
            dragz__tcrco.append(get_overload_const_str(eag__akbis))
        return tuple(dragz__tcrco)
...
if types.SliceLiteral in numba.core.pythonapi._boxers.functions:
    ...
    def box_slice_literal(typ, val, c):
        nmsg__hqdmp = typ.literal_value
        rhwd__pjf = []
        for zmf__gkx in ('start', 'stop', 'step'):
            dsjoz__cjcbl = getattr(typ.literal_value, zmf__gkx)
            tdob__wxw = c.pyapi.make_none(
                ) if dsjoz__cjcbl is None else c.pyapi.from_native_value(types
                .literal(dsjoz__cjcbl), dsjoz__cjcbl, c.env_manager)
            rhwd__pjf.append(tdob__wxw)
        szzt__baxd = c.pyapi.unserialize(c.pyapi.serialize_object(slice))
        sewe__cjjh = c.pyapi.call_function_objargs(szzt__baxd, rhwd__pjf)
        for a in rhwd__pjf:
            c.pyapi.decref(a)
        c.pyapi.decref(szzt__baxd)
        return sewe__cjjh
...
def box_list_literal(typ, val, c):
    lxo__skx = typ.literal_value
    btg__zhm = [c.pyapi.from_native_value(types.literal(dhq__guh), dhq__guh,
        c.env_manager) for dhq__guh in lxo__skx]
    jgoq__kxbil = c.pyapi.list_pack(btg__zhm)
    for a in btg__zhm:
        c.pyapi.decref(a)
    return jgoq__kxbil
...
def list_literal_to_list(context, builder, fromty, toty, val):
    ...
    cgv__nip = types.List(toty.dtype)
...
def is_literal_type(t):
    ...
    return isinstance(t, (types.Literal, types.Omitted)) and not isinstance(t,
        ...
        is_literal_type(dhq__guh) for dhq__guh in t.types
...
def get_literal_value(t):
    ...
    if isinstance(t, types.Literal):
        if isinstance(t, types.LiteralStrKeyDict):
            return {get_literal_value(auzc__dcx): get_literal_value(
                dhq__guh) for auzc__dcx, dhq__guh in t.literal_value.items()}
        if isinstance(t, types.LiteralList):
            return [get_literal_value(dhq__guh) for dhq__guh in t.literal_value
                ]
        return t.literal_value
    if isinstance(t, types.Omitted):
        return t.value
    if isinstance(t, types.BaseTuple):
        return tuple(get_literal_value(dhq__guh) for dhq__guh in t.types)
...
def dtype_to_array_type(dtype):
    ...
    if isinstance(dtype, (types.Number, types.Boolean, types.NPDatetime,
        types.NPTimedelta)):
        mvgf__ldcla = types.Array(dtype, 1, 'C')
        if eerlb__nncjc:
            return to_nullable_type(mvgf__ldcla)
        return mvgf__ldcla
...
def get_udf_out_arr_type(f_return_type, return_nullable=False):
    ...
    tsfyf__zgoo = dtype_to_array_type(f_return_type)
    tsfyf__zgoo = to_nullable_type(tsfyf__zgoo
        ) if return_nullable else tsfyf__zgoo
    return tsfyf__zgoo
...
def types_equality_exists(t1, t2):
    wbx__thhc = numba.core.registry.cpu_target.typing_context
    try:
        wbx__thhc.resolve_function_type(operator.eq, (t1, t2), {})
        return True
    except:
        return False
...
def is_hashable_type(t):
    whitelist_types = (types.UnicodeType, types.StringLiteral, types.
        ...
        PandasTimestampType)
    ebejp__fzrfq = (types.bool_, bodo.datetime64ns, bodo.timedelta64ns,
        bodo.pd_timedelta_type)
    if isinstance(t, whitelist_types) or t in ebejp__fzrfq:
        return True
    wbx__thhc = numba.core.registry.cpu_target.typing_context
    try:
        wbx__thhc.resolve_function_type(hash, (t,), {})
        return True
    except:
        return False
...
def to_nullable_type(t):
    ...
    if isinstance(t, DataFrameType):
        gvqr__gzn = tuple(to_nullable_type(t) for t in t.data)
        return DataFrameType(gvqr__gzn, t.index, t.columns, t.dist, t.
            is_table_format)
...
def is_common_scalar_dtype(scalar_types):
    imit__sojng, scyma__djao = get_common_scalar_dtype(scalar_types)
    return scyma__djao
...
def get_common_scalar_dtype(scalar_types):
    ...
    try:
        eak__dptcp = np.find_common_type([numba.np.numpy_support.as_dtype(t
            ) for t in scalar_types], [])
        if eak__dptcp != object:
            return numba.np.numpy_support.from_dtype(eak__dptcp), True
    except numba.core.errors.NumbaNotImplementedError as tvdj__ngrb:
        pass
    ...
    wlhl__yonnz = itertools.groupby(scalar_types)
    if next(wlhl__yonnz, True) and not next(wlhl__yonnz, False):
...
def get_nullable_and_non_nullable_types(array_of_types):
    ...
    for typ in array_of_types:
        if typ == bodo.libs.bool_arr_ext.boolean_array:
            saqqb__mfqvs.append(types.Array(types.bool_, 1, 'C'))
        elif isinstance(typ, bodo.libs.int_arr_ext.IntegerArrayType):
            saqqb__mfqvs.append(types.Array(typ.dtype, 1, 'C'))
        elif isinstance(typ, types.Array):
            if typ.dtype == types.bool_:
                saqqb__mfqvs.append(bodo.libs.bool_arr_ext.boolean_array)
            if isinstance(typ.dtype, types.Integer):
                saqqb__mfqvs.append(bodo.libs.int_arr_ext.IntegerArrayType(
                    typ.dtype))
        saqqb__mfqvs.append(typ)
    return saqqb__mfqvs
...
def _gen_objmode_overload(func, output_type, attr_name=None, is_function=
    ...
    if is_function:
        fepq__azl = getfullargspec(func)
        assert fepq__azl.varargs is None, 'varargs not supported'
        assert fepq__azl.varkw is None, 'varkw not supported'
        defaults = [] if fepq__azl.defaults is None else fepq__azl.defaults
        fojw__qfoli = len(fepq__azl.args) - len(defaults)
        args = fepq__azl.args[1:] if attr_name else fepq__azl.args[:]
        ilys__jruhl = []
        for i, fwf__mcpue in enumerate(fepq__azl.args):
            if i < fojw__qfoli:
                ilys__jruhl.append(fwf__mcpue)
            elif str(defaults[i - fojw__qfoli]) != '<deprecated parameter>':
                ilys__jruhl.append(fwf__mcpue + '=' + str(defaults[i -
                    fojw__qfoli]))
            else:
                args.remove(fwf__mcpue)
        if fepq__azl.kwonlyargs is not None:
            for fwf__mcpue in fepq__azl.kwonlyargs:
                args.append(f'{fwf__mcpue}={fwf__mcpue}')
                ilys__jruhl.append(
                    f'{fwf__mcpue}={str(fepq__azl.kwonlydefaults[fwf__mcpue])}'
                    )
        sig = ', '.join(ilys__jruhl)
        args = ', '.join(args)
    else:
        sig = 'self'
    ...
    gvjf__rpcc = f'self.{attr_name}' if attr_name else f'{func_name}'
    eas__omlpf = f'def overload_impl({sig}):\n'
    eas__omlpf += f'    def impl({sig}):\n'
    if single_rank:
        eas__omlpf += f'        if bodo.get_rank() == 0:\n'
        euqhj__zoiea = '    '
    else:
        euqhj__zoiea = ''
    eas__omlpf += (
        f"        {euqhj__zoiea}with numba.objmode(res='{type_name}'):\n")
    if is_function:
        eas__omlpf += f'            {euqhj__zoiea}res = {gvjf__rpcc}({args})\n'
    else:
        eas__omlpf += f'            {euqhj__zoiea}res = {gvjf__rpcc}\n'
    eas__omlpf += f'        return res\n'
    eas__omlpf += f'    return impl\n'
    mgqw__tsxjc = {}
    bzl__tief = globals()
    if not attr_name:
        bzl__tief[func_name] = func
    exec(eas__omlpf, bzl__tief, mgqw__tsxjc)
    jxva__zhv = mgqw__tsxjc['overload_impl']
    return jxva__zhv


def gen_objmode_func_overload(func, output_type=None, single_rank=False):
    try:
        jxva__zhv = _gen_objmode_overload(func, output_type, is_function=
            True, single_rank=single_rank)
        overload(func, no_unliteral=True)(jxva__zhv)
    except Exception as tvdj__ngrb:
        pass
...
def gen_objmode_method_overload(obj_type, method_name, method, output_type=
    ...
    try:
        jxva__zhv = _gen_objmode_overload(method, output_type, method_name,
            True, single_rank)
        overload_method(obj_type, method_name, no_unliteral=True)(jxva__zhv)
    except Exception as tvdj__ngrb:
        pass
...
def gen_objmode_attr_overload(obj_type, attr_name, attr, output_type=None,
    single_rank=False):
    try:
        jxva__zhv = _gen_objmode_overload(attr, output_type, attr_name, 
            False, single_rank)
        overload_attribute(obj_type, attr_name, no_unliteral=True)(jxva__zhv)
    except Exception as tvdj__ngrb:
        pass
...
class NumTypeStaticGetItem(AbstractTemplate):
    ...
    def generic(self, args, kws):
        val, rwhqr__imxn = args
        if isinstance(rwhqr__imxn, slice) and (isinstance(val, types.
            NumberClass) or isinstance(val, types.TypeRef) and isinstance(
            val.instance_type, (types.NPDatetime, types.NPTimedelta))):
            return signature(types.TypeRef(val.instance_type[rwhqr__imxn]),
                *args)
...
def construct_pysig(arg_names, defaults):
    eas__omlpf = f'def stub('
    for fwf__mcpue in arg_names:
        eas__omlpf += fwf__mcpue
        if fwf__mcpue in defaults:
            if isinstance(defaults[fwf__mcpue], str):
                eas__omlpf += f"='{defaults[fwf__mcpue]}'"
            else:
                eas__omlpf += f'={defaults[fwf__mcpue]}'
        eas__omlpf += ', '
    eas__omlpf += '):\n'
    eas__omlpf += '    pass\n'
    mgqw__tsxjc = {}
    exec(eas__omlpf, {}, mgqw__tsxjc)
    iftfo__fzpe = mgqw__tsxjc['stub']
    return numba.core.utils.pysignature(iftfo__fzpe)


def fold_typing_args(func_name, args, kws, arg_names, defaults,
    unsupported_arg_names=()):
    kws = dict(kws)
    zur__zxnd = len(arg_names)
    dzrjk__gpkvq = len(args) + len(kws)
    if dzrjk__gpkvq > zur__zxnd:
        crpdt__klhy = 'argument' if zur__zxnd == 1 else 'arguments'
        hdvbb__dch = 'was' if dzrjk__gpkvq == 1 else 'were'
        raise BodoError(
            f'{func_name}(): Too many arguments specified. Function takes {zur__zxnd} {crpdt__klhy}, but {dzrjk__gpkvq} {hdvbb__dch} provided.'
            )
    hzrk__href = bodo.utils.typing.construct_pysig(arg_names, defaults)
    try:
        qkbc__zlf = bodo.utils.transform.fold_argument_types(hzrk__href,
            args, kws)
    except Exception as zpsap__mybi:
        raise_bodo_error(f'{func_name}(): {zpsap__mybi}')
    if unsupported_arg_names:
        ...
        for i, arg_name in enumerate(arg_names):
            if arg_name in unsupported_arg_names:
                ...
                fyjm__yuja[arg_name] = qkbc__zlf[i]
                vmnh__smnts[arg_name] = defaults[arg_name]
        check_unsupported_args(func_name, fyjm__yuja, vmnh__smnts)
    return hzrk__href, qkbc__zlf
...
class BodoArrayIteratorModel(models.StructModel):
    ...
    def __init__(self, dmm, fe_type):
        yzynj__qmev = [('index', types.EphemeralPointer(types.uintp)), (
            'array', fe_type.arr_type)]
        super(BodoArrayIteratorModel, self).__init__(dmm, fe_type, yzynj__qmev)
...
def iternext_bodo_array(context, builder, sig, args, result):
    [cwt__mpdcm] = sig.args
    [vtta__iquf] = args
    bvo__tfvy = context.make_helper(builder, cwt__mpdcm, value=vtta__iquf)
    jrkl__bcp = signature(types.intp, cwt__mpdcm.arr_type)
    pwzn__cie = context.compile_internal(builder, lambda a: len(a),
        jrkl__bcp, [bvo__tfvy.array])
    hpe__vuv = builder.load(bvo__tfvy.index)
    aprag__lyxm = builder.icmp_signed('<', hpe__vuv, pwzn__cie)
    result.set_valid(aprag__lyxm)
    with builder.if_then(aprag__lyxm):
        owdsr__qouu = signature(cwt__mpdcm.yield_type, cwt__mpdcm.arr_type,
            types.intp)
        value = context.compile_internal(builder, lambda a, i: a[i],
            owdsr__qouu, [bvo__tfvy.array, hpe__vuv])
        result.yield_(value)
        yuy__baqwy = cgutils.increment_index(builder, hpe__vuv)
        builder.store(yuy__baqwy, bvo__tfvy.index)


def index_typ_from_dtype_name_arr(elem_dtype, name, arr_typ):
    ximtb__hvm = type(get_index_type_from_dtype(elem_dtype))
    if name is None:
        mcfz__tzjp = None
    else:
        mcfz__tzjp = types.StringLiteral(name)
    if ximtb__hvm == bodo.hiframes.pd_index_ext.NumericIndexType:
        pvse__zcqvs = ximtb__hvm(elem_dtype, mcfz__tzjp, arr_typ)
    elif ximtb__hvm == bodo.hiframes.pd_index_ext.CategoricalIndexType:
        pvse__zcqvs = ximtb__hvm(bodo.CategoricalArrayType(elem_dtype),
            mcfz__tzjp, arr_typ)
    else:
        pvse__zcqvs = ximtb__hvm(mcfz__tzjp, arr_typ)
    return pvse__zcqvs
...
def check_objmode_output_type(ret_tup, ret_type):
    return tuple(_check_objmode_type(dhq__guh, t) for dhq__guh, t in zip(
...
def _fix_objmode_df_type(val, val_typ, typ):
    ...
    if val_typ != typ:
        elot__dnvvl = pd.Index(val_typ.columns)
        ugvq__css = pd.Index(typ.columns)
        dmgv__uqzzj = elot__dnvvl.argsort()
        faqf__sutol = ugvq__css.argsort()
        yemop__cwtyu = val_typ.copy(data=tuple(np.array(val_typ.data)[
            dmgv__uqzzj]), columns=tuple(elot__dnvvl[dmgv__uqzzj]))
        kywi__yiw = typ.copy(data=tuple(np.array(typ.data)[faqf__sutol]),
            columns=tuple(ugvq__css[faqf__sutol]))
        if yemop__cwtyu == kywi__yiw:
            val_typ = typ
