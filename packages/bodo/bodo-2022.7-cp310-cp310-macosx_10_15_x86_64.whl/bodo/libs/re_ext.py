...
def box_regex_flag(typ, val, c):
    wwmbc__tvgr = c.box(types.int64, val)
    hko__uzs = c.pyapi.unserialize(c.pyapi.serialize_object(re.RegexFlag))
    return c.pyapi.call_function_objargs(hko__uzs, (wwmbc__tvgr,))


@unbox(RegexFlagsType)
def unbox_regex_flag(typ, obj, c):
    wwmbc__tvgr = c.pyapi.object_getattr_string(obj, 'value')
    return c.unbox(types.int64, wwmbc__tvgr)
...
def cast_match_obj_bool(context, builder, fromty, toty, val):
    out = cgutils.alloca_once_value(builder, context.get_constant(types.
        bool_, True))
    daevp__wrv = context.get_python_api(builder)
    kas__ayk = builder.icmp_signed('==', val, daevp__wrv.borrow_none())
    with builder.if_then(kas__ayk):
...
def init_const_pattern(typingctx, pat, pat_const=None):
    sawe__vjyr = get_overload_const_str(pat_const)

    def codegen(context, builder, sig, args):
        return impl_ret_borrowed(context, builder, sig.return_type, args[0])
    return RePatternType(sawe__vjyr)(pat, pat_const), codegen
...
def overload_pat_findall(p, string, pos=0, endpos=9223372036854775807):
    if p.pat_const:
        utom__dnhje = re.compile(p.pat_const).groups
        typ = types.List(string_type)
        if utom__dnhje > 1:
            typ = types.List(types.Tuple([string_type] * utom__dnhje))
...
def overload_match_group(m, *args):
    ...
    tbdx__zrar = types.optional(string_type)
    if len(args) == 1:

        def _match_group_impl_one(m, *args):
            movo__bvik = args[0]
            with numba.objmode(out=tbdx__zrar):
                out = m.group(movo__bvik)
            return out
        return _match_group_impl_one
    lfc__mjccg = 'tuple_str_{}'.format(len(args))
    setattr(types, lfc__mjccg, types.Tuple([tbdx__zrar] * len(args)))
    pya__ynagi = ', '.join('group{}'.format(svnnp__lfrg + 1) for
        svnnp__lfrg in range(len(args)))
    bfuvj__swok = 'def _match_group_impl(m, *args):\n'
    bfuvj__swok += '  ({}) = args\n'.format(pya__ynagi)
    bfuvj__swok += "  with numba.objmode(out='{}'):\n".format(lfc__mjccg)
    bfuvj__swok += '    out = m.group({})\n'.format(pya__ynagi)
    bfuvj__swok += '  return out\n'
    ywxno__oslyi = {}
    exec(bfuvj__swok, globals(), ywxno__oslyi)
    pnax__eby = ywxno__oslyi['_match_group_impl']
    return pnax__eby
...
def overload_match_groups(m, default=None):
    rutge__xtre = types.List(types.optional(string_type))

    def _match_groups_impl(m, default=None):
        with numba.objmode(out=rutge__xtre):
...
def overload_match_groupdict(m, default=None):
    ...
    def _check_dict_none(out):
        if any(ggl__sescb is None for ggl__sescb in out.values()):
            raise BodoError(
...
def overload_match_lastgroup(p):
    ...
    def _match_lastgroup_impl(p):
        with numba.objmode(lastgroup=tbdx__zrar):
