...
class SeriesIatType(types.Type):
    ...
    def __init__(self, stype):
        self.stype = stype
        pquj__qhvas = 'SeriesIatType({})'.format(stype)
        super(SeriesIatType, self).__init__(pquj__qhvas)
...
class SeriesIatModel(models.StructModel):
    ...
    def __init__(self, dmm, fe_type):
        kgnis__uytm = [('obj', fe_type.stype)]
        super(SeriesIatModel, self).__init__(dmm, fe_type, kgnis__uytm)
...
def init_series_iat(typingctx, obj=None):
    ...
    def codegen(context, builder, signature, args):
        fsd__lglk, = args
        fed__evvzk = signature.return_type
        akp__frsv = cgutils.create_struct_proxy(fed__evvzk)(context, builder)
        akp__frsv.obj = fsd__lglk
        context.nrt.incref(builder, signature.args[0], fsd__lglk)
        return akp__frsv._getvalue()
...
def overload_series_iat_setitem(I, idx, val):
    if isinstance(I, SeriesIatType):
        ...
        if I.stype.dtype == types.NPTimedelta('ns'
            ...
            def impl_dt(I, idx, val):
                ylpiy__uoz = (bodo.hiframes.datetime_timedelta_ext.
                    _to_nanoseconds(val))
...
class SeriesIlocType(types.Type):
    ...
    def __init__(self, stype):
        self.stype = stype
        pquj__qhvas = 'SeriesIlocType({})'.format(stype)
        super(SeriesIlocType, self).__init__(pquj__qhvas)
...
class SeriesIlocModel(models.StructModel):
    ...
    def __init__(self, dmm, fe_type):
        kgnis__uytm = [('obj', fe_type.stype)]
        super(SeriesIlocModel, self).__init__(dmm, fe_type, kgnis__uytm)
...
def init_series_iloc(typingctx, obj=None):
    ...
    def codegen(context, builder, signature, args):
        fsd__lglk, = args
        fgur__ztn = signature.return_type
        ynr__ovygi = cgutils.create_struct_proxy(fgur__ztn)(context, builder)
        ynr__ovygi.obj = fsd__lglk
        context.nrt.incref(builder, signature.args[0], fsd__lglk)
        return ynr__ovygi._getvalue()
...
def overload_series_iloc_getitem(I, idx):
    if isinstance(I, SeriesIlocType):
        ...
        if is_list_like_index_type(idx) and isinstance(idx.dtype, (types.
            ...
            def impl(I, idx):
                S = I._obj
                wzzf__wytl = bodo.utils.conversion.coerce_to_ndarray(idx)
                rzwyc__xna = bodo.hiframes.pd_series_ext.get_series_data(S)[
                    wzzf__wytl]
                eamjc__aenya = bodo.hiframes.pd_series_ext.get_series_index(S)[
                    wzzf__wytl]
                pquj__qhvas = bodo.hiframes.pd_series_ext.get_series_name(S)
                return bodo.hiframes.pd_series_ext.init_series(rzwyc__xna,
                    eamjc__aenya, pquj__qhvas)
            return impl
        if isinstance(idx, types.SliceType):

            def impl_slice(I, idx):
                S = I._obj
                rzwyc__xna = bodo.hiframes.pd_series_ext.get_series_data(S)[idx
                    ]
                eamjc__aenya = bodo.hiframes.pd_series_ext.get_series_index(S)[
                    idx]
                pquj__qhvas = bodo.hiframes.pd_series_ext.get_series_name(S)
        ...
        if is_list_like_index_type(idx) and isinstance(idx.dtype, (types.
            ...
            if is_scalar_type(val):
                if I.stype.dtype == types.NPDatetime('ns'
                    ...
                    def impl_dt(I, idx, val):
                        s = integer_to_dt64(val.value)
                        wzzf__wytl = bodo.utils.conversion.coerce_to_ndarray(
                            idx)
                ...
                if I.stype.dtype == types.NPTimedelta('ns'
                    ...
                    def impl_dt(I, idx, val):
                        ylpiy__uoz = (bodo.hiframes.datetime_timedelta_ext.
                            _to_nanoseconds(val))
                        s = (bodo.hiframes.pd_timestamp_ext.
                            integer_to_timedelta64(ylpiy__uoz))
                        wzzf__wytl = bodo.utils.conversion.coerce_to_ndarray(
                            idx)
                        bodo.hiframes.pd_series_ext.get_series_data(I._obj)[
                            wzzf__wytl] = s
            ...
            def impl_arr(I, idx, val):
                wzzf__wytl = bodo.utils.conversion.coerce_to_ndarray(idx)
                bodo.hiframes.pd_series_ext.get_series_data(I._obj)[wzzf__wytl
                    ] = bodo.utils.conversion.coerce_to_array(val, False)
            return impl_arr
...
class SeriesLocType(types.Type):
    ...
    def __init__(self, stype):
        self.stype = stype
        pquj__qhvas = 'SeriesLocType({})'.format(stype)
        super(SeriesLocType, self).__init__(pquj__qhvas)
...
class SeriesLocModel(models.StructModel):
    ...
    def __init__(self, dmm, fe_type):
        kgnis__uytm = [('obj', fe_type.stype)]
        super(SeriesLocModel, self).__init__(dmm, fe_type, kgnis__uytm)
...
def init_series_loc(typingctx, obj=None):
    ...
    def codegen(context, builder, signature, args):
        fsd__lglk, = args
        lfw__fpyry = signature.return_type
        ofain__icbi = cgutils.create_struct_proxy(lfw__fpyry)(context, builder)
        ofain__icbi.obj = fsd__lglk
        context.nrt.incref(builder, signature.args[0], fsd__lglk)
        return ofain__icbi._getvalue()
...
def overload_series_loc_getitem(I, idx):
    ...
    if isinstance(idx, types.Integer) and isinstance(I.stype.index,
        ...
        def impl_range_index_int(I, idx):
            ...
            rzwyc__xna = bodo.hiframes.pd_series_ext.get_series_data(S)
            eamjc__aenya = bodo.hiframes.pd_series_ext.get_series_index(S)
            qhwq__bdcgb = (idx - eamjc__aenya._start) // eamjc__aenya._step
            return rzwyc__xna[qhwq__bdcgb]
        ...
        if isinstance(idx, types.Integer):
            ...
            if isinstance(S.index, RangeIndexType):
                ...
                def impl(S, idx):
                    ...
                    wzzf__wytl = idx * I._step + I._start
                    return bodo.utils.conversion.box_if_dt64(rzwyc__xna[
                        wzzf__wytl])
                return impl
        ...
        if idx == bodo.string_type or is_overload_constant_str(idx):
            if isinstance(S.index, bodo.hiframes.pd_index_ext.StringIndexType):
                ...
                def impl_str_getitem(S, idx):
                    rxe__gqczy = bodo.hiframes.pd_series_ext.get_series_index(S
                        )
                    shpkv__baw = rxe__gqczy.get_loc(idx)
                    if shpkv__baw is None:
                        raise IndexError
                    else:
                        val = bodo.hiframes.pd_series_ext.get_series_data(S)[
                            bodo.utils.indexing.unoptional(shpkv__baw)]
                    return val
    ...
    if isinstance(S, SeriesType):
        ...
        if is_list_like_index_type(idx) and isinstance(idx.dtype, (types.
            ...
            def impl_arr(S, idx, val):
                wzzf__wytl = bodo.utils.conversion.coerce_to_ndarray(idx)
                bodo.hiframes.pd_series_ext.get_series_data(S)[wzzf__wytl
                    ] = bodo.utils.conversion.coerce_to_array(val, False)
            return impl_arr
...
def overload_const_index_series_getitem(S, idx):
    if isinstance(S, (SeriesType, HeterogeneousSeriesType)) and isinstance(S
        ...
        index.data):
        aago__vuuhr = get_overload_const_tuple(S.index.data)
        if isinstance(idx, types.Integer) and not any(isinstance(bfj__qhagn,
            int) for bfj__qhagn in aago__vuuhr):
            return lambda S, idx: bodo.hiframes.pd_series_ext.get_series_data(S
                )[idx]
        if is_literal_type(idx):
            ydgo__mwca = get_literal_value(idx)
            if ydgo__mwca not in aago__vuuhr:
                raise_bodo_error(
                    f"Series label-based getitem: '{ydgo__mwca}' not in {aago__vuuhr}"
                    )
            arr_ind = aago__vuuhr.index(ydgo__mwca)
            return lambda S, idx: bodo.hiframes.pd_series_ext.get_series_data(S
                )[arr_ind]
...
def cast_series_iat(context, builder, fromty, toty, val):
    akp__frsv = cgutils.create_struct_proxy(fromty)(context, builder, val)
    gqw__qkquq = context.cast(builder, akp__frsv.obj, fromty.stype, toty.stype)
    tlyqj__dozq = cgutils.create_struct_proxy(toty)(context, builder)
    tlyqj__dozq.obj = gqw__qkquq
    return tlyqj__dozq._getvalue()
