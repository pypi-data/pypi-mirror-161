...
from .objects.IMTDLMSBase import IMTDLMSBase
...
class MTDLMSServer(object):
    ...
    def __init__(self, logicalNameReferencing, interfaceType):
        self.info = MTReplyData()
        # Received data.
        self.receivedData = MTByteBuffer()
        # Reply data.
        self.replyData = MTByteBuffer()
        # Long get or read transaction information.
        self.transaction = None
        # Server settings.
        self.settings = MTDLMSSettings(True)
 ...
 #
    ...
    def addData(self, obj, index, buff):
        ...
        _MTCommon.setData(self.settings, buff, dt, value)
    ...
    def generateDataNotificationMessages(self, time, data):
        if self.useLogicalNameReferencing:
            p = MTDLMSLNParameters(self.settings, 0, Command.DATA_NOTIFICATION, 0, None, data, 0xff)
            if time is None:
                p.time = None
            else:
            ...
            p2 = MTDLMSSNParameters(self.settings, Command.DATA_NOTIFICATION, 1, 0, data, None)
    ...
    def generatePushSetupMessages(self, date, push):
        ...
        _MTCommon.setObjectCount(len(push.pushObjectList), buff)
    ...
    def initialize(self):
        ...
        while pos != len(self.settings.objects):
            ...
            it.start = self
            if isinstance(it, (MTDLMSAssociationShortName,)) and not self.useLogicalNameReferencing:
                if len(it.objectList) == 0:
                    it.objectList.append(self.items)
                associationObject = it
            elif isinstance(it, (MTDLMSAssociationLogicalName,)) and self.useLogicalNameReferencing:
                ln = it
            ...
            elif not isinstance(it, IMTDLMSBase):
                self.settings.objects.remove(pos)
                pos -= 1
            pos += 1
        if not associationObject:
            if self.useLogicalNameReferencing:
                it = MTDLMSAssociationLogicalName()
                ...
                it2 = MTDLMSAssociationShortName()
    ...
    def updateShortNames(self, force):
        ...
        for it in self.settings.objects:
            if not isinstance(it, (MTDLMSAssociationShortName, MTDLMSAssociationLogicalName)):
                if force or it.shortName == 0:
                    it.setShortName(sn)
                    MTDLMS.getActionInfo(it.objectType, offset, count)
                    if count[0] != 0:
    ...
    def handleAarqRequest(self, data, connectionInfo):
        ...
        try:
            diagnostic = _MTAPDU.parsePDU(self.settings, self.settings.cipher, data, None)
        ...
        except MTDLMSConfirmedServiceError as e:
            result = AssociationResult.PERMANENT_REJECTED
            diagnostic = SourceDiagnostic.NO_REASON_GIVEN
            error = MTByteBuffer()
            error.setUInt8(0xE)
        ...
        except MTDLMSException as e:
            result = e.result
            diagnostic = e.diagnostic
        if self.settings.interfaceType == InterfaceType.HDLC or self.settings.interfaceType == InterfaceType.HDLC_WITH_MODE_E:
            self.replyData.set(_MTCommon.LLC_REPLY_BYTES)
        if self.settings.authentication > Authentication.LOW:
            self.settings.setStoCChallenge(MTSecure.generateChallenge())
        _MTAPDU.generateAARE(self.settings, self.replyData, result, diagnostic, self.settings.cipher, error, None)

    def handleReleaseRequest(self, data):
        # pylint: disable=unused-argument
        if self.settings.interfaceType == InterfaceType.HDLC or self.settings.interfaceType == InterfaceType.HDLC_WITH_MODE_E:
            self.replyData.set(0, _MTCommon.LLC_REPLY_BYTES)
        tmp = _MTAPDU.getUserInformation(self.settings, self.settings.cipher)
    ...
    def handleSnrmRequest(self, data):
        if self.hdlc:
            ...
            self.settings.hdlc.maxInfoRX = MTHdlcSettings.DEFAULT_MAX_INFO_RX
            self.settings.hdlc.maxInfoTX = MTHdlcSettings.DEFAULT_MAX_INFO_TX
            self.settings.hdlc.windowSizeRX = MTHdlcSettings.DEFAULT_WINDOWS_SIZE_RX
            self.settings.hdlc.windowSizeTX = MTHdlcSettings.DEFAULT_WINDOWS_SIZE_TX
        ...
        MTDLMS.appendHdlcParameter(self.replyData, self.hdlc.maxInfoTX)
        self.replyData.setUInt8(_HDLCInfo.MAX_INFO_RX)
        MTDLMS.appendHdlcParameter(self.replyData, self.hdlc.maxInfoRX)
    ...
    def handleRequest(self, sr):
        ...
        try:
            if not sr.isStreaming():
                ...
                try:
                    MTDLMS.getData(self.settings, self.receivedData, self.info, None)
                except Exception:
                    self.dataReceived = datetime.datetime.now()
                    self.receivedData.size = 0
                    sr.setReply(MTDLMS.getHdlcFrame(self.settings, Command.UNACCEPTABLE_FRAME, self.replyData))
                    return
                ...
                if self.info.command == Command.DISCONNECT_REQUEST and (self.settings.connected == ConnectionState.NONE):
                    sr.setReply(MTDLMS.getHdlcFrame(self.settings, Command.DISCONNECT_MODE, self.replyData))
                    self.info.clear()
                    return
                ...
                if (self.info.moreData & RequestTypes.FRAME) == RequestTypes.FRAME:
                    self.dataReceived = datetime.datetime.now()
                    sr.setReply(MTDLMS.getHdlcFrame(self.settings, self.settings.getReceiverReady(), self.replyData))
                    return
            ...
            if isinstance(e, (MTDLMSConfirmedServiceError,)):
    ...
    def reportConfirmedServiceError(self, e):
        self.replyData.clear()
        if self.settings.interfaceType == InterfaceType.HDLC or self.settings.interfaceType == InterfaceType.HDLC_WITH_MODE_E:
            MTDLMS.addLLCBytes(self.settings, self.replyData)
        ...
        if self.settings.interfaceType == InterfaceType.WRAPPER:
            return MTDLMS.getWrapperFrame(self.settings, Command.CONFIRMED_SERVICE_ERROR, self.replyData)
        return MTDLMS.getHdlcFrame(self.settings, int(0), self.replyData)
    ...
    def reportError(self, command, error):
        ...
        if self.settings.getUseLogicalNameReferencing():
            p = MTDLMSLNParameters(self.settings, 0, cmd, 1, None, None, error)
            MTDLMS.getLNPdu(p, self.replyData)
        else:
            bb = MTByteBuffer()
            bb.setUInt8(error)
            p2 = MTDLMSSNParameters(self.settings, cmd, 1, 1, None, bb)
            MTDLMS.getSNPdu(p2, self.replyData)
        if self.settings.interfaceType == InterfaceType.WRAPPER:
            return MTDLMS.getWrapperFrame(self.settings, command, self.replyData)
        return MTDLMS.getHdlcFrame(self.settings, int(0), self.replyData)
    ...
    def handleCommand(self, cmd, data, sr):
        ...
        if cmd == Command.ACCESS_REQUEST:
            MTDLMSLNCommandHandler.handleAccessRequest(self.settings, self, data, self.replyData, None)
        elif cmd == Command.SET_REQUEST:
            MTDLMSLNCommandHandler.handleSetRequest(self.settings, self, data, self.replyData, None)
        elif cmd == Command.WRITE_REQUEST:
            MTDLMSSNCommandHandler.handleWriteRequest(self.settings, self, data, self.replyData, None)
        elif cmd == Command.GET_REQUEST:
            if data:
                MTDLMSLNCommandHandler.handleGetRequest(self.settings, self, data, self.replyData, None)
        elif cmd == Command.READ_REQUEST:
            MTDLMSSNCommandHandler.handleReadRequest(self.settings, self, data, self.replyData, None)
        elif cmd == Command.METHOD_REQUEST:
            MTDLMSLNCommandHandler.handleMethodRequest(self.settings, self, data, sr.getConnectionInfo(), self.replyData, None)
        ...
        if self.settings.interfaceType == InterfaceType.WRAPPER:
            reply = MTDLMS.getWrapperFrame(self.settings, cmd, self.replyData)
        else:
            reply = MTDLMS.getHdlcFrame(self.settings, frame_, self.replyData)
    ...
    def handleGeneralBlockTransfer(self, data, sr):
        if self.transaction:
            if self.transaction.command == Command.GET_REQUEST:
                ...
                MTDLMSLNCommandHandler.getRequestNextDataBlock(self.settings, 0, self, data, self.replyData, None, True)
                if sr.count != 0:
                ...
                len_ = _MTCommon.getObjectCount(data)
            ...
            if len_ > len(data) - data.position:
                ...
                self.transaction = MTDLMSLongTransaction(None, data.getUInt8(), data)
                self.replyData.setUInt8(Command.GENERAL_BLOCK_TRANSFER)
