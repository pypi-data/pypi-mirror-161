...
class MTStandardObisCodeCollection(list):
    """
    ...
    def getBytes(cls, ln):
        ...
        if len(tmp) != 6:
            ...
            tmp = MTByteBuffer.hexToBytes(ln)
            if len(tmp) != 6:
    ...
    def find2(self, obisCode, ic):
        ...
        for it in self:
            ...
            if self.equalsInterface(it, ic) and self.equalsObisCode(it.obis, obisCode):
                tmp = MTStandardObisCode(it.obis[0:], it.description, it, it.dataType)
        ...
        if not list_:
            tmp = MTStandardObisCode(None, "Invalid", str(int(ic)), "")
            obis = tmp.obis
            obis[0] = str(obisCode[0])
            obis[1] = str(obisCode[1])
            obis[2] = str(obisCode[2])
