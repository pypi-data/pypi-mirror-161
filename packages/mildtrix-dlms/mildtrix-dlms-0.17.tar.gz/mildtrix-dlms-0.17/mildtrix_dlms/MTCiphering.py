...
class MTCiphering(MTICipher):
    ...
    def decrypt(cls, c, p, data):
        ...
        tmp = MTDLMSChippering.decryptAesGcm(p, data)
    ...
    def encrypt(cls, p, data):
        if p.security != Security.NONE:
            tmp = MTDLMSChippering.encryptAesGcm(p, data)
            return tmp
        return data
    ...
    def generateGmacPassword(self, challenge):
        ...
        bb = MTByteBuffer()
        MTDLMSChippering.encryptAesGcm(p, challenge)
        bb.setUInt8(0x10)
