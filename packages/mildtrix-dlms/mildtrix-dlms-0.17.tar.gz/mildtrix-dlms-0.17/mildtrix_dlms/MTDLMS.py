...
from .internal._MTDataInfo import _MTDataInfo
...
class MTDLMS:
    """
    MTDLMS implements methods to communicate with DLMS/COSEM metering devices.
    """
    ...
    def receiverReady(cls, settings, reply):
        ...
        if (reply.moreData & RequestTypes.FRAME) != 0:
            ...
            return MTDLMS.getHdlcFrame(settings, id_, None)
        cmd = settings.command
        if reply.moreData == RequestTypes.GBT:
            p = MTDLMSLNParameters(settings, 0, Command.GENERAL_BLOCK_TRANSFER, 0, None, None, 0xff)
            p.gbtWindowSize = reply.gbtWindowSize
            p.blockNumberAck = reply.blockNumber
            p.blockIndex = settings.blockNumber
            reply = MTDLMS.getLnMessages(p)
        else:
            #  Get next block.
            bb = MTByteBuffer(4)
            ...
            if settings.getUseLogicalNameReferencing():
                p = MTDLMSLNParameters(settings, 0, cmd, GetCommandType.NEXT_DATA_BLOCK, bb, None, 0xff)
                reply = MTDLMS.getLnMessages(p)
            else:
                p = MTDLMSSNParameters(settings, cmd, 1, VariableAccessSpecification.BLOCK_NUMBER_ACCESS, bb, None)
                reply = MTDLMS.getSnMessages(p)
        return reply[0]
        ...
        if tp == DataType.ARRAY:
            ...
            if isinstance(value, str) and tp == DataType.OCTET_STRING:
                ...
                if ui == DataType.STRING:
                    _MTCommon.setData(settings, bb, tp, value.encode())
                    return
        _MTCommon.setData(settings, bb, tp, value)
    ...
    def addLLCBytes(cls, settings, data):
        ...
        if settings.isServer:
            data.set(_MTCommon.LLC_REPLY_BYTES)
        else:
            data.set(_MTCommon.LLC_SEND_BYTES)
        data.set(tmp)
    ...
    def multipleBlocks(cls, p, reply, ciphering):
        ...
        if ciphering:
            len_ += MTDLMS.__CIPHERING_HEADER_SIZE
    ...
    def getLNPdu(cls, p, reply):
        ...
        if p.command == Command.AARQ:
            ...
            if p.command == Command.EVENT_NOTIFICATION or p.command == Command.DATA_NOTIFICATION or p.command == Command.ACCESS_REQUEST or p.command == Command.ACCESS_RESPONSE:
                if p.command != Command.EVENT_NOTIFICATION:
                    if p.invokeId != 0:
                        ...
                        reply.setUInt32(MTDLMS.getLongInvokeIDPriority(p.settings))
                if p.time is None:
                    reply.setUInt8(DataType.NONE)
                else:
                    pos = len(reply)
                    _MTCommon.setData(p.settings, reply, DataType.OCTET_STRING, p.getTime())
                    if p.command != Command.EVENT_NOTIFICATION:
                        reply.move(pos + 1, pos, len(reply) - pos - 1)
                MTDLMS.multipleBlocks(p, reply, ciphering)
                ...
                if p.command != Command.GENERAL_BLOCK_TRANSFER:
                    ...
                    if p.invokeId != 0:
                        ...
                        reply.setUInt8(MTDLMS.getInvokeIDPriority(p.settings))
            reply.set(p.attributeDescriptor)
            if p.multipleBlocks and (p.settings.negotiatedConformance & Conformance.GENERAL_BLOCK_TRANSFER) == 0:
                ...
                if ciphering:
                    totalLength += MTDLMS.__CIPHERING_HEADER_SIZE
                if totalLength > p.settings.maxPduSize:
                    len_ = p.settings.maxPduSize - len(reply)
                    if ciphering:
                        len_ -= MTDLMS.__CIPHERING_HEADER_SIZE
                    len_ -= _MTCommon.getObjectCountSizeInBytes(len_)
                _MTCommon.setObjectCount(len_, reply)
            ...
            if ciphering and reply and p.settings.negotiatedConformance & Conformance.GENERAL_BLOCK_TRANSFER == Conformance.NONE and p.command != Command.RELEASE_REQUEST:
                tmp = []
                if p.settings.cipher.securitySuite == SecuritySuite.AES_GCM_128:
                    tmp = MTDLMS.cipher0(p, reply.array())
                reply.size = 0
                reply.set(tmp)
        if p.command == Command.GENERAL_BLOCK_TRANSFER or (p.multipleBlocks and p.settings.negotiatedConformance & Conformance.GENERAL_BLOCK_TRANSFER != Conformance.NONE):
            ...
            if p.command != Command.DATA_NOTIFICATION and p.blockNumberAck != 0:
                ...
                p.blockNumberAck = -1
                reply.setUInt16(0)
            _MTCommon.setObjectCount(len(bb), reply)
            reply.set(bb)
            if p.command != Command.GENERAL_BLOCK_TRANSFER:
        ...
        if p.settings.interfaceType == InterfaceType.HDLC or p.settings.interfaceType == InterfaceType.HDLC_WITH_MODE_E:
            MTDLMS.addLLCBytes(p.settings, reply)
    ...
    def cipher0(cls, p, data):
        ...
        if ((p.settings.connected & ConnectionState.DLMS) == 0 or (p.settings.negotiatedConformance & Conformance.GENERAL_PROTECTION) == 0) and \
            ...
            if cipher.dedicatedKey and (p.settings.connected & ConnectionState.DLMS) != 0:
                cmd = MTDLMS.getDedMessage(p.command)
                key = cipher.dedicatedKey
            else:
                cmd = MTDLMS.getGloMessage(p.command)
        ...
        tmp = MTCiphering.encrypt(s, data)
        if p.command == Command.DATA_NOTIFICATION or p.command == Command.GENERAL_GLO_CIPHERING or p.command == Command.GENERAL_DED_CIPHERING:
            ...
            if p.settings.getStandard() == Standard.ITALY:
                ...
                _MTCommon.setObjectCount(len(p.settings.cipher.systemTitle), reply)
        ...
        while True:
            ...
            while reply.position != len(reply):
                if p.settings.interfaceType == InterfaceType.WRAPPER:
                    messages.append(MTDLMS.getWrapperFrame(p.settings, p.command, reply))
                elif p.settings.interfaceType == InterfaceType.HDLC or p.settings.interfaceType == InterfaceType.HDLC_WITH_MODE_E:
                    messages.append(MTDLMS.getHdlcFrame(p.settings, frame_, reply))
    ...
    def appendMultipleSNBlocks(cls, p, reply):
        ...
        if p.command == Command.WRITE_REQUEST or p.command == Command.READ_REQUEST:
            hSize += 1 + _MTCommon.getObjectCountSizeInBytes(p.count)
        maxSize = p.settings.maxPduSize - hSize
        if ciphering:
            maxSize -= MTDLMS.__CIPHERING_HEADER_SIZE
            if p.settings.interfaceType == InterfaceType.HDLC or p.settings.interfaceType == InterfaceType.HDLC_WITH_MODE_E:
                maxSize -= 3
        maxSize -= _MTCommon.getObjectCountSizeInBytes(maxSize)
        ...
        if p.command == Command.WRITE_REQUEST:
            p.blockIndex = p.blockIndex + 1
            _MTCommon.setObjectCount(p.count, reply)
            reply.setUInt8(DataType.OCTET_STRING)
        elif p.command == Command.READ_REQUEST:
            p.blockIndex = p.blockIndex + 1
        _MTCommon.setObjectCount(maxSize, reply)
        return maxSize
    ...
    def getSNPdu(cls, p, reply):
        ...
        if not ciphering and (p.settings.interfaceType == InterfaceType.HDLC or p.settings.interfaceType == InterfaceType.HDLC_WITH_MODE_E):
            if p.settings.isServer:
                reply.set(_MTCommon.LLC_REPLY_BYTES)
            elif not reply:
                reply.set(_MTCommon.LLC_SEND_BYTES)
        cnt = 0
        cipherSize = 0
        if ciphering:
            cipherSize = MTDLMS.__CIPHERING_HEADER_SIZE
        if p.data:
        ...
        if p.command == Command.INFORMATION_REPORT:
            ...
            if not p.time:
                ...
                pos = len(reply)
                _MTCommon.setData(p.settings, reply, DataType.OCTET_STRING, p.time)
                reply.move(pos + 1, pos, len(reply) - pos - 1)
            _MTCommon.setObjectCount(p.count, reply)
            reply.set(p.attributeDescriptor)
            ...
            if not p.multipleBlocks:
                ...
                if p.multipleBlocks:
                    ...
                    if p.requestType != 0xFF:
                        reply.setUInt8(p.requestType)
                    cnt = MTDLMS.appendMultipleSNBlocks(p, reply)
            else:
                cnt = MTDLMS.appendMultipleSNBlocks(p, reply)
        if p.data:
        ...
        if ciphering and p.command != Command.AARQ and p.command != Command.AARE:
            cipher = p.settings.cipher
            s = AesGcmParameter(MTDLMS.getGloMessage(p.command), cipher.systemTitle, cipher.blockCipherKey, cipher.authenticationKey)
            s.security = cipher.security
            s.invocationCounter = cipher.invocationCounter
            tmp = MTCiphering.encrypt(s, reply.array())
            assert not tmp
            reply.size = 0
            if p.settings.interfaceType == InterfaceType.HDLC or p.settings.interfaceType == InterfaceType.HDLC_WITH_MODE_E:
                if p.settings.isServer:
    ...
    def getAddressBytes(cls, value, size):
        tmp = MTDLMS.getAddress(value, size)
        bb = MTByteBuffer()
    ...
    def getHdlcFrame(cls, settings, frame_, data):
        ...
        if settings.isServer:
            if frame_ == 0x13 and settings.pushClientAddress != 0:
                primaryAddress = MTDLMS.getAddressBytes(settings.pushClientAddress, 1)
            else:
                primaryAddress = MTDLMS.getAddressBytes(settings.clientAddress, 1)
            secondaryAddress = MTDLMS.getAddressBytes(settings.serverAddress, settings.serverAddressSize)
        else:
            primaryAddress = MTDLMS.getAddressBytes(settings.serverAddress, settings.serverAddressSize)
            secondaryAddress = MTDLMS.getAddressBytes(settings.clientAddress, 1)
        bb.setUInt8(_MTCommon.HDLC_FRAME_START_END)
        ...
        if frame_ == 0:
            ...
            bb.setUInt8(frame_)
        crc = _MTFCS16.countFCS16(bb._data, 1, bb.size - 1)
        bb.setUInt16(crc)
        if len1 != 0:
            bb.set(data, data.position, len1)
            crc = _MTFCS16.countFCS16(bb._data, 1, len(bb) - 1)
            bb.setUInt16(crc)
        bb.setUInt8(_MTCommon.HDLC_FRAME_START_END)
        if settings.isServer:
            if data:
    ...
    def getLLCBytes(cls, server, data):
        if server:
            return data.compare(_MTCommon.LLC_SEND_BYTES)
        return data.compare(_MTCommon.LLC_REPLY_BYTES)
    ...
    def getHdlcData(cls, server, settings, reply, data, notify):
        ...
        while pos < len(reply):
            ch = reply.getUInt8()
            if ch == _MTCommon.HDLC_FRAME_START_END:
                packetStartID = pos
                break
            pos += 1
        if reply.position == len(reply):
        ...
        if (frame_ & 0xF0) != 0xA0:
            ...
            return MTDLMS.getHdlcData(server, settings, reply, data, notify)
        if (frame_ & 0x7) != 0:
        ...
        try:
            ...
            ret = MTDLMS.checkHdlcAddress(server, settings, reply, eopPos, addresses)
        except Exception:
            ret = False
        if not ret:
            #If not notify.
        ...
        if (frame_ & 0x8) != 0:
            ...
            if isNotify:
                ...
                data.moreData = data.moreData & ~RequestTypes.FRAME
        crc = _MTFCS16.countFCS16(reply, packetStartID + 1, reply.position - packetStartID - 1)
        ...
        if reply.position != packetStartID + frameLen + 1:
            crc = _MTFCS16.countFCS16(reply, packetStartID + 1, frameLen - 2)
        ...
        if cf != 0x13 and cf != 0x3 and (cf & HdlcFrameType.U_FRAME) == HdlcFrameType.U_FRAME:
            ...
            if reply.position == packetStartID + frameLen + 1:
                ...
                if not MTDLMS.getLLCBytes(server, reply) and data.xml:
                    MTDLMS.getLLCBytes(not server, reply)
        return cf
    ...
    def checkHdlcAddress(cls, server, settings, reply, index, addresses):
        # pylint: disable=too-many-arguments
        target = _MTCommon.getHDLCAddress(reply)
        source = _MTCommon.getHDLCAddress(reply)
        addresses[0] = source
        addresses[1] = target
        if server:
            ...
            if settings.serverAddress != source and (settings.serverAddress and 0x7F) != 0x7F and (settings.serverAddress and 0x3FFF) != 0x3FFF:
                ...
                logical = [0]
                physical = [0]
                MTDLMS.getServerAddress(source, readLogical, readPhysical)
                MTDLMS.getServerAddress(settings.serverAddress, logical, physical)
    ...
    def __getTcpData(cls, settings, buff, data, notify):
        ...
        while buff.position < len(buff) - 1:
            ...
            if value == 1:
                if not MTDLMS.checkWrapperAddress(settings, buff, data, notify):
                    target = notify
                    isData = False
                value = buff.getUInt16()
    ...
    def __getWiredMBusData(cls, settings, buff, data):
        ...
        if buff.getUInt8() != 0x68 or buff.available() < 5:
            ...
            if buff.getUInt8() != len_ or buff.available() < 3 + len_ or buff.getUInt8() != 0x68:
                ...
                crc = MTDLMS.__validateCheckSum(buff, len_)
    ...
    def __getWirelessMBusData(cls, settings, buff, data):
        ...
        if len(buff) < len_ - 1:
            ...
            man = _MTCommon.decryptManufacturer(manufacturerID)
            #id =
            buff.getUInt32()
            ...
            if data.xml and data.xml.comments:
                ...
                settings.mBus.manufacturerId = _MTCommon.decryptManufacturer(manufacturerID)
    ...
    def __getPlcData(cls, settings, buff, data):
        ...
        if buff.available < len_:
            ...
            if buff.size < len_ + padLen + 2:
                ...
                #Skip padding.
                if data.complete:
                    crcCount = _MTFCS16.countFCS16(buff.data, 0, len_ + padLen)
    ...
    def __getPlcHdlcData(cls, settings, buff, data):
        ...
        frame = 0
        #SN field.
        frameLen = MTDLMS.getPlcSfskFrameSize(buff)
        if frameLen == 0:
        ...
        if buff.Available < frameLen:
            ...
            if data.IsComplete:
                #PAD length.
                padLen = buff.getUInt8()
                frame = MTDLMS.getHdlcData(settings.isServer, settings, buff, data, None)
                MTDLMS.getDataFromFrame(buff, data, True)
                buff.position = buff.position + padLen
                crcCount = _MTFCS16.countFCS24(buff.data, index, buff.position - index)
    ...
    def readResponseDataBlockResult(cls, settings, reply, index):
        ...
        blockLength = _MTCommon.getObjectCount(data)
        if lastBlock == 0:
        ...
        if (reply.moreData & RequestTypes.FRAME) != 0:
            MTDLMS.getDataFromBlock(data, index)
            return False
        ...
        if reply.xml:
            ...
            return False
        MTDLMS.getDataFromBlock(reply.data, index)
    ...
    def handleReadResponse(cls, settings, reply, index):
        ...
        if first:
            cnt = _MTCommon.getObjectCount(reply.data)
        ...
        if cnt != 1:
            ...
            if reply.isMoreData():
                MTDLMS.getDataFromBlock(reply.data, 0)
                return False
            if not first:
        ...
        while pos != cnt:
            ...
            if type_ == SingleReadResponse.DATA:
                ...
                if reply.xml:
                    ...
                    di = _MTDataInfo()
                    di.xml = (reply.xml)
                    _MTCommon.getData(settings, reply.data, di)
                    ...
                    MTDLMS.getValueFromData(settings, reply)
                ...
                if reply.xml:
                    if standardXml:
                        reply.xml.appendStartTag(TranslatorTags.CHOICE)
                    reply.xml.appendLine(Command.READ_RESPONSE << 8 | SingleReadResponse.DATA_ACCESS_ERROR, None, MTDLMS.errorCodeToString(reply.xml.outputType, reply.error))
                    if standardXml:
                        reply.xml.appendEndTag(TranslatorTags.CHOICE)
            elif type_ == SingleReadResponse.DATA_BLOCK_RESULT:
                if not MTDLMS.readResponseDataBlockResult(settings, reply, index):
                    if reply.xml:
    ...
    def handleActionResponseNormal(cls, settings, data):
        ...
        if data.xml:
            ...
            data.xml.appendLine(TranslatorTags.RESULT, None, MTDLMS.errorCodeToString(data.xml.outputType, data.error))
        settings.resetBlockIndex()
        if data.data.position < data.data.size:
            ret = data.data.getUInt8()
            if ret == 0:
                MTDLMS.getDataFromBlock(data.data, 0)
            elif ret == 1:
            ...
            if data.xml and (ret != 0 or data.data.position < data.data.size):
                ...
                if ret != 0:
                    data.xml.appendLine(TranslatorTags.DATA_ACCESS_ERROR, None, MTDLMS.errorCodeToString(data.xml.outputType, data.error))
                else:
                    data.xml.appendStartTag(Command.READ_RESPONSE, SingleReadResponse.DATA)
                    di = _MTDataInfo()
                    di.xml = (data.xml)
                    _MTCommon.getData(settings, data.data, di)
    ...
    def handleActionResponseWithBlock(cls, settings, reply, index):
        ...
        if reply.xml:
            if reply.data.available() != 0:
                # Get data size.
                blockLength = _MTCommon.getObjectCount(reply.data)
                ...
                reply.xml.appendLine(TranslatorTags.RAW_DATA, None, MTByteBuffer.toHex(reply.data.data, False, reply.data.position, reply.data.available))
    ...
    def handleMethodResponse(cls, settings, data, index):
        ...
        if type_ == ActionResponseType.NORMAL:
            MTDLMS.handleActionResponseNormal(settings, data)
        elif type_ == 2:
            MTDLMS.handleActionResponseWithBlock(settings, data, index)
        elif type_ == 3:
            raise ValueError("Invalid Command.")
        elif type_ == 4:
    ...
    def handleAccessResponse(cls, settings, reply):
        ...
        if len_ != 0:
            tmp = bytearray(len_)
            data.get(tmp)
            reply.time = _MTCommon.changeType(settings, tmp, DataType.DATETIME)
        if reply.xml:
            ...
            reply.xml.appendLine(TranslatorTags.DATE_TIME, None, MTByteBuffer.hex(tmp, False))
            reply.data.getUInt8()
            len_ = _MTCommon.getObjectCount(reply.data)
            ...
            while pos != len_:
                ...
                reply.xml.appendLine(TranslatorTags.RESULT, None, MTDLMS.errorCodeToString(reply.xml, err))
    ...
    def handleDataNotification(cls, settings, reply):
        ...
        if len_ != 0:
            ...
            info = _MTDataInfo()
            info.type_ = dt
            reply.time = _MTCommon.getData(settings, MTByteBuffer(tmp), info)
        if reply.xml:
            ...
            MTDLMS.getDataFromBlock(reply.data, start)
            MTDLMS.getValueFromData(settings, reply)
    ...
    def handleSetResponse(cls, data):
        ...
        if type_ == SetResponseType.NORMAL:
            ...
            cnt = _MTCommon.getObjectCount(data.data)
            if data.xml:
                data.xml.appendStartTag(TranslatorTags.RESULT, "Qty", str(cnt))
                pos = 0
                while pos != cnt:
                    err = data.data.getUInt8()
                    data.xml.appendLine(TranslatorTags.DATA_ACCESS_RESULT, None, MTDLMS.errorCodeToString(data.xml.outputType, err))
                    pos += 1
    ...
    def handleWriteResponse(cls, data):
        ...
        while pos != cnt:
            ...
            if data.xml:
                if ret == 0:
                    data.xml.appendLine("<" + MTDLMS.errorCodeToString(data.xml.outputType, ret) + " />")
                else:
                    data.xml.appendLine(TranslatorTags.DATA_ACCESS_ERROR, None, MTDLMS.errorCodeToString(data.xml.outputType, data.error))
            pos += 1
        if data.xml:
            ...
            if reply.xml:
                ...
                if reply.error:
                    reply.xml.appendLine(TranslatorTags.DATA_ACCESS_ERROR, None,\
                        MTDLMS.errorCodeToString(reply.xml.outputType, reply.error))
                else:
    ...
    def handleGetResponseNextDataBlock(cls, settings, reply, index):
        ...
        if reply.xml:
            reply.xml.appendStartTag(TranslatorTags.RESULT)
            if reply.getError() != 0:
                reply.xml.appendLine(TranslatorTags.DATA_ACCESS_RESULT, None, MTDLMS.errorCodeToString(reply.xml.outputType, reply.error))
            elif reply.data.available() != 0:
                blockLength = _MTCommon.getObjectCount(reply.data)
                if (reply.moreData & RequestTypes.FRAME) == 0:
            ...
            if reply.moreData == RequestTypes.NONE and settings and \
                ...
                MTDLMS.handleGetResponseWithList(settings, reply)
                ret = False
        return ret
    ...
    def handleGetResponse(cls, settings, reply, index):
        ...
        if type_ == GetCommandType.NORMAL:
            empty = MTDLMS.handleGetResponseNormal(settings, reply)
        elif type_ == GetCommandType.NEXT_DATA_BLOCK:
            MTDLMS.handleGetResponseNextDataBlock(settings, reply, index)
    ...
    def handleGbt(cls, settings, data):
        ...
        data.command = Command.NONE
        len_ = _MTCommon.getObjectCount(data.data)
        ...
        if data.xml:
            ...
            return
        MTDLMS.getDataFromBlock(data.data, index)
        if (bc & 0x80) == 0:
            ...
            if data.data.position != data.data.size and (data.command == Command.READ_RESPONSE or data.command == Command.GET_RESPONSE) and (data.moreData == RequestTypes.NONE or data.peek):
                data.data.position = 0
                MTDLMS.getValueFromData(settings, data)
    ...
    def getPdu(cls, settings, data):
        ...
        if data.command == Command.NONE:
            ...
            if cmd == Command.READ_RESPONSE:
                if not MTDLMS.handleReadResponse(settings, data, index):
                    return
            elif cmd == Command.GET_RESPONSE:
                if not MTDLMS.handleGetResponse(settings, data, index):
                    return
            elif cmd == Command.SET_RESPONSE:
                MTDLMS.handleSetResponse(data)
            elif cmd == Command.WRITE_RESPONSE:
                MTDLMS.handleWriteResponse(data)
            elif cmd == Command.METHOD_RESPONSE:
                MTDLMS.handleMethodResponse(settings, data, index)
            elif cmd == Command.ACCESS_RESPONSE:
                MTDLMS.handleAccessResponse(settings, data)
            elif cmd == Command.GENERAL_BLOCK_TRANSFER:
                if data.xml or (not settings.isServer and (data.moreData & RequestTypes.FRAME) == 0):
                    MTDLMS.handleGbt(settings, data)
                ...
                MTDLMS.handleConfirmedServiceError(data)
            elif cmd == Command.EXCEPTION_RESPONSE:
                MTDLMS.handleExceptionResponse(data)
                ...
                MTDLMS.handleGloDedRequest(settings, data)
            ...
            if cmd == Command.GENERAL_BLOCK_TRANSFER:
                ...
                if cmd == Command.READ_RESPONSE and data.totalCount > 1:
                    if not MTDLMS.handleReadResponse(settings, data, 0):
                        return
    ...
    def handleConfirmedServiceError(cls, data):
        if data.xml:
            ...
            type_ = data.data.getUInt8()
            raise MTDLMSConfirmedServiceError(service, type_, data.data.getUInt8())
    ...
    def handleExceptionResponse(cls, data):
        ...
        if data.xml:
            ...
            raise MTDLMSExceptionResponse(state, error, value)
    ...
    def handleGloDedRequest(cls, settings, data):
        ...
        if (data.moreData & RequestTypes.FRAME) == 0:
            ...
            tmp = MTCiphering.decrypt(settings.cipher, p, data.data)
            data.data.clear()
            data.data.set(tmp)
            ...
            if data.command == Command.DATA_NOTIFICATION or data.command == Command.INFORMATION_REPORT:
                data.command = Command.NONE
                data.data.position = data.data.position - 1
                MTDLMS.getPdu(cls, settings, data)
        else:
            data.data.position = data.data.position - 1
    ...
    def handleGloDedResponse(cls, settings, data, index):
        if data.xml and not data.xml.comments:
            ...
            if (data.moreData & RequestTypes.FRAME) == 0:
                ...
                data.data.set(MTCiphering.decrypt(settings.cipher, p, bb))
    ...
    def handleGeneralCiphering(cls, settings, data):
        ...
        if (data.moreData & RequestTypes.FRAME) == 0:
            ...
            if data.xml:
                ...
                data.xml.appendLine(TranslatorTags.ORIGINATOR_SYSTEM_TITLE, None, MTByteBuffer.hex(p.systemTitle, False))
                data.xml.appendLine(TranslatorTags.RECIPIENT_SYSTEM_TITLE, None, MTByteBuffer.hex(p.recipientSystemTitle, False))
                data.xml.appendLine(TranslatorTags.DATE_TIME, None, MTByteBuffer.hex(p.dateTime, False))
                data.xml.appendLine(TranslatorTags.OTHER_INFORMATION, None, MTByteBuffer.hex(p.otherInformation, False))
                ...
                data.xml.appendLine(TranslatorTags.KEY_CIPHERED_DATA, None, MTByteBuffer.hex(p.keyCipheredData, False))
                data.xml.appendEndTag(TranslatorTags.AGREED_KEY)
                data.xml.appendEndTag(TranslatorTags.KEY_INFO)
                data.xml.appendLine(TranslatorTags.CIPHERED_CONTENT, None, MTByteBuffer.hex(p.cipheredContent, False))
    ...
    def getValueFromData(cls, settings, reply):
        ...
        data.position = reply.readPosition
        try:
            value = _MTCommon.getData(settings, data, info)
    ...
    def getData(cls, settings, reply, data, notify):
        ...
        if settings.interfaceType == InterfaceType.HDLC or settings.interfaceType == InterfaceType.HDLC_WITH_MODE_E:
            frame_ = MTDLMS.getHdlcData(settings.isServer, settings, reply, target, notify)
            ...
            if not MTDLMS.__getTcpData(settings, reply, target, notify):
                target = notify
                isNotify = True
        elif settings.interfaceType == InterfaceType.WIRELESS_MBUS:
            MTDLMS.__getWirelessMBusData(settings, reply, target)
            ...
            MTDLMS.__getPlcData(settings, reply, data)
        elif settings.interfaceType == InterfaceType.PLC_HDLC:
            frame_ = MTDLMS.__getPlcHdlcData(settings, reply, data)
        elif settings.interfaceType == InterfaceType.WIRED_MBUS:
            MTDLMS.__getWiredMBusData(settings, reply, data)
        else:
        ...
        if settings.interfaceType != InterfaceType.PLC_HDLC:
            MTDLMS.getDataFromFrame(reply, target, MTDLMS.useHdlc(settings.interfaceType))
        #If keepalive or get next frame request.
        if data.xml or ((frame_ not in (0x13, 0x3) or data.isMoreData()) and (frame_ & 0x1) != 0):
            if frame_ == 0x3 and target.isMoreData():
                tmp = MTDLMS.getData(settings, reply, data, notify)
        ...
        if not isLast or (data.moreData == RequestTypes.GBT and reply.available() != 0):
            return MTDLMS.getData(settings, reply, data, notify)
        if isNotify:
            return False
        return True
