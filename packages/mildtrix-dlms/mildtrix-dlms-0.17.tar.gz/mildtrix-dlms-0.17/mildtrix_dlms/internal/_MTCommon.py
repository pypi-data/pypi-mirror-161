...
from ._MTDataInfo import _MTDataInfo
...
class _MTCommon:
    ...
    def addString(cls, value, bb):
        ...
        if not value:
            ...
            _MTCommon.setObjectCount(len(value), bb)
            bb.set(value.encode())
    ...
    def changeType(cls, settings, value, type_):
        ...
        if value is None:
            ...
            ret = MTByteBuffer.hex(value, True)
        elif type_ in (DataType.STRING, DataType.OCTET_STRING) and not value:
            ret = ""
        elif type_ == DataType.OCTET_STRING:
            ret = MTByteBuffer(value)
        elif type_ == DataType.STRING and not MTByteBuffer.isAsciiString(value):
            ret = MTByteBuffer(value)
    ...
    def integerToHex(cls, value, desimals):
        ...
        if not desimals or desimals == len(str_):
            return str_
        return _MTCommon.zeroes[0: desimals - len(str_)] + str_.upper()
    ...
    def integerString(cls, value, desimals):
        str_ = str(value)
        if desimals == 0 or len(_MTCommon.zeroes) == len(str_):
            return str_
        return _MTCommon.zeroes[0: desimals - len(str_)] + str_
    ...
    def getArray(cls, settings, buff, info, index):
        value = None
        if info.count == 0:
            info.count = _MTCommon.getObjectCount(buff)
        if info.xml:
    ...
    def getDateTime(cls, settings, buff, info):
        ...
        try:
            ...
            if tz is None:
                ...
                dt.value = datetime(year, month, day, hour, minute, second, ms, tzinfo=MTTimeZone(tz))
            dt.skip = skip
            value = dt
        except Exception as ex:
            if info.xml is None:
    ...
    def getDouble(cls, settings, buff, info):
        ...
        if info.xml:
            ...
            info.xml.appendLine(info.xml.getDataType(info.type_), None, MTByteBuffer.toHex(False, 1, len(tmp) - 1))
        return MTFloat64(value)
    ...
    def appendDataTypeAsXml(cls, cols, info):
        for it in cols:
            ...
            elif isinstance(it, MTStructure):
    ...
    def getCompactArray(cls, settings, buff, info):
        ...
        len_ = _MTCommon.getObjectCount(buff)
        list_ = list()
        if dt == DataType.STRUCTURE:
            ...
            while buff.position - start < len_:
                ...
                pos = 0
                while pos != len(cols):
                    if isinstance(cols[pos], MTArray):
                        cls.getCompactArrayItem(settings, buff, cols[pos], row, 1)
                    elif isinstance(cols[pos], MTStructure):
                        tmp2 = list()
                ...
                for row in list_:
                    for it in row:
                        if isinstance(it, bytearray):
                            sb += MTByteBuffer.hex(it)
                        elif isinstance(it, list):
                            start = len(sb)
                            for it2 in it:
                                if isinstance(it2, bytearray):
                                    sb += MTByteBuffer.hex(it2)
                                else:
                                    sb += str(it2)
                                sb += ";"
                            if start != len(sb):
            ...
            if info.xml and info.xml.outputType == TranslatorOutputType.SIMPLE_XML:
                for it in list_:
                    if isinstance(it, bytearray):
                        info.xml.append(MTByteBuffer.hex(it))
                    else:
    ...
    def getOctetString(cls, settings, buff, info, knownType):
        ...
        if info.xml:
                    ...
                    if len(tmp) == 12 or len(tmp) == 5 or len(tmp) == 4:
                        try:
                            ...
                            if len(tmp) == 12:
                                ...
                                type_ = DataType.TIME
                            dt = _MTCommon.changeType(settings, tmp, type_)
                            year = dt.value.year
                            if 1970 < year < 2100:
            ...
            info.xml.appendLine(info.xml.getDataType(info.type_), None, MTByteBuffer.hex(tmp, False))
        return value
    ...
    def setData(cls, settings, buff, dataType, value):
        if dataType in (DataType.ARRAY, DataType.STRUCTURE) and isinstance(value, (MTByteBuffer, bytearray, bytes)):
        ...
        if dataType == DataType.NONE:
            ...
            if isinstance(value, MTDate):
                #  Add size
                buff.setUInt8(5)
                cls.setDate(buff, value)
            elif isinstance(value, MTTime):
                #  Add size
                buff.setUInt8(4)
                cls.setTime(buff, value)
            elif isinstance(value, (MTDateTime, datetime)):
                buff.setUInt8(12)
    ...
    def setTime(cls, buff, value):
        dt = _MTCommon.__getDateTime(value)
        #  Add time.
    ...
    def setArray(cls, settings, buff, value):
        if value:
            _MTCommon.setObjectCount(len(value), buff)
            for it in value:
                cls.setData(settings, buff, cls.getDLMSDataType(it), it)
        else:
            _MTCommon.setObjectCount(0, buff)

    @classmethod
    def setOctetString(cls, buff, value):
        if isinstance(value, str):
            tmp = MTByteBuffer.hexToBytes(value)
            _MTCommon.setObjectCount(len(tmp), buff)
            buff.set(tmp)
        elif isinstance(value, MTByteBuffer):
    ...
    def setString(cls, buff, value):
        if value:
            _MTCommon.setObjectCount(len(value), buff)
            buff.set(_MTCommon.getBytes(value))
        else:
            buff.setUInt8(0)
    ...
    def setBitString(cls, buff, val1, addCount):
        value = val1
        if isinstance(value, MTBitString):
            value = value.value
        if isinstance(value, str):
            val = 0
            str_ = str(value)
            if addCount:
                _MTCommon.setObjectCount(len(str_), buff)
            index = 7
            pos = 0
            while pos != len(str_):
                it = str_[pos]
            ...
            _MTCommon.setObjectCount(8 * len(value), buff)
            buff.set(value)
        elif isinstance(value, int):
            _MTCommon.setObjectCount(8, buff)
            buff.setUInt8(value)
        elif value is None:
            buff.setUInt8(0)
        else:
    ...
    def getDataType(cls, value):
        if value == DataType.NONE:
            ...
            ret = MTFloat32.__class__
        elif value == DataType.FLOAT64:
            ret = MTFloat64.__class__
        elif value == DataType.BITSTRING:
            ret = MTBitString.__class__
        else:
    ...
    def getDLMSDataType(cls, value):
        ...
        if value is None:
            ret = DataType.NONE
        elif isinstance(value, (bytes, bytearray, MTByteBuffer)):
            ret = DataType.OCTET_STRING
        elif isinstance(value, (MTEnum)):
            ret = DataType.ENUM
        elif isinstance(value, (MTInt8)):
            ret = DataType.INT8
        elif isinstance(value, (MTInt16)):
            ret = DataType.INT16
        elif isinstance(value, (MTInt64)):
            ret = DataType.INT64
        elif isinstance(value, (MTUInt8)):
            ret = DataType.UINT8
        elif isinstance(value, (MTUInt16)):
            ret = DataType.UINT16
        elif isinstance(value, (MTUInt32)):
            ret = DataType.UINT32
        elif isinstance(value, (MTUInt64)):
            ret = DataType.UINT64
        elif isinstance(value, (bool)):
            ret = DataType.BOOLEAN
        elif isinstance(value, (MTInt32, int)):
            ret = DataType.INT32
        elif isinstance(value, (MTTime)):
            ret = DataType.TIME
        elif isinstance(value, (MTDate)):
            ret = DataType.DATE
        elif isinstance(value, (datetime, MTDateTime)):
            ret = DataType.DATETIME
        elif isinstance(value, (MTStructure)):
            ret = DataType.STRUCTURE
        elif isinstance(value, (MTArray, list)):
            ret = DataType.ARRAY
        elif isinstance(value, (str)):
            ret = DataType.STRING
        elif isinstance(value, (MTFloat64)):
            ret = DataType.FLOAT64
        elif isinstance(value, (MTFloat32, complex, float)):
            ret = DataType.FLOAT32
        elif isinstance(value, (MTBitString)):
    ...
    def getGeneralizedTime(cls, dateString):
        ...
        if dateString.endsWith("Z"):
            ...
            return datetime(year, month, day, hour, minute, second, 0, tzinfo=MTTimeZone(0))

        if len(dateString) > 17:
            second = int(dateString.substring(12, 14))
        tz = dateString[dateString.length() - 4:]
        return datetime(year, month, day, hour, minute, second, 0, tzinfo=MTTimeZone(tz))
    ...
    def idisSystemTitleToString(cls, st):
        ...
        sb += _MTCommon.__getChar(st[0]) + _MTCommon.__getChar(st[1]) + _MTCommon.__getChar(st[2])
        sb += "\nFunction type: "
        ft = st[4] >> 4
        add = False
        if (ft & 0x1) != 0:
    ...
    def uniSystemTitleToString(cls, st):
        ...
        sb += "\nSerial number: "
        sb += MTByteBuffer.toHex((st[7], st[6], st[5], st[4], st[3], st[2]), False)
        return sb
    ...
    def systemTitleToString(cls, standard, st):
        ...
        if standard == Standard.ITALY or not _MTCommon.__getChar(st[0]).isalpha() or \
            not cls.__getChar(st[1]).isalpha() or not cls.__getChar(st[2]).isalpha():
            return cls.uniSystemTitleToString(st)
        if standard == Standard.IDIS or not _MTCommon.__getChar(st[3]).isdigit() or \
            not _MTCommon.__getChar(st[4]).isdigit() or not _MTCommon.__getChar(st[5]).isdigit() or \
            not _MTCommon.__getChar(st[6]).isdigit() or not _MTCommon.__getChar(st[7]).isdigit():
