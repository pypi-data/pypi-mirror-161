...
class MTDLMSProfileGeneric(MTDLMSObject, IMTDLMSBase):
    """
    Online help:
    http://www.mildtrix.fi/mildtrix.DLMS.Objects.MTDLMSProfileGeneric
    """
    ...
    def __init__(self, ln=None, sn=0):
        ...
        super(MTDLMSProfileGeneric, self).__init__(ObjectType.PROFILE_GENERIC, ln, sn)
        self.version = 1
        self.buffer = list()
    ...
    def addCaptureObject(self, item, attributeIndex, dataIndex):
        ...
        co = MTDLMSCaptureObject(attributeIndex, dataIndex)
    ...
    def __getColumns(self, settings):
        ...
        #  Add count
        _MTCommon.setObjectCount(cnt, data)
        for k, v in self.captureObjects:
            data.setUInt8(DataType.STRUCTURE)
            #  Count
            data.setUInt8(4)
            #  ClassID
            _MTCommon.setData(settings, data, DataType.UINT16, k.objectType)
            _MTCommon.setData(settings, data, DataType.OCTET_STRING, _MTCommon.logicalNameToBytes(k.logicalName))
            _MTCommon.setData(settings, data, DataType.INT8, v.attributeIndex)
            _MTCommon.setData(settings, data, DataType.UINT16, v.dataIndex)
        return data
    ...
    def getData(self, settings, e, table, columns):
        ...
        if settings.index == 0:
            data.setUInt8(DataType.ARRAY)
            if e.rowEndIndex != 0:
                _MTCommon.setObjectCount(e.rowEndIndex - e.rowBeginIndex, data)
            else:
                _MTCommon.setObjectCount(len(table), data)
        ...
        for row in table:
            ...
            if not columns:
                ...
                _MTCommon.setObjectCount(len(columns), data)
            pos = 0
            for value in items:
                if columns is None or self.captureObjects[pos] in columns:
                    tp = types[pos]
                    if tp == DataType.NONE:
                        tp = _MTCommon.getDLMSDataType(value)
                        types[pos] = tp
                    _MTCommon.setData(settings, data, tp, value)
                pos += 1
    ...
    def getColumns(self, cols):
        ...
        if cols:
            ...
            for it in cols:
                ot = ObjectType(it[0])
                ln = _MTCommon.toLogicalName(it[1])
                attributeIndex = it[2]
                dataIndex = it[3]
                for k, v in self.captureObjects:
    ...
    def __getProfileGenericData(self, settings, e):
        ...
        if e.selector == 1:
            ...
            start = _MTCommon.getData(settings, MTByteBuffer(arr[1]), info).value
            info.clear()
            info.type_ = DataType.DATETIME
            end = _MTCommon.getData(settings, MTByteBuffer(arr[2]), info).value
            for row in self.buffer:
                tm = None
                tmp = (row)[0]
                if isinstance(tmp, MTDateTime):
                    tm = tmp.value
                else:
                    tm = tmp
    ...
    def getValue(self, settings, e):
        ...
        if e.index == 1:
            ...
            if self.sortObject is None:
                _MTCommon.setData(settings, data, DataType.UINT16, 0)
                _MTCommon.setData(settings, data, DataType.OCTET_STRING, bytearray(6))
                _MTCommon.setData(settings, data, DataType.INT8, 0)
                _MTCommon.setData(settings, data, DataType.UINT16, 0)
            else:
                _MTCommon.setData(settings, data, DataType.UINT16, self.sortObject.objectType)
                _MTCommon.setData(settings, data, DataType.OCTET_STRING, _MTCommon.logicalNameToBytes(self.sortObject.logicalName))
                _MTCommon.setData(settings, data, DataType.INT8, self.sortObjectAttributeIndex)
                _MTCommon.setData(settings, data, DataType.UINT16, self.sortObjectDataIndex)
            ret = data.array()
        elif e.index == 7:
        ...
        if e.index == 1:
            ...
            if e.value:
                for it in e.value:
                    ...
                    type_ = tmp[0]
                    ln = _MTCommon.toLogicalName(tmp[1])
                    obj = None
            ...
            if e.value is None:
                ...
                if self.sortObject is None:
                    self.sortObject = _MTObjectFactory.createObject(type_)
            ...
            for it in e.value:
                ...
                for colIndex in range(len(row)):
                    ...
                    if type_ != DataType.NONE and isinstance(data, bytearray):
                        data = _MTCommon.changeType(settings, data, type_)
                        if isinstance(data, MTDateTime):
                            lastDate = data.value
                        row[colIndex] = data
                        ...
                        if lastDate:
                            if self.sortMethod in(SortMethod.FIFO, SortMethod.SMALLEST):
                                ...
                                lastDate -= timedelta(seconds=self.capturePeriod)
                            row[colIndex] = MTDateTime(lastDate)
                    elif type_ == DataType.DATETIME and not isinstance(row[colIndex], MTDateTime):
                        row[colIndex] = MTDateTime.fromUnixTime(row[colIndex])
                    item = cols[colIndex]
                    if isinstance(item[0], MTDLMSRegister) and item[1].attributeIndex == 2:
                    ...
                    elif isinstance(item[0], MTDLMSDemandRegister) and (item[1].attributeIndex == 2 or item[1].attributeIndex == 3):
                        scaler_ = item[0].scaler
                        if scaler_ != 1 and data:
                            try:
                                row[colIndex] = data * scaler_
    ...
    def load(self, reader):
        ...
        if reader.isStartElement("CaptureObjects", True):
            while reader.isStartElement("Item", True):
                ...
                co = MTDLMSCaptureObject(ai, di)
                obj = reader.objects.findByLN(ot, ln)
                if obj is None:
                    obj = _MTObjectFactory.createObject(ot)
                    obj.logicalName = ln
