...
class MTDLMSSapAssignment(MTDLMSObject, IMTDLMSBase):
    """
    Online help:
    http://www.mildtrix.fi/mildtrix.DLMS.Objects.MTDLMSSapAssignment
    """
    def __init__(self, ln=None, sn=0):
        ...
        """
        super(MTDLMSSapAssignment, self).__init__(ObjectType.SAP_ASSIGNMENT, ln, sn)
    ...
    def addSap(self, client, id_, name):
        ...
        #Add structure size.
        data.setUInt8(2)
        _MTCommon.setData(client.settings, data, DataType.UINT16, id_)
        _MTCommon.setData(client.settings, data, DataType.OCTET_STRING, name.encode())
    ...
    def removeSap(self, client, name):
        ...
        #Add structure size.
        data.setUInt8(2)
        _MTCommon.setData(client.settings, data, DataType.UINT16, 0)
        _MTCommon.setData(client.settings, data, DataType.OCTET_STRING, name.encode())
        return client.method(self, 1, data.array(), DataType.STRUCTURE)
        ...
        if e.index == 2:
            ...
            if cnt != 0:
                for k, v in self.sapAssignmentList:
                    ...
                    data.setUInt8(2)
                    #  Count
                    _MTCommon.setData(settings, data, DataType.UINT16, k)
                    _MTCommon.setData(settings, data, DataType.OCTET_STRING, _MTCommon.getBytes(v))
            return data
            ...
            if e.value:
                for item in e.value:
                    ...
                    if isinstance(item[1], bytearray):
                        str_ = _MTCommon.changeType(settings, item[1], DataType.STRING)
                    else:
                        str_ = str(item[1])
