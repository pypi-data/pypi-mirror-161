...
class MTDLMSLimiter(MTDLMSObject, IMTDLMSBase):
    """
    Online help:
    http://www.mildtrix.fi/mildtrix.DLMS.Objects.MTDLMSLimiter
    ...
    def __init__(self, ln=None, sn=0):
        ...
        super(MTDLMSLimiter, self).__init__(ObjectType.LIMITER, ln, sn)
        ...
        self.emergencyProfile = MTDLMSEmergencyProfile()
        self.actionOverThreshold = MTDLMSActionItem()
        self.actionUnderThreshold = MTDLMSActionItem()
    ...
    def getDataType(self, index):
        ...
        if index == 1:
            ...
            ret = super(MTDLMSLimiter, self).getDataType(index)
        elif index == 4:
            ret = super(MTDLMSLimiter, self).getDataType(index)
        elif index == 5:
            ret = super(MTDLMSLimiter, self).getDataType(index)
    ...
    def getValue(self, settings, e):
        ...
        if e.index == 1:
            ...
            if self.monitoredValue is None:
                _MTCommon.setData(settings, data, DataType.INT16, int(0))
                _MTCommon.setData(settings, data, DataType.OCTET_STRING, _MTCommon.logicalNameToBytes(None))
                _MTCommon.setData(settings, data, DataType.INT8, 0)
            else:
                _MTCommon.setData(settings, data, DataType.INT16, int(self.monitoredValue.objectType))
                _MTCommon.setData(settings, data, DataType.OCTET_STRING, _MTCommon.logicalNameToBytes(self.monitoredValue.logicalName))
                _MTCommon.setData(settings, data, DataType.INT8, self.monitoredAttributeIndex)
            ret = data
        elif e.index == 3:
            ...
            data.setUInt8(DataType.STRUCTURE)
            data.setUInt8(3)
            _MTCommon.setData(settings, data, DataType.UINT16, self.emergencyProfile.id)
            _MTCommon.setData(settings, data, DataType.OCTET_STRING, self.emergencyProfile.activationTime)
            _MTCommon.setData(settings, data, DataType.UINT32, self.emergencyProfile.duration)
            ret = data
        elif e.index == 9:
            data = MTByteBuffer()
            ...
            for it in self.emergencyProfileGroupIDs:
                _MTCommon.setData(settings, data, DataType.UINT16, it)
            ret = data
        elif e.index == 10:
            ...
            data.setUInt8(DataType.STRUCTURE)
            data.setUInt8(2)
            _MTCommon.setData(settings, data, DataType.OCTET_STRING, _MTCommon.logicalNameToBytes(self.actionOverThreshold.logicalName))
            _MTCommon.setData(settings, data, DataType.UINT16, self.actionOverThreshold.scriptSelector)
            data.setUInt8(DataType.STRUCTURE)
            data.setUInt8(2)
            _MTCommon.setData(settings, data, DataType.OCTET_STRING, _MTCommon.logicalNameToBytes(self.actionUnderThreshold.logicalName))
            _MTCommon.setData(settings, data, DataType.UINT16, self.actionUnderThreshold.scriptSelector)
            ret = data
        else:
    ...
    def setValue(self, settings, e):
        if e.index == 1:
            ...
            ot = e.value[0]
            ln = _MTCommon.toLogicalName(e.value[1])
            ...
            self.emergencyProfile.activationTime = _MTCommon.changeType(settings, e.value[1], DataType.DATETIME)
            ...
            self.actionOverThreshold.logicalName = _MTCommon.toLogicalName(e.value[0][0])
            self.actionOverThreshold.scriptSelector = e.value[0][1]
            self.actionUnderThreshold.logicalName = _MTCommon.toLogicalName(e.value[1][0])
    ...
    def load(self, reader):
        ...
        if reader.isStartElement("MonitoredValue", True):
            ...
            if ot != ObjectType.NONE and ln:
                ...
                if self.monitoredValue is None:
                    self.monitoredValue = _MTObjectFactory.createObject(ot)
