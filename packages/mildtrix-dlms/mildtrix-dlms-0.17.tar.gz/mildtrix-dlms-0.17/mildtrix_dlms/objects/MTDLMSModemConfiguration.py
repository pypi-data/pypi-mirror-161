...
class MTDLMSModemConfiguration(MTDLMSObject, IMTDLMSBase):
    """
    Online help:
    http://www.mildtrix.fi/mildtrix.DLMS.Objects.MTDLMSModemConfiguration
    """
    ...
    def __init__(self, ln=None, sn=0):
        ...
        super(MTDLMSModemConfiguration, self).__init__(ObjectType.MODEM_CONFIGURATION, ln, sn)
        ...
        if e.index == 3:
            ...
            if cnt != 0:
                for it in self.initialisationStrings:
                    ...
                    data.setUInt8(3)
                    #  Count
                    _MTCommon.setData(settings, data, DataType.OCTET_STRING, _MTCommon.getBytes(it.request))
                    _MTCommon.setData(settings, data, DataType.OCTET_STRING, _MTCommon.getBytes(it.response))
                    _MTCommon.setData(settings, data, DataType.UINT16, it.delay)
            return data
        if e.index == 4:
            data = MTByteBuffer()
            data.setUInt8(DataType.ARRAY)
    ...
    def setValue(self, settings, e):
        if e.index == 1:
            ...
            if e.value:
                for it in e.value:
                    item = MTDLMSModemInitialisation()
