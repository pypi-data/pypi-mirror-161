...
class MTDLMSPppSetup(MTDLMSObject, IMTDLMSBase):
    """
    Online help:
    http://www.mildtrix.fi/mildtrix.DLMS.Objects.MTDLMSPppSetup
    """
    ...
    def __init__(self, ln=None, sn=0):
        ...
        super(MTDLMSPppSetup, self).__init__(ObjectType.PPP_SETUP, ln, sn)
    ...
    def getValue(self, settings, e):
        if e.index == 1:
            ...
            ret = _MTCommon.logicalNameToBytes(self.phyReference)
        elif e.index == 3:
            data = MTByteBuffer()
            ...
            if not self.lcpOptions:
                ...
                for it in self.lcpOptions:
                    ...
                    data.setUInt8(3)
                    _MTCommon.setData(settings, data, DataType.UINT8, it.type_.value)
                    _MTCommon.setData(settings, data, DataType.UINT8, it.length)
                    _MTCommon.setData(settings, data, _MTCommon.getDLMSDataType(it.data), it.data)
            ret = data.array()
        elif e.index == 4:
            data = MTByteBuffer()
            data.setUInt8(DataType.ARRAY)
            ...
            if self.userName:
                ...
                data.setUInt8(DataType.STRUCTURE)
                data.setUInt8(2)
                _MTCommon.setData(settings, data, DataType.OCTET_STRING, self.userName)
                _MTCommon.setData(settings, data, DataType.OCTET_STRING, self.password)
                ret = data.array()
            else:
                ret = None
        else:
    ...
    def setValue(self, settings, e):
        if e.index == 1:
            ...
            self.phyReference = _MTCommon.toLogicalName(e.value)
        elif e.index == 3:
            self.lcpOptions = []
            if e.value:
                for item in e.value:
                    it1 = MTDLMSPppSetupLcpOption()
    ...
    def load(self, reader):
        ...
        self.userName = MTByteBuffer.hexToBytes(reader.readElementContentAsString("UserName"))
        self.password = MTByteBuffer.hexToBytes(reader.readElementContentAsString("Password"))
    ...
    def save(self, writer):
        ...
        if self.ipcpOptions:
            for it in self.ipcpOptions:
                ...
                writer.writeEndElement()
        writer.writeEndElement()
        writer.writeElementString("UserName", MTByteBuffer.hex(self.userName))
        writer.writeElementString("Password", MTByteBuffer.hex(self.password))
