...
class MTDLMSTcpUdpSetup(MTDLMSObject, IMTDLMSBase):
    """
    Online help:
    http://www.mildtrix.fi/mildtrix.DLMS.Objects.MTDLMSTcpUdpSetup
    """
    def __init__(self, ln="0.0.25.0.0.255", sn=0):
        ...
        """
        super(MTDLMSTcpUdpSetup, self).__init__(ObjectType.TCP_UDP_SETUP, ln, sn)
        self.port = 4059
    ...
    def getValue(self, settings, e):
        if e.index == 1:
            ...
            ret = _MTCommon.logicalNameToBytes(self.ipReference)
        elif e.index == 4:
        ...
        if e.index == 1:
            ...
            if e.value is None:
                ...
                if isinstance(e.value, bytearray):
                    self.ipReference = _MTCommon.toLogicalName(e.value)
                else:
