...
class MTDLMSTokenGateway(MTDLMSObject, IMTDLMSBase):
    """
    Online help:
    ...
    def __init__(self, ln="0.0.19.40.0.255", sn=0):
        ...
        """
        super(MTDLMSTokenGateway, self).__init__(ObjectType.TOKEN_GATEWAY, ln, sn)
        # Descriptions.
    ...
    def getValue(self, settings, e):
        ...
        if e.index == 1:
            ...
            bb.setUInt8(2)
            _MTCommon.setData(settings, bb, DataType.ENUM, self.statusCode)
            _MTCommon.setData(settings, bb, DataType.BITSTRING, self.dataValue)
            ret = bb
        else:
    ...
    def load(self, reader):
        self.token = MTByteBuffer.hexToBytes(reader.readElementContentAsString("Token"))
    ...
    def save(self, writer):
        writer.writeElementString("Token", MTByteBuffer.hex(self.token, False))
