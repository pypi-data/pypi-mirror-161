...
class MTDLMSPushSetup(MTDLMSObject, IMTDLMSBase):
    """
    Online help:
    http://www.mildtrix.fi/mildtrix.DLMS.Objects.MTDLMSPushSetup
    """
    ...
    def __init__(self, ln="0.7.25.9.0.255", sn=0):
        ...
        super(MTDLMSPushSetup, self).__init__(ObjectType.PUSH_SETUP, ln, sn)
    ...
    def getValue(self, settings, e):
        ...
        if e.index == 1:
            ...
            _MTCommon.setObjectCount(len(self.pushObjectList), buff)
            for k, v in self.pushObjectList:
                buff.setUInt8(DataType.STRUCTURE)
                buff.setUInt8(4)
                _MTCommon.setData(settings, buff, DataType.UINT16, k.objectType)
                _MTCommon.setData(settings, buff, DataType.OCTET_STRING, _MTCommon.logicalNameToBytes(k.logicalName))
                _MTCommon.setData(settings, buff, DataType.INT8, v.attributeIndex)
                _MTCommon.setData(settings, buff, DataType.UINT16, v.dataIndex)
            ret = buff
        elif e.index == 3:
            buff.setUInt8(DataType.STRUCTURE)
            buff.setUInt8(3)
            _MTCommon.setData(settings, buff, DataType.ENUM, self.service)
            if self.destination:
                _MTCommon.setData(settings, buff, DataType.OCTET_STRING, self.destination.encode())
            else:
                _MTCommon.setData(settings, buff, DataType.OCTET_STRING, None)
            _MTCommon.setData(settings, buff, DataType.ENUM, self.message)
            ret = buff
        elif e.index == 4:
            buff.setUInt8(DataType.ARRAY)
            _MTCommon.setObjectCount(len(self.communicationWindow), buff)
            for k, v in self.communicationWindow:
                buff.setUInt8(DataType.STRUCTURE)
                buff.setUInt8(2)
                _MTCommon.setData(settings, buff, DataType.OCTET_STRING, k)
                _MTCommon.setData(settings, buff, DataType.OCTET_STRING, v)
            return buff
        elif e.index == 5:
            ...
            if e.value:
                ...
                try:
                    if self.service == ServiceType.HDLC:
                        self.destination = _MTCommon.toLogicalName(e.value[1])
                    else:
                        self.destination = e.value[1].decode()
                except Exception:
                    self.destination = MTByteBuffer.toHex(e.value[1])
                self.message = e.value[2]
        elif e.index == 4:
            self.communicationWindow = []
            if e.value:
                for it in e.value:
                    start = _MTCommon.changeType(settings, it[0], DataType.DATETIME)
                    end = _MTCommon.changeType(settings, it[1], DataType.DATETIME)
