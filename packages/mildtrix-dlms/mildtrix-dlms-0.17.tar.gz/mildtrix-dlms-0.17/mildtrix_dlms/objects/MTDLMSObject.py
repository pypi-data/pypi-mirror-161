...
from ..manufacturersettings import MTAttributeCollection, MTDLMSAttributeSettings
...
class MTDLMSObject(object):
    ...
    def __init__(self, ot, ln=None, sn=0):
        ...
        self.description = ""
        self.attributes = MTAttributeCollection()
        self.methodAttributes = MTAttributeCollection()
        if ln:
            items = ln.split('.')
            if len(items) != 6:
    ...
    def validateLogicalName(cls, value):
        _MTCommon.logicalNameToBytes(value)
    ...
    def setAccess(self, index, access):
        att = self.attributes.find(index)
        if att is None:
            att = MTDLMSAttributeSettings(index)
