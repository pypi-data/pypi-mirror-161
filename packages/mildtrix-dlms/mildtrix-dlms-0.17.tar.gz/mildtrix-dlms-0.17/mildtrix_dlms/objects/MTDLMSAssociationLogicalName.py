...
class MTDLMSAssociationLogicalName(MTDLMSObject, IMTDLMSBase):
    """
    Online help:
    http://www.mildtrix.fi/mildtrix.DLMS.Objects.MTDLMSAssociationLogicalName
    """
    ...
    def __init__(self, ln="0.0.40.0.0.255"):
        ...
        """
        MTDLMSObject.__init__(self, ObjectType.ASSOCIATION_LOGICAL_NAME, ln)
        self.objectList = MTDLMSObjectCollection(self)
        self.applicationContextName = MTApplicationContextName()
        self.xDLMSContextInfo = MTxDLMSContextType()
        self.authenticationMechanismName = MTAuthenticationMechanismName()
        self.userList = list()
        self.version = 2
    ...
    def addUser(self, client, id_, name):
        ...
        #  Add structure size.
        data.setUInt8(2)
        _MTCommon.setData(None, data, DataType.UINT8, id_)
        _MTCommon.setData(None, data, DataType.STRING, name)
    ...
    def invoke(self, settings, e):
        ...
        if e.index == 1:
            ...
            if settings.authentication == Authentication.HIGH_GMAC:
                ...
                bb = MTByteBuffer(int(e.parameters))
                bb.getUInt8()
                ic = bb.getUInt32()
            else:
                readSecret = self.secret
            serverChallenge = MTSecure.secure(settings, settings.cipher, ic, settings.stoCChallenge, readSecret)
            ...
            if accept:
                if settings.authentication == Authentication.HIGH_GMAC or settings.authentication == Authentication.HIGH_ECDSA:
                    ...
                    readSecret = self.secret
                tmp = MTSecure.secure(settings, settings.cipher, ic, settings.getCtoSChallenge(), readSecret)
    ...
    def getObjects(self, settings, e):
        ...
        if settings.index == 0:
            ...
            _MTCommon.setObjectCount(len(self.objectList), data)
        pos = 0
        for it in self.objectList:
            ...
            if not pos <= settings.index:
                data.setUInt8(DataType.STRUCTURE)
                data.setUInt8(4)
                _MTCommon.setData(settings, data, DataType.UINT16, it.objectType)
                _MTCommon.setData(settings, data, DataType.UINT8, it.version)
                _MTCommon.setData(settings, data, DataType.OCTET_STRING, _MTCommon.logicalNameToBytes(it.logicalName))
    ...
    def __getAccessRights(self, settings, item, server, data):
        ...
        if server is None:
            ...
            while pos != cnt:
                ...
                data.setUInt8(DataType.STRUCTURE)
                data.setUInt8(3)
                _MTCommon.setData(settings, data, DataType.INT8, pos + 1)
                _MTCommon.setData(settings, data, DataType.ENUM, m)
                _MTCommon.setData(settings, data, DataType.NONE, None)
                pos += 1
            data.setUInt8(DataType.ARRAY)
            ...
            while pos != cnt:
                ...
                if self.version == 0:
                    _MTCommon.setData(settings, data, DataType.BOOLEAN, m != 0)
                else:
                    _MTCommon.setData(settings, data, DataType.ENUM, m)
                pos += 1
    ...
    def getUserList(self, settings):
        ...
        if settings.index == 0:
            ...
            data.setUInt8(DataType.ARRAY)
            _MTCommon.setObjectCount(len(self.userList), data)
        pos = 0
        for k, v in self.userList:
            pos += 1
            if not pos <= settings.index:
                ...
                data.setUInt8(DataType.STRUCTURE)
                data.setUInt8(2)
                _MTCommon.setData(settings, data, DataType.UINT8, k)
                _MTCommon.setData(settings, data, DataType.STRING, v)
        return data
    ...
    def getValue(self, settings, e):
        ...
        if e.index == 1:
            ...
            data.setUInt8(DataType.STRUCTURE)
            data.setUInt8(0x7)
            _MTCommon.setData(settings, data, DataType.UINT8, self.applicationContextName.jointIsoCtt)
            _MTCommon.setData(settings, data, DataType.UINT8, self.applicationContextName.country)
            _MTCommon.setData(settings, data, DataType.UINT16, self.applicationContextName.countryName)
            _MTCommon.setData(settings, data, DataType.UINT8, self.applicationContextName.identifiedOrganization)
            _MTCommon.setData(settings, data, DataType.UINT8, self.applicationContextName.dlmsUA)
            _MTCommon.setData(settings, data, DataType.UINT8, self.applicationContextName.applicationContext)
            _MTCommon.setData(settings, data, DataType.UINT8, self.applicationContextName.contextId)
            ret = data
        elif e.index == 5:
            data = MTByteBuffer()
            data.setUInt8(DataType.STRUCTURE)
            data.setUInt8(6)
            _MTCommon.setData(settings, data, DataType.BITSTRING, MTBitString.toBitString(self.xDLMSContextInfo.conformance, 24))
            _MTCommon.setData(settings, data, DataType.UINT16, self.xDLMSContextInfo.maxReceivePduSize)
            _MTCommon.setData(settings, data, DataType.UINT16, self.xDLMSContextInfo.maxSendPduSize)
            _MTCommon.setData(settings, data, DataType.UINT8, self.xDLMSContextInfo.dlmsVersionNumber)
            _MTCommon.setData(settings, data, DataType.INT8, self.xDLMSContextInfo.qualityOfService)
            _MTCommon.setData(settings, data, DataType.OCTET_STRING, self.xDLMSContextInfo.cypheringInfo)
            ret = data
        elif e.index == 6:
            data = MTByteBuffer()
            data.setUInt8(DataType.STRUCTURE)
            data.setUInt8(0x7)
            _MTCommon.setData(settings, data, DataType.UINT8, self.authenticationMechanismName.jointIsoCtt)
            _MTCommon.setData(settings, data, DataType.UINT8, self.authenticationMechanismName.country)
            _MTCommon.setData(settings, data, DataType.UINT16, self.authenticationMechanismName.countryName)
            _MTCommon.setData(settings, data, DataType.UINT8, self.authenticationMechanismName.identifiedOrganization)
            _MTCommon.setData(settings, data, DataType.UINT8, self.authenticationMechanismName.dlmsUA)
            _MTCommon.setData(settings, data, DataType.UINT8, self.authenticationMechanismName.authenticationMechanismName)
            _MTCommon.setData(settings, data, DataType.UINT8, self.authenticationMechanismName.mechanismId)
            ret = data
        elif e.index == 7:
            ret = self.secret
        elif e.index == 8:
            ...
            ret = _MTCommon.logicalNameToBytes(self.securitySetupReference)
        elif e.index == 10:
            ...
            if self.currentUser is None:
                _MTCommon.setData(settings, data, DataType.UINT8, 0)
                _MTCommon.setData(settings, data, DataType.STRING, None)
            else:
                _MTCommon.setData(settings, data, DataType.UINT8, self.currentUser[0])
                _MTCommon.setData(settings, data, DataType.STRING, self.currentUser[1])
            ret = data
        else:
            e.error = ErrorCode.READ_WRITE_DENIED
        return ret
    ...
    def updateObjectList(cls, settings, target, value):
        ...
        if value:
            for item in value:
                type_ = item[0]
                version = item[1]
                ln = _MTCommon.toLogicalName(item[2])
                obj = settings.objects.findByLN(type_, ln)
                if obj is None:
                    obj = _MTObjectFactory.createObject(type_)
                    obj.logicalName = ln
                    obj.version = version
                if type(obj) != MTDLMSObject:
                    cls.updateAccessRights(obj, item[3])
                    target.append(obj)

    def updateApplicationContextName(self, value):
        if isinstance(value, bytearray):
            buff = MTByteBuffer(value)
    ...
    def setValue(self, settings, e):
        if e.index == 1:
            ...
            self.securitySetupReference = _MTCommon.toLogicalName(e.value)
        elif e.index == 10:
            self.userList = []
            if e.value:
                for tmp in e.value:
    ...
    def load(self, reader):
        ...
        if reader.isStartElement("XDLMSContextInfo", True):
            ...
            self.xDLMSContextInfo.cypheringInfo = MTByteBuffer.hexToBytes(reader.readElementContentAsString("CypheringInfo"))
        ...
        if str_ is None:
            ...
            self.secret = MTByteBuffer.hexToBytes(str_)
    ...
    def save(self, writer):
        ...
        if self.xDLMSContextInfo:
            ...
            writer.writeElementString("CypheringInfo", MTByteBuffer.hex(self.xDLMSContextInfo.cypheringInfo))
            writer.writeEndElement()
        if self.authenticationMechanismName:
            ...
            writer.writeEndElement()
        writer.writeElementString("Secret", MTByteBuffer.hex(self.secret))
