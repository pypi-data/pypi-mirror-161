...
class MTDLMSImageTransfer(MTDLMSObject, IMTDLMSBase):
    """
    Online help:
    http://www.mildtrix.fi/mildtrix.DLMS.Objects.MTDLMSImageTransfer
    """
    ...
    def __init__(self, ln="0.0.44.0.0.255", sn=0):
        ...
        """
        MTDLMSObject.__init__(self, ObjectType.IMAGE_TRANSFER, ln, sn)
    ...
    def getValue(self, settings, e):
        ...
        if e.index == 1:
            ...
            for it in self.imageActivateInfo:
                ...
                #  Item count.
                data.setUInt8(int(3))
                _MTCommon.setData(settings, data, DataType.UINT32, it.size)
                _MTCommon.setData(settings, data, DataType.OCTET_STRING, it.identification)
                _MTCommon.setData(settings, data, DataType.OCTET_STRING, it.signature)
            ret = data
        else:
            e.error = ErrorCode.READ_WRITE_DENIED
        return ret
    ...
    def imageTransferInitiate(self, client, imageIdentifier, forImageSize):
        ...
        data.setUInt8(DataType.STRUCTURE)
        data.setUInt8(2)
        _MTCommon.setData(None, data, DataType.OCTET_STRING, _MTCommon.getBytes(imageIdentifier))
        _MTCommon.setData(None, data, DataType.UINT32, forImageSize)
    ...
    def imageBlockTransfer(self, client, imageBlockValue, imageBlockCount):
        ...
        while pos != cnt:
            ...
            data.setUInt8(DataType.STRUCTURE)
            data.setUInt8(2)
            _MTCommon.setData(None, data, DataType.UINT32, pos)
            tmp = None
            ...
            _MTCommon.setData(None, data, DataType.OCTET_STRING, tmp)
    ...
    def load(self, reader):
        ...
        if reader.isStartElement("ImageActivateInfo", True):
            while reader.isStartElement("Item", True):
                it = MTDLMSImageActivateInfo()
                it.size = reader.readElementContentAsULong("Size")
                it.identification = MTByteBuffer.hexToBytes(reader.readElementContentAsString("Identification"))
                it.signature = MTByteBuffer.hexToBytes(reader.readElementContentAsString("Signature"))
    ...
    def save(self, writer):
        ...
        if self.imageActivateInfo:
            for it in self.imageActivateInfo:
                ...
                writer.writeElementString("Identification", MTByteBuffer.hex(it.identification, False))
                writer.writeElementString("Signature", MTByteBuffer.hex(it.signature, False))
