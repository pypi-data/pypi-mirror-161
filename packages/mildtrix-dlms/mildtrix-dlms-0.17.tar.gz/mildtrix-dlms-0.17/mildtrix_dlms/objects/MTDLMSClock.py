...
class MTDLMSClock(MTDLMSObject, IMTDLMSBase):
    """
    Online help:
    http://www.mildtrix.fi/mildtrix.DLMS.Objects.MTDLMSClock
    """
    ...
    def __init__(self, ln="0.0.1.0.0.255", sn=0):
        ...
        """
        MTDLMSObject.__init__(self, ObjectType.CLOCK, ln, sn)
        self.time = None
        self.timeZone = 0
        self.status = ClockStatus.OK
        self.deviation = 0
        self.begin = MTDateTime()
        self.end = MTDateTime()
        self.enabled = False
    ...
    def getUIDataType(self, index):
        ...
        return super(MTDLMSClock, self).getUIDataType(index)
    ...
    def presetAdjustingTime(self, client, presetTime, validityIntervalStart, validityIntervalEnd):
        buff = MTByteBuffer(44)
        buff.setUInt8(DataType.STRUCTURE)
        buff.setUInt8(3)
        _MTCommon.setData(None, buff, DataType.OCTET_STRING, presetTime)
        _MTCommon.setData(None, buff, DataType.OCTET_STRING, validityIntervalStart)
        _MTCommon.setData(None, buff, DataType.OCTET_STRING, validityIntervalEnd)
    ...
    def setValue(self, settings, e):
        ...
        if e.index == 1:
            ...
            if e.value is None:
                ...
                self.begin = _MTCommon.changeType(settings, e.value, DataType.DATETIME)
            else:
                self.begin = e.value
        elif e.index == 6:
            if e.value is None:
                ...
                self.end = _MTCommon.changeType(settings, e.value, DataType.DATETIME)
            else:
                self.end = e.value
        elif e.index == 7:
            if e.value is None:
