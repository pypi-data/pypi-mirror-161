...
class MTDLMSExtendedRegister(MTDLMSObject, IMTDLMSBase):
    """
    Online help:
    http://www.mildtrix.fi/mildtrix.DLMS.Objects.MTDLMSExtendedRegister
    """
    ...
    def __init__(self, ln=None, sn=0):
        ...
        """
        MTDLMSObject.__init__(self, ObjectType.EXTENDED_REGISTER, ln, sn)
        self.value = None
        self.scaler = 1
        self.unit = Unit.NONE
        self.status = None
    ...
    def getUIDataType(self, index):
        ...
        return super(MTDLMSExtendedRegister, self).getUIDataType(index)
    ...
    def getDataType(self, index):
        ...
        if index == 1:
            ...
            ret = super(MTDLMSExtendedRegister, self).getDataType(index)
        elif index == 3:
            ret = DataType.ARRAY
        elif index == 4:
            ret = super(MTDLMSExtendedRegister, self).getDataType(index)
    ...
    def getValue(self, settings, e):
        ...
        if e.index == 3:
            ...
            data.setUInt8(2)
            _MTCommon.setData(settings, data, DataType.INT8, math.floor(math.log(self.scaler, 10)))
            _MTCommon.setData(settings, data, DataType.ENUM, int(self.unit))
            return data.array()
        if e.index == 4:
            return self.status
        if e.index == 5:
