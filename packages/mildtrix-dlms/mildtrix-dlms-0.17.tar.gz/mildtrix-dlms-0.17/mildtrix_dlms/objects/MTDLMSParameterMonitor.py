...
class MTDLMSParameterMonitor(MTDLMSObject, IMTDLMSBase):
    """
    Online help:
    http://www.mildtrix.fi/mildtrix.DLMS.Objects.MTDLMSParameterMonitor
    """
    def __init__(self, ln="0.0.16.2.0.255", sn=0):
        ...
        super(MTDLMSParameterMonitor, self).__init__(ObjectType.PARAMETER_MONITOR, ln, sn)
        self.parameters = list()
        self.captureTime = None
        self.changedParameter = MTDLMSTarget()
    ...
    def insert(self, client, entry):
        ...
        bb.setUInt8(3)
        _MTCommon.setData(None, bb, DataType.UINT16, entry.target.objectType)
        _MTCommon.setData(None, bb, DataType.OCTET_STRING, _MTCommon.logicalNameToBytes(entry.target.logicalName))
        _MTCommon.setData(None, bb, DataType.INT8, entry.attributeIndex)
        ...
        if e.index != 1 and e.index != 2:
            ...
            if e.index == 1:
                ...
                if it.target is None:
                    it.target = _MTObjectFactory.createObject(type_)
    ...
    def getValue(self, settings, e):
        ...
        if e.index == 1:
            ...
            if self.changedParameter is None:
                ...
                _MTCommon.setData(settings, data, DataType.OCTET_STRING, [0, 0, 0, 0, 0, 0])
                _MTCommon.setData(settings, data, DataType.INT8, 1)
                _MTCommon.setData(settings, data, DataType.NONE, None)
            else:
                _MTCommon.setData(settings, data, DataType.UINT16, self.changedParameter.target.objectType)
                _MTCommon.setData(settings, data, DataType.OCTET_STRING, _MTCommon.logicalNameToBytes(self.changedParameter.target.logicalName))
                _MTCommon.setData(settings, data, DataType.INT8, self.changedParameter.attributeIndex)
                _MTCommon.setData(settings, data, _MTCommon.getDLMSDataType(self.changedParameter.value), self.changedParameter.value)
            ret = data
        elif e.index == 3:
            ...
            if self.parameters is None:
                ...
                for it in self.parameters:
                    data.setUInt8(DataType.STRUCTURE)
                    data.setUInt8(3)
                    _MTCommon.setData(settings, data, DataType.UINT16, it.target.objectType)
                    _MTCommon.setData(settings, data, DataType.OCTET_STRING, _MTCommon.logicalNameToBytes(it.target.logicalName))
                    _MTCommon.setData(settings, data, DataType.INT8, it.attributeIndex)
            ret = data
        else:
            e.error = ErrorCode.READ_WRITE_DENIED
        return ret
    ...
    def setValue(self, settings, e):
        ...
        if e.index == 1:
            ...
            if self.changedParameter.target is None:
                self.changedParameter.target = _MTObjectFactory.createObject(type_)
            ...
            if e.value is None:
                ...
                if isinstance(e.value, bytearray):
                    self.captureTime = _MTCommon.changeType(settings, e.value, DataType.DATETIME)
                else:
            ...
            if e.value:
                for i in e.value:
                    ...
                    ln = _MTCommon.toLogicalName(i[1])
                    obj.target = settings.objects.findByLN(type_, ln)
                    if obj.target is None:
                        obj.target = _MTObjectFactory.createObject(type_)
    ...
    def load(self, reader):
        ...
        if reader.isStartElement("ChangedParameter", True):
            ...
            if self.changedParameter.target is None:
                self.changedParameter.target = _MTObjectFactory.createObject(ot)
                self.changedParameter.target.logicalName = ln
        ...
        if reader.isStartElement("Parameters", True):
            while reader.isStartElement("Item", True):
                ...
                if obj.target is None:
                    obj.target = _MTObjectFactory.createObject(ot)
                    obj.target.logicalName = ln
