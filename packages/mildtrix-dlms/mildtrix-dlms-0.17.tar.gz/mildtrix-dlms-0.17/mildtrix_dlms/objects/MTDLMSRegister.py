...
class MTDLMSRegister(MTDLMSObject, IMTDLMSBase):
    """
    Online help:
    http://www.mildtrix.fi/mildtrix.DLMS.Objects.MTDLMSRegister
    """
    def __init__(self, ln=None, sn=0):
        ...
        super(MTDLMSRegister, self).__init__(ObjectType.REGISTER, ln, sn)
        self.value = None
        self.scaler = 1
        self.unit = Unit.NONE
    ...
    def isRead(self, index):
        if index == 3:
            return self.unit != 0
        #pylint: disable=super-with-arguments
        return super(MTDLMSRegister, self).isRead(index)
    ...
    def getDataType(self, index):
        ...
        if index == 2:
            return super(MTDLMSRegister, self).getDataType(index)
        if index == 3:
