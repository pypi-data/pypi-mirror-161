...
class MTDLMSIp6Setup(MTDLMSObject, IMTDLMSBase):
    """
    Online help:
    http://www.mildtrix.fi/mildtrix.DLMS.Objects.MTDLMSIp6Setup
    """
    def __init__(self, ln="0.0.25.7.0.255", sn=0):
        ...
        super(MTDLMSIp6Setup, self).__init__(ObjectType.IP6_SETUP, ln, sn)
    ...
    def getValue(self, settings, e):
        ...
        if e.index == 1:
            ...
            ret = _MTCommon.logicalNameToBytes(self.dataLinkLayerReference)
        elif e.index == 3:
            ...
            if not self.unicastIPAddress:
                ...
                _MTCommon.setObjectCount(len(self.unicastIPAddress), data)
                for it in self.unicastIPAddress:
                    _MTCommon.setData(settings, data, DataType.OCTET_STRING, socket.inet_pton(socket.AF_INET6, it))
            ret = data
        elif e.index == 5:
            data = MTByteBuffer()
            data.setUInt8(DataType.ARRAY)
            if not self.multicastIPAddress:
                ...
                _MTCommon.setObjectCount(len(self.multicastIPAddress), data)
            ...
            if not self.gatewayIPAddress:
                ...
                _MTCommon.setObjectCount(len(self.gatewayIPAddress), data)
            ...
            if self.neighborDiscoverySetup is None:
                ...
                _MTCommon.setObjectCount(len(self.neighborDiscoverySetup), data)
                for it in self.neighborDiscoverySetup:
                    data.setUInt8(DataType.STRUCTURE)
                    data.setUInt8(3)
                    _MTCommon.setData(settings, data, DataType.UINT8, it.maxRetry)
                    _MTCommon.setData(settings, data, DataType.UINT16, it.retryWaitTime)
                    _MTCommon.setData(settings, data, DataType.UINT32, it.sendPeriod)
            ret = data
        else:
    ...
    def setValue(self, settings, e):
        if e.index == 1:
            ...
            if isinstance(e.value, str):
                ...
                self.dataLinkLayerReference = _MTCommon.toLogicalName(e.value)
        elif e.index == 3:
