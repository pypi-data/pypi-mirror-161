...
class MTDLMSAutoAnswer(MTDLMSObject, IMTDLMSBase):
    """
    Online help:
    http://www.mildtrix.fi/mildtrix.DLMS.Objects.MTDLMSAutoAnswer
    """
    ...
    def __init__(self, ln="0.0.2.2.0.255", sn=0):
        ...
        """
        MTDLMSObject.__init__(self, ObjectType.AUTO_ANSWER, ln, sn)
    ...
    def getValue(self, settings, e):
        ...
        if e.index == 1:
            ...
            #  Add count
            _MTCommon.setObjectCount(cnt, buff)
            if cnt != 0:
                for it in self.listeningWindow:
                    ...
                    buff.setUInt8(2)
                    #  Start time
                    _MTCommon.setData(settings, buff, DataType.OCTET_STRING, it[0])
                    #  End time
                    _MTCommon.setData(settings, buff, DataType.OCTET_STRING, it[1])
            ret = buff.array()
        elif e.index == 4:
            ret = self.status
        elif e.index == 5:
            ...
            buff = MTByteBuffer()
            buff.setUInt8(DataType.STRUCTURE)
            _MTCommon.setObjectCount(2, buff)
            _MTCommon.setData(settings, buff, DataType.UINT8, self.numberOfRingsInListeningWindow)
            _MTCommon.setData(settings, buff, DataType.UINT8, self.numberOfRingsOutListeningWindow)
            ret = buff.array()
        else:
