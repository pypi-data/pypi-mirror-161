...
class MTDLMSScriptTable(MTDLMSObject, IMTDLMSBase):
    """
    Online help:
    http://www.mildtrix.fi/mildtrix.DLMS.Objects.MTDLMSScriptTable
    """
    def __init__(self, ln=None, sn=0):
        ...
        """
        super(MTDLMSScriptTable, self).__init__(ObjectType.SCRIPT_TABLE, ln, sn)
        self.scripts = list()
    ...
    def getValue(self, settings, e):
        ...
        if e.index == 2:
            ...
            for it in self.scripts:
                ...
                data.setUInt8(2)
                #  Script_identifier:
                _MTCommon.setData(settings, data, DataType.UINT16, it.id)
                data.setUInt8(DataType.ARRAY)
                #  Count
                ...
                for a in it.actions:
                    ...
                    data.setUInt8(5)
                    #  service_id
                    _MTCommon.setData(settings, data, DataType.ENUM, a.type_)
                    if not a.target:
                        #  class_id
                        _MTCommon.setData(settings, data, DataType.UINT16, a.objectType)
                        #  logical_name
                        _MTCommon.setData(settings, data, DataType.OCTET_STRING, _MTCommon.logicalNameToBytes(a.logicalName))
                    else:
                        #  class_id
                        _MTCommon.setData(settings, data, DataType.UINT16, a.target.objectType)
                        _MTCommon.setData(settings, data, DataType.OCTET_STRING, _MTCommon.logicalNameToBytes(a.target.logicalName))
                    _MTCommon.setData(settings, data, DataType.INT8, a.index)
                    _MTCommon.setData(settings, data, a.parameterDataType, a.parameter)
            return data
            ...
            if isinstance(e.value, list) and e.value:
                if isinstance(e.value[0], list):
                    for item in e.value:
                        ...
                        for arr in item[1]:
                            it = MTDLMSScriptAction()
                            it.type_ = arr[0]
                            ot = arr[1]
                            ln = _MTCommon.toLogicalName(arr[2])
                            t = settings.objects.findByLN(ot, ln)
                            if t is None:
                                t = _MTObjectFactory.createObject(ot)
                                t.logicalName = ln
                            it.target = t
                            it.index = arr[3]
                            it.parameter = arr[4]
                            it.parameterDataType = _MTCommon.getDLMSDataType(it.parameter)
    ...
    def execute(self, client, script):
        if isinstance(script, MTDLMSScript):
            script = script.id
