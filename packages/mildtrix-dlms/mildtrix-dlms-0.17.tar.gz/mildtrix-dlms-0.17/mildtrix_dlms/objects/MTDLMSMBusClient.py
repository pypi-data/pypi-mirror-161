...
class MTDLMSMBusClient(MTDLMSObject, IMTDLMSBase):
    """
    Online help:
    http://www.mildtrix.fi/mildtrix.DLMS.Objects.MTDLMSMBusClient
    """
    ...
    def __init__(self, ln=None, sn=0):
        ...
        super(MTDLMSMBusClient, self).__init__(ObjectType.MBUS_CLIENT, ln, sn)
    ...
    def sendData(self, client, data):
        ...
        _MTCommon.setObjectCount(len(data), bb)
        for it in data:
            bb.setUInt8(DataType.STRUCTURE)
            bb.setUInt8(3)
            bb.setUInt8(DataType.OCTET_STRING)
            _MTCommon.setObjectCount(it.DataInformation.Length, bb)
            bb.set(it.DataInformation)
            bb.setUInt8(DataType.OCTET_STRING)
            _MTCommon.setObjectCount(it.ValueInformation.Length, bb)
            bb.set(it.ValueInformation)
            _MTCommon.setData(None, bb, _MTCommon.getDLMSDataType(it.Data), it.Data)
    ...
    def setEncryptionKey(self, client, encryptionKey):
        ...
        if not encryptionKey:
            ...
            _MTCommon.setObjectCount(encryptionKey.Length, bb)
    ...
    def getValue(self, settings, e):
        ...
        if e.index == 1:
            ...
            ret = _MTCommon.logicalNameToBytes(self.mBusPortReference)
        elif e.index == 3:
            buff = MTByteBuffer()
            buff.setUInt8(DataType.ARRAY)
            _MTCommon.setObjectCount(len(self.captureDefinition), buff)
            for k, v in self.captureDefinition:
                buff.setUInt8(DataType.STRUCTURE)
                buff.setUInt8(2)
                _MTCommon.setData(settings, buff, DataType.UINT8, k)
                if not v:
                    _MTCommon.setData(settings, buff, DataType.OCTET_STRING, None)
                else:
                    _MTCommon.setData(settings, buff, DataType.OCTET_STRING, v.encode())
            ret = buff
        elif e.index == 4:
    ...
    def setValue(self, settings, e):
        if e.index == 1:
            ...
            self.mBusPortReference = _MTCommon.toLogicalName(e.value)
        elif e.index == 3:
            self.captureDefinition = []
            if e.value:
                for it in e.value:
                    self.captureDefinition.append((_MTCommon.changeType(settings, it[0], DataType.OCTET_STRING), _MTCommon.changeType(settings, it[1], DataType.OCTET_STRING)))
        elif e.index == 4:
