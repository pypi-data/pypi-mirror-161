...
class MTDLMSAutoConnect(MTDLMSObject, IMTDLMSBase):
    """
    Online help:
    http://www.mildtrix.fi/mildtrix.DLMS.Objects.MTDLMSAutoConnect
    """
    ...
    def __init__(self, ln="0.0.2.1.0.255", sn=0):
        ...
        """
        MTDLMSObject.__init__(self, ObjectType.AUTO_CONNECT, ln, sn)
    ...
    def getValue(self, settings, e):
        ...
        if e.index == 1:
            ...
            #  Add count
            _MTCommon.setObjectCount(len(self.callingWindow), data)
            for k, v in self.callingWindow:
                ...
                #  End time
                _MTCommon.setData(settings, data, DataType.OCTET_STRING, v)
            ret = data.array()
        elif e.index == 6:
            data = MTByteBuffer()
            data.setUInt8(DataType.ARRAY)
            if not self.destinations:
                ...
                #  Add count
                _MTCommon.setObjectCount(len(self.destinations), data)
                #  destination
                for it in self.destinations:
                    _MTCommon.setData(settings, data, DataType.OCTET_STRING, _MTCommon.getBytes(it))
            ret = data.array()
        else:
            e.error = ErrorCode.READ_WRITE_DENIED
        return ret
    ...
    def setValue(self, settings, e):
        if e.index == 1:
            ...
            if e.value:
                for item in e.value:
                    start = _MTCommon.changeType(settings, item[0], DataType.DATETIME)
                    end = _MTCommon.changeType(settings, item[1], DataType.DATETIME)
                    self.callingWindow.append((start, end))
        elif e.index == 6:
            self.destinations = []
            if e.value:
                for item in e.value:
                    it = _MTCommon.changeType(settings, item, DataType.STRING)
