...
class MTXmlReader:
        ...
        if name == self.name():
            ...
            if tp in (DataType.ARRAY, DataType.STRUCTURE):
                ...
                if uiType == DataType.OCTET_STRING:
                    ret = MTByteBuffer.hexToBytes(str_)
                elif uiType == DataType.DATETIME:
                    ret = MTDateTime(str_, "%m/%d/%Y %H:%M:%S")
                elif uiType == DataType.DATE:
                    ret = MTDate(str_, "%m/%d/%Y")
                elif uiType == DataType.TIME:
                    ret = MTTime(str_, "%H:%M:%S")
                elif uiType == DataType.NONE:
                    ret = str_
                else:
                    ret = MTDLMSConverter.changeType(str_, uiType)
                self.getNext()
            return ret
        return defaultValue
    ...
    def readElementContentAsDateTime(self, name):
        if name == self.currentElement.tag:
            ...
            self.getNext()
            if str_:
                return MTDateTime(str_, "%m/%d/%Y %H:%M:%S")
        return None
