...
class MTDLMSSpecialDaysTable(MTDLMSObject, IMTDLMSBase):
    """
    Online help:
    http://www.mildtrix.fi/mildtrix.DLMS.Objects.MTDLMSSpecialDaysTable
    """
    ...
    def __init__(self, ln="0.0.11.0.0.255", sn=0):
        ...
        """
        super(MTDLMSSpecialDaysTable, self).__init__(ObjectType.SPECIAL_DAYS_TABLE, ln, sn)
        # Special day entries.
        self.entries = list()
    ...
    def insert(self, client, entry):
        ...
        bb.setUInt8(3)
        _MTCommon.setData(None, bb, DataType.UINT16, entry.index)
        _MTCommon.setData(None, bb, DataType.OCTET_STRING, entry.date)
        _MTCommon.setData(None, bb, DataType.UINT8, entry.dayId)
        ...
        if e.index == 2:
            ...
            if not self.entries:
                ...
                for it in self.entries:
                    ...
                    data.setUInt8(3)
                    #  Count
                    _MTCommon.setData(settings, data, DataType.UINT16, it.index)
                    _MTCommon.setData(settings, data, DataType.OCTET_STRING, it.date)
        ...
        if e.index == 1:
            ...
            if e.value:
                for item in e.value:
                    it = MTDLMSSpecialDay()
                    it.index = item[0]
                    it.date = _MTCommon.changeType(settings, item[1], DataType.DATE)
                    it.dayId = item[2]
