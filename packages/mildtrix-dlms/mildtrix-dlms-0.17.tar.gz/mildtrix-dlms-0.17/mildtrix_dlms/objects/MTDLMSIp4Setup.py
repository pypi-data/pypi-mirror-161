...
class MTDLMSIp4Setup(MTDLMSObject, IMTDLMSBase):
    """
    Online help:
    http://www.mildtrix.fi/mildtrix.DLMS.Objects.MTDLMSIp4Setup
    """
    def __init__(self, ln="0.0.25.1.0.255", sn=0):
        ...
        super(MTDLMSIp4Setup, self).__init__(ObjectType.IP4_SETUP, ln, sn)
        ...
        if e.index == 1:
            ...
            if not self.multicastIPAddress:
                ...
                for it in self.multicastIPAddress:
                    _MTCommon.setData(settings, data, DataType.UINT16, struct.unpack("!I", socket.inet_aton(it))[0])
            ret = data
        elif e.index == 5:
            data = MTByteBuffer()
            data.setUInt8(DataType.ARRAY)
            if not self.ipOptions:
                ...
                _MTCommon.setObjectCount(len(self.ipOptions), data)
                for it in self.ipOptions:
                    data.setUInt8(DataType.STRUCTURE)
                    data.setUInt8(3)
                    _MTCommon.setData(settings, data, DataType.UINT8, it.type_)
                    _MTCommon.setData(settings, data, DataType.UINT8, it.length)
                    _MTCommon.setData(settings, data, DataType.OCTET_STRING, it.data)
            ret = data.array()
        elif e.index == 6:
    ...
    def load(self, reader):
        ...
        if reader.isStartElement("IPOptions", True):
            while reader.isStartElement("IPOption", True):
                it = MTDLMSIp4SetupIpOption()
                it.type_ = reader.readElementContentAsInt("Type")
                it.length = reader.readElementContentAsInt("Length")
                it.data = MTByteBuffer.hexToBytes(reader.readElementContentAsString("Data"))
    ...
    def save(self, writer):
        ...
        if self.ipOptions:
            for it in self.ipOptions:
                ...
                writer.writeElementString("Data", MTByteBuffer.hex(it.data))
                writer.writeEndElement()
        writer.writeEndElement()
