...
class MTDLMSCharge(MTDLMSObject, IMTDLMSBase):
    """
    Online help:
    http://www.mildtrix.fi/mildtrix.DLMS.Objects.MTDLMSCharge
    """
    ...
    def __init__(self, ln="0.0.19.20.0.255", sn=0):
        ...
        """
        MTDLMSObject.__init__(self, ObjectType.CHARGE, ln, sn)
        ...
        self.proportion = 0
        self.unitChargeActive = MTUnitCharge()
        self.unitChargePassive = MTUnitCharge()
    ...
    def getUIDataType(self, index):
        ...
        return super(MTDLMSCharge, self).getUIDataType(index)
    ...
    def getUnitCharge(cls, settings, charge):
        ...
        bb.setUInt8(DataType.STRUCTURE)
        bb.setUInt8(2)
        _MTCommon.setData(settings, bb, DataType.INT8, charge.chargePerUnitScaling.commodityScale)
        _MTCommon.setData(settings, bb, DataType.INT8, charge.chargePerUnitScaling.priceScale)
        bb.setUInt8(DataType.STRUCTURE)
        bb.setUInt8(3)
        if charge.commodity.target is None:
            _MTCommon.setData(settings, bb, DataType.UINT16, 0)
            ...
            bb.setUInt8(0)
            bb.setUInt8(0)
            _MTCommon.setData(settings, bb, DataType.INT8, 0)
        else:
            _MTCommon.setData(settings, bb, DataType.UINT16, charge.commodity.target.objectType)
            _MTCommon.setData(settings, bb, DataType.OCTET_STRING, _MTCommon.logicalNameToBytes(charge.commodity.target.logicalName))
            _MTCommon.setData(settings, bb, DataType.INT8, charge.commodity.index)
        bb.setUInt8(DataType.ARRAY)
        if charge.chargeTables is None:
            bb.setUInt8(0)
        else:
            _MTCommon.setObjectCount(len(charge.chargeTables), bb)
            for it in charge.chargeTables:
                bb.setUInt8(DataType.STRUCTURE)
                bb.setUInt8(2)
                _MTCommon.setData(settings, bb, DataType.OCTET_STRING, it.index)
                _MTCommon.setData(settings, bb, DataType.INT16, it.chargePerUnit)
        return bb.array()
    ...
    def getValue(self, settings, e):
        ...
        if e.index == 1:
            ...
            ret = MTBitString.toBitString(self.chargeConfiguration, 2)
        elif e.index == 10:
    ...
    def setUnitCharge(cls, settings, charge, value):
        ...
        tmp2 = tmp[1]
        ot = tmp2[0]
        ln = _MTCommon.toLogicalName(tmp2[1])
    ...
    def setValue(self, settings, e):
        if e.index == 1:
            ...
            if isinstance(e.value, bytearray):
                self.unitChargeActivationTime = _MTCommon.changeType(settings, e.value, DataType.DATETIME)
            else:
            ...
            if isinstance(e.value, bytearray):
                self.lastCollectionTime = _MTCommon.changeType(settings, e.value, DataType.DATETIME)
            else:
