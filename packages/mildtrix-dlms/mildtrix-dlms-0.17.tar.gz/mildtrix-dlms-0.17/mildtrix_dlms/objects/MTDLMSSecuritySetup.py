...
class MTDLMSSecuritySetup(MTDLMSObject, IMTDLMSBase):
    """
    Online help:
    http://www.mildtrix.fi/mildtrix.DLMS.Objects.MTDLMSSecuritySetup
    """
    def __init__(self, ln="0.0.43.0.0.255", sn=0):
        ...
        """
        super(MTDLMSSecuritySetup, self).__init__(ObjectType.SECURITY_SETUP, ln, sn)
        self.version = 1
    ...
    def globalKeyTransfer(self, client, kek, list_):
        ...
        from ..secure.MTDLMSSecureClient import MTDLMSSecureClient
        if not list_:
        ...
        for it in list_:
            ...
            bb.setUInt8(2)
            _MTCommon.setData(None, bb, DataType.ENUM, it.getKey())
            tmp = MTDLMSSecureClient.encrypt(kek, it)
            _MTCommon.setData(None, bb, DataType.OCTET_STRING, tmp)
    ...
    def __keyAgreement(self, client, list_):
        ...
        for it in list_:
            ...
            bb.setUInt8(2)
            _MTCommon.setData(None, bb, DataType.ENUM, it.getKey())
            _MTCommon.setData(None, bb, DataType.OCTET_STRING, it)
    ...
    def keyAgreement(self, client, type_):
        ...
        # sign = MTASymmetric.getEphemeralPublicKeySignature(type_ ,
        # client.ciphering.ephemeralKeyPair.public,
        # client.ciphering.signingKeyPair.private)
        #  bb.set(sign)
        # print("Data: " + MTByteBuffer.hex(data))
        #print("Sign: " + MTByteBuffer.hex(sign))
        list_ = list()
    ...
    def exportCertificateByEntity(self, client, entity, type_, systemTitle):
        ...
        bb.setUInt8(DataType.ENUM)
        bb.setUInt8(type_)
        _MTCommon.setData(None, bb, DataType.OCTET_STRING, systemTitle)
    ...
    def exportCertificateBySerial(self, client, serialNumber, issuer):
        ...
        bb.setUInt8(DataType.STRUCTURE)
        bb.setUInt8(2)
        _MTCommon.setData(None, bb, DataType.OCTET_STRING, serialNumber.encode())
        _MTCommon.setData(None, bb, DataType.OCTET_STRING, issuer.encode())
        return client.method(self, 7, bb.array(), DataType.STRUCTURE)
        ...
        if e.index == 1:
            ...
            for tmp in e.parameters:
                ...
                if type_ == GlobalKeyType.UNICAST_ENCRYPTION:
                    MTDLMSSecureClient.decrypt(settings.kek, item[1])
    ...
    def applyKeys(cls, settings, e):
        ...
        for tmp in e.parameters:
            item = tmp
            type_ = GlobalKeyType(item[0])
            data = MTDLMSSecureClient.decrypt(settings.kek, item[1])
            if type_ == GlobalKeyType.UNICAST_ENCRYPTION:
    ...
    def getCertificatesByteArray(cls, settings):
        ...
        if settings.cipher.certificates:
            _MTCommon.setObjectCount(len(settings.cipher.certificates), bb)
            for it in settings.cipher.certificates:
                bb.setUInt8(DataType.STRUCTURE)
                _MTCommon.setObjectCount(6, bb)
                bb.setUInt8(DataType.ENUM)
                bb.setUInt8(CertificateEntity.SERVER)
                bb.setUInt8(DataType.ENUM)
                bb.setUInt8(CertificateType.DIGITAL_SIGNATURE)
                _MTCommon.addString(it.serialNumber, bb)
                _MTCommon.addString(it.issuer, bb)
                _MTCommon.addString(it.subject, bb)
                _MTCommon.addString("", bb)
        else:
            bb.setUInt8(0)
        return bb.array()
    ...
    def getEphemeralPublicKeyData(cls, keyId, ephemeralKey):
        ...
        #tmp =
        #(MTAsn1Converter.fromByteArray(ephemeralKey.getEncoded())).get(1)
        #epk = MTByteBuffer(tmp.value)
    ...
    def load(self, reader):
        ...
        if str_ is None:
            ...
            self.clientSystemTitle = MTByteBuffer.hexToBytes(str_)
        str_ = reader.readElementContentAsString("ServerSystemTitle")
        if str_ is None:
            self.serverSystemTitle = None
        else:
            self.serverSystemTitle = MTByteBuffer.hexToBytes(str_)
        self.certificates = []
        if reader.isStartElement("Certificates", True):
            while reader.isStartElement("Item", True):
                it = MTDLMSCertificateInfo()
    ...
    def save(self, writer):
        ...
        writer.writeElementString("ClientSystemTitle", MTByteBuffer.hex(self.clientSystemTitle))
        writer.writeElementString("ServerSystemTitle", MTByteBuffer.hex(self.serverSystemTitle))
