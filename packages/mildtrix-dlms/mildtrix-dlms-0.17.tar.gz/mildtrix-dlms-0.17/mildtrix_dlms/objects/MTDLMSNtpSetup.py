...
class MTDLMSNtpSetup(MTDLMSObject, IMTDLMSBase):
    """
    Online help:
    http://www.mildtrix.fi/mildtrix.DLMS.Objects.MTDLMSNtpSetup
    """
    ...
    def __init__(self, ln="0.0.25.10.0.255", sn=0):
        ...
        """
        MTDLMSObject.__init__(self, ObjectType.NTP_SETUP, ln, sn)
        self.value = None
    ...
    def addAuthenticationKey(self, client, id_, key):
        ...
        _MTCommon.setObjectCount(key.length, bb)
        bb.set(key)
    ...
    def getUIDataType(self, index):
        ...
        return super(MTDLMSNtpSetup, self).getUIDataType(index)
    ...
    def getValue(self, settings, e):
        if e.index == 1:
            ...
            #Add count
            _MTCommon.setObjectCount(len(self.keys), bb)
            for it in self.keys:
                bb.setUInt8(DataType.STRUCTURE)
                #Count
                bb.setUInt8(2)
                ...
                bb.setUInt32(it.key)
                bb.setUInt8(DataType.OCTET_STRING)
                _MTCommon.setObjectCount(len(it.value), bb)
                bb.set(it.value)
            ret = bb.array()
        elif e.index == 7:
            ret = self.clientKey
    ...
    def load(self, reader):
        ...
        if reader.isStartElement("Keys", True):
            while reader.isStartElement("Item", True):
                ...
                key = MTByteBuffer.hexToBytes(reader.readElementContentAsString("Key"))
                self.keys[id_] = key
        self.clientKey = MTByteBuffer.hexToBytes(reader.readElementContentAsString("ServerAddress", None))
    ...
    def save(self, writer):
        ...
        for it in self.keys:
            ...
            writer.writeElementString("Key", MTByteBuffer.toHex(it.value, False))
            writer.writeEndElement()
        writer.writeEndElement()
        #Keys
        writer.writeElementString("ClientKey", MTByteBuffer.toHex(self.clientKey, False))
