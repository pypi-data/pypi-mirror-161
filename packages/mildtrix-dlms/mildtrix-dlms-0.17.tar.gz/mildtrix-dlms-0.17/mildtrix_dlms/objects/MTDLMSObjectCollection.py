...
class MTDLMSObjectCollection(list):
    """
    ...
    def __init__(self, forParent=None):
        ...
        super(MTDLMSObjectCollection, self).__init__()
        self.parent = forParent

    def append(self, item):
        if not isinstance(item, MTDLMSObject):
            raise TypeError('item is not of type MTDLMSObject')
        #pylint: disable=super-with-arguments
        super(MTDLMSObjectCollection, self).append(item)
        item.parent = self

    def extend(self, items):
        #pylint: disable=super-with-arguments
        if not isinstance(items, MTDLMSObjectCollection):
            raise TypeError('items is not of type MTDLMSObjectCollection')
        for it in items:
            super(MTDLMSObjectCollection, self).append(it)
            it.parent = self

    def getObjects(self, type_):
        if isinstance(type_, (int, ObjectType)):
            type_ = [type_]
        items = MTDLMSObjectCollection()
        for it in self:
    ...
    def load(cls, file_):
        ...
        target = None
        type_ = None
        reader = MTXmlReader(file_, MTDLMSObjectCollection())
        reader.getNext()
        while not reader.isEOF():
            if reader.isStartElement():
                ...
                if "Objects".lower() == target.lower():
                    reader.getNext()
                elif target.find("MTDLMS") == 0:
                    try:
                        type_ = MTDLMSConverter.valueOfObjectType(target[6:])
                    except Exception:
    ...
    def save(self, name, settings=None):
        ...
        for it in self:
            node = ET.SubElement(objects, "MTDLMS" + MTDLMSConverter.objectTypeToString(it.objectType))
            if it.shortName != 0:
