...
class MTDLMSAssociationShortName(MTDLMSObject, IMTDLMSBase):
    """
    Online help:
    http://www.mildtrix.fi/mildtrix.DLMS.Objects.MTDLMSAssociationShortName
    """
    ...
    def __init__(self, ln="0.0.40.0.0.255", sn=0xFA00):
        ...
        """
        MTDLMSObject.__init__(self, ObjectType.ASSOCIATION_SHORT_NAME, ln, sn)
        ...
        if e.index == 8:
            ...
            if settings.authentication == Authentication.HIGH_ECDSA:
                ...
                if settings.authentication == Authentication.HIGH_GMAC:
                    ...
                    bb = MTByteBuffer(e.parameters)
                    bb.getUInt8()
                    ic = bb.getUInt32()
                else:
                    readSecret = self.secret
                serverChallenge = MTSecure.secure(settings, settings.cipher, ic, settings.getStoCChallenge(), readSecret)
            ...
            if accept:
                ...
                return MTSecure.secure(settings, settings.cipher, ic, settings.getCtoSChallenge(), readSecret)
        else:
            settings.setConnected(settings.connected & ~ConnectionState.DLMS)
    ...
    def __getAccessRights_(cls, settings, item, data):
        ...
        data.setUInt8(3)
        _MTCommon.setData(settings, data, DataType.UINT16, item.shortName)
        data.setUInt8(DataType.ARRAY)
        data.setUInt8(len(item.attributes))
        for att in item.attributes:
            data.setUInt8(DataType.STRUCTURE)
            data.setUInt8(3)
            _MTCommon.setData(settings, data, DataType.INT8, att.index)
            _MTCommon.setData(settings, data, DataType.ENUM, att.access.value)
        ...
        for it in item.methodAttributes:
            data.setUInt8(DataType.STRUCTURE)
            data.setUInt8(2)
            _MTCommon.setData(settings, data, DataType.INT8, it.index)
            _MTCommon.setData(settings, data, DataType.ENUM, it.getMethodAccess().value)
        ...
        if cnt != 0:
            for it in self.objectList:
                ...
                if not pos <= settings.index:
                    ...
                    bb.setUInt8(4)
                    _MTCommon.setData(settings, bb, DataType.INT16, it.shortName)
                    _MTCommon.setData(settings, bb, DataType.UINT16, it.objectType)
                    _MTCommon.setData(settings, bb, DataType.UINT8, 0)
                    _MTCommon.setData(settings, bb, DataType.OCTET_STRING, _MTCommon.logicalNameToBytes(it.logicalName))
    ...
    def getValue(self, settings, e):
        ...
        if e.index == 1:
            ...
            ret = _MTCommon.getBytes(self.securitySetupReference)
        e.error = ErrorCode.READ_WRITE_DENIED
        return ret
        ...
        if e.index == 1:
            ...
            if e.value:
                for item in e.value:
                    ...
                    ot = item[1]
                    version = item[2]
                    ln = _MTCommon.toLogicalName(item[3])
