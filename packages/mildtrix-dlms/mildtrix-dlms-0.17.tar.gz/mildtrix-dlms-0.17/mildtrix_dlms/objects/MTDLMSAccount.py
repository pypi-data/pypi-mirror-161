...
class MTDLMSAccount(MTDLMSObject, IMTDLMSBase):
    ...
    def __init__(self, ln="0.0.19.0.0.255", sn=0):
        ...
        """
        MTDLMSObject.__init__(self, ObjectType.ACCOUNT, ln, sn)
    ...
    def getValue(self, settings, e):
        ...
        if e.index == 1:
            ...
            ret = MTBitString.toBitString(self.currentCreditStatus, 8)
        elif e.index == 5:
            ...
            if not self.creditReferences:
                ...
                _MTCommon.setObjectCount(len(self.creditReferences), bb)
                for it in self.creditReferences:
                    bb.setUInt8(DataType.OCTET_STRING)
                    bb.setUInt8(6)
                    bb.set(_MTCommon.logicalNameToBytes(it))
            ret = bb.array()
        elif e.index == 10:
            bb = MTByteBuffer()
            bb.setUInt8(DataType.ARRAY)
            if not self.chargeReferences:
                ...
                _MTCommon.setObjectCount(len(self.chargeReferences), bb)
            ...
            if not self.creditChargeConfigurations:
                ...
                _MTCommon.setObjectCount(len(self.creditChargeConfigurations), bb)
                for it in self.creditChargeConfigurations:
                    bb.setUInt8(DataType.STRUCTURE)
                    bb.setUInt8(3)
                    bb.setUInt8(DataType.OCTET_STRING)
                    bb.setUInt8(6)
                    bb.set(_MTCommon.logicalNameToBytes(it.creditReference))
                    bb.setUInt8(DataType.OCTET_STRING)
                    bb.setUInt8(6)
                    bb.set(_MTCommon.logicalNameToBytes(it.chargeReference))
                    _MTCommon.setData(settings, bb, DataType.BITSTRING, MTBitString.toBitString(it.collectionConfiguration, 3))
            ret = bb.array()
        elif e.index == 12:
            bb = MTByteBuffer()
            bb.setUInt8(DataType.ARRAY)
            if not self.tokenGatewayConfigurations:
                ...
                _MTCommon.setObjectCount(len(self.tokenGatewayConfigurations), bb)
            ...
            bb.setUInt8(DataType.STRUCTURE)
            bb.setUInt8(3)
            _MTCommon.setData(settings, bb, DataType.STRING_UTF8, self.currency.name)
            _MTCommon.setData(settings, bb, DataType.INT8, self.currency.scale)
            _MTCommon.setData(settings, bb, DataType.ENUM, self.currency.unit)
            ret = bb.array()
        elif e.index == 16:
    ...
    def setValue(self, settings, e):
        if e.index == 1:
            ...
            if e.value:
                for it in e.value:
                    self.creditReferences.append(_MTCommon.toLogicalName(it))
        elif e.index == 10:
            self.chargeReferences = []
            if e.value:
                for it in e.value:
                    self.chargeReferences.append(_MTCommon.toLogicalName(it))
        elif e.index == 11:
            #pylint: disable=bad-option-value,redefined-variable-type
            self.creditChargeConfigurations = []
            if e.value:
                for it in e.value:
                    item = MTCreditChargeConfiguration()
                    item.creditReference = _MTCommon.toLogicalName(it[0])
                    item.chargeReference = _MTCommon.toLogicalName(it[1])
            ...
            if e.value:
                for it in e.value:
                    item = MTTokenGatewayConfiguration()
                    item.creditReference = _MTCommon.toLogicalName(it[0])
                    item.tokenProportion = it[1]
                    self.tokenGatewayConfigurations.append(item)
        elif e.index == 13:
            if not e.value:
                self.accountActivationTime = MTDateTime()
            else:
                tmp = None
                if isinstance(e.value, bytearray):
                    tmp = _MTCommon.changeType(settings, e.value, DataType.DATETIME)
                else:
                    tmp = e.value
                self.accountActivationTime = tmp
        elif e.index == 14:
            if not e.value:
                self.accountClosureTime = MTDateTime()
            else:
                tmp = None
                if isinstance(e.value, bytearray):
                    tmp = _MTCommon.changeType(settings, e.value, DataType.DATETIME)
    ...
    def loadCreditChargeConfigurations(cls, reader, list_):
        ...
        if reader.isStartElement("CreditChargeConfigurations", True):
            while reader.isStartElement("Item", True):
                it = MTCreditChargeConfiguration()
    ...
    def loadTokenGatewayConfigurations(cls, reader, list_):
        ...
        if reader.isStartElement("TokenGatewayConfigurations", True):
            while reader.isStartElement("Item", True):
                it = MTTokenGatewayConfiguration()
                it.creditReference = reader.readElementContentAsString("Credit")
