...
class MTDLMSGprsSetup(MTDLMSObject, IMTDLMSBase):
    """
    Online help:
    http://www.mildtrix.fi/mildtrix.DLMS.Objects.MTDLMSGprsSetup
    """
    ...
    def __init__(self, ln="0.0.25.4.0.255", sn=0):
        ...
        """
        MTDLMSObject.__init__(self, ObjectType.GPRS_SETUP, ln, sn)
        self.apn = ""
        self.pinCode = 0
        self.defaultQualityOfService = MTDLMSQualityOfService()
        self.requestedQualityOfService = MTDLMSQualityOfService()
    ...
    def getValue(self, settings, e):
        ...
        if e.index == 1:
            ...
            data.setUInt8(DataType.STRUCTURE)
            data.setUInt8(5)
            _MTCommon.setData(settings, data, DataType.UINT8, self.defaultQualityOfService.precedence)
            _MTCommon.setData(settings, data, DataType.UINT8, self.defaultQualityOfService.delay)
            _MTCommon.setData(settings, data, DataType.UINT8, self.defaultQualityOfService.reliability)
            _MTCommon.setData(settings, data, DataType.UINT8, self.defaultQualityOfService.peakThroughput)
            _MTCommon.setData(settings, data, DataType.UINT8, self.defaultQualityOfService.meanThroughput)
            data.setUInt8(DataType.STRUCTURE)
            data.setUInt8(5)
            _MTCommon.setData(settings, data, DataType.UINT8, self.requestedQualityOfService.precedence)
            _MTCommon.setData(settings, data, DataType.UINT8, self.requestedQualityOfService.delay)
            _MTCommon.setData(settings, data, DataType.UINT8, self.requestedQualityOfService.reliability)
            _MTCommon.setData(settings, data, DataType.UINT8, self.requestedQualityOfService.peakThroughput)
            _MTCommon.setData(settings, data, DataType.UINT8, self.requestedQualityOfService.meanThroughput)
            ret = data.array()
        else:
    ...
    def setValue(self, settings, e):
        if e.index == 1:
            ...
            if isinstance(e.value, str):
                ...
                self.apn = _MTCommon.changeType(settings, e.value, DataType.STRING)
        elif e.index == 3:
