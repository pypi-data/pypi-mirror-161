...
class MTDLMSActivityCalendar(MTDLMSObject, IMTDLMSBase):
    """
    Online help:
    http://www.mildtrix.fi/mildtrix.DLMS.Objects.MTDLMSActivityCalendar
    """
    ...
    def __init__(self, ln="0.0.13.0.0.255", sn=0):
        ...
        """
        MTDLMSObject.__init__(self, ObjectType.ACTIVITY_CALENDAR, ln, sn)
    ...
    def __getSeasonProfile(cls, settings, target):
        ...
        if target is None:
            #  Add count
            _MTCommon.setObjectCount(0, data)
        else:
            #  Add count
            _MTCommon.setObjectCount(len(target), data)
            for it in target:
                data.setUInt8(DataType.STRUCTURE)
                data.setUInt8(3)
                _MTCommon.setData(settings, data, DataType.OCTET_STRING, it.name)
                _MTCommon.setData(settings, data, DataType.OCTET_STRING, it.start)
                _MTCommon.setData(settings, data, DataType.OCTET_STRING, it.weekName)
        return data.array()
    ...
    def __getWeekProfileTable(cls, settings, target):
        ...
        if target is None:
            ...
            for it in target:
                ...
                data.setUInt8(8)
                _MTCommon.setData(settings, data, DataType.OCTET_STRING, it.name)
                _MTCommon.setData(settings, data, DataType.UINT8, it.monday)
                _MTCommon.setData(settings, data, DataType.UINT8, it.tuesday)
                _MTCommon.setData(settings, data, DataType.UINT8, it.wednesday)
                _MTCommon.setData(settings, data, DataType.UINT8, it.thursday)
                _MTCommon.setData(settings, data, DataType.UINT8, it.friday)
                _MTCommon.setData(settings, data, DataType.UINT8, it.saturday)
                _MTCommon.setData(settings, data, DataType.UINT8, it.sunday)
        return data.array()
    ...
    def __getDayProfileTable(cls, settings, target):
        ...
        if target is None:
            ...
            for it in target:
                data.setUInt8(DataType.STRUCTURE)
                data.setUInt8(2)
                _MTCommon.setData(settings, data, DataType.UINT8, it.dayId)
                data.setUInt8(DataType.ARRAY)
                #  Add count
                _MTCommon.setObjectCount(len(it.daySchedules), data)
                for action in it.daySchedules:
                    data.setUInt8(DataType.STRUCTURE)
                    data.setUInt8(3)
                    _MTCommon.setData(settings, data, DataType.OCTET_STRING, action.startTime)
                    _MTCommon.setData(settings, data, DataType.OCTET_STRING, _MTCommon.logicalNameToBytes(action.scriptLogicalName))
                    _MTCommon.setData(settings, data, DataType.UINT16, int(action.scriptSelector))
        return data.array()
    ...
    def getValue(self, settings, e):
        if e.index == 1:
            ...
            if not self.calendarNameActive:
                ...
                ret = MTByteBuffer.hexToBytes(self.calendarNameActive)
            else:
            ...
            if self.calendarNamePassive is None:
                ...
                ret = MTByteBuffer.hexToBytes(self.calendarNamePassive)
            else:
    ...
    def __setSeasonProfile(cls, settings, value):
        ...
        if value:
            for item in value:
                it = MTDLMSSeasonProfile()
                it.name = item[0]
                it.start = _MTCommon.changeType(settings, item[1], DataType.DATETIME)
                it.weekName = item[2]
                items.append(it)
        return items
    ...
    def __setDayProfileTable(cls, settings, value):
        ...
        if value:
            for item in value:
                ...
                for it2 in item[1]:
                    ac = MTDLMSDayProfileAction()
                    ac.startTime = _MTCommon.changeType(settings, it2[0], DataType.TIME)
                    ac.scriptLogicalName = _MTCommon.toLogicalName(it2[1])
    ...
    def setValue(self, settings, e):
        if e.index == 1:
            ...
            if self.isSec or not MTByteBuffer.isAsciiString(e.value):
                self.calendarNameActive = MTByteBuffer.hex(e.value)
            else:
            ...
            if self.isSec or not MTByteBuffer.isAsciiString(e.value):
                self.calendarNamePassive = MTByteBuffer.hex(e.value)
            else:
            ...
            if isinstance(e.value, MTDateTime):
                self.time = e.value
            else:
                self.time = _MTCommon.changeType(settings, e.value, DataType.DATETIME)
        else:
            e.error = ErrorCode.READ_WRITE_DENIED
    ...
    def loadSeasonProfile(cls, reader, name):
        ...
        if reader.isStartElement(name, True):
            while reader.isStartElement("Item", True):
                it = MTDLMSSeasonProfile()
                it.name = MTByteBuffer.hexToBytes(reader.readElementContentAsString("Name"))
                it.start = reader.readElementContentAsDateTime("Start")
                it.weekName = MTByteBuffer.hexToBytes(reader.readElementContentAsString("WeekName"))
                list_.append(it)
        ...
        if list_:
            ...
            for it in list_:
                ...
                if isinstance(str_, (bytearray, bytes)):
                    str_ = MTByteBuffer.hex(str_)
                writer.writeElementString("Name", str_)
                writer.writeElementString("Start", it.start)
                writer.writeElementString("WeekName", MTByteBuffer.hex(it.weekName))
    ...
    def saveWeekProfileTable(cls, writer, list_, name):
        if list_:
            ...
            for it in list_:
                writer.writeStartElement("Item")
                writer.writeElementString("Name", MTByteBuffer.hex(it.name))
