...
class MTDLMSIECLocalPortSetup(MTDLMSObject, IMTDLMSBase):
    """
    Online help:
    http://www.mildtrix.fi/mildtrix.DLMS.Objects.MTDLMSIECLocalPortSetup
    """
    ...
    def __init__(self, ln="0.0.20.0.0.255", sn=0):
        ...
        """
        MTDLMSObject.__init__(self, ObjectType.IEC_LOCAL_PORT_SETUP, ln, sn)
    ...
    def getValue(self, settings, e):
        if e.index == 1:
            ...
            ret = _MTCommon.getBytes(self.deviceAddress)
        elif e.index == 7:
            ret = _MTCommon.getBytes(self.password1)
        elif e.index == 8:
            ret = _MTCommon.getBytes(self.password2)
        elif e.index == 9:
            ret = _MTCommon.getBytes(self.password5)
        else:
    ...
    def setValue(self, settings, e):
        if e.index == 1:
            ...
            self.deviceAddress = _MTCommon.changeType(settings, e.value, DataType.STRING)
        elif e.index == 7:
            self.password1 = _MTCommon.changeType(settings, e.value, DataType.STRING)
        elif e.index == 8:
            self.password2 = _MTCommon.changeType(settings, e.value, DataType.STRING)
        elif e.index == 9:
            self.password5 = _MTCommon.changeType(settings, e.value, DataType.STRING)
        else:
            e.error = ErrorCode.READ_WRITE_DENIED
...
class MTDLMSIECOpticalPortSetup(MTDLMSIECLocalPortSetup):
    """Obsolete. Use MTDLMSIECLocalPortSetup instead."""
