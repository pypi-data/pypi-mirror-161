...
class MTDateTime:
        ...
        if isinstance(value, datetime.datetime):
            if value.tzinfo is None:
                if time.localtime().tm_isdst:
                    #If DST is used.
                    tz = MTTimeZone(int(-time.altzone / 60))
                else:
                    tz = MTTimeZone(int(-time.timezone / 60))
    ...
    def fromString(self, value, pattern=None):
        ...
        if value:
            ...
            addTimeZone = type(self).__name__ == MTDateTime.__name__
            if value.find('*') != -1:
                lastFormatIndex = -1
                pos = 0
                while pos < len(value):
            ...
            if addTimeZone:
                ...
                if tz is None:
                    ...
                    tmp = datetime.datetime(tmp.year, tmp.month, tmp.day, tmp.hour, tmp.minute, tmp.second, 0, tzinfo=MTTimeZone(tz))
                return tmp
...
#        return str_
    ...
    def fromUnixTime(cls, unixTime):
        return MTDateTime(datetime.datetime(unixTime * 1000))
    ...
    def fromHighResolutionTime(cls, unixTime):
        return MTDateTime(datetime.datetime(unixTime))
