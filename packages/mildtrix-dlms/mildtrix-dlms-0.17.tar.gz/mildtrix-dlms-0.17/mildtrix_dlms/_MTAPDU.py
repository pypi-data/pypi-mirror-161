...
class _MTAPDU:
    ...
    def generateApplicationContextName(cls, settings, data, cipher):
        ...
        if settings.protocolVersion:
            ...
            _MTCommon.setBitString(data, settings.protocolVersion, False)
    ...
    def getConformanceToArray(cls, data):
        ret = _MTCommon.swapBits(data.getUInt8())
        ret |= _MTCommon.swapBits(data.getUInt8()) << 8
        ret |= _MTCommon.swapBits(data.getUInt8()) << 16
        return ret

    # Reserved for internal use.
    @classmethod
    def setConformanceToArray(cls, value, data):
        data.setUInt8(_MTCommon.swapBits(int(value) & 0xFF))
        data.setUInt8(_MTCommon.swapBits((int(value) >> 8) & 0xFF))
        data.setUInt8(_MTCommon.swapBits((int(value) >> 16) & 0xFF))
    ...
    def getInitiateRequest(cls, settings, data):
        ...
        if not settings.cipher or settings.cipher.security == Security.NONE or not settings.cipher.dedicatedKey:
            ...
            data.setUInt8(1)
            _MTCommon.setObjectCount(len(settings.cipher.dedicatedKey), data)
    ...
    def generateUserInformation(cls, settings, cipher, encryptedData, data):
        ...
        if not cipher or not cipher.isCiphered():
            ...
            if encryptedData:
                ...
                crypted = MTCiphering.encrypt(p, tmp.array())
                #  Length for AARQ user field
    ...
    def parseUserInformation(cls, settings, cipher, data, xml):
        ...
        if xml and xml.outputType == TranslatorOutputType.STANDARD_XML:
            ...
            return
        _MTAPDU.parseInitiate(False, settings, cipher, data, xml)
    ...
    def parse(cls, initiateRequest, settings, cipher, data, xml, tag2):
        ...
        if response:
            ...
            if tag != 0:
                ...
                if xml:
                    xml.appendLine(TranslatorGeneralTags.DEDICATED_KEY, None, MTByteBuffer.hex(tmp, False))
            ...
            if xml:
                ...
                return
            raise MTDLMSConfirmedServiceError(ConfirmedServiceError(data.getUInt8()), ServiceError(data.getUInt8()), data.getUInt8())
        else:
            if xml:
        ...
        if not response:
            ...
            if xml is None and pdu < 64:
                raise MTDLMSConfirmedServiceError(ConfirmedServiceError.INITIATE_ERROR, ServiceError.SERVICE, Service.PDU_SIZE)
            #  Max PDU size.
            settings.maxPduSize = pdu
            if xml:
    ...
    def parseInitiate(cls, initiateRequest, settings, cipher, data, xml):
        ...
        if tag in (Command.GLO_INITIATE_RESPONSE, Command.GLO_INITIATE_REQUEST,
            ...
            if xml:
                ...
                if st and cipher and cipher.blockCipherKey and cipher.authenticationKey and xml.comments:
                    ...
                    try:
                        ...
                        p.xml = (xml)
                        tmp = MTCiphering.decrypt(settings.cipher, p, data)
                        data.clear()
                        data.set(tmp)
                ...
                xml.appendLine(tag, None, MTByteBuffer.hex(encrypted, False))
                return
            ...
            tag = data.getUInt8()
        _MTAPDU.parse(initiateRequest, settings, cipher, data, xml, tag)
    ...
    def parsePDU(cls, settings, cipher, buff, xml):
        ...
        ret = _MTAPDU.parsePDU2(settings, cipher, buff, xml)
        #  Closing tags
        if xml:
            if settings.isServer:
    ...
    def parseProtocolVersion(cls, settings, buff, xml):
        ...
        value = buff.getUInt8()
        sb = _MTCommon.toBitString(value, 8 - unusedBits)
        ...
        while buff.position < len(buff):
            ...
            if tag == BerType.CONTEXT | BerType.CONSTRUCTED | PduType.APPLICATION_CONTEXT_NAME:
                if not cls.parseApplicationContextName(settings, buff, xml):
                    raise MTDLMSException(AssociationResult.PERMANENT_REJECTED, SourceDiagnostic.NOT_SUPPORTED)
                ...
                if settings.isServer:
                    ...
                    if xml:
                        #  RespondingAPTitle
                        if xml.comments:
                            xml.appendComment(_MTCommon.systemTitleToString(settings.standard, settings.sourceSystemTitle))
                        xml.appendLine(TranslatorTags.CALLED_AP_TITLE, "Value", MTByteBuffer.hex(tmp, False))
                else:
                    #  Choice for result (INTEGER, universal)
                ...
                #  0xA3
                resultDiagnosticValue = _MTAPDU.parseSourceDiagnostic(settings, buff, xml)
            elif tag == BerType.CONTEXT | BerType.CONSTRUCTED | PduType.CALLED_AP_INVOCATION_ID:
                #  0xA4
                _MTAPDU.parseResult(settings, buff, xml)
                ...
                try:
                    ...
                    if xml is None:
                        raise ex
                _MTAPDU.appendClientSystemTitleToXml(settings, xml)
                ...
                _MTAPDU.appendServerSystemTitleToXml(settings, xml, tag)
                ...
                #  0x89
                _MTAPDU.updateAuthentication(settings, buff)
                if xml:
                    if xml.outputType == TranslatorOutputType.SIMPLE_XML:
                ...
                #  0xAC
                _MTAPDU.updatePassword(settings, buff, xml)
            elif tag == BerType.CONTEXT | BerType.CONSTRUCTED | PduType.USER_INFORMATION:
                #  0xBE
                try:
                    _MTAPDU.parseUserInformation(settings, cipher, buff, xml)
                except Exception:
                    if xml is None:
                        ...
                        if xml is None and resultComponent != AssociationResult.ACCEPTED and resultDiagnosticValue != SourceDiagnostic.NONE:
                            raise MTDLMSException(resultComponent, resultDiagnosticValue)
                        if xml is None and resultComponent != AssociationResult.ACCEPTED and resultDiagnosticValue != AcseServiceProvider.NONE:
                            raise MTDLMSException(resultComponent, resultDiagnosticValue)
                        raise MTDLMSException(AssociationResult.PERMANENT_REJECTED, SourceDiagnostic.NO_REASON_GIVEN)
    ...
    def parseSourceDiagnostic(cls, settings, buff, xml):
        ...
        if settings.isServer:
            ...
            if xml:
                xml.appendLine(TranslatorTags.CALLED_AE_QUALIFIER, "Value", MTByteBuffer.hex(calledAEQualifier, False))
        else:
    ...
    def appendServerSystemTitleToXml(cls, settings, xml, tag):
        if xml:
            ...
            if xml.outputType == TranslatorOutputType.SIMPLE_XML:
                xml.appendLine(tag, None, MTByteBuffer.hex(settings.getStoCChallenge(), False))
            else:
                xml.append(tag, True)
                xml.append(TranslatorGeneralTags.CHAR_STRING, True)
                xml.append(MTByteBuffer.hex(settings.getStoCChallenge(), False))
    ...
    def appendClientSystemTitleToXml(cls, settings, xml):
        if xml:
            #  CallingAPTitle
            xml.appendLine(TranslatorGeneralTags.CALLING_AP_TITLE, "Value", MTByteBuffer.hex(settings.sourceSystemTitle, False))

    @classmethod
    def appendResultToXml(cls, settings, xml):
        if xml:
            #  RespondingAPTitle
            xml.appendLine(TranslatorGeneralTags.RESPONDING_AP_TITLE, "Value", MTByteBuffer.hex(settings.sourceSystemTitle, False))
    ...
    def updatePassword(cls, settings, buff, xml):
        ...
        if xml:
            if xml.outputType == TranslatorOutputType.SIMPLE_XML:
                if MTByteBuffer.isAsciiString(tmp):
                    xml.appendComment(tmp.decode("utf-8"))
                xml.appendLine(TranslatorGeneralTags.CALLING_AUTHENTICATION, "Value", MTByteBuffer.hex(tmp, False))
            else:
                xml.appendStartTag(TranslatorGeneralTags.CALLING_AUTHENTICATION)
                xml.appendStartTag(TranslatorGeneralTags.CHAR_STRING)
                if settings.authentication == Authentication.LOW:
                    xml.append(MTByteBuffer.hex(settings.password, False))
                else:
                    xml.append(MTByteBuffer.hex(settings.ctoSChallenge, False))
    ...
    def getUserInformation(cls, settings, cipher):
        ...
        if cipher and cipher.isCiphered():
            ...
            p.security = cipher.security
            p.invocationCounter = cipher.invocationCounter
            return MTCiphering.encrypt(p, data.array())
    ...
    def generateAARE(cls, settings, data, result, diagnostic, cipher, errorData, encryptedData):
        ...
        if result == AssociationResult.ACCEPTED or not cipher or cipher.security == Security.NONE:
            ...
            data.setUInt8(BerType.CONTEXT | BerType.CONSTRUCTED | PduType.USER_INFORMATION)
            if encryptedData:
                tmp2 = MTByteBuffer(2 + len(encryptedData))
                tmp2.setUInt8(Command.GLO_INITIATE_RESPONSE)
                _MTCommon.setObjectCount(len(encryptedData), tmp2)
