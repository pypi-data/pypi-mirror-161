...
from .objects import MTDLMSObject, MTDLMSObjectCollection, MTDLMSData
...
class MTDLMSClient(object):
    ...
    def __init__(self, useLogicalNameReferencing=True, clientAddress=16, serverAddress=1, forAuthentication=Authentication.NONE, password=None, interfaceType=InterfaceType.HDLC):
        # DLMS settings.
        self.settings = MTDLMSSettings(False)
        ...
        if not password:
            ...
            self.password = _MTCommon.getBytes(password)
    ...
    # 0xFFFF.
    #
    # @see MTDLMSClient.clientAddress
    # @see MTDLMSClient.serverAddress
    # @see MTDLMSClient.useLogicalNameReferencing
    ...
    #
    # @see MTDLMSClient#getAuthentication
    # Used password.
    #
    ...
    #
    # @see MTDLMSClient#getPassword
    # @see MTDLMSClient#getClientAddress
    # Used authentication.
    #
    ...
    #
    # @see MTDLMSClient#getClientAddress
    # @see MTDLMSClient#getServerAddress
    # @see MTDLMSClient#parseUAResponse
    ...
    def snrmRequest(self):
        ...
        if MTHdlcSettings.DEFAULT_MAX_INFO_TX != self.hdlcSettings.maxInfoTX:
            data.setUInt8(_HDLCInfo.MAX_INFO_TX)
            MTDLMS.appendHdlcParameter(data, self.hdlcSettings.maxInfoTX)
        if MTHdlcSettings.DEFAULT_MAX_INFO_RX != self.hdlcSettings.maxInfoRX:
            data.setUInt8(_HDLCInfo.MAX_INFO_RX)
            MTDLMS.appendHdlcParameter(data, self.hdlcSettings.maxInfoRX)
        if MTHdlcSettings.DEFAULT_WINDOWS_SIZE_TX != self.hdlcSettings.windowSizeTX:
            data.setUInt8(_HDLCInfo.WINDOW_SIZE_TX)
            data.setUInt8(4)
            data.setUInt32(self.hdlcSettings.windowSizeTX)
        if MTHdlcSettings.DEFAULT_WINDOWS_SIZE_RX != self.hdlcSettings.windowSizeRX:
        ...
        if data.size != 3:
            ...
            data = None
        return MTDLMS.getHdlcFrame(self.settings, Command.SNRM, data)

    #
    # Parses UAResponse from byte array.
    #
    # data: # Received message from the server.
    # @see MTDLMSClient#snrmRequest
    #
    def parseUAResponse(self, data):
        if not isinstance(data, MTByteBuffer):
            data = MTByteBuffer(data)

        MTDLMS.parseSnrmUaResponse(data, self.settings.hdlc)
    ...
    # @see MTDLMSClient#parseAareResponse
    #
    def aarqRequest(self):
        ...
        buff = MTByteBuffer(20)
        self.settings.resetBlockIndex()
        MTDLMS.checkInit(self.settings)
        ...
        if self.authentication > Authentication.LOW:
            if not self.settings.useCustomChallenge:
                self.settings.ctoSChallenge = MTSecure.generateChallenge()
        else:
            self.settings.setCtoSChallenge(None)
        _MTAPDU.generateAarq(self.settings, self.settings.cipher, None, buff)
        reply = None
        if self.settings.getUseLogicalNameReferencing():
            p = MTDLMSLNParameters(self.settings, 0, Command.AARQ, 0, buff, None, 0xff)
            reply = MTDLMS.getLnMessages(p)
        else:
            p = MTDLMSSNParameters(self.settings, Command.AARQ, 0, 0, None, buff)
    ...
    # reply
    # Received data.
    # @see MTDLMSClient#aarqRequest
    # @see MTDLMSClient#useLogicalNameReferencing
    # @see MTDLMSClient#negotiatedConformance
    # @see MTDLMSClient#proposedConformance
    #
    def parseAareResponse(self, reply):
        self.isAuthenticationRequired = _MTAPDU.parsePDU(self.settings, self.settings.cipher, reply, None) == SourceDiagnostic.AUTHENTICATION_REQUIRED
    ...
    def getApplicationAssociationRequest(self):
        ...
        challenge = MTSecure.secure(self.settings, self.settings.cipher, self.settings.cipher.invocationCounter, self.settings.getStoCChallenge(), pw)
        if self.useLogicalNameReferencing:
            return self.__method("0.0.40.0.0.255", ObjectType.ASSOCIATION_LOGICAL_NAME, 1, challenge, DataType.OCTET_STRING)
        return self.__method(0xFA00, ObjectType.ASSOCIATION_SHORT_NAME, 8, challenge, DataType.OCTET_STRING)
    ...
    def parseApplicationAssociationResponse(self, reply):
        ...
        if self.manufacturerId == "LGZ" and self.settings.authentication == Authentication.HIGH:
            ...
            equals = False
            ic = 0
            value = _MTCommon.getData(self.settings, reply, info)
            if value:
                ...
                tmp = MTSecure.secure(self.settings, self.settings.cipher, ic, self.settings.getCtoSChallenge(), secret)
                challenge = MTByteBuffer(tmp)
                equals = challenge.compare(value)
                if not equals:
                    print("Invalid StoC:" + MTByteBuffer.hex(value, True) + "-" + MTByteBuffer.hex(tmp, True))
            else:
    ...
    def releaseRequest(self):
        ...
        if self.useProtectedRelease:
            ...
            _MTAPDU.generateUserInformation(self.settings, self.settings.cipher, None, buff)
        ...
        if self.useLogicalNameReferencing:
            p = MTDLMSLNParameters(self.settings, 0, Command.RELEASE_REQUEST, 0, buff, None, 0xff)
            reply = MTDLMS.getLnMessages(p)
        else:
            reply = MTDLMS.getSnMessages(MTDLMSSNParameters(self.settings, Command.RELEASE_REQUEST, 0xFF, 0xFF, None, buff))
        self.settings.connected = self.settings.connected & ~ConnectionState.DLMS
        return reply
    ...
    def disconnectRequest(self, force=False):
        if not force and self.settings.connected == ConnectionState.NONE:
            return None
        if MTDLMS.useHdlc(self.interfaceType):
            self.settings.connected = ConnectionState.NONE
            reply = MTDLMS.getHdlcFrame(self.settings, Command.DISCONNECT_REQUEST, None)
    ...
    def __createDLMSObject(cls, classID, version, baseName, ln, accessRights):
        ...
        MTDLMSClient.__updateObjectData(obj, type_, version, baseName, ln, accessRights)
        return obj
    ...
    def parseSNObjects(self, buff, onlyKnownObjects, ignoreInactiveObjects):
        ...
        items = MTDLMSObjectCollection(self)
        cnt = _MTCommon.getObjectCount(buff)
        info = _MTDataInfo()
        objPos = 0
        while objPos != cnt:
            ...
            objects = _MTCommon.getData(self.settings, buff, info)
            if len(objects) != 4:
                raise Exception("Invalid structure format.")
            classID = objects[1]
            baseName = int(objects[0]) & 0xFFFF
            comp = MTDLMSClient.__createDLMSObject(classID, objects[2], baseName, objects[3], None)
            if (not onlyKnownObjects or type(comp) != MTDLMSObject):
    ...
    def __updateObjectData(cls, obj, objectType, version, baseName, logicalName, accessRights):
        ...
        obj.logicalName = _MTCommon.toLogicalName(logicalName)
    ...
    def parseObjects(self, data, onlyKnownObjects=True, ignoreInactiveObjects=True):
        ...
        c = MTDLMSConverter(self.standard)
    ...
    def parseLNObjects(self, buff, onlyKnownObjects, ignoreInactiveObjects):
        ...
        while objPos != cnt:
            ...
            classID = objects[0]
            if classID > 0:
                comp = MTDLMSClient.__createDLMSObject(classID, objects[1], 0, objects[2], objects[3])
                if (not onlyKnownObjects or type(comp) != MTDLMSObject):
                    if not ignoreInactiveObjects or comp.logicalName != "0.0.127.0.0.0":
                        items.append(comp)
                else:
                    print("Unknown object : " + str(classID) + " " + _MTCommon.toLogicalName(objects[2]))
            objPos += 1
        return items
    ...
    def getValue(cls, data, useUtc):
        ...
        info = _MTDataInfo()
        return _MTCommon.getData(settings, data, info)
    ...
    def changeType(cls, value, type_, useUtc=False):
        settings = MTDLMSSettings(False)
        settings.useUtc2NormalTime = useUtc
        return _MTCommon.changeType(settings, value, type_)
    ...
    def __method(self, name, objectType, methodIndex, value, dataType=DataType.NONE):
        ...
        reply = None
        data = MTByteBuffer()
        attributeDescriptor = MTByteBuffer()
        _MTCommon.setData(self.settings, data, type_, value)
        if self.useLogicalNameReferencing:
            attributeDescriptor.setUInt16(objectType)
            attributeDescriptor.set(_MTCommon.logicalNameToBytes(str(name)))
            attributeDescriptor.setUInt8(int(methodIndex))
            if type_ == DataType.NONE:
                attributeDescriptor.setUInt8(0)
            else:
                attributeDescriptor.setUInt8(1)
            p = MTDLMSLNParameters(self.settings, 0, Command.METHOD_REQUEST, ActionRequestType.NORMAL, attributeDescriptor, data, 0xff)
            reply = MTDLMS.getLnMessages(p)
        else:
            ind = [0]
            count = [0]
            MTDLMS.getActionInfo(objectType, ind, count)
            if index > count[0]:
            ...
            if type_ != DataType.NONE:
                ...
                attributeDescriptor.setUInt8(0)
            p = MTDLMSSNParameters(self.settings, Command.WRITE_REQUEST, 1, VariableAccessSpecification.VARIABLE_NAME, attributeDescriptor, data)
            reply = MTDLMS.getSnMessages(p)
        return reply
    ...
    def __write(self, name, value, dataType, objectType, index):
        ...
        if self.useLogicalNameReferencing:
            ...
            attributeDescriptor.setUInt8(0)
            p = MTDLMSLNParameters(self.settings, 0, Command.SET_REQUEST, SetRequestType.NORMAL, attributeDescriptor, data, 0xff)
    ...
    def writeList(self, list_):
        ...
        if self.useLogicalNameReferencing:
            ...
            for it in list_:
                ...
                bb.set(_MTCommon.logicalNameToBytes(it.target.logicalName))
            ...
            for it in list_:
                ...
                sn += (it.index - 1) * 8
                bb.setUInt16(sn)
        _MTCommon.setObjectCount(len(list_), bb)
        for it in list_:
            ...
            _MTCommon.setData(self.settings, data, type_, value)
        if self.useLogicalNameReferencing:
            p = MTDLMSLNParameters(self.settings, 0, Command.SET_REQUEST, SetRequestType.WITH_LIST, bb, data, 0xff)
            reply = MTDLMS.getLnMessages(p)
        else:
            p2 = MTDLMSSNParameters(self.settings, Command.WRITE_REQUEST, len(list_), 4, bb, data)
            reply = MTDLMS.getSnMessages(p2)
        return reply
    ...
    def _read(self, name, objectType, attributeOrdinal, data=None):
        ...
        if self.useLogicalNameReferencing:
            ...
            if not data:
                ...
                attributeDescriptor.setUInt8(1)
            p = MTDLMSLNParameters(self.settings, 0, Command.GET_REQUEST, GetCommandType.NORMAL, attributeDescriptor, data, 0xFF)
            reply = MTDLMS.getLnMessages(p)
        else:
            #pylint: disable=bad-option-value,redefined-variable-type
            sn = name
            ...
            p = MTDLMSSNParameters(self.settings, Command.READ_REQUEST, 1, requestType, attributeDescriptor, data)
            reply = MTDLMS.getSnMessages(p)
        return reply
    ...
    def readList(self, list_):
        ...
        self.settings.resetBlockIndex()
        if self.useLogicalNameReferencing:
            p = MTDLMSLNParameters(self.settings, 0, Command.GET_REQUEST, GetCommandType.WITH_LIST, data, None, 0xff)
            pos = 0
            ...
            if count > 10:
                count = 10
            _MTCommon.setObjectCount(count, data)
            for k, v in list_:
                data.setUInt16(k.objectType)
                data.set(_MTCommon.logicalNameToBytes(k.logicalName))
                data.setUInt8(v)
                data.setUInt8(0)
                pos += 1
                if pos % count == 0 and len(list_) != pos:
                    messages.append(MTDLMS.getLnMessages(p))
                    data.clear()
                    if len(list_) - pos < count:
                        _MTCommon.setObjectCount(len(list_) - pos, data)
                    else:
                        _MTCommon.setObjectCount(count, data)
            messages.append(MTDLMS.getLnMessages(p))
        else:
            p2 = MTDLMSSNParameters(self.settings, Command.READ_REQUEST, len(list_), 0xFF, data, None)
            for k, v in list_:
                data.setUInt8(VariableAccessSpecification.VARIABLE_NAME)
                sn = k.shortName
                sn += (v - 1) * 8
                data.setUInt16(sn)
            messages.append(MTDLMS.getSnMessages(p2))
        return messages

    def keepAlive(self):
        if self.interfaceType == InterfaceType.WRAPPER:
            return None
        return MTDLMS.getHdlcFrame(self.settings, self.settings.getReceiverReady(), None)
    ...
    def readRowsByEntry(self, pg, index, count, columns=None):
        ...
        pg.buffer = list()
        buff = MTByteBuffer(19)
        buff.setUInt8(0x02)
        buff.setUInt8(DataType.STRUCTURE)
        buff.setUInt8(0x04)
        _MTCommon.setData(self.settings, buff, DataType.UINT32, index)
        if count == 0:
            _MTCommon.setData(self.settings, buff, DataType.UINT32, count)
        else:
            _MTCommon.setData(self.settings, buff, DataType.UINT32, index + count - 1)
        columnIndex = 1
        columnCount = 0
        pos = 0
        if columns:
        ...
        _MTCommon.setData(self.settings, buff, DataType.UINT16, columnIndex)
        _MTCommon.setData(self.settings, buff, DataType.UINT16, columnCount)
        return self._read(pg.name, ObjectType.PROFILE_GENERIC, 2, buff)

    def readRowsByRange(self, pg, start, end, columns=None):
        pg.buffer = list()
        self.settings.resetBlockIndex()
        if not isinstance(start, MTDateTime):
            start = MTDateTime(start)
        if not isinstance(end, MTDateTime):
            end = MTDateTime(end)
        sort = pg.sortObject
        if not sort and pg.captureObjects:
            sort = pg.captureObjects[0][0]
        buff = MTByteBuffer(51)
        buff.setUInt8(0x01)
        buff.setUInt8(DataType.STRUCTURE)
        buff.setUInt8(0x04)
        buff.setUInt8(DataType.STRUCTURE)
        ...
        if sort:
            ...
            ot = ObjectType.CLOCK
            ln = "0.0.1.0.0.255"
        _MTCommon.setData(self.settings, buff, DataType.UINT16, ot)
        _MTCommon.setData(self.settings, buff, DataType.OCTET_STRING, _MTCommon.logicalNameToBytes(ln))
        _MTCommon.setData(self.settings, buff, DataType.INT8, 2)
        _MTCommon.setData(self.settings, buff, DataType.UINT16, 0)
        if sort and isinstance(sort, MTDLMSData) and sort.logicalName == "0.0.1.1.0.255":
            _MTCommon.setData(self.settings, buff, DataType.UINT32, MTDateTime.toUnixTime(start))
            _MTCommon.setData(self.settings, buff, DataType.UINT32, MTDateTime.toUnixTime(end))
        else:
            _MTCommon.setData(self.settings, buff, DataType.OCTET_STRING, start)
            _MTCommon.setData(self.settings, buff, DataType.OCTET_STRING, end)
        buff.setUInt8(DataType.ARRAY)
        if not columns:
            buff.setUInt8(0x00)
        else:
            _MTCommon.setObjectCount(len(columns), buff)
            for it in columns:
                buff.setUInt8(DataType.STRUCTURE)
                buff.setUInt8(4)
                _MTCommon.setData(self.settings, buff, DataType.UINT16, it[0].objectType)
                _MTCommon.setData(self.settings, buff, DataType.OCTET_STRING, _MTCommon.logicalNameToBytes(it[0].logicalName))
                _MTCommon.setData(self.settings, buff, DataType.INT8, it[1].attributeIndex)
                _MTCommon.setData(self.settings, buff, DataType.UINT16, it[1].dataIndex)
        return self._read(pg.name, ObjectType.PROFILE_GENERIC, 2, buff)

    @classmethod
    def createObject(cls, type_):
        return _MTObjectFactory.createObject(type_)

    def receiverReady(self, type_):
        return MTDLMS.receiverReady(self.settings, type_)
    ...
    def getData(self, reply, data, notify=None):
        ...
        if isinstance(reply, bytearray):
            reply = MTByteBuffer(reply)
        elif isinstance(reply, bytes):
            reply = MTByteBuffer(reply)
        try:
            ret = MTDLMS.getData(self.settings, reply, data, notify)
        ...
        if ret and self.translator and data.moreData == RequestTypes.NONE:
            if data.xml is None:
                data.xml = (MTDLMSTranslatorStructure(self.translator.outputType, self.translator.omitXmlNameSpace, self.translator.hex, self.translator.showStringAsHex, self.translator.comments, self.translator.tags))
            pos = data.data.position
            try:
                data2 = data.data
                if data.command == Command.GET_RESPONSE:
                    tmp = MTByteBuffer((4 + data.data.size))
                    ...
                    tmp = MTByteBuffer((6 + data.data.size))
                    tmp.setUInt8(data.command)
                    tmp.setUInt8(GetCommandType.NORMAL)
                    tmp.setUInt8(int(data.invokeId))
                    tmp.setUInt8(0)
                    ...
                    tmp = MTByteBuffer(3 + data.data.size)
                    tmp.setUInt8(data.command)
    ...
    def accessRequest(self, time, list_):
        ...
        for it in list_:
            if it.command == AccessServiceCommandType.GET:
                ...
                if type_ == DataType.NONE:
                    raise Exception("Invalid parameter. In python value type must give.")
                _MTCommon.setData(self.settings, bb, type_, value)
            else:
                raise ValueError("Invalid command.")

        p = MTDLMSLNParameters(self.settings, 0, Command.ACCESS_REQUEST, 0xFF, None, bb, 0xff)
        if time:
            p.time = MTDateTime(time)
        return MTDLMS.getLnMessages(p)

    def parseAccessResponse(self, list_, data):
        info = _MTDataInfo()
        cnt = _MTCommon.getObjectCount(data)
        if len(list_) != cnt:
            raise ValueError("List size and values size do not match.")
        for it in list_:
            info.clear()
            it.value = _MTCommon.getData(self.settings, data, info)
        cnt = _MTCommon.getObjectCount(data)
        if len(list_) != cnt:
            raise ValueError("List size and values size do not match.")
        for it in list_:
    ...
    def getInitialConformance(cls, useLogicalNameReferencing):
        return MTDLMSSettings.getInitialConformance(useLogicalNameReferencing)

    def parseReport(self, reply, list_):
        if reply.command == Command.EVENT_NOTIFICATION:
            MTDLMSLNCommandHandler.handleEventNotification(self.settings, reply, list_)
            return None
        if reply.command == Command.INFORMATION_REPORT:
            MTDLMSSNCommandHandler.handleInformationReport(self.settings, reply, list_)
            return None
    ...
    def parsePushObjects(self, data):
        ...
        if data:
            ...
            for it in data:
                ...
                if classID > 0:
                    comp = None
                    comp = self.objects.findByLN(classID, _MTCommon.toLogicalName(tmp[1]))
                    if comp is None:
                        comp = MTDLMSClient.__createDLMSObject(classID, 0, 0, tmp[1], None)
                        self.settings.objects.append(comp)
                        c.updateOBISCodeInformation(comp)
                    if comp.__class__ != MTDLMSObject.__class__:
                        objects.append((comp, tmp[2]))
                    else:
                        print("Unknown object: " + str(classID) + " " + _MTCommon.toLogicalName(tmp[1]))
        return objects
