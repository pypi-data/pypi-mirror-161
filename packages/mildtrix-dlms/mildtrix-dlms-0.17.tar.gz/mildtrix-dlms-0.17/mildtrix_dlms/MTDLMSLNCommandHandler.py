...
class MTDLMSLNCommandHandler:
    ...
    def handleGetRequest(cls, settings, server, data, replyData, xml):
        ...
        if type_ == GetCommandType.NORMAL:
            ...
            MTDLMS.getLNPdu(MTDLMSLNParameters(settings, invokeID, Command.GET_RESPONSE, type_, None, bb, ErrorCode.OK), replyData)
        if xml:
    ...
    def handleSetRequest(cls, settings, server, data, replyData, xml):
        ...
        p = MTDLMSLNParameters(settings, invoke, Command.SET_RESPONSE, type_, None, None, 0)
        if xml:
    ...
    def appendAttributeDescriptor(cls, xml, ci, ln, attributeIndex):
        ...
        xml.appendComment(_MTCommon.toLogicalName(ln))
        xml.appendLine(TranslatorTags.INSTANCE_ID, "Value", MTByteBuffer.hex(ln, False))
    ...
    def getRequestNormal(cls, settings, invokeID, server, data, replyData, xml):
        ...
        if xml:
            ...
            if selection != 0:
                ...
                _MTCommon.getData(settings, data, info)
                xml.appendEndTag(TranslatorTags.ACCESS_PARAMETERS)
                xml.appendEndTag(TranslatorTags.ACCESS_SELECTION)
            return
        if selection != 0:
            parameters = _MTCommon.getData(settings, data, info)
        ot = ObjectType(ci)
        obj = settings.objects.findByLN(ot, _MTCommon.toLogicalName(ln))
        if obj is None:
            obj = server.onFindObject(ot, 0, _MTCommon.toLogicalName(ln))
        ...
        if obj is None:
            ...
            if server.onGetAttributeAccess(e) == AccessMode.NO_ACCESS:
                ...
                if isinstance(obj, (MTDLMSProfileGeneric,)) and attributeIndex == 2:
                    dt = None
                    rowsize = 0
                    pg = e.target
                    ...
                    for k, v in pg.captureObjects:
                        ...
                        if dt == DataType.OCTET_STRING:
                            ...
                            if dt == DataType.DATETIME:
                                rowsize += _MTCommon.getDataTypeSize(DataType.DATETIME)
                            elif dt == DataType.DATE:
                                rowsize += _MTCommon.getDataTypeSize(DataType.DATE)
                            elif dt == DataType.TIME:
                                rowsize += _MTCommon.getDataTypeSize(DataType.TIME)
                        elif dt == DataType.NONE:
                            rowsize += 2
                        else:
                            rowsize += _MTCommon.getDataTypeSize(dt)
                    if rowsize != 0:
                ...
                if e.byteArray:
                    ...
                    MTDLMS.appendData(settings, obj, attributeIndex, bb, value)
                status = e.error
        MTDLMS.getLNPdu(MTDLMSLNParameters(settings, e.invokeId, Command.GET_RESPONSE, 1, None, bb, status), replyData)
        if settings.count != settings.index or len(bb) != bb.position:
            server.setTransaction(MTDLMSLongTransaction(e, Command.GET_REQUEST, bb))
    ...
    def getRequestNextDataBlock(cls, settings, invokeID, server, data, replyData, xml, streaming):
        ...
        if not streaming:
            ...
            if index != settings.blockIndex:
                MTDLMS.getLNPdu(MTDLMSLNParameters(settings, invokeID, Command.GET_RESPONSE, 2, None, bb, ErrorCode.DATA_BLOCK_NUMBER_INVALID), replyData)
                return
        settings.increaseBlockIndex()
        p = MTDLMSLNParameters(settings, invokeID, Command.GENERAL_BLOCK_TRANSFER if streaming else Command.GET_RESPONSE, 2, None, bb, ErrorCode.OK)
                ...
                if len(bb) < settings.getMaxPduSize():
                    ...
                    for arg in server.transaction.targets:
                        ...
                        if arg.byteArray:
                            ...
                            MTDLMS.appendData(settings, arg.target, arg.index, bb, value)
                    moreData = settings.index != settings.getCount()
    ...
    def getRequestWithList(cls, settings, invokeID, server, data, replyData, xml):
        ...
        _MTCommon.setObjectCount(cnt, bb)
        list_ = list()
        if xml:
        ...
        while pos != cnt:
            ...
            if selection != 0:
                selector = data.getUInt8()
                i = _MTDataInfo()
                parameters = _MTCommon.getData(settings, data, i)
            if xml:
                ...
                obj = settings.objects.findByLN(ci, _MTCommon.toLogicalName(ln))
                if obj is None:
                    obj = server.onFindObject(ci, 0, _MTCommon.toLogicalName(ln))
        ...
        value = None
        pos = 0
        p = MTDLMSLNParameters(settings, invokeID, Command.GET_RESPONSE, 3, None, bb, 0xFF)
        for it in list_:
            try:
                if it.handled:
                    value = it.value
                ...
                if it.byteArray:
                    ...
                    MTDLMS.appendData(settings, it.target, it.index, bb, value)
                p.invokeId = it.invokeId
            except Exception:
                bb.setUInt8(ErrorCode.HARDWARE_FAULT)
            if settings.index != settings.count:
                server.setTransaction(MTDLMSLongTransaction(list_, Command.GET_REQUEST, None))
            pos += 1
    ...
    def handleSetRequestNormal(cls, settings, server, data, type_, p, xml):
        ...
        if type_ == 2:
            ...
            if blockNumber != settings.blockIndex:
                ...
                return
            settings.increaseBlockIndex()
            size = _MTCommon.getObjectCount(data)
        ...
        if xml:
            ...
            di = _MTDataInfo()
            di.xml = (xml)
            value = _MTCommon.getData(settings, data, di)
            if not di.complete:
                value = MTByteBuffer.hex(data.data, False, data.position, len(data) - data.position)
            elif isinstance(value, bytearray):
                value = MTByteBuffer.hex(value, False)
            xml.appendEndTag(TranslatorTags.VALUE)
            return
        if not p.isMultipleBlocks():
            settings.resetBlockIndex()
            value = _MTCommon.getData(settings, data, reply)
        obj = settings.objects.findByLN(ot, _MTCommon.toLogicalName(ln))
        if obj is None:
            obj = server.onFindObject(ot, 0, _MTCommon.toLogicalName(ln))
                ...
                try:
                    if isinstance(value, bytearray):
                        ...
                        if dt not in (DataType.NONE, DataType.OCTET_STRING):
                            value = _MTCommon.changeType(settings, value, dt)
                    e.value = value
                    list_ = list(e)
                    if p.isMultipleBlocks():
                        server.setTransaction(MTDLMSLongTransaction(list_, Command.GET_REQUEST, data))
    ...
    def hanleSetRequestWithDataBlock(cls, settings, server, data, p, xml):
        ...
        if xml is None and blockNumber != settings.blockIndex:
            ...
            if not p.isMultipleBlocks():
                try:
                    value = _MTCommon.getData(settings, server.getTransaction().data, reply)
                    if isinstance(value, bytearray):
    ...
    def handleMethodRequest(cls, settings, server, data, connectionInfo, replyData, xml):
        ...
        if xml:
            ...
            if type_ == ActionRequestType.NORMAL:
                ...
                if selection != 0:
                    ...
                    di = _MTDataInfo()
                    di.xml = (xml)
                    _MTCommon.getData(settings, data, di)
            ...
            if server.onGetMethodAccess(e) == MethodAccessMode.NO_ACCESS:
                ...
                if actionReply and e.error == ErrorCode.OK:
                    ...
                    if e.byteArray:
                        ...
                        _MTCommon.setData(settings, bb, _MTCommon.getDLMSDataType(actionReply), actionReply)
                else:
                    error = e.error
                    bb.setUInt8(0)
                invokeId = int(e.invokeId)
        p = MTDLMSLNParameters(settings, invokeId, Command.METHOD_RESPONSE, 1, None, bb, error)
        MTDLMS.getLNPdu(p, replyData)
        if isinstance(obj, (MTDLMSAssociationLogicalName,)) and id_ == 1:
        ...
        if e is not None and error == 0 and isinstance(obj, MTDLMSSecuritySetup) and id_ == 2:
    ...
    def handleAccessRequest(cls, settings, server, data, reply, xml):
        ...
        if len_ != 0:
            ...
            if xml is None:
                ...
                info = _MTDataInfo()
                info.type = dt
                _MTCommon.getData(settings, MTByteBuffer(tmp), info)
        cnt = _MTCommon.getObjectCount(data)
        if xml:
            xml.appendStartTag(Command.ACCESS_REQUEST)
            xml.appendLine(TranslatorTags.LONG_INVOKE_ID, "Value", xml.integerToHex(invokeId, 8))
            xml.appendLine(TranslatorTags.DATE_TIME, "Value", MTByteBuffer.hex(tmp, False))
    ...
    def handleEventNotification(cls, settings, reply, list_):
        ...
        if reply.xml:
            ...
            if reply.time:
                ...
                reply.xml.appendLine(TranslatorTags.TIME, None, MTByteBuffer.hex(tmp, False))
        ci = data.getUInt16()
        ln = bytearray(6)
        data.get(ln)
        index = data.getUInt8()
        ...
        di = _MTDataInfo()
        di.xml = reply.xml
        value = _MTCommon.getData(settings, reply.data, di)
        if reply.xml:
            reply.xml.appendEndTag(TranslatorTags.ATTRIBUTE_VALUE)
            reply.xml.appendEndTag(Command.EVENT_NOTIFICATION)
        else:
            obj = settings.objects.findByLN(ObjectType(ci), _MTCommon.toLogicalName(ln))
            if obj:
                ...
                print("InformationReport message. Unknown object : " + str(ObjectType(ci)) + " " + _MTCommon.toLogicalName(ln))
