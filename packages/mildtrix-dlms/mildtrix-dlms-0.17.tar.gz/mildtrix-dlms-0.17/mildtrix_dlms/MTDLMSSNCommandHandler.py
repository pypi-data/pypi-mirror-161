...
class MTDLMSSNCommandHandler:
    ...
    def handleRead(cls, settings, server, type_, data, list_, reads, actions, replyData, xml):
        ...
        if type_ == VariableAccessSpecification.PARAMETERISED_ACCESS:
            ...
            di = _MTDataInfo()
            e.parameters = _MTCommon.getData(settings, data, di)
    ...
    def handleReadBlockNumberAccess(cls, settings, server, data, replyData, xml):
        ...
        if blockNumber != settings.blockIndex:
            ...
            MTDLMS.getSNPdu(MTDLMSSNParameters(settings, Command.READ_RESPONSE, 1, SingleReadResponse.DATA_ACCESS_ERROR, bb, None), replyData)
        ...
        p = MTDLMSSNParameters(settings, Command.READ_RESPONSE, 1, SingleReadResponse.DATA_BLOCK_RESULT, bb, server.transaction.data)
        p.multipleBlocks = True
        MTDLMS.getSNPdu(p, replyData)
    ...
    def getReadData(cls, settings, list_, data):
        ...
        for e in list_:
            ...
            if e.error == ErrorCode.OK:
                ...
                if e.action:
                    _MTCommon.setData(settings, data, _MTCommon.getDLMSDataType(value), value)
                else:
                    MTDLMS.appendData(settings, e.target, e.index, data, value)
            else:
                if not first and list_:
    ...
    def handleReadDataBlockAccess(cls, settings, server, command, data, cnt, replyData, xml):
        ...
        if blockNumber != settings.blockIndex:
            bb.setUInt8(ErrorCode.DATA_BLOCK_NUMBER_INVALID)
            MTDLMS.getSNPdu(MTDLMSSNParameters(settings, command, 1, SingleReadResponse.DATA_ACCESS_ERROR, bb, None), replyData)
            settings.resetBlockIndex()
            return
        count = 1
        ...
        if count != 1 or type1 != DataType.OCTET_STRING or size != realSize:
            ...
            MTDLMS.getSNPdu(MTDLMSSNParameters(settings, command, cnt, SingleReadResponse.DATA_ACCESS_ERROR, bb, None), replyData)
            settings.resetBlockIndex()
            return
        if server.transaction is None:
            server.setTransaction(MTDLMSLongTransaction(None, command, data))
        else:
        ...
        if lastBlock == 0:
            ...
            MTDLMS.getSNPdu(MTDLMSSNParameters(settings, command, cnt, type2, None, bb), replyData)
            return
    ...
    def handleReadRequest(cls, settings, server, data, replyData, xml):
        ...
        p = MTDLMSSNParameters(settings, Command.READ_RESPONSE, cnt, requestType, None, bb)
        MTDLMS.getSNPdu(p, replyData)
        if server.transaction() is None and (len(bb) != bb.position or settings.count != settings.index):
            ...
            if reads:
                server.onPostRead(reads)
            server.setTransaction(MTDLMSLongTransaction(reads, Command.READ_REQUEST, bb))
    ...
    def returnSNError(cls, settings, cmd, error, replyData):
        bb = MTByteBuffer()
        bb.setUInt8(error)
        MTDLMS.getSNPdu(MTDLMSSNParameters(settings, cmd, 1, SingleReadResponse.DATA_ACCESS_ERROR, bb, None), replyData)
        settings.resetBlockIndex()
    ...
    def handleWriteRequest(cls, settings, server, data, replyData, xml):
        ...
        while pos != cnt:
            ...
            if xml:
                ...
                if not di.complete:
                    ...
                    xml.appendLine(_MTCommon.DATA_TYPE_OFFSET + di.type, "Value", str(value))
                if xml.outputType == TranslatorOutputType.STANDARD_XML:
        ...
        while pos != cnt:
            ...
            bb.setUInt8(ret)
            pos += 1
        p = MTDLMSSNParameters(settings, Command.WRITE_RESPONSE, cnt, 0xFF, None, bb)
        MTDLMS.getSNPdu(p, replyData)
    ...
    def handleInformationReport(cls, settings, reply, list_):
        ...
        if len_ != 0:
            tmp = bytearray(len_)
            data.get(tmp)
            reply.t = _MTCommon.changeType(settings, tmp, DataType.DATETIME)
        type_ = 0
        ot = TranslatorOutputType.SIMPLE_XML
        if reply.xml:
            ot = reply.xml.outputType
        count = _MTCommon.getObjectCount(reply.data)
        if reply.xml:
            ...
            if reply.time:
                ...
                if ot == TranslatorOutputType.SIMPLE_XML:
                    reply.xml.appendLine(TranslatorTags.CURRENT_TIME, None, MTByteBuffer.hex(tmp, False))
                else:
                    reply.xml.appendLine(TranslatorTags.CURRENT_TIME, None, _MTCommon.generalizedTime(reply.time))
        ...
        while pos != count:
            ...
            if reply.xml:
                ...
                v.value = _MTCommon.getData(settings, reply.data, di)
