...
class MTDLMSChipperingStream:
    ...
    def __init__(self, forSecurity, forEncrypt, blockCipherKey, forAad, iv, forTag):
        ...
        if self.encrypt:
            bufLength = MTDLMSChipperingStream.BLOCK_SIZE
        else:
            bufLength = (MTDLMSChipperingStream.BLOCK_SIZE + MTDLMSChipperingStream.TAG_SIZE)
        self.bufBlock = bytearray(bufLength)
        self.aad = forAad
        self.h = bytearray(MTDLMSChipperingStream.BLOCK_SIZE)
        if iv:
    ...
    def flushFinalBlock(self):
        ...
        if not self.encrypt:
            if not self.tagsEquals(self.tag, generatedTag):
                print(MTByteBuffer.hex(self.tag, False) + "-" + MTByteBuffer.hex(generatedTag, False))
