...
from .MTManufacturer import MTManufacturer
from .MTObisCode import MTObisCode
from .MTAuthentication import MTAuthentication
from .MTServerAddress import MTServerAddress
from .MTDLMSAttribute import MTDLMSAttribute
...
#
class MTManufacturerCollection(list):
    #
    ...
    def readManufacturerSettings(cls, manufacturers, path):
        ...
        if files:
            for it in files:
                if it.endswith(".obx"):
                    try:
                        manufacturers.append(MTManufacturerCollection.__parse(os.path.join(path, it)))
                    except Exception as e:
                        print(e)
                        continue
    ...
    def __parse(cls, file):
        ...
        for it in ET.parse(file).iter():
            if it.tag == "MTManufacturer":
                man = MTManufacturer()
            elif it.tag == "MTObisCode":
                obisCode = MTObisCode()
                man.obisCodes.append(obisCode)
            elif it.tag == "MTAuthentication":
                authentication = MTAuthentication()
                man.settings.append(authentication)
            elif it.tag == "MTServerAddress":
                serveraddress = MTServerAddress()
                man.serverSettings.append(serveraddress)
            elif it.tag == "MTDLMSAttributeSettings":
                att = MTDLMSAttribute()
