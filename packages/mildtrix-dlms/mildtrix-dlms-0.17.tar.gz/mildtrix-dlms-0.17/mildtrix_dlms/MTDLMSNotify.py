...
class MTDLMSNotify(object):
    ...
    def getData(self, reply, data):
        return MTDLMS.getData(self.settings, reply, data, None)
    ...
    def generateReport(self, time, list_):
        ...
        if self.useLogicalNameReferencing:
            for k, v in list_:
                ...
                buff.set(_MTCommon.logicalNameToBytes(k.logicalName))
                buff.setUInt8(v)
                self.addData(k, v, buff)
            p = MTDLMSLNParameters(self.settings, 0, Command.EVENT_NOTIFICATION, 0, None, buff, 0xff)
            p.time = time
            reply = MTDLMS.getLnMessages(p)
        else:
            p = MTDLMSSNParameters(self.settings, Command.INFORMATION_REPORT, len(list_), 0xFF, None, buff)
            for k, v in list_:
                buff.setUInt8(VariableAccessSpecification.VARIABLE_NAME)
                sn = k.shortName
                sn += (v - 1) * 8
                buff.setUInt16(sn)
            _MTCommon.setObjectCount(len(list_), buff)
            for k, v in list_:
                self.addData(k, v, buff)
