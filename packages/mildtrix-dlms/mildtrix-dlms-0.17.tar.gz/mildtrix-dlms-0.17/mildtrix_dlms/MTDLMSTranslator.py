...
class MTDLMSTranslator:
    ...
    def findNextFrame(self, msg, pdu):
        if not isinstance(msg, (MTDLMSTranslatorMessage)):
            if not isinstance(msg, (MTByteBuffer)):
                data = MTByteBuffer(msg)
            else:
                data = msg
            msg = MTDLMSTranslatorMessage()
            msg.message = data
        msg.sourceAddress = msg.targetAddress = 0
        original = msg.message.position
        msg.exception = None
        data = msg.message
        settings = MTDLMSSettings(True)
        reply = MTReplyData()
        reply.moreData = msg.moreData
        reply.xml = (MTDLMSTranslatorStructure(self.outputType, self.omitXmlNameSpace, self.hex, self.showStringAsHex, self.comments, self.tags))
        pos = 0
        found = False
        while data.position < len(data):
            if msg.interfaceType in (None, InterfaceType.HDLC, InterfaceType.HDLC_WITH_MODE_E) and data.getUInt8(data.position) == 0x7e:
                ...
                found = MTDLMS.getData(settings, data, reply, None)
                data.position = pos
                if found:
                    break
                ...
                if found:
                    break
            elif msg.interfaceType in (None, InterfaceType.WIRED_MBUS) and MTDLMS.isWiredMBusData(msg.message):
                pos = data.position
                ...
                if found:
                    break
            elif msg.interfaceType in (None, InterfaceType.WIRELESS_MBUS) and MTDLMS.isWirelessMBusData(msg.message):
                pos = data.position
    ...
    def getPdu(self, value):
        return self.getPdu(MTByteBuffer(value))
    ...
    def getDlmsFraming(cls, value):
        ...
        while pos != len(value):
            ...
            if MTDLMS.isWirelessMBusData(value):
    ...
    def getPdu_0(self, value):
        ...
        data = MTReplyData()
        data.xml = (MTDLMSTranslatorStructure(self.outputType, self.omitXmlNameSpace, self.hex, self.showStringAsHex, self.comments, self.tags))
        settings = MTDLMSSettings(True)
        settings.interfaceType = framing
        MTDLMS.getData(settings, value, data, None)
        return data.data

    def getCiphering(self, settings, force):
        if force or self.security != Security.NONE:
            c = MTCiphering(self.systemTitle)
    ...
    def messageToXml(self, msg):
        ...
        msg.exception = None
        xml = MTDLMSTranslatorStructure(self.outputType, self.omitXmlNameSpace, self.hex, self.showStringAsHex, self.comments, self.tags)
        data = MTReplyData()
        settings = MTDLMSSettings(True)
        ...
        try:
            ...
            if msg.interfaceType in (None, InterfaceType.HDLC, InterfaceType.HDLC_WITH_MODE_E) and msg.message.getUInt8(msg.message.position) == 0x7e:
                ...
                if MTDLMS.getData(settings, msg.message, data, None):
            ...
            if msg.interfaceType in (None, InterfaceType.WRAPPER) and (msg.message.available() > 1) and msg.message.getUInt16(msg.message.position) == 1:
                ...
                MTDLMS.getData(settings, msg.message, data, None)
                msg.moreData = data.moreData
                msg.sourceAddress = data.sourceAddress
                msg.targetAddress = data.targetAddress
            ...
            if msg.interfaceType in (None, InterfaceType.PLC) and msg.message.getUInt8(msg.message.position) == 2:
                ...
                return msg.xml
            #If Wired M-Bus.
            if msg.interfaceType in (None, InterfaceType.WIRED_MBUS) and MTDLMS.isWiredMBusData(msg.message):
                ...
                return msg.xml
            #If Wireless M-Bus.
            if msg.interfaceType in (None, InterfaceType.WIRELESS_MBUS) and MTDLMS.isWirelessMBusData(msg.message):
    ...
    #
    # @param pdu
    #            Converted hex string or MTByteBuffer.
    # Converted XML.
    #
    def pduToXml(self, pdu):
        if not isinstance(pdu, MTByteBuffer):
            pdu = MTByteBuffer(pdu)
    ...
    def getUa(cls, data, xml):
        ...
        while data.position < len(data):
            ...
            if len_ == 1:
                ...
                raise MTDLMSException("Invalid Exception.")
            if id_ == _HDLCInfo.MAX_INFO_TX:
                ...
                raise MTDLMSException("Invalid UA response.")
    ...
    def _pduToXml2(self, xml, value, omitDeclaration, omitNameSpace, allowUnknownCommand=True):
        ...
        if cmd == Command.AARQ:
            value.position = 0
            _MTAPDU.parsePDU(settings, settings.cipher, value, xml)
        elif cmd == Command.INITIATE_REQUEST:
            value.position = 0
            settings = MTDLMSSettings(True)
            _MTAPDU.parseInitiate(True, settings, settings.cipher, value, xml)
        elif cmd == Command.INITIATE_RESPONSE:
            value.position = 0
            settings = MTDLMSSettings(False)
            self.getCiphering(settings, True)
            _MTAPDU.parseInitiate(True, settings, settings.cipher, value, xml)
        elif cmd == 0x81:
            #  Ua
            ...
            MTDLMSLNCommandHandler.handleGetRequest(settings, None, value, None, xml)
        elif cmd == Command.SET_REQUEST:
            MTDLMSLNCommandHandler.handleSetRequest(settings, None, value, None, xml)
        elif cmd == Command.READ_REQUEST:
            MTDLMSSNCommandHandler.handleReadRequest(settings, None, value, None, xml)
        elif cmd == Command.METHOD_REQUEST:
            MTDLMSLNCommandHandler.handleMethodRequest(settings, None, value, None, None, xml)
        elif cmd == Command.WRITE_REQUEST:
            MTDLMSSNCommandHandler.handleWriteRequest(settings, None, value, None, xml)
        elif cmd == Command.ACCESS_REQUEST:
            MTDLMSLNCommandHandler.handleAccessRequest(settings, None, value, None, xml)
            ...
            data.xml = (xml)
            data.data = value
            MTDLMSSNCommandHandler.handleInformationReport(settings, data, None)
        elif cmd == Command.EVENT_NOTIFICATION:
            data.xml = (xml)
            data.data = value
            MTDLMSLNCommandHandler.handleEventNotification(settings, data, None)
            ...
            settings.cipher = MTCiphering("ABCDEFGH".encode())
            data.xml = (xml)
            data.data = value
            value.position = 0
            MTDLMS.getPdu(settings, data)
            ...
            if value.available() != 0:
                ...
                if value.available() != 0:
                    _MTAPDU.parsePDU2(settings, settings.cipher, value, xml)
            ...
            if settings.cipher and self.comments:
                ...
                try:
                    ...
                    if st or cmd in (Command.GENERAL_GLO_CIPHERING, Command.GENERAL_DED_CIPHERING):
                        ...
                        if p.blockCipherKey:
                            data2 = MTByteBuffer(MTCiphering.decrypt(settings.cipher, p, value))
            ...
            cnt = _MTCommon.getObjectCount(value)
            ...
            if settings.cipher and self.comments:
                ...
                try:
                    ...
                    p.xml = xml
                    tmp = MTByteBuffer(MTCiphering.decrypt(settings.cipher, p, tmp))
                    len_ = xml.getXmlLength()
                    ...
                    xml.setXmlLength(len_)
                value.position = originalPosition
            len_ = _MTCommon.getObjectCount(value)
            tmp = bytearray(len_)
            value.get(tmp)
            xml.appendStartTag(Command.GENERAL_GLO_CIPHERING)
            xml.appendLine(TranslatorTags.SYSTEM_TITLE, None, MTByteBuffer.hex(tmp, False, 0, len_))
            len_ = _MTCommon.getObjectCount(value)
            tmp = bytearray(len_)
            value.get(tmp)
            xml.appendLine(TranslatorTags.CIPHERED_SERVICE, None, MTByteBuffer.hex(tmp, False, 0, len_))
            ...
            xml.appendLine(TranslatorTags.PHYSICAL_DEVICE_ADDRESS, None, MTByteBuffer.hex(tmp, False, 0, len_))
            self._pduToXml2(xml, MTByteBuffer(value.remaining()), omitDeclaration, omitNameSpace, allowUnknownCommand)
            xml.appendEndTag(cmd)
    ...
    def getCommand(cls, node, s, tag):
        ...
        if tag in (Command.SNRM, Command.AARQ, Command.READ_REQUEST, Command.WRITE_REQUEST,\
           ...
           Command.DED_GET_REQUEST, Command.DED_SET_REQUEST, Command.DED_METHOD_REQUEST):
            s.settings.server = False
            tmp = MTByteBuffer.hexToBytes(cls.getValue(node, s))
    ...
    def handleAarqAare(cls, node, s, tag):
        ...
        if tag == TranslatorGeneralTags.APPLICATION_CONTEXT_NAME:
            ...
            if s.outputType == TranslatorOutputType.STANDARD_XML:
                ...
                _MTAPDU.parseInitiate(False, s.settings, s.settings.getCipher(), bb, None)
            ...
            if s.settings.authentication == Authentication.LOW:
                s.settings.password = MTByteBuffer.hexToBytes(cls.getValue(node, s))
            else:
                s.settings.setCtoSChallenge(MTByteBuffer.hexToBytes(cls.getValue(node, s)))
            ...
            s.settings.setStoCChallenge(MTByteBuffer.hexToBytes(cls.getValue(node, s)))
        elif tag == 0xBE03:
            pass
        elif tag == 0xBE05:
            if s.settings.isServer:
            ...
            _MTCommon.setBitString(pv, str_, False)
    ...
    def readNode(cls, node, s):
        ...
        if s.command == Command.NONE:
            ...
            if not cls.handleAarqAare(node, s, tag):
                return
        elif tag >= _MTCommon.DATA_TYPE_OFFSET:
            if tag == DataType.DATETIME + _MTCommon.DATA_TYPE_OFFSET or (s.command == Command.EVENT_NOTIFICATION and not s.attributeDescriptor):
            ...
            if tag in (Command.GET_REQUEST << 8 | GetCommandType.NORMAL,
                ...
                if s.command == Command.WRITE_REQUEST:
                    _MTCommon.setObjectCount(cls.getNodeCount(node), s.data)
                ...
                if s.outputType == TranslatorOutputType.SIMPLE_XML:
                    ...
                    str_ = cls.getValue(node, s)
                    s.setTime(MTDateTime(_MTCommon.getGeneralizedTime(str_)))
                ...
                s.settings.stoCChallenge = MTByteBuffer.hexToBytes(cls.getValue(node, s))
                ...
                s.attributeDescriptor.set(MTByteBuffer.hexToBytes(cls.getValue(node, s)))
                ...
                if s.command == Command.GET_RESPONSE:
                    s.data.setUInt8(0)
                tmp = MTByteBuffer.hexToBytes(cls.getValue(node, s))
                _MTCommon.setObjectCount(len(tmp), s.data)
                s.data.set(tmp)
                ...
                if s.command == Command.GET_REQUEST or s.requestType == 3:
                    _MTCommon.setObjectCount(node.getChildNodes().getLength(), s.attributeDescriptor)
                ...
                _MTCommon.setObjectCount(cls.getNodeCount(node), s.attributeDescriptor)
                ...
                if s.command == Command.GENERAL_CIPHERING:
                    ...
                    _MTCommon.setObjectCount(4 + len(tmp), s.data)
                    s.data.setUInt8(4)
                    _MTCommon.setObjectCount(2 + len(tmp), s.data)
                ...
                s.data.set(MTByteBuffer.hexToBytes(cls.getValue(node, s)))
                ...
                s.setPhysicalDeviceAddress(MTByteBuffer.hexToBytes(cls.getValue(node, s)))
        ...
        for node2 in node:
            ...
            cnt += 1
        if preData:
            _MTCommon.setObjectCount(cnt, preData)
            preData.set(s.data)
            s.data.size = 0
            s.data.set(preData)
            ...
            for r in str_.split("\n"):
                if r.strip() and not r == "\r":
                    ...
                    for it in r.strip().split(";"):
                        tmp.clear()
                        _MTCommon.setData(None, tmp, types[len(types)], MTDLMSConverter.changeType(it, types[len(types)]))
                        if len(tmp) == 1:
                            s.data.setUInt8(0)
                        else:
                        ...
                        col += 1
            row += 1
        _MTCommon.setObjectCount(len(tmp2), s.data)
        s.data.set(tmp2)
    ...
    def getNodeTypes(cls, s, node):
        ...
        if len1 > 1:
            ...
            _MTCommon.setObjectCount(len, s.data)
        for node2 in node:
            if s.outputType == TranslatorOutputType.SIMPLE_XML:
                ...
                str_ = node2.tag
            tag = s.tags.get(str_)
            s.data.setUInt8(tag - _MTCommon.DATA_TYPE_OFFSET)

    @classmethod
    def updateDateTime(cls, node, s, preData):
        bb = preData
        if s.requestType != 0xFF:
            bb = cls.updateDataType(node, s, DataType.DATETIME + _MTCommon.DATA_TYPE_OFFSET)
        else:
            ...
            if tmp:
                if len(tmp) == 5:
                    ...
                    dt = DataType.TIME
                s.setTime(_MTCommon.changeType(None, tmp, dt))
        return bb
    ...
    def updateDataType(cls, node, s, tag):
        ...
        if s.template or v == "*":
            s.template = True
            return preData
        dt = DataType(tag - _MTCommon.DATA_TYPE_OFFSET)
        if dt == DataType.ARRAY:
            s.data.setUInt8(DataType.ARRAY)
            preData = MTByteBuffer(s.data)
            s.data.size = 0
        elif dt == DataType.BCD:
            _MTCommon.setData(None, s.data, DataType.BCD, s.parseShort(cls.getValue(node, s)))
        elif dt == DataType.BITSTRING:
            _MTCommon.setData(None, s.data, DataType.BITSTRING, cls.getValue(node, s))
        elif dt == DataType.BOOLEAN:
            _MTCommon.setData(None, s.data, DataType.BOOLEAN, s.parseShort(cls.getValue(node, s)))
        elif dt == DataType.DATE:
            _MTCommon.setData(None, s.data, DataType.DATE, _MTCommon.changeType(None, MTByteBuffer.hexToBytes(cls.getValue(node, s)), DataType.DATE))
            ...
            if len(tmp) == 5:
                ...
                dt = DataType.TIME
            _MTCommon.setData(None, s.data, dt, _MTCommon.changeType(None, tmp, dt))
        elif dt == DataType.ENUM:
            _MTCommon.setData(None, s.data, DataType.ENUM, s.parseShort(cls.getValue(node, s)))
        elif dt == DataType.FLOAT32:
            cls.getFloat32(node, s)
        elif dt == DataType.FLOAT64:
            cls.getFloat64(node, s)
        elif dt == DataType.INT16:
            _MTCommon.setData(None, s.data, DataType.INT16, s.parseShort(cls.getValue(node, s)))
        elif dt == DataType.INT32:
            _MTCommon.setData(None, s.data, DataType.INT32, s.parseInt(cls.getValue(node, s)))
        elif dt == DataType.INT64:
            _MTCommon.setData(None, s.data, DataType.INT64, s.parseLong(cls.getValue(node, s)))
        elif dt == DataType.INT8:
            _MTCommon.setData(None, s.data, DataType.INT8, s.parseShort(cls.getValue(node, s)))
        elif dt == DataType.NONE:
            _MTCommon.setData(None, s.data, DataType.NONE, None)
        elif dt == DataType.OCTET_STRING:
            cls.getOctetString(node, s)
        elif dt == DataType.STRING:
            if s.showStringAsHex:
                _MTCommon.setData(None, s.data, DataType.STRING, MTByteBuffer.hexToBytes(cls.getValue(node, s)))
            else:
                _MTCommon.setData(None, s.data, DataType.STRING, cls.getValue(node, s))
        elif dt == DataType.STRING_UTF8:
            if s.showStringAsHex:
                _MTCommon.setData(None, s.data, DataType.STRING_UTF8, MTByteBuffer.hexToBytes(cls.getValue(node, s)))
            else:
                _MTCommon.setData(None, s.data, DataType.STRING_UTF8, cls.getValue(node, s))
        elif dt == DataType.STRUCTURE:
            s.data.setUInt8(DataType.STRUCTURE)
            preData = MTByteBuffer(s.data)
            s.data.size = 0
        elif dt == DataType.TIME:
            _MTCommon.setData(None, s.data, DataType.TIME, _MTCommon.changeType(None, MTByteBuffer.hexToBytes(cls.getValue(node, s)), DataType.TIME))
        elif dt == DataType.UINT16:
            _MTCommon.setData(None, s.data, DataType.UINT16, s.parseShort(cls.getValue(node, s)))
        elif dt == DataType.UINT32:
            _MTCommon.setData(None, s.data, DataType.UINT32, s.parseLong(cls.getValue(node, s)))
        elif dt == DataType.UINT64:
            _MTCommon.setData(None, s.data, DataType.UINT64, s.parseLong(cls.getValue(node, s)))
        elif dt == DataType.UINT8:
            _MTCommon.setData(None, s.data, DataType.UINT8, s.parseShort(cls.getValue(node, s)))
    ...
    def getOctetString(cls, node, s):
        ...
        _MTCommon.setData(None, s.data, DataType.OCTET_STRING, bb.array())

    @classmethod
    def getFloat32(cls, node, s):
        bb = MTByteBuffer()
        bb.setHexString(cls.getValue(node, s))
        _MTCommon.setData(None, s.data, DataType.FLOAT32, bb.getFloat())

    @classmethod
    def getFloat64(cls, node, s):
        bb = MTByteBuffer()
        bb.setHexString(cls.getValue(node, s))
        _MTCommon.setData(None, s.data, DataType.FLOAT64, bb.getDouble())

    def xmlToHexPdu(self, xml, addSpace=False):
        return MTByteBuffer.hex(self.xmlToPdu(xml, None), addSpace)

    def xmlToPdu(self, xml, settings=None):
        s = settings
        if s is None:
            s = MTDLMSXmlSettings(self.outputType, self.hex, self.showStringAsHex, self.tagsByName)
        ...
        if s.command == Command.INITIATE_REQUEST:
            _MTAPDU.getInitiateRequest(s.settings, bb)
        elif s.command == Command.INITIATE_RESPONSE:
            bb.set(_MTAPDU.getUserInformation(s.settings, s.settings.cipher))
        elif s.command in (Command.READ_REQUEST, Command.WRITE_REQUEST, Command.READ_RESPONSE, Command.WRITE_RESPONSE):
            sn = MTDLMSSNParameters(s.settings, s.command, s.count, s.requestType, s.attributeDescriptor, s.data)
            MTDLMS.getSNPdu(sn, bb)
            ...
            ln = MTDLMSLNParameters(s.settings, 0, s.command, s.requestType, s.attributeDescriptor, s.data, 0xff)
            MTDLMS.getLNPdu(ln, bb)
            ...
            _MTCommon.setObjectCount(len(s.data), bb)
            bb.set(s.data)
            ...
            if s.data:
                ...
                s.data.set(bb)
                bb.clear()
            bb.set(MTDLMS.getHdlcFrame(s.settings, int(Command.SNRM), s.data))
            ...
            if s.data:
                ...
                s.data.set(bb)
                bb.clear()
            bb.set(MTDLMS.getHdlcFrame(s.settings, s.command, s.data))
        elif s.command in (Command.AARQ, Command.GLO_INITIATE_REQUEST):
            _MTAPDU.generateAarq(s.settings, s.settings.cipher, s.data, bb)
        elif s.command in (Command.AARE, Command.GLO_INITIATE_RESPONSE):
            _MTAPDU.generateAARE(s.settings, bb, s.result, s.diagnostic, s.settings.cipher, s.attributeDescriptor, s.data)
            ...
            ln = MTDLMSLNParameters(s.settings, 0, s.command, s.requestType, s.attributeDescriptor, s.data, 0xff, Command.NONE)
            MTDLMS.getLNPdu(ln, bb)
            ...
            bb.setUInt8(s.command)
            _MTCommon.setObjectCount(len(s.settings.sourceSystemTitle), bb)
        ...
        if s.physicalDeviceAddress:
            ...
            _MTCommon.setObjectCount(len(s.physicalDeviceAddress), bb2)
    ...
    def dataToXml(self, data):
        ...
        xml = MTDLMSTranslatorStructure(self.outputType, self.omitXmlNameSpace, self.hex, self.showStringAsHex, self.comments, self.tags)
        di.xml = xml
        _MTCommon.getData(None, data, di)
        return str(di.xml)
    ...
    def __getAllDataNodes(self, nodes, s):
        ...
        for it in nodes:
            ...
            if tag == TranslatorTags.RAW_DATA:
                ...
                if preData:
                    _MTCommon.setObjectCount(it.getChildNodes().getLength(), preData)
                    preData.set(s.data)
                    s.data.size(0)
                    s.data.set(preData)
