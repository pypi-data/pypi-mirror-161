...
from .manufacturersettings.MTObisCode import MTObisCode

###Python 2 requires this
#pylint: disable=bad-option-value,old-style-class
class MTDLMSConverter:
    #
    # Constructor.
    #
    # value: Used standard.
    ...
    def __init__(self, value=Standard.DLMS):
        ...
        self.codes = MTStandardObisCodeCollection()
        ...
        if code_:
            ...
            if not code_.uiDataType:
                if "10" in code_.dataType:
                    ...
                    if (MTStandardObisCodeCollection.equalsMask2("0.0-64.96.7.10-14.255", ln) or MTStandardObisCodeCollection.equalsMask2("0.0-64.0.1.5.0-99,255", ln) or MTStandardObisCodeCollection.equalsMask2("0.0-64.0.1.2.0-99,255", ln) or MTStandardObisCodeCollection.equalsMask2("1.0-64.0.1.2.0-99,255", ln) or MTStandardObisCodeCollection.equalsMask2("1.0-64.0.1.5.0-99,255", ln) or MTStandardObisCodeCollection.equalsMask2("1.0-64.0.9.0.255", ln) or MTStandardObisCodeCollection.equalsMask2("1.0-64.0.9.6.255", ln) or MTStandardObisCodeCollection.equalsMask2("1.0-64.0.9.7.255", ln) or MTStandardObisCodeCollection.equalsMask2("1.0-64.0.9.13.255", ln) or MTStandardObisCodeCollection.equalsMask2("1.0-64.0.9.14.255", ln) or MTStandardObisCodeCollection.equalsMask2("1.0-64.0.9.15.255", ln)):
                        code_.uiDataType = "25"
                    #Local time
                    elif MTStandardObisCodeCollection.equalsMask2("1.0-64.0.9.1.255", ln):
                        code_.uiDataType = "27"
                    #Local date
                    elif MTStandardObisCodeCollection.equalsMask2("1.0-64.0.9.2.255", ln):
                        code_.uiDataType = "26"
                    #Active firmware identifier
                    elif MTStandardObisCodeCollection.equalsMask2("1.0.0.2.0.255", ln):
                        code_.uiDataType = "10"
                #Unix time
                elif it.objectType == ObjectType.DATA and MTStandardObisCodeCollection.equalsMask2("0.0.1.1.0.255", it.logicalName):
                    code_.uiDataType = "25"
    ...
    def __getObjects(cls, standard):
        ...
        for it in rows:
            if it and not it.startswith("#"):
                ...
                if len(items) > 1:
                    ot = int(items[0])
                    ln = _MTCommon.toLogicalName(_MTCommon.logicalNameToBytes(items[1]))
                    version = int(items[2])
                    desc = items[3]
                    code_ = MTObisCode(ln, ot, 0, desc)
    ...
    def __readStandardObisInfo(cls, standard, codes):
        if standard != Standard.DLMS:
            for it in MTDLMSConverter.__getObjects(standard):
                tmp = MTStandardObisCode(it.logicalName.split('.'))
        ...
        for it in rows:
            if it and not it.startswith("#"):
                ...
                obis = items[0].split('.')
                try:
                    code_ = MTStandardObisCode(obis, str(items[3]) + "; " + str(items[4]) + "; " + str(items[5]) + "; " + str(items[6]) + "; " + str(items[7]), str(items[1]), str(items[2]))
                    codes.append(code_)
    ...
    def changeType(cls, value, type_):
        if _MTCommon.getDLMSDataType(value) == type_:
            return value
        ...
        if type_ == DataType.BCD:
            ...
            if isinstance(value, str):
                ret = MTByteBuffer.hexToBytes(str(value))
            else:
