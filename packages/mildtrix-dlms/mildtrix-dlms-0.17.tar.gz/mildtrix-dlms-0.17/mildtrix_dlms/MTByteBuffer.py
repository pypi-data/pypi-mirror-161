...
class MTByteBuffer(__base):
    ...
    def set(self, value, index=None, count=None):
        ...
        if value:
            if index is None:
                if isinstance(value, MTByteBuffer):
    ...
    def ___getValue(cls, c):
        ...
        if c.islower():
            c = c.upper()
        pos = MTByteBuffer.__HEX_ARRAY.find(c)
        if pos == -1:
    ...
    def hexToBytes(cls, value):
        ...
        if value:
            ...
            for ch in value:
                if ch != ' ':
                    if lastValue == -1:
                        lastValue = MTByteBuffer.___getValue(ch)
                    elif lastValue != -1:
                        buff.append(lastValue << MTByteBuffer.__NIBBLE | MTByteBuffer.___getValue(ch))
                        lastValue = -1
                elif lastValue != -1:
                    buff.append(MTByteBuffer.___getValue(ch))
                    lastValue = -1
        return buff
    ...
    def hex(cls, value, addSpace=True, index=0, count=None):
        ...
        for it in value[index:index+count]:
            hexChars += MTByteBuffer.__HEX_ARRAY[it >> MTByteBuffer.__NIBBLE]
            hexChars += MTByteBuffer.__HEX_ARRAY[it & MTByteBuffer.__LOW_BYTE_PART]
            if addSpace:
                hexChars += ' '
