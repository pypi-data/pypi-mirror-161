...
from .objects.MTDLMSObjectCollection import MTDLMSObjectCollection
...
class MTDLMSSettings:
    ...
    def __init__(self, isServer):
        ...
        self.objects = MTDLMSObjectCollection()
        self.hdlc = MTHdlcSettings()
        self.gateway = None
        self.proposedConformance = MTDLMSSettings.getInitialConformance(self.__useLogicalNameReferencing)
    ...
    def checkFrame(self, frame_, xml):
        ...
        if (self.senderFrame & 0x1) == 0:
            expected = MTDLMSSettings.increaseReceiverSequence(MTDLMSSettings.increaseSendSequence(self.receiverFrame))
            if frame_ == expected:
