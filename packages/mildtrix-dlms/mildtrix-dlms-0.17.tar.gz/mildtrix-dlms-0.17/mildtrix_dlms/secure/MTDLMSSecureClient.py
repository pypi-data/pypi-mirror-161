...
class MTDLMSSecureClient(MTDLMSClient):
    """
    MTDLMSSecureClient implements secure client where all messages are secured
    ...
    def __init__(self, useLogicalNameReferencing=False, clientAddress=16, serverAddress=1, forAuthentication=Authentication.NONE, password=None, interfaceType=InterfaceType.HDLC):
        MTDLMSClient.__init__(self, useLogicalNameReferencing, clientAddress, serverAddress, forAuthentication, password, interfaceType)
        # Ciphering settings.
        self.settings.cipher = MTCiphering("ABCDEFGH".encode())
    ...
    def encrypt(cls, kek, data):
        ...
        gcm = MTDLMSChipperingStream(None, True, kek, None, None, 0)
    ...
    def decrypt(cls, kek, data):
        ...
        if not data:
            raise ValueError("data")
        gcm = MTDLMSChipperingStream(None, False, kek, None, None, 0)
