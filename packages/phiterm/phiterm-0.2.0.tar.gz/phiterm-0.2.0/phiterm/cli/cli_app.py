...
cli_app = typer.Typer(
    help="""\b
phidata is a toolkit for building data products.
\b
Get started:
1. Run `phi init` to initialize phidata
2. Run `phi ws init` to create a new workspace
3. Run `phi ws setup` setup an existing workspace
4. Run `phi ws up` to deploy the workspace
5. Run `phi ws down` to shutdown the workspace
""",
