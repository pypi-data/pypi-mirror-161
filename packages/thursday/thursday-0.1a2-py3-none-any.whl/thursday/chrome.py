...
class ChromeDriver():
    """
    ChromeDriver provides a selenium webdriver and action chains for the current
    installation of Google Chrome as well as a set of generic functions for common
    actions such as locating elements and user input.
    ...
    def _download_chromedriver(self):
        if not path.exists(self._chromedriver_exe):
            ...
            with requests.get(f"https://chromedriver.storage.googleapis.com/{latest_release}/chromedriver_win32.zip", stream=True) as download:
                ...
                with open(path.join(self._chromedriver_zip), "wb") as chromedriver_zip:
    ...
    def _get_version_number(self):
        for bit_version in GoogleChrome:
            version = check_output(bit_version.value, shell=True).decode("UTF-8")
            if bool(version):
                try:
                    version = search("([0-9]+.){3}[0-9]+", version)[0].split(".")
    ...
    def _setup_chromedriver(self):
        ...
        self._chromedriver_zip = path.join(getcwd(), "chromedriver_win32.zip")
        ...
        try:
            ...
            remove(self._chromedriver_exe)
            self._setup_chromedriver()
        else:
            return webdriver

    def _unzip_chromedriver(self):
        with ZipFile(self._chromedriver_zip, "r") as zip_file:
            zip_file.extractall(getcwd())
        remove(self._chromedriver_zip)