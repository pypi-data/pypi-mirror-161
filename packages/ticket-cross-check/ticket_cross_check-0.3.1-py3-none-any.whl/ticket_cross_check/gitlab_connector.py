...
class GitlabConnector:
    ...
    def _get(self, url: str, expect: type) -> list | dict:
        """
        ...
        json = result.json()
        if not isinstance(json, expect):
            raise ImportError(f"Result is not of type {type} it's {type(json)}")
