...
class DSLibrary(object):
    ...
    def open_resource(self, resource_name: str, mode: str='rb', **kwargs) -> io.RawIOBase:
        ...
        if "root" in kwargs:
            ...
            uri = os.path.join(kwargs.get("root"), uri)
    ...
    def sql_select(self, sql: str, parameters: list=None, engine: str=None, data=None, table_loader: callable=None):
        """
        ...
        if not n_sources:
            ...
            def default_table_loader(table_spec: tuple):
                return self.load_dataframe(table_spec[-1])
            table_loader = default_table_loader
        if engine:
            return self.load_dataframe(engine, sql=sql, sql_parameters=parameters)
