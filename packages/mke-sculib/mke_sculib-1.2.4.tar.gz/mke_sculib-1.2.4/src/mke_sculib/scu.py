...
class scu():
    ...
    def run_track_table(self, t=None, az=None, el=None, df=None, columns=['time', 'az', 'el'], activate_logging=True):
        ...
        self.wait_for_pos('az', az[-1], tolerance=0.01)
        self.wait_for_pos('el', el[-1], tolerance=0.01)
        self.stop_logger()
        self.wait_duration(5)
    ...
    def upload_track_table(self, t=None, az=None, el=None, df=None, columns=['time', 'az', 'el'], wait_for_start=False):
        ...
        print('Running Table ({}, {:5.3f}, {:5.3f}) --> ({}, {:5.3f}, {:5.3f})'.format(Time(t[0], format='mjd').datetime, az[0], el[0], Time(t[-1], format='mjd').datetime, az[-1], el[-1]))
    
        self.acu_ska_track(self.format_body(t, az, el))
        if wait_for_start:
            self.wait_track_start()

        
        return Time(t[-1], format='mjd').unix - self.t_internal
