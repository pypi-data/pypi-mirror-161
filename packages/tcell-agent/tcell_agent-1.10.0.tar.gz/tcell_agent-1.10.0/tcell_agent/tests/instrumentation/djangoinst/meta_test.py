...
class InitializeTcellContextTest(unittest.TestCase):
    def test_initialize_tcell_context(self):
        ...
        with patch("tcell_agent.instrumentation.djangoinst.meta.get_remote_address",
                   return_value="1.1.1.1") as patched_get_remote_address:
            with patch("tcell_agent.instrumentation.djangoinst.meta.get_reverse_proxy_header",
                       return_value="2.2.2.2") as patched_get_reverse_proxy_header:
                initialize_tcell_context(request)
                tcell_context = request._tcell_context
                self.assertTrue(patched_get_remote_address.called)
                self.assertTrue(patched_get_reverse_proxy_header.called)
                ...
                self.assertIsNotNone(tcell_context.reverse_proxy_header_value, "2.2.2.2")
