...
if DJANGO_VERSION[:2] not in [(1, 5), (1, 6)]:
    ...
    class DLPMiddlewareTest(TestCase):
        def setUp(self):
            ...
            self.grm = GlobalRequestMiddleware(get_response)
            self.aam = AfterAuthMiddleware(get_response)
            self.bfm = BodyFilterMiddleware(get_response)
            self.data_exposure_middleware = TCellDataExposureMiddleware(get_response)
            self.tcell_last_middleware = TCellLastMiddleware(get_response)

            if get_version().split(".")[0] == "1":
                self.cm = CommonMiddleware()
            else:
                self.cm = CommonMiddleware(get_response)
