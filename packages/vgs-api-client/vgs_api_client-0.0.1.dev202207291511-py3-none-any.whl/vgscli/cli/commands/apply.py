...
def apply_service_account(ctx: click.Context, org_id: ResourceId, file) -> None:
    ...
    """
    data = validate_yaml(file, "validation/service-account-schema.yaml")["data"]
...
def apply_vault(ctx: click.Context, org_id: Optional[ResourceId], file) -> None:
    ...
    """
    data = validate_yaml(file, "validation/vault-schema.yaml")["data"]
    ...
    response = account_mgmt.vaults.create_or_update(
        body={
            "data": {
                "attributes": {"name": data["name"], "environment": data["environment"]},
                "type": "vaults",
                "relationships": {
                    "organization": {
