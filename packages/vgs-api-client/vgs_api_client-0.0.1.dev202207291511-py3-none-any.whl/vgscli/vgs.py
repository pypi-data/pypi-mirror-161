...
from vgscli.accounts_api import (
    create_api as create_accounts_api,
...
@click.option("--vault", "-V", help="Vault ID", required=True)
@click.option(
    "--filename", "-f", help="Filename for the input data", type=click.File("r"), required=True
)
@click.pass_context
@handle_errors()
def apply_routes(ctx, vault, filename):
    ...
    sync_all_routes(
        vault_management_api, route_data, lambda route_id: click.echo(f"Route {route_id} processed")
    )
...
@click.option(
    "--template",
    "-t",
    type=click.Choice(["vgs-cli", "calm", "checkout", "sub-account-checkout", "payments-admin"]),
    help="Predefined service account template configuration",
    required=True,
)
@click.option(
    "--var", "variables", type=VariableParamType(), multiple=True, help="Template variables."
)
@click.pass_context
@handle_errors()
def generate_service_account(ctx, template, variables):
    ...
    environment = Environment(
        loader=PackageLoader(package_name="vgscli", package_path="templates/serviceaccount"),
