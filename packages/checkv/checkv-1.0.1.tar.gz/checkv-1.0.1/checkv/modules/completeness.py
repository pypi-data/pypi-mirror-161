...
def main(args):
    ...
    utility.check_executables(["prodigal-gv", "diamond"])
    ...
    if os.path.exists(args["diamond_checkpoint"]):
        ...
        diamond_result = utility.run_diamond(
            args["blastp"], args["db"], args["faa"], args["threads"]
        )
        if diamond_result:
            with open(args["diamond_checkpoint"], "w") as fout:
                pass
        else:
            sys.exit("DIAMOND task failed. Program should be rerun.")
