...
def get_min_max_pipe(pipes, property_name, scale_fac=1.0):
    ...
    tr_min_data = np.hstack(
        [np.array(p.get_property(property_name).get_extr_min_pipe()) for p in pipes]
    )
    tr_max_data = np.hstack(
        [np.array(p.get_property(property_name).get_extr_max_pipe()) for p in pipes]
    )
...
def get_min_max_pipe_relative(pipes, property_name):
    ...
    ss_data = np.hstack(
        [np.array(p.get_property(property_name).get_series_pipe())[:, 0] for p in pipes]
    )
...
def get_route_data(model, pipes, annotations, prop, times):
    ...
    for p in pipes:
        profile_data_s_h = np.array(
            p.get_property("Profile").get_table().get_float_data()[:3]
        ).transpose()[:, -1:0:-1]

        s_dist_pipe = np.linspace(
            profile_data_s_h[0, 0], profile_data_s_h[-1, 0], p.get_num_elements() + 1
        )
    ...
    for p in pipes:
        ...
        data_min = np.array(wanda_prop.get_extr_min_pipe()) * wanda_prop.get_unit_factor()
        data_max = np.array(wanda_prop.get_extr_max_pipe()) * wanda_prop.get_unit_factor()
        if direction == -1:
            data_list.append(np.flipud(data_array))
            data_list_min += np.flipud(data_min).tolist()
            data_list_max += np.flipud(data_max).tolist()
        else:
            data_list.append(data_array)
            data_list_min += data_min.tolist()
            data_list_max += data_max.tolist()
    data = np.vstack(data_list)
    data_min = np.vstack(data_list_min)
    data_max = np.vstack(data_list_max)

    output_location_series = {}
    for t in times:
        if isinstance(t, str) and t.lower().startswith("max"):
            output_location_series[t] = data_max
        elif isinstance(t, str) and t.lower().startswith("min"):
            output_location_series[t] = data_min
        else:
            ind = int(round(t / dt))
