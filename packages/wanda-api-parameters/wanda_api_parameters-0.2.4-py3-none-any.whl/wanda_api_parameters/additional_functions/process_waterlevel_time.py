    ...
    for idx_scenario, scenario in enumerate(list(scenario_list.keys())):
        ...
        for idx_Hi, Hi in enumerate(Hrange):
            plt.plot(
                ...
                label="{} H<={} m".format(scenario, Hi),
                markerfacecolor=scenario_color[idx_scenario],
            )
    plt.legend(frameon=False, loc="upper right", fontsize="x-small")
    plt.xlim([2_000, 5_000])
    plt.ylim([0, 400])
    plt.xlabel(r"$C = P_{lucht} \times V_{lucht} \ \mathrm{(kJ)}$")
    plt.ylabel(r"$\mathrm{Leeglooptijd} \ \mathrm{(s)}$")
    plt.tight_layout()

    plt.savefig(os.path.join(FN_FIGURE, "Leeglooptijd_no_restart.png"))
