...
if __name__ == "__main__":
    case_list = ['txl_db_max_case30',  'txl_db_max_case32',
                 'txl_db_avg_case65', 'txl_db_avg_case68']
    case_label = ['Maximaal debiet C = 3,000 kJ',  'Maximaal debiet C = 5,000 kJ',
                  'Gemiddeld debiet C = 2,000 kJ', 'Gemiddeld debiet C = 5,000 kJ']
    ...
    plt.text(200, 1.0, "minimum hoogte zuigleiding", fontsize='x-small', verticalalignment='bottom',
    ...
    for idx_case, case in enumerate(case_list):
        ...
        # Deteremine time below zero
        ttemp = []
        for Hi in Hrange:
            idx_t = np.where(Hlevel[:, idx_case] < Hi)[0]
            if len(idx_t) == 0:
                ttemp.append(max(tlevel))
            else:
                try:
                    idx_range = np.arange(idx_t[0]-1,idx_t[0]+1)
                    ttemp.append(float(interp1d(Hlevel[idx_range, idx_case], tlevel[idx_range], kind='linear')(Hi)))
                except:
                    ttemp.append(tlevel[idx_t[0]])
        toutput.append(ttemp)
        print("Bij het {} is voor Hmin t = {} s en voor Hzuig t = {} s".format(case_label[idx_case], ttemp[0], ttemp[1]))
        # Plot case
    ...
    plt.savefig(
        os.path.join(FN_FIGURE, "waterlevel_over_time_no_start.png")
    )
...
#             plt.tight_layout()
#             plt.savefig(
#                 os.path.join(FN_FIGURE, "{}_{:2.0f}.png".format(fn, indx_sub_set))
#             )
#
#         Hbottom = 3.0
#         Hmin = 0.4
