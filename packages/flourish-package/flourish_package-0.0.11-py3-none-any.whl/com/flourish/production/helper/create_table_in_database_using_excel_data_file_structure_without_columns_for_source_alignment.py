...
def create(fileDetailsId, locationInsideBucket, fileName, sheetName, headerStartRow, columnSpan, schemaName, rawTableName, insert_into_file_column_to_table_column_mapping_table):
    result = "SUCCESS"
    try:
        ...
        data_from_file = pandas.read_excel(locationInsideBucket+'/'+fileName, sheet_name=(0 if (pandas.isna(sheetName) or not sheetName) else sheetName), header=headerStartRow, usecols=helper_methods.getColumnSpan(columnSpan))
        ...
        if insert_into_file_column_to_table_column_mapping_table:
