        ...
        if(len(master_ingestion_raw_source_file_details) > 0):
            ...
            if(len(runDetails) > 0):
                ...
                if(len(key_columns_in_table) > 0):
                    ...
                    mergedDf1 = data_from_raw_table.merge(data_from_source_aligned_table.drop_duplicates(), on=key_columns_in_table.table_column_name.tolist(), how='left', indicator=True)
                    ...
                    mergedDf2 = data_from_source_aligned_table.merge(data_from_raw_table.drop_duplicates(), on=key_columns_in_table.table_column_name.tolist(), how='left', indicator=True)
                    to_delete_df_since_present_in_source_aligned_but_not_present_in_raw = pandas.DataFrame(mergedDf2[mergedDf2['_merge'] == 'left_only'])
                    to_delete_df_since_present_in_source_aligned_but_not_present_in_raw.drop(helper_methods.drop_columns_ending_with_suffix(to_delete_df_since_present_in_source_aligned_but_not_present_in_raw.columns, '_y'), axis=1, inplace=True)
                    to_delete_df_since_present_in_source_aligned_but_not_present_in_raw.drop(['_merge'], axis=1, inplace=True)
                    to_delete_df_since_present_in_source_aligned_but_not_present_in_raw.rename(columns=helper_methods.change_column_names_by_removing_suffix(to_delete_df_since_present_in_source_aligned_but_not_present_in_raw.columns, '_x'), inplace=True)
                    ...
                    if(len(final_data_to_insert) > 0):
                        ...
                        """ % (schemaName+'.'+sourceAlignedTableName+'_history', schemaName+'.'+sourceAlignedTableName, schemaName+'.'+sourceAlignedTableName, executionDateWithTimeWithDashes))
                        final_data_to_insert.to_sql(name=sourceAlignedTableName, con=dBConnectionEngine, schema=schemaName, if_exists='append', index=False)
