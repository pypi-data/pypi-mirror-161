...
def execute(schema, run_details, rule_mapping, rule):
    ...
    number_of_rows_in_file = run_details.number_of_rows
    ...
    number_of_rows_in_last_file = 0
    if len(last_run_details) > 0:
        number_of_rows_in_last_file = last_run_details.number_of_rows
        if(abs(number_of_rows_in_file - number_of_rows_in_last_file)*100/number_of_rows_in_last_file <= float(variance)):
            rule_execution_details["status_code"] = 'S'
            rule_execution_details["status_description"] = 'Variance in number of rows in accepted limits'
            print('Variance in number of rows in accepted limits')
        else:
            rule_execution_details["status_code"] = 'F'
            rule_execution_details["status_description"] = 'Variance in number of rows > ' + str(variance)
            print('Variance in number of rows > ' + str(variance))
            result = 'FAILURE'
    else:
        rule_execution_details["status_code"] = 'S'
