...
def execute(schema, run_details, rule_mapping, rule):
    ...
    if(len(master_ingestion_raw_source_file_details) > 0):
        ...
        #Step 3: Using file_details_id, name_delimiter and actual_file_name from run_details, get the number of rows
        number_of_rows = get_number_of_rows_in_file_by_splitting_file_name(file_details_id, file_name_delimiter, run_details.actual_file_name)
        ...
        if number_of_rows > -1:
            if(number_of_rows == run_details.number_of_rows):
                rule_execution_details["status_code"] = 'S'
                rule_execution_details["status_description"] = 'Count of rows in file same as that in the file name'
                print('Count of rows in file same as that in the file name')
            else:
                rule_execution_details["status_code"] = 'F'
                rule_execution_details["status_description"] = 'Count of rows in file NOT same as that in the file name'
                print('Count of rows in file NOT same as that in the file name')
                result = 'FAILURE'
        else:
            rule_execution_details["status_code"] = 'S'
            rule_execution_details["status_description"] = 'Count of rows NOT present in file name'
            print('Count of rows NOT present in file name')
        db_operations.insert_rule_check_details_for_run(schema, rule_execution_details)
    return result

def get_number_of_rows_in_file_by_splitting_file_name(file_details_id, file_name_delimiter, actual_file_name):
    actual_file_name_without_extension = actual_file_name[:actual_file_name.rfind(".")]
    file_name_parts_array = actual_file_name_without_extension.split(file_name_delimiter)
    print('File Name Split by Delimiter: ', file_name_parts_array)
    i = 0
    ...
    for row in master_ingestion_raw_source_file_name_details.itertuples():
        if row.meaning == 'ROW_COUNT' or row.meaning == 'ROWCOUNT':
            return int(file_name_parts_array[int(row.index_in_file_name)])
        else:
            i=i+1
    return -1
