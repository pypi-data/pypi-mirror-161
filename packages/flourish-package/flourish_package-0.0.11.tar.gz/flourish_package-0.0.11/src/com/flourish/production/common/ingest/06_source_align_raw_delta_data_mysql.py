...
def execute_for_file_name_pattern(source, schemaName, rawTableName, sourceAlignedTableName, bucketName, destinationFolderInsideBucket, destinationFileNameWithoutExtension, fileNamePattern):
    updateRunDetails = {}
    try:
        condition = {'table_name': rawTableName, 'name_pattern': fileNamePattern}
        ...
        if(len(master_ingestion_raw_source_file_details) == 1):
            master_ingestion_raw_source_file_detail = master_ingestion_raw_source_file_details.iloc[0]
            updateRunDetails = execute_common(source, schemaName, rawTableName, sourceAlignedTableName, bucketName, destinationFolderInsideBucket, destinationFileNameWithoutExtension, master_ingestion_raw_source_file_detail)
...
def execute_common(source, schemaName, rawTableName, sourceAlignedTableName, bucketName, destinationFolderInsideBucket, destinationFileNameWithoutExtension, master_ingestion_raw_source_file_detail):
    ...
    try:
        ...
        if(len(runDetails) > 0):
            ...
            if(len(key_columns_in_table) > 0):
                ...
                #data_from_raw_table renamed to data_of_updated_or_inserted_rows_from_raw_table since for delta files there will be a column
                #called is_deleted (in the future we might have to make this a configurable column which can be different for different source 
                #delta files but as of now the delta file needs to have a column called is_deleted that will either have TRUE or 1 when the 
                #row is deleted and will have FALSE or 0 when the row is updated or inserted in the source). The is_deleted is currently
                #assumed to be varchar but can later by changed to boolean in the database if that makes more sense during ingestion.
                data_of_updated_or_inserted_rows_from_raw_table = pandas.read_sql("select * from "+schemaName+"."+rawTableName+" where (upper(is_deleted) = 'FALSE' or is_deleted = '0')", dBConnection)
                data_of_updated_or_inserted_rows_from_raw_table = helper_methods.remove_database_audit_columns_from_dataframe(data_of_updated_or_inserted_rows_from_raw_table)
                data_of_updated_or_inserted_rows_from_raw_table.drop(['is_deleted'], axis=1, inplace=True)
                data_of_updated_or_inserted_rows_from_raw_table = helper_methods.change_datatype_of_columns_based_on_master_configuration(data_of_updated_or_inserted_rows_from_raw_table, source_file_column_to_table_column_mapping)
                #data_of_updated_or_inserted_rows_from_raw_table.set_index(key_columns_in_table.table_column_name.tolist(), inplace=True, verify_integrity=True)
                #data_of_updated_or_inserted_rows_from_raw_table = data_of_updated_or_inserted_rows_from_raw_table.replace(numpy.nan, '', regex=True)
                print('********** data_of_updated_or_inserted_rows_from_raw_table: ', len(data_of_updated_or_inserted_rows_from_raw_table))

                for column in data_of_updated_or_inserted_rows_from_raw_table.columns:
                    if is_integer_dtype(data_of_updated_or_inserted_rows_from_raw_table[column]):
                        data_of_updated_or_inserted_rows_from_raw_table[column] = data_of_updated_or_inserted_rows_from_raw_table[column].astype('Int64')
                    elif is_bool_dtype(data_of_updated_or_inserted_rows_from_raw_table[column]):
                        data_of_updated_or_inserted_rows_from_raw_table[column] = data_of_updated_or_inserted_rows_from_raw_table[column].astype('boolean')
                ...
                mergedDf1 = data_of_updated_or_inserted_rows_from_raw_table.merge(data_from_source_aligned_table.drop_duplicates(), on=key_columns_in_table.table_column_name.tolist(), how='outer', indicator=True)

                columns_in_raw_table = data_of_updated_or_inserted_rows_from_raw_table.columns
                ...
                to_do_nothing_df_since_present_in_source_aligned_but_not_present_in_raw = pandas.DataFrame(mergedDf1[mergedDf1['_merge'] == 'right_only'])
                to_do_nothing_df_since_present_in_source_aligned_but_not_present_in_raw.drop(helper_methods.drop_columns_ending_with_suffix(to_do_nothing_df_since_present_in_source_aligned_but_not_present_in_raw.columns, '_x'), axis=1, inplace=True)
                to_do_nothing_df_since_present_in_source_aligned_but_not_present_in_raw.drop(['_merge'], axis=1, inplace=True)
                to_do_nothing_df_since_present_in_source_aligned_but_not_present_in_raw.rename(columns=helper_methods.change_column_names_by_removing_suffix(to_do_nothing_df_since_present_in_source_aligned_but_not_present_in_raw.columns, '_y'), inplace=True)
                print('Rows where nothing needs to be done since they are present only in source aligned and not in raw')
                print(len(to_do_nothing_df_since_present_in_source_aligned_but_not_present_in_raw))

                concatenated_df_raw_and_source_aligned = pandas.concat([data_from_source_aligned_table, data_of_updated_or_inserted_rows_from_raw_table])
                grouped_df_on_all_columns_from_concatenated_df = concatenated_df_raw_and_source_aligned.fillna(0).groupby(list(data_of_updated_or_inserted_rows_from_raw_table.columns))
                ...
                print('The above count should always be 0 for a delta file. If NOT, we need to assess the reason.')
                ...
                data_of_deleted_rows_from_raw_table = pandas.read_sql("select * from "+schemaName+"."+rawTableName+" where (upper(is_deleted) = 'TRUE' or is_deleted = '1')", dBConnection)
                data_of_deleted_rows_from_raw_table = helper_methods.remove_database_audit_columns_from_dataframe(data_of_deleted_rows_from_raw_table)
                data_of_deleted_rows_from_raw_table.drop(['is_deleted'], axis=1, inplace=True)
                data_of_deleted_rows_from_raw_table = helper_methods.change_datatype_of_columns_based_on_master_configuration(data_of_deleted_rows_from_raw_table, source_file_column_to_table_column_mapping)
                #data_of_deleted_rows_from_raw_table.set_index(key_columns_in_table.table_column_name.tolist(), inplace=True, verify_integrity=True)
                #data_of_deleted_rows_from_raw_table = data_of_deleted_rows_from_raw_table.replace(numpy.nan, '', regex=True)
                print('********** data_of_deleted_rows_from_raw_table: ', len(data_of_deleted_rows_from_raw_table))

                for column in data_of_deleted_rows_from_raw_table.columns:
                    if is_integer_dtype(data_of_deleted_rows_from_raw_table[column]):
                        data_of_deleted_rows_from_raw_table[column] = data_of_deleted_rows_from_raw_table[column].astype('Int64')
                    elif is_bool_dtype(data_of_deleted_rows_from_raw_table[column]):
                        data_of_deleted_rows_from_raw_table[column] = data_of_deleted_rows_from_raw_table[column].astype('boolean')

                data_to_delete_from_source_aligned_table = data_of_deleted_rows_from_raw_table.merge(data_from_source_aligned_table.drop_duplicates(), on=key_columns_in_table.table_column_name.tolist(), how='inner')

                data_to_delete_from_source_aligned_table.drop(helper_methods.drop_columns_ending_with_suffix(data_to_delete_from_source_aligned_table.columns, '_x'), axis=1, inplace=True)
                data_to_delete_from_source_aligned_table.rename(columns=helper_methods.change_column_names_by_removing_suffix(data_to_delete_from_source_aligned_table.columns, '_y'), inplace=True)
                #print(data_to_delete_from_source_aligned_table)
                data_to_delete_from_source_aligned_table.set_index(key_columns_in_table.table_column_name.tolist(), inplace=True, verify_integrity=True)
                data_to_delete_from_source_aligned_table["row_end_date"] = executionDateWithTimeWithDashes
                print('Rows to update row_end_date column to current_timestamp since they have been deleted in the source system.')
                print(len(data_to_delete_from_source_aligned_table))
                print(data_to_delete_from_source_aligned_table.columns.tolist())





                final_data_to_insert = pandas.concat([to_insert_df_since_present_in_raw_but_not_present_in_source_aligned, to_do_nothing_df_since_present_in_source_aligned_but_not_present_in_raw, to_insert_with_active_status_df, to_update_status_to_inactive_df, to_do_nothing_df_as_no_difference_between_raw_and_source_aligned_records, data_to_delete_from_source_aligned_table])
                ...
                if(len(final_data_to_insert) > 0):
                    ...
                    """ % (schemaName+'.'+sourceAlignedTableName+'_history', schemaName+'.'+sourceAlignedTableName))
                    
                    metadataObject = MetaData(bind=dBConnectionEngine, reflect=True, schema=schemaName)
                    metadataObject.reflect()
                    source_aligned_table_object = Table(sourceAlignedTableName, metadataObject)
                    delete_from_source_aligned_table = source_aligned_table_object.delete().where(source_aligned_table_object.c.row_status_code == 1)
                    dBConnection.execute(delete_from_source_aligned_table)
                    
                    final_data_to_insert['load_timestamp'] = final_data_to_insert['load_timestamp'].astype(str)
                    final_data_to_insert['row_start_date'] = final_data_to_insert['row_start_date'].astype(str)
                    final_data_to_insert['row_end_date'] = final_data_to_insert['row_end_date'].astype(str)
                    final_data_to_insert.to_sql(name=sourceAlignedTableName, con=dBConnectionEngine, schema=schemaName, if_exists='append', index=False)
                ...
                print("No key columns for a delta file so source alignment can't be done and run details weill be updated to SAF (Source alignment unsuccessful).")
