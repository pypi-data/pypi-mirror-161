...
ingestDataWithDelimiterIntoRawTableWithBlankColumnsInTheEnd = getattr(ingest_csv_data_file_into_raw_table, "ingestDataWithDelimiterIntoRawTableWithBlankColumnsInTheEnd")

def execute(source, schema, tableName, blankColumnsToAddInTheEnd):
    ...
    for row in master_ingestion_raw_source_file_details.itertuples():
        ...
        if(len(runDetails) > 0):
            ...
            if(not pandas.isna(row.file_encoding) and row.file_encoding and not row.file_encoding is 'utf-8'):
                ...
                result = ingestDataWithDelimiterIntoRawTableWithBlankColumnsInTheEnd(
