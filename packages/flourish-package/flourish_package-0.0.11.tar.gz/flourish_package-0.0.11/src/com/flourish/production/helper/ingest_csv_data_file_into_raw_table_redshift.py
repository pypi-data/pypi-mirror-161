...
def ingestDataIntoRawTableCommon(fileNameWithS3Location, fileEncoding, schemaName, tableName, data_from_file):
    ...
    try:
        ...
        dBConnection.execute("""
            ...
            ACCEPTINVCHARS AS '-'
            csv;
            update %s set load_timestamp = '%s';
        """ % (schemaName+'.'+tableName, dataFileName, schemaName+'.'+tableName, executionDateWithTimeWithDashes))
...
def ingestDataIntoRawTableCommonWithReportingPeriod(fileNameWithS3Location, fileEncoding, schemaName, tableName, data_from_file, reportingPeriod):
    ...
    try:
        ...
        dBConnection.execute("""
            ...
            ACCEPTINVCHARS AS '-'
            csv;
            update %s set load_timestamp = '%s', reporting_period = '%s';
        """ % (schemaName+'.'+tableName, dataFileName, schemaName+'.'+tableName, executionDateWithTimeWithDashes, reportingPeriod))
    except Exception as e:
        print('Error while ingesting data into the raw table')
        print(e)
        result = "FAILURE"
