...
class AnalysisSTEAM:
    ...
    def __init__(self, file_name_analysis: str = None,
        ...
        if not self.file_name_analysis:
            if not input_dictionary:
                raise Exception('No .yaml input file or dict provided.')
            else:
                self.data_analysis = DataAnalysis(**input_dictionary)
        else:
            # Check that .yaml is provided or not [for consistency with other tools]
            if not self.file_name_analysis.endswith('.yaml'):
                self.file_name_analysis = self.file_name_analysis +  '.yaml'
    ...
    def setup_sim_FiQuS(self, simulation_name, sim_number):
        """
