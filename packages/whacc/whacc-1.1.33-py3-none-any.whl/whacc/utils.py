...
def make_path(name_in):
    name_in = assert_path(name_in) # if its a file it will create the basename.
...
def make_list(x, suppress_warning=False):
    if not isinstance(x, list):
        ...
        if type(x) is np.str_:
            ...
            print(type(x))
            assert False, '''see module whacc.utils.make_list, we have not official protocol for this input type ''' + str(type(x))
