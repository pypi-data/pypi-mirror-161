...
class tc_settings extends s {
  ...
  render() {
    return $`<textarea align="center">TODO</textarea><br><button aligh="center">hi</button>`;
  }
}
...
class tinycloud extends s {
  ...
  render() {
    ...
    return $`<body><div id="header">Tinycloud0.1<div align="right">${menu.map((x) => $`<a href="#${x[2]}">${x[0]}</a> `)}</div><hr></div><div id="content">${contFunc()}</div></body>`;
  }
}
...
class tc_filelist extends s {
  ...
  render() {
    ...
    if (this.url != "/") {
      prev = $`<a class="dir" href="#${this.urlRoot}/${prev}">../</a><br>`;
    } else {
      prev = $``;
    }
        ...
        (file) =>
          $`${o(file.type, [
            [
              "dir",
              () =>
                $`<a id="file-${file.name}" tc-filename="${file.name}" class="dir" href="#${this.urlRoot}/${this.url}/${file.name}/">${file.name}/</a>`,
            ],
            [
              "broken",
              () =>
                $`<a id="file-${file.name}" tc-filename="${file.name}" class="broken" href="#${this.urlRoot}/${this.url}/${file.name}/">${file.name}/</a>`,
            ],
            [
              "file",
              () =>
                $`<a class="file" id="file-${file.name}" tc-filename="${file.name}" href="/dav/${this.urlRoot}/${this.url}/${file.name}" download="${file.name}">${file.name}</a>`,
            ],
            [
              "uploading",
              () =>
            ...
            [
              "mountpoint",
              () =>
                $`<a class="mountpoint" id="file-${file.name}" tc-filename="${file.name}" href="#${this.urlRoot}/${this.url}/${file.name}">${file.name}</a>`,
            ],
          ])}<br>`
      )}</div>`;
  }
