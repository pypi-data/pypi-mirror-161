...
def get_SIFTS(SIFTS_file="uniprot_segments_observed.tsv.gz"):
    ...
    with gzip.open('uniprot_segments_observed.tsv.gz','rb') as fin:        
        for line in fin:        
...
def main():
    ...
    if len(pdbs) == 0:
        ...
        if args.update:
            ...
            get_SIFTS("uniprot_segments_observed.tsv.gz")


        # check if PDB-UP mapping json exists
        ...
        try:
            ...
            print("-> ERROR couldn't find SIFTS file, attempting to download from PDBe")
            get_SIFTS("uniprot_segments_observed.tsv.gz")
        ...
        try:
                    ...
                    for chain in pdb_chain_up_start_end[pdbid]:
                        for upid in pdb_chain_up_start_end[pdbid][chain]:
                            ...
                            try:
                                ...
                                print("-> could not find match for "+pdbid)
                                pass
            ...
            print("-> couldn't find or download SIFTS file, quitting")
            sys.exit(1)
