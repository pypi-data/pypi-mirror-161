...
class DiTree:
    ...
    def find_in(self) -> Dict[str, List[Vertex]]:
        ...
        for vertex in self._steps[-1]:
            ...
            _vertices = [_v for _v in vertex.predecessors() if _v[r'name'] not in self._nodes]

            if _vertices:
                vertices.extend(_vertices)
                nodes[vertex[r'name']] = [_v[r'name'] for _v in _vertices]
    ...
    def find_out(self) -> Dict[str, List[Vertex]]:
        ...
        for vertex in self._steps[-1]:
            ...
            _vertices = [_v for _v in vertex.successors() if _v[r'name'] not in self._nodes]
...
class DiGraph:
    ...
    def del_edges(self, source: Vertex, target: Vertex):
        ...
        if edges:
            self._graph.delete_edges([edge.index for edge in edges])
