...
class MaterializationPlan(object):
    _base_data_frame: Optional[DataFrame]
    ...
    def base_data_frame(self) -> Optional[DataFrame]:
        if self._base_data_frame is None and conf.get_bool("ALPHA_QUERYTREE_ENABLED") and self.query_tree is not None:
            # can't really use a tecton.DataFrame here, but it would be nice to not have all this tree manipulation boilerplate around
            self._base_data_frame = translate.spark_convert(self.query_tree).to_dataframe(self.spark)
...
def get_batch_materialization_plan(
    *,
    ...
    plan.query_tree = build_run_querytree(feature_definition, feature_data_time_limits=feature_data_time_limits)
